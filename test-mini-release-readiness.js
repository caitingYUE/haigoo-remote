...
assert.ok(miniSession.includes('memberType?: string'), 'Mini session must retain the current Club plan type')
assert.ok(membershipPage.includes('memberType === plan.memberType'), 'active members may only renew their current Club plan')
assert.ok(membershipPage.includes('到期后可更换'), 'incompatible active Club plans must not expose a failing purchase action')
...
assert.ok(!cloudrun.includes('jobsSourceOrigin !== apiOrigin'), 'subscription reads must not reference an undefined job source variable')
assert.ok(!cloudrun.includes('getSubscribedCachedJobs'), 'subscription updates must not fabricate immediate matches outside email delivery history')
assert.ok(cloudrun.includes('getCachedJobs(data.jobIds || [], 5)'), 'subscription updates must mirror at most five jobs from the latest email')
assert.ok(gateway.includes('WITH latest_sent_run AS') && gateway.includes('LIMIT 5'), 'subscription reads must use only the latest successful email batch')
...
assert.ok(jobCard.includes("job-card__new") && jobCard.includes('job.isNew'), 'recent jobs must show the same New signal as the website')
...
assert.ok(jobDetail.includes('邮件正文：') && jobDetail.includes("confirmText: '复制全部'"), 'email apply must preview recipient, subject, and body before copying')
...
assert.ok(membershipPage.includes('haigoo-advisor.png') && membershipPage.includes('haigoo-community.png'), 'membership consultation and community QR entries must be present')
assert.ok(membershipPage.includes('subscription-topic-search') && membershipPage.includes('subscription-topic-groups') && membershipPage.includes('maximumTopics'), 'member subscription topics must support grouped tabs, search, and selection limits')
assert.ok(!membershipPage.includes('topic.count'), 'subscription topic chips must not display backend job counts')
assert.ok(!membershipPage.includes('MY JOB UPDATES') && !jobDetail.includes('COMPANY PROFILE'), 'decorative English title labels must not ship')
assert.ok(membershipPage.includes('membership-qr-dialog') && membershipPage.includes('showMenuByLongpress'), 'Club QR entries must open an enlargable in-app dialog')
assert.ok(taroConfig.includes('home-hero-bg.jpg') && taroConfig.includes('haigoo-community.png'), 'experience builds must use packaged JPEG/PNG assets')
assert.ok(taroConfig.includes('../../public/avatars'), 'generated website avatars must be packaged for the Mini Program')
