...
assert.match(unreadUpdates, /markCareerWatchUpdatesRead\(unreadIds\)/)
assert.match(unreadUpdates, /Taro\.eventCenter\.trigger\('haigoo:unread-change', 0\)/)
assert.match(unreadUpdates, /订阅岗位更新/)
assert.match(profile, /const subscribedJobCount = dashboard\?\.watch\.followedUpdates\.length/)
assert.match(profile, /profile-facts__unread-dot/)
assert.match(profile, /订阅岗位更新/)
assert.doesNotMatch(profile, /trigger\('haigoo:unread-change', watch\.followedUpdates\.length\)/)
