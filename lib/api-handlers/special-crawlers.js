...
export async function crawlBraintrustJobs(url) {
    ...
    try {
        ...
        // Try to extract from URL path: /employers/123/
        const matchPath = url.match(/employers\/(\d+)/)
        if (matchPath) {
            employerId = matchPath[1]
        } else {
            // Try to extract from query param: ?employer_id=123
            try {
                // If url doesn't start with http, it might be partial? No, we expect full url.
