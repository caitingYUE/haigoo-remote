...
async function listOrGetBugReport(req, res, user) {
  ...
  if (user) {
    ...
    const userResult = await neonHelper.query('SELECT roles, email FROM users WHERE user_id = $1', [user.userId])
    if (userResult && userResult.length > 0) {
      const userRoles = userResult[0].roles
      const email = userResult[0].email
      // Hardcoded fallback for known admins
      isAdmin = (userRoles && userRoles.admin === true) || email === 'caitlinyct@gmail.com' || email === 'mrzhangzy1996@gmail.com'
      console.log('[BugReport] User roles:', JSON.stringify(userRoles), 'Email:', email, 'IsAdmin:', isAdmin)
    } else {
  ...
  try {
    ...
    console.log(`[BugReport] Query returned ${rows ? rows.length : 0} rows`)
    return res.json({ success: true, data: rows || [] })
  } catch (error) {
