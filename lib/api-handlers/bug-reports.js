...
async function createBugReport(req, res, user) {
  const { title, description, imageUrl, userNickname } = req.body
...
async function listOrGetBugReport(req, res, user) {
  ...
  try {
    ...
    let query = `
        SELECT 
            id, user_id, user_nickname, title, description, status, created_at, updated_at,
...
async function updateBugReport(req, res, user) {
    ...
    try {
        const query = `UPDATE ${TABLE_NAME} SET status = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING *`
        const rows = await neonHelper.query(query, [status, id])
