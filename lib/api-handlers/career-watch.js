  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      const [feed, filterOptions] = await Promise.all([
        getCareerWatchFeed({ userId: user.user_id, profile, isMember, fixedFree: access.matchState === 'fixed_free', refreshExpired: true }),
        getCareerWatchFilterOptions()
      ])
    ...
    if (req.method === 'GET') {
      ...
      const access = await getCareerWatchAccessState(user.user_id, isMember)
      const feed = await getCareerWatchFeed({ userId: user.user_id, profile, isMember, fixedFree: access.matchState === 'fixed_free', refreshExpired: true })
      return res.status(200).json({
        success: true,
        authenticated: true,
        profile,
