...
请返回以下 JSON（中文），所有文本字段必须填写实际内容，禁止留空：
{
  ...
  "milestones": [
    {
      "month": "<阶段名，使用相对时间，如：第1-2周 / 准备冲刺期 / 投递强化期>",
      "focus": "<本阶段核心任务，必须结合用户职业方向${role ? `（${role}）` : ''}写具体，10-20字，禁止留空>",
      "tasks": [
        "<具体行动1：结合${role || '用户背景'}的实际可执行步骤，30字内>",
        "<具体行动2：使用具体工具/平台/方法，如 LinkedIn/Notion/GitHub/JD分析等>",
        "<具体行动3：可量化的目标，如'完成X件作品/投递Y份/练习Z次'>"
      ]
    }
  ]
}
...
关键行动路线生成规则（严格遵守）：
1. 必须生成 4-6 个阶段，覆盖整个 ${timelineWeeks} 周时间线，做 Backward Planning（从目标倒推）
...
6. 根据目标类型（${goalLabel}）调整策略：全职优先质量投递，兼职/副业优化接单平台，了解市场聚焦信息收集
7. tasks 禁止出现 JSON 对象，必须是纯字符串数组

milestones 示例（严格按此格式）：
{ "month": "第1-2周", "focus": "打磨${role || '目标岗位'}远程简历与作品集", "tasks": ["将过往项目按 STAR 法则重写，突出量化成果", "更新 LinkedIn 标题/摘要并设置 Open to Work", "收集 5 个目标 JD，提炼高频关键词清单"] }
{ "month": "第3-4周", "focus": "建立远程求职基础设施", "tasks": ["针对目标岗位制作 1 份定制化英文简历模板", "注册 ${role?.includes('工程') || role?.includes('开发') ? 'GitHub/TopTal/Upwork' : 'LinkedIn/People Per Hour/Remote.co'} 并完善档案", "练习 1 次 30 分钟英文自我介绍视频录制并复盘"] }
