...
请返回以下 JSON（中文），所有文本字段必须填写实际内容，禁止留空：
{
  ...
  "milestones": [
    { "month": "<阶段名，如：第1-2周>", "focus": "<本阶段核心任务，必填，10-20字>", "tasks": ["<具体行动1，30字内>", "<具体行动2，30字内>", "<具体行动3，30字内>"] },
    { "month": "<阶段名>", "focus": "<本阶段核心任务，必填>", "tasks": ["<行动1>", "<行动2>"] },
    { "month": "<阶段名>", "focus": "<本阶段核心任务，必填>", "tasks": ["<行动1>", "<行动2>"] }
  ]
}

milestones 示例（请严格按此格式，禁止 focus 留空）：
{ "month": "第1-2周", "focus": "打磨远程简历与作品集", "tasks": ["将过往项目按 STAR 法则重写简历", "更新 LinkedIn 并设置 Open to Work", "收集目标岗位 JD 关键词"] }

要求：
- recommendations 必须优先匹配用户目标类型；若用户目标为兼职/副业增收，不要首推全职岗位。
- 若推荐岗位与目标存在偏差，必须在 reason 中给予说明。
- milestones 必须生成 3-6 个阶段；month 命名为相对时间（如"第1-2周""准备期""冲刺期"），focus 必须概括本阶段核心任务（禁止留空！），tasks 必须是字符串数组（每项≤30字，不能是对象）。
- 根据用户预期时间（${timelineWeeks}周）和每周投入（${investedHours || '5-10'}小时）进行 Backward Planning 排期，确保行动量级可执行。`;
