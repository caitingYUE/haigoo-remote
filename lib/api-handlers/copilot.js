...
import { buildMatchingProfile, getUserProfileForMatching, scoreJobForUserProfile } from '../services/matching-engine.js';
...
function hasBlockingConstraintFlags(flags = {}) {
  return Boolean(
...
function composeGoalAwareScore({ baseScore = 0, breakdown = {}, goal = '', language = '', row = {} } = {}) {
  ...
  if (!GOAL_AWARE_SCORING_ENABLED) {
    ...
    return {
      ...
      aiRecommended: AI_RECOMMENDED_TAG_ENABLED && base >= 70 && goalFitDisabled >= AI_RECOMMENDED_MIN_GOAL_FIT
    };
  }
  ...
  if (skillAdjacency < 20 || skillFit < 20) {
    weighted = Math.min(weighted, 60);
  } else if (skillAdjacency < 35 || skillFit < 35) {
    weighted = Math.min(weighted, 65);
  }
  ...
  const aiRecommended = AI_RECOMMENDED_TAG_ENABLED && weighted >= 70 && goalFit >= AI_RECOMMENDED_MIN_GOAL_FIT;
...
function buildCopilotMatchDetails({ score = 0, details = {}, row = {} }) {
  ...
  const insights = buildMatchInsights({ score, details, jobRow: row });

  return {
    ...insights,
    summary: `${row?.title || row?.category || '该岗位'} 属于${resolveMatchLabel(level) || '可尝试'}岗位。 ${insights.summary}`.trim()
  };
}

function getRecommendationDateKey(date = new Date()) {
  return new Date(date).toISOString().slice(0, 10);
}

function getRecommendationJobId(item = {}) {
  const raw = item?.jobId ?? item?.job_id ?? item?.id;
  return raw == null ? '' : String(raw);
}

function buildRecommendationMeta(recommendations = [], previousRecommendationIds = []) {
  const previousIds = new Set((previousRecommendationIds || []).map(id => String(id)).filter(Boolean));
  const jobIds = recommendations.map(item => getRecommendationJobId(item)).filter(Boolean);
  const overlapCount = jobIds.filter(id => previousIds.has(id)).length;
  const overlapRatio = jobIds.length > 0 ? Number((overlapCount / jobIds.length).toFixed(2)) : 0;

  return {
    refreshedAt: new Date().toISOString(),
    refreshDateKey: getRecommendationDateKey(),
    jobIds,
    overlapCount,
    overlapRatio
  };
...
function shouldRefreshRecommendationMeta(meta = null) {
  const refreshDateKey = String(meta?.refreshDateKey || '').trim();
  return refreshDateKey !== getRecommendationDateKey();
}

function applyRecommendationOverlapPolicy(candidates = [], previousRecommendationIds = [], topN = 5) {
  const normalizedTopN = Math.max(0, Number(topN) || 0);
  if (normalizedTopN === 0) return [];

  const previousIds = new Set((previousRecommendationIds || []).map(id => String(id)).filter(Boolean));
  if (!previousIds.size) return candidates.slice(0, normalizedTopN);

  const maxOverlap = Math.floor(normalizedTopN * 0.6);
  const selected = [];
  const selectedIds = new Set();
  let overlapCount = 0;

  for (const candidate of candidates) {
    const candidateId = getRecommendationJobId(candidate?.job || candidate);
    const isRepeated = previousIds.has(candidateId);
    if (selected.length >= normalizedTopN) break;
    if (isRepeated && overlapCount >= maxOverlap) continue;
    selected.push(candidate);
    selectedIds.add(candidateId);
    if (isRepeated) overlapCount += 1;
  }

  if (selected.length < normalizedTopN) {
    for (const candidate of candidates) {
      const candidateId = getRecommendationJobId(candidate?.job || candidate);
...
function mapCandidateJobsToPlanRecommendations(candidateJobs = []) {
...
async function getCopilotMembershipStatus(userId) {
  if (!userId) return false;
  try {
    const userResult = await neonHelper.query(
      `SELECT member_status, member_expire_at, member_cycle_start_at FROM users WHERE user_id = $1`,
      [userId]
    );
    const row = userResult?.[0];
    if (!row) return false;
    return isMembershipActive(row);
  } catch (_err) {
    return false;
  }
}

async function persistCopilotPlanData(userId, planData) {
  ...
  try {
    await neonHelper.query(
      `UPDATE copilot_sessions
       SET plan_data = $1
       WHERE id = (
         SELECT id FROM copilot_sessions
         WHERE user_id = $2
    ...
    console.warn('[Copilot] Failed to persist session plan_data:', err?.message || err);
  }

  try {
    await neonHelper.query(
      `INSERT INTO copilot_user_state (user_id, plan_data, updated_at)
       VALUES ($1, $2, NOW())
       ON CONFLICT (user_id)
    ...
    console.warn('[Copilot] Failed to persist copilot_user_state plan_data:', err?.message || err);
  }
}

async function maybeRefreshDailyRecommendations({
  userId,
  plan = null,
  goal = '',
  background = {},
  ...
  if (!userId || !plan || typeof plan !== 'object') return plan;

  const recommendationMeta = plan?.plan_v2?.recommendationMeta || plan?.recommendationMeta || null;
  const currentRecommendations = Array.isArray(plan?.recommendations) ? plan.recommendations : [];
  const previousRecommendationIds = recommendationMeta?.jobIds || currentRecommendations.map(item => getRecommendationJobId(item)).filter(Boolean);

  if (currentRecommendations.length > 0 && !shouldRefreshRecommendationMeta(recommendationMeta)) {
    return plan;
  }

  const refreshedJobs = await fetchCandidateJobs({
    userId,
    background,
    goal,
    topN: isMember ? 5 : 1,
  ...
  const refreshedRecommendations = mapCandidateJobsToPlanRecommendations(refreshedJobs);
  const nextRecommendationMeta = buildRecommendationMeta(refreshedRecommendations, previousRecommendationIds);
  const nextPlan = {
    ...plan,
    recommendations: refreshedRecommendations,
    recommendationMeta: nextRecommendationMeta,
    plan_v2: plan.plan_v2 ? {
      ...plan.plan_v2,
      recommendations: refreshedRecommendations,
      recommendationMeta: nextRecommendationMeta
    } : undefined
  ...
  await persistCopilotPlanData(userId, nextPlan);
  return nextPlan;
}
...
export default async function handler(req, res) {
  ...
  // Authenticated identity must only come from the signed token. Trusting a
  // client-supplied userId would allow account impersonation and could bypass
  // the account's email-verification state.
  try {
    const token = extractToken({ headers: req.headers });
    if (token) {
      const payload = verifyToken(token);
      userId = payload?.userId || null;
    }
  } catch (err) {
    console.warn('[Copilot] token extraction failed', err.message);
  }

  if (userId && requestedUserId && String(userId) !== String(requestedUserId)) {
    return res.status(403).json({
      success: false,
      error: '请求用户与登录用户不一致，请重新登录后重试。'
    });
  }
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      try {
        ...
        const savedPlan = parseJsonSafe(stateResult?.state?.planData, stateResult?.state?.planData) || null;
        if (savedPlan) {
          // 致命 Bug 修复：V1.3 核心中不再把推荐打包进 planData，而是独立在了 jobMatches。这会导致旧版 UI 的 .map 崩溃。
          if (!savedPlan.recommendations) {
            if (stateResult.jobMatches && stateResult.jobMatches.length > 0) {
              savedPlan.recommendations = stateResult.jobMatches.slice(0, 5).map(j => ({
                  title: j.title || '',
                  company: j.company || '',
                  match: resolveMatchLabel(resolveMatchLevel(j.match_score)) || j.match_reason || '高匹配',
                  jobId: j.job_id || ''
              }));
            } else {
              savedPlan.recommendations = [];
            }
          }

          let lastSession = null;
          try {
            const sessions = await neonHelper.query(
                `SELECT goal, timeline, background FROM copilot_sessions WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1`,
                [userId]
            );
            if (sessions && sessions.length > 0) lastSession = sessions[0];
          } catch(e) {}

          let backgroundParams = { role: '', industry: '', years: '中级', seniority: '中级', education: '本科', language: '英语-工作 (B2)' };
          if (lastSession && lastSession.background) {
              try {
                  const p = typeof lastSession.background === 'string' ? JSON.parse(lastSession.background) : lastSession.background;
                  backgroundParams = { ...backgroundParams, ...p, role: p.industry || p.role || '' };
              } catch(e) {}
          }

          const latestGoal = lastSession?.goal || savedPlan?.plan_v2?.goalContext?.goal || 'full-time'
          const isMemberForView = await getCopilotMembershipStatus(userId)
          const refreshedPlan = await maybeRefreshDailyRecommendations({
            userId,
            plan: savedPlan,
            goal: latestGoal,
            background: backgroundParams,
            isMember: isMemberForView
          })
      ...
      if (sessionResult && sessionResult.length > 0) {
        ...
        const sessionPlan = parseJsonSafe(session.plan_data, session.plan_data) || null;
    ...
    if (req.method === 'POST') {
      ...
      if (action && action !== 'generate') {
          ...
          try {
            if (!isGuest) {
              const userRows = await neonHelper.query(
                `SELECT email_verified FROM users WHERE user_id = $1 LIMIT 1`,
                [userId]
              );
              if (userRows?.[0]?.email_verified !== true) {
                return res.status(403).json({
                  error: '请先验证邮箱后再获取岗位推荐',
        ...
        try {
          const userResult = await neonHelper.query(
            `SELECT member_status, member_expire_at, member_cycle_start_at FROM users WHERE user_id = $1`, [userId]
          );
          const row = userResult?.[0];
          if (row) isMemberForAction = isMembershipActive(row);
        } catch (e) { /* proceed as free user */ }

        if (action === 'refresh-recommendations') {
          const topN = isMemberForAction ? 5 : 1;
          const latestPlanRows = await neonHelper.query(
            `SELECT plan_data FROM copilot_sessions WHERE user_id = $1 ORDER BY created_at DESC LIMIT 1`,
            [userId]
          ).catch(() => []);
          const latestPlan = parseJsonSafe(latestPlanRows?.[0]?.plan_data, {}) || {};
          const previousRecommendationIds =
            latestPlan?.plan_v2?.recommendationMeta?.jobIds ||
            latestPlan?.recommendationMeta?.jobIds ||
            (Array.isArray(latestPlan?.recommendations) ? latestPlan.recommendations.map(item => getRecommendationJobId(item)).filter(Boolean) : []);
  ...
  "milestones": [
    {
      ...
      "tasks": ["<深度打磨后的行动1>", "<行动2>", ...]
        ...
        if (memberOnlyActions.includes(action) && !isMemberForAction) {
          return res.status(403).json({
            error: '此功能仅限会员使用',
            code: 'MEMBER_ONLY',
            message: '了解会员服务以解锁完整 Copilot 功能。',
          });
        }
      ...
      if (!isGuest) {
        ...
        try {
          const userResult = await neonHelper.query(
            `SELECT has_used_copilot_trial, member_status, member_expire_at, member_cycle_start_at
        ...
        if (userRow) {
          isMember = isMembershipActive(userRow);
        ...
        if (!isMember && hasUsedTrial) {
          return res.status(403).json({
            ...
            code: 'TRIAL_EXPIRED',
            message: '您的免费生成次数已用完，了解会员服务可获取无限次使用权限。',
          });
        }
      }
 ...
 */
async function fetchCandidateJobs({ userId, background = {}, goal = '', topN = 5, isMember = false, forceRefresh = false, previousRecommendationIds = [] } = {}) {
    ...
    if (userId) {
      ...
      if (userProfile) {
        const personalized = rows.map(job => {
          ...
          const baseTrueScore = clampScore(Number(result?.trueScore ?? result?.totalScore) || 0);
          const breakdown = normalizeBreakdown(result?.breakdown || {});
          const goalAware = composeGoalAwareScore({
            baseScore: baseTrueScore,
            breakdown,
            goal: normalizedGoal,
            language,
            row: job
          });
          const trueScore = baseTrueScore;
          const calibration = calibrateDisplayScore({
            trueScore,
            constraintFlags: result?.constraintFlags || {},
            evidenceCoverage: result?.breakdown?.evidenceCoverage
          });
          const matchLevel = resolveMatchLevelFromDisplayBand(calibration.displayBand);
          const matchDetails = buildCopilotMatchDetails({ score: trueScore, details: breakdown, row: job });
          return {
            job,
            trueScore,
            displayScore: calibration.displayScore,
            displayBand: calibration.displayBand,
            matchLevel,
            matchDetails,
            ...
            constraintFlags: calibration.constraintFlags
          };
        });
        ...
        let visible = personalized.filter(item => item.displayBand !== 'hidden');
        let candidates = applyRecommendationOverlapPolicy(visible, previousRecommendationIds, topN);

        if (candidates.length === 0 && personalized.length > 0) {
          // Generic fallback: keep hard-constrained jobs hidden; only return low-score non-blocked candidates.
          const eligibleFallbacks = personalized
            .filter(item => item.trueScore >= 30 && !hasBlockingConstraintFlags(item.constraintFlags));
          candidates = applyRecommendationOverlapPolicy(eligibleFallbacks, previousRecommendationIds, topN);
        ...
        const result = candidates.map(({ job, trueScore, displayScore, displayBand, matchLevel, matchDetails, goalFitScore, skillAdjacencyScore, aiRecommended, reason }) => ({
          id: job.job_id,
          title: job.title,
          company: job.company,
          ...
          displayMatchScore: displayScore,
          displayBand,
          matchLevel,
          ...
          skillAdjacencyScore,
          aiRecommended,
          matchDetails: isMember && displayBand === 'high' ? matchDetails : null,
          matchDetailsLocked: !isMember && displayBand === 'high',
...
function rankAndFormat(rows, role, seniority, topN, options = {}) {
  ...
  const matchingProfile = buildMatchingProfile({
    targetRole: role,
    experienceYears: experienceBySeniority[String(seniority || '').toLowerCase()] || 0,
    resumeText: `${role} ${seniority}`,
  ...
  const scored = rows.map(job => {
    const matchResult = scoreJobForUserProfile(matchingProfile, job);
    const baseTrueScore = Number(matchResult?.trueScore ?? matchResult?.totalScore) || 0;
    const goalAware = composeGoalAwareScore({
      baseScore: baseTrueScore,
      breakdown: {
        skillMatch: baseTrueScore,
        keywordSimilarity: baseTrueScore,
        experienceMatch: 65,
        preferenceMatch: 60
      },
      goal,
    ...
    const matchLevel = resolveMatchLevelFromDisplayBand(calibration.displayBand);
    const matchDetails = buildCopilotMatchDetails({ score: trueScore, details: matchResult?.breakdown || {}, row: job });
    return {
      job,
      trueScore,
      displayScore: calibration.displayScore,
  ...
  scored.sort((a, b) => b.trueScore - a.trueScore);
  let visible = applyRecommendationOverlapPolicy(
    scored.filter(item => item.displayBand !== 'hidden'),
    previousRecommendationIds,
    topN
  );

  if (visible.length === 0 && scored.length > 0) {
    visible = applyRecommendationOverlapPolicy(
      scored.filter(item => item.trueScore >= 30),
      previousRecommendationIds,
      topN
    );
    visible.forEach(c => {
  ...
  return visible.map(({ job, trueScore, displayScore, displayBand, matchLevel, matchDetails, goalFitScore, skillAdjacencyScore, aiRecommended, reason }) => ({
    id: job.job_id,
    title: job.title,
    company: job.company,
    category: job.category,
  ...
  try {
    ...
    if (result && result.content) {
      ...
      try {
        ...
        const candidateRecommendations = mapCandidateJobsToPlanRecommendations(candidateJobs);
        ...
        const recommendationMeta = buildRecommendationMeta(normalizedRecommendations, []);
        const normalized = {
...
function buildFallbackPlan({ role, timeline, candidateJobs, isMember, goal, language }) {
  ...
  const base = {
    ...
    recommendations: candidateJobs.length > 0
      ? mapCandidateJobsToPlanRecommendations(candidateJobs).map(j => ({
        ...j,
  ...
  if (PLAN_V2_ENABLED) {
    ...
    base.plan_v2 = {
      ...
      recommendationMeta: buildRecommendationMeta(base.recommendations, []),
      modules
    };
  }

  base.recommendationMeta = buildRecommendationMeta(base.recommendations, []);
