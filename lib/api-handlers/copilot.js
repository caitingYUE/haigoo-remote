...
function composeGoalAwareScore({ baseScore = 0, breakdown = {}, goal = '', language = '', row = {} } = {}) {
  ...
  if (!GOAL_AWARE_SCORING_ENABLED) {
    ...
    return {
      ...
      aiRecommended: base >= MATCH_MEDIUM_THRESHOLD && goalFitDisabled >= AI_RECOMMENDED_MIN_GOAL_FIT
    };
  }
  ...
  const aiRecommended = weighted >= MATCH_MEDIUM_THRESHOLD && goalFit >= AI_RECOMMENDED_MIN_GOAL_FIT;
      ...
      try {
        ...
        const aiRecommendations = (parsed.recommendations?.map(r => {
          ...
          return {
            ...
            aiRecommended: inferredLevel === 'high' || inferredLevel === 'medium',
            matchDetails: null,
        ...
        const candidateRecommendations = (candidateJobs || []).map((job) => {
          ...
          return {
            ...
            aiRecommended: Boolean(job.aiRecommended),
  ...
  const base = {
    ...
    recommendations: candidateJobs.length > 0
      ? candidateJobs.map(j => ({
        ...
        aiRecommended: Boolean(j.aiRecommended),
