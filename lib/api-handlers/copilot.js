...
function buildExpandModulePrompt({ module, intent, context = {} } = {}) {
  ...
  const systemPrompt = '你是一名远程求职实战教练。请严格返回 JSON，不要输出 markdown 代码块，不要输出多余说明和铺垫。';
...
要求：roadmap 最多 3 阶段；每阶段 tasks 最多 2 条；resources 最多 3 个。`;
  ...
  if (module === 'apply') {
    const userPrompt = `请为用户输出“投递执行模块”的排期方案。
用户信息：
- 目标：${goalLabel}
- 方向排期：${timeline}
- 核心背景：${role} / ${seniority}
...
要求：
1. 千万不要喊口号，直接基于推荐的岗位类型，制定投递漏斗转化策略。
2. 明确建议第一周投几个试水、通过什么特定渠道（如 RemoteOK, LinkedIn, 官网）。
3. 请为 outreach 提供一份有针对性的极简 Cold Email / LinkedIn 私信模板。

返回 JSON：
{
  "summary": "<投递破局策略，1句话>",
  "weeklyPlan": [
    { "week": "<第X周>", "targetCount": <建议投递数(数字)>, "channels": ["<渠道名1>", "<渠道名2>"], "tasks": ["<执行动作1>", "<动作2>"] }
  ],
  "outreachTemplates": [
    { "scene": "Cold Message / Email", "template": "<模板正文，需留出[Bracket]供用户替换>" }
  ]
}
要求：weeklyPlan 最多 4 阶段，outreachTemplates 1-2 条。`;
    return { systemPrompt, userPrompt, model: 'qwen-plus', maxTokens: 1200 };
  }

  const userPrompt = `请为用户输出“面试实战模块”的定制化方案。
用户信息：
- 目标：${goalLabel}
- 方向：${role} / ${seniority}
- 核心技能提取：${skills || '未解析'}
...
你生成的面试题，绝对不能是通用的泛泛而谈（如“介绍你自己”），必须：
1. 直接针对上方提供的“核心技能”或“目标岗位”中最具争议或难点的场景进行提问。
2. 至少有 2 题是硬核专业结合远程协作能力的。
3. 每题必须附带极高含金量的 STAR 法则回答框架。

返回 JSON：
{
  "summary": "<1句针对 ${role} 岗位的面试破局点>",
  "questions": [
    { "question": "<高质量的刁钻问题>", "why": "<面试官在此题上真正想考察的点>", "answerFramework": ["<S: 情境搭建>", "<T: 核心矛盾>", "<A: 具体行动（结合${skills}）>", "<R: 数据化结果>"] }
  ]
}
要求：questions 控制在 3-5 道，answerFramework 必须是 STAR 结构。`;
...
- applicationPlan.milestones 请按时间顺序规划，覆盖${timelineWeeks}周。绝对禁止超过 5 个大阶段里程碑，请将相近的时间大段合并为 3-5 个阶段输出（严禁按单周生成十几或二十几个阶段的流水账）。每个阶段必须包含多个有价值的行动。`;
