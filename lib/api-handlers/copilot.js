...
function buildCopilotMatchDetails({ score = 0, details = {}, row = {} }) {
  ...
  if (breakdown.titleMatch >= 85) strengths.push('职业方向与岗位标题高度一致');
  if (breakdown.roleTypeMatch >= 82) strengths.push('岗位角色类型与当前定位贴合');
  if (breakdown.skillMatch >= 72) strengths.push('核心技能贴合岗位要求');
  if (breakdown.keywordSimilarity >= 66) strengths.push('简历经历与JD职责关联度较高');
  if (breakdown.experienceMatch >= 70) strengths.push('经验层级匹配岗位预期');
  if (breakdown.preferenceMatch >= 65) strengths.push('岗位类型与求职偏好一致');

  if (breakdown.titleMatch < 55) suggestions.push('把目标岗位方向写得更具体');
  if (breakdown.roleTypeMatch < 55) suggestions.push('补充更贴近该岗位类型的项目成果');
...
function rankAndFormat(rows, role, seniority, topN, options = {}) {
  ...
  let visible = scored.filter(item => item.displayBand !== 'hidden').slice(0, topN);

  if (visible.length === 0 && scored.length > 0) {
    visible = scored.filter(item => item.trueScore >= 30).slice(0, topN);
    visible.forEach(c => {
      c.aiRecommended = false;
      c.reason = '未发现完全匹配该方向的高级岗位，已为您降维匹配当前包含相关技能或通用属性的岗位。';
    });
