    ...
    if (userId) {
      ...
      if (userProfile) {
        const personalized = rows.map(job => {
          ...
          const trueScore = goalAware.adjustedScore;
...
function rankAndFormat(rows, role, seniority, topN, options = {}) {
  ...
  const scored = rows.map(job => {
    ...
    const baseTrueScore = Math.round(calculateSimilarity(userProfile, jobText) * 100);
    const goalAware = composeGoalAwareScore({
      baseScore: baseTrueScore,
      breakdown: {
    ...
    const trueScore = goalAware.adjustedScore;
    const calibration = calibrateDisplayScore({ trueScore });
    const matchLevel = resolveMatchLevelFromDisplayBand(calibration.displayBand);
    const matchDetails = buildCopilotMatchDetails({ score: trueScore, details: {}, row: job });
    return {
      job,
      trueScore,
      displayScore: calibration.displayScore,
