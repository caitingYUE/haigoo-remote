  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (sessionResult && sessionResult.length > 0) {
        ...
        let background = { role: '', years: '中级', education: '本科', language: '英语-工作 (B2)' };
        try { background = JSON.parse(session.background); } catch (e) { }
    ...
    if (req.method === 'POST') {
      ...
      if (action && action !== 'generate') {
        ...
        const memberOnlyActions = ['create-plan', 'update-progress', 'align-resume', 'interview-prep', 'mock-interview', 'expand-module', 'generate-interview-plan', 'generate-answer'];
...
请根据以上前提，生成投递与面试节奏规划，指导如何破局。

要求：
1. 请根据每周投入时长合理分配任务密度（高频投递或是精准低频投递）。
2. 提供一版有针对性的冷启动 Outreach/Cover Letter 模板。
...
要求：weeklyPlan 最多 4 阶段，outreachTemplates 1-2 条。`;
  ...
  const userPrompt = `根据以下岗位与候选人背景，生成针对远程工作的实战模拟英文面试问题及参考答案。
用户信息：
目标：${goalLabel}
设定：${role} / ${seniority}
英语水平：${language}
...
2. 给出至少 3 个具体面试题，并严格提供高含金量的 STAR 回答架构。
3. 提供一句总体破局建议。
...
输出JSON：
{
  ...
  "questions": [
    {
      "question": "<高质量英文刁钻问题>",
      "why": "<面试官在此题考察点（含沟通与专业）>",
      "answerFramework": ["<S: 情境搭建>", "<T: 核心矛盾>", "<A: 具体行动（如结合${skills}）>", "<R: 数据化结果>"]
    }
  ],
  "tips": ["<语言交流与面试进阶技巧1>", "<技巧2>"]
}
要求：questions 控制在 3-5 道。`;
...
请返回以下 JSON（中文）：
{
  ...
  "interviewPrep": {
    "keyThemes": ["<核心面试考察点1>", "<考察点2>"],
    "sampleQA": [
      { "question": "<高频面试题>", "answerHint": "<回答要点>" }
    ],
    "languageTip": "<针对目标岗位的英语沟通建议>"
  }
}
