...
function buildInitialPlanModules(parsed = {}) {
  const interviewQuestions = Array.isArray(parsed?.interviewPrep?.sampleQA)
    ? parsed.interviewPrep.sampleQA.map(item => item?.question).filter(Boolean).slice(0, 5)
    : [];
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action && action !== 'generate') {
        ...
        const memberOnlyActions = ['create-plan', 'update-progress', 'align-resume', 'interview-prep', 'mock-interview', 'expand-module'];
  ...
  try {
    ...
    if (userId) {
      ...
      if (userProfile) {
        ...
        const result = candidates.map(({ job, score, matchLevel, matchDetails, goalFitScore, skillAdjacencyScore, aiRecommended }) => ({
          id: job.job_id,
          title: job.title,
          company: job.company,
...
function rankAndFormat(rows, role, seniority, topN, options = {}) {
  ...
  return visible.map(({ job, score, matchLevel, matchDetails, goalFitScore, skillAdjacencyScore, aiRecommended }) => ({
    id: job.job_id,
    title: job.title,
    company: job.company,
    category: job.category,
...
function buildExpandModulePrompt({ module, intent, context = {} } = {}) {
  ...
  const recommendations = (context.recommendations || []).slice(0, 5)
    .map((item, idx) => `${idx + 1}. ${item.title} @ ${item.company || 'N/A'} (${item.matchLabel || ''})`)
    .join('\n');
  const skills = Array.isArray(context.resumeSkills) ? context.resumeSkills.slice(0, 10).join('、') : '';
  const question = context.question ? `\n用户指定问题：${context.question}` : '';

  const systemPrompt = '你是一名远程求职教练。请严格返回 JSON，不要输出 markdown 代码块，不要输出多余说明。';

  if (module === 'language') {
    const userPrompt = `请为用户输出“语言准备模块”的深度方案。
用户画像：
- 目标：${goalLabel}
- 时间：${timeline}
- 方向：${role} / ${seniority}
- 语言水平：${language}
- 远程适配结论：${readinessLevel} ${readiness}
- 技能关键词：${skills || '暂无'}
- 推荐岗位：
${recommendations || '暂无'}

当前请求：${intent === 'resources' ? '扩展学习资源与工具建议' : '生成完整语言提升路线'}

返回 JSON：
{
  "summary": "<1-2句结论>",
  "roadmap": [
    { "phase": "<阶段，如第1周>", "focus": "<重点>", "tasks": ["<任务1>", "<任务2>"] }
  ],
  "tips": ["<建议1>", "<建议2>"],
  "resources": [
    { "name": "<资源名>", "type": "<app|site|course|community>", "url": "<可选>", "reason": "<推荐理由>" }
  ]
}
要求：roadmap 最多 4 阶段；每阶段 tasks 最多 3 条；resources 最多 6 条。`;
    return { systemPrompt, userPrompt, model: 'qwen-plus', maxTokens: 1200 };
  }

  if (module === 'apply') {
    const applyStrategy = intent === 'sprint' ? '强力的 2 周投递冲刺节奏，要求分定级（保底、激进、稳妥）进行策略排期' : '基于推荐岗位匹配情况的完整投递推进方案';
    const userPrompt = `请为用户输出“投递计划模块”的深度方案。
用户信息与上下文：
- 目标：${goalLabel}
- 时间序列排期预估：${timeline}
- 核心背景：${role} / ${seniority}
- 当前远程适配综合情况：${readinessLevel} ${readiness}
- 简历提取的核心技能：${skills || '尚未解析或暂无'}
- 系统目前为您锁定出的高优推荐岗位参考：
${recommendations || '暂无数据'}

当前请求：为用户制订 ${applyStrategy}。你在制订排期和建议渠道时，必须结合上方提供的“简历核心技能”与“推荐岗位参考”来举例（比如建议在第几周定向投递上方列表中的某个匹配度最高的岗位或同类岗位）。

返回 JSON 数据格式要求：
{
  "summary": "<整体策略>",
  "weeklyPlan": [
    { "week": "<周次>", "targetCount": <数字>, "channels": ["<渠道1>", "<渠道2>"], "tasks": ["<任务1>", "<任务2>"] }
  ],
  "outreachTemplates": [
    { "scene": "<场景>", "template": "<联系模板>" }
  ],
  "metrics": [
    { "name": "<指标>", "target": "<目标值>", "why": "<原因>" }
  ]
}
要求：weeklyPlan 最多 4 周；template 最多 2 条；metrics 3 条以内。`;
    return { systemPrompt, userPrompt, model: 'qwen-plus', maxTokens: 1200 };
  }

  const interviewGoal = intent === 'mock-answer' ? '重点针对推荐岗位的特质生成优质模拟回答与优化建议' : intent === 'more-questions' ? '生成极具针对性的高频实战问题' : '生成基于该用户背景的完整面试准备方案';
  const userPrompt = `请为用户输出“面试准备模块”的深度方案。
用户信息与上下文：
- 目标：${goalLabel}
- 方向级别：${role} / ${seniority}
- 语言水平：${language}
- 当前远程适配情况：${readinessLevel} ${readiness}
- 简历提取的核心经历/技能点：${skills || '尚未解析'}
- 系统目标投递的参考岗位：
${recommendations || '暂无数据'}${question}

当前请求任务：${interviewGoal}。
你生成的这 3 到 5 道面试题，绝不能只是干瘪的行为测试题，必须有至少两道题能够紧密贴合上方提供的“简历核心技能点”，以及“目标参考岗位”的实际场景。另外必须附带一份基于 STAR 法则的黄金答题结构解析。数量控制在最经典代表性的 3 道题即可。

返回 JSON 数据格式要求：
{
  "summary": "<1句总结>",
  "questions": [
    { "question": "<问题>", "why": "<考察点>", "answerFramework": ["<要点1>", "<要点2>", "<要点3>"] }
  ],
  "mockAnswer": {
    "question": "<问题>",
    "answer": "<示例回答>",
...
要求：questions 5-8 条；answerFramework 每题最多3条；mockAnswer 可为空。`;
  return { systemPrompt, userPrompt, model: 'qwen-plus', maxTokens: 1400 };
}
...
async function handleExpandModule({ userId, body = {}, isMember = false } = {}) {
  ...
  const latestRows = await neonHelper.query(
    `SELECT goal, timeline, background, plan_data, created_at
...
要求：
- recommendations 必须优先匹配用户目标类型；若用户目标为兼职/副业增收，不要首推全职岗位。
- 若推荐岗位与目标存在偏差，必须在 reason 中给出偏差解释和过渡路径。
applicationPlan.milestones 请按时间顺序规划，覆盖${timelineWeeks}周。`;
  ...
  由于未登录且未提供详细简历，请返回如下 JSON 结构的数据兜底分析：
  {
    "remoteReadiness": {
      "level": "transformable",
      "score": 50,
      "summary": "当前可尝试远程求职，但建议先补齐基础能力后集中投递。",
      "gaps": ["远程协作经验证明不足", "语言表达有待加强"],
      "actions": ["先完善1个可展示项目", "准备英文自我介绍与常见问答"]
    },
