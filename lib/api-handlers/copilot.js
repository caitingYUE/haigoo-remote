...
请返回以下 JSON（中文）：
{
  ...
  "interviewPrep": {
    ...
    "sampleQA": [
      { "question": "<高质量英文面试题1>", "answerHint": "<针对候选人背景的回答要点，STAR原则>" },
      { "question": "<题2>", "answerHint": "<要点>" },
      { "question": "<题3>", "answerHint": "<要点>" },
      { "question": "<题4>", "answerHint": "<要点>" },
      { "question": "<题5>", "answerHint": "<要点>" }
    ],
...
- applicationPlan.milestones（即关键行动路线）必须生成 3-7 个阶段。请根据用户预期时间和投入时间（每周${investedHours || '5-10'}小时）进行整体倒推排期（Backward Planning），确保行动量级符合其时间精力！阶段命名要体现递进关系。
- 绝对禁止单周流水账，务必提炼出 3-7 个具备极强执行指导意义的核心阶段，并在 actions 里给出具体的周度拆解行动方案。
- actions 字段下每一项必须是简洁的中文行动字符串（不能是 JSON 对象），每条控制在 30 字以内。`;
  ...
  const maxTokens = isMember ? 3500 : 2000; // Increased: member plan with 3-7 milestones + sampleQA needs headroom
