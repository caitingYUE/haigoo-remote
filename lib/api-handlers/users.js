...
function shouldCreateMembershipActivationNotification(previousUser, updatedUser) {
  if (!previousUser || !updatedUser?.user_id) return false
  return !isMembershipActive(previousUser) && isMembershipActive(updatedUser)
}
...
async function createMembershipActivatedNotification(user) {
  if (!neonHelper?.isConfigured || !user?.user_id) return

  const memberTypeLabel = getMembershipTypeLabel(user.memberType || user.member_type || user.membershipLevel || user.membership_level)
  const startAt = formatDateTimeLabel(user.memberCycleStartAt || user.member_cycle_start_at || user.memberSince || user.member_since)
  const expireAt = formatDateTimeLabel(user.memberExpireAt || user.member_expire_at)
  const title = '会员已生效'
  const content = [
    `Hi ${user.username || user.profile?.fullName || user.fullName || '你好'}，您的${memberTypeLabel}已开通成功。`,
    `生效时间：${startAt}`,
    `失效时间：${expireAt}`
  ].join('\n')
  ...
  await neonHelper.query(
    ...
    [user.user_id, 'membership_activated', title, content]
  )
}
  ...
  if (req.method === 'PATCH') {
    try {
      ...
      if (shouldSendMembershipActivationEmail(previousUser, result.user)) {
        ...
        if (shouldCreateMembershipActivationNotification(previousUser, result.user)) {
          try {
            await createMembershipActivatedNotification(result.user)
          } catch (notificationError) {
            console.error('[users] Failed to create membership notification:', notificationError)
          }
        }
      }
