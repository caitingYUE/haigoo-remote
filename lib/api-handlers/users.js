...
export default async function handler(req, res) {
  ...
  if (req.method === 'PATCH') {
    try {
      const { id, status, username, roles, memberStatus, memberExpireAt, memberType, autoApplyMemberDuration } = req.body || {}
      if (!id) {
      ...
      const result = await userHelper.updateUser(
        id,
        { status, username, roles, memberStatus, memberExpireAt, memberType, autoApplyMemberDuration },
        { isAdmin: true }
      )
