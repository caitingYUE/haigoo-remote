...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && action === 'status') {
      ...
      return res.status(200).json({
        ...
        membership: {
          ...
          isActive: user.membershipExpireAt ? new Date(user.membershipExpireAt) > new Date() : false
        }
      })
    }
    ...
    if (req.method === 'POST' && action === 'confirm-payment') {
      ...
      // Direct SQL update because userHelper.updateUser doesn't support these fields yet (or I need to add them to allow list)
      // Actually I updated userHelper to READ them, but update logic needs to be checked.
      
      await neonHelper.update('users', {
          membership_level: plan.id.includes('club_go') ? 'club_go' : 'goo_plus',
