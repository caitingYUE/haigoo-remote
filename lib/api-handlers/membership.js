...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && action === 'confirm-payment') {
      ...
      // Ensure member_display_id exists
      let memberDisplayId = user.memberDisplayId || user.member_display_id
      if (!memberDisplayId) {
          try {
             // Try to get next value from sequence
             // Note: Sequence must exist (created via DDL)
             const seqRes = await neonHelper.query("SELECT nextval('member_id_seq') as id")
             if (seqRes && seqRes[0]) {
                 memberDisplayId = seqRes[0].id
             }
          } catch (e) {
             console.error('Failed to generate member_display_id:', e)
             // Fallback or ignore? Ideally we should have the sequence.
             // If sequence fails, we might leave it null for now.
          }
      }
      ...
      const updateData = {
          ...
          member_since: now.toISOString(), // Always update member_since to latest start date
      ...
      if (memberDisplayId) {
          updateData.member_display_id = memberDisplayId
      }
