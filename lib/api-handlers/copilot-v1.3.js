...
async function findMatchingJobs({ structured = null, manualDirection = '', positionType = 'full-time', resumeHints = [], limit = 20, recentRecommendationIds = [] }) {
...
function selectDiversifiedHeroMatches(scored = [], limit = 5, recentRecommendationIds = []) {
    ...
    const tryAppend = (item, allowRecentRepeat = false) => {
...
export async function handleHeroRecommendations(userId, body) {
    ...
    const matches = await findMatchingJobs({
        ...
        positionType,
        resumeHints,
        recentRecommendationIds: normalizeRecentRecommendationIds(recentRecommendationIds),
