...
export async function handleCreatePlan(userId, body) {
    ...
    const prompt = `根据用户目标与时间规划，生成针对性的远程求职里程碑计划。

用户信息：
目标：${goalLabel}
可用时间：总计 ${timelineWeeks} 周
当前远程就绪度评估：${readiness?.remote_readiness_score || '未评估'}
${gapsBlock}
${skillsBlock}

【核心指令】：
该行动计划虽然横跨 ${timelineWeeks} 周，但你必须将其聚合成仅仅 3 到 5 个【大阶段核心里程碑】。
绝对禁止按周拆分出超过 5 个阶段（例如，严禁直接输出 24 个单周阶段，那会导致过度繁杂）！
每个大阶段的 phase_name 必须注明涵盖的时间跨度（例如："第1-4周：简历重塑与基建"）。

必须输出严格符合此JSON格式的结果：
{
  "phases": [
    {
      "phase_name": "第X-Y周：具体的阶段性核心目标（如：简历重塑与基建）",
      "phase_key": "准备或投递或面试或入职",
      "duration_weeks": 包含周数的数字或区间,
      "focus": "本阶段核心聚焦的破局点或难点",
      "tasks": [
        {
          "task_name": "具体微观任务要求（必须非常具体且可执行）",
          "type": "准备或投递或面试或入职",
...
要求：
1. 每个阶段必须包含真实的周度排期在 phase_name 前面（例如 "第1-2周：..."）。
2. 每阶段必须带有 2 到 4 个任务。
3. 请只输出要求的纯JSON内容，不要附带任何解释文本。`;
