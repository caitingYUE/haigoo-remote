...
async function upsertUserState(userId, updates) {
  ...
  `;
    ...
    try {
        ...
        console.warn('[Copilot V1.3] upsertUserState skipped (table may not exist):', e.message);
    }
}

// Safe query against copilot_user_state — returns [] if table doesn't exist
async function safeQueryUserState(selectCols, userId) {
    try {
        return await neonHelper.query(
            `SELECT ${selectCols} FROM copilot_user_state WHERE user_id = $1`, [userId]
        ) || [];
    } catch (e) {
        console.warn('[Copilot V1.3] safeQueryUserState skipped:', e.message);
        return [];
    }
}
...
输出JSON：
{
  ...
  "remote_related_experience": []
    ...
    try {
        ...
        // 3. Save to state
        const currentState = await safeQueryUserState('resume_version', userId);
...
export async function handleAssess(userId, body) {
    ...
    if (!forceRefresh) {
        const existing = await safeQueryUserState('readiness_data, readiness_generated_at, resume_version', userId);
    ...
    const stateRows = await safeQueryUserState('resume_structured', userId);
...
export async function handleCreatePlan(userId, body) {
    ...
    const stateRows = await safeQueryUserState('readiness_data, resume_structured', userId);
...
export async function handleGenerateInterviewPlan(userId, body) {
    ...
    const stateRows = await safeQueryUserState('resume_structured, readiness_data', userId);
    const state = stateRows?.[0];
    const resumeStructured = state?.resume_structured;
    const readiness = state?.readiness_data;
...
export async function handleGenerateAnswer(userId, body) {
    ...
    // Get resume data for personalization (graceful fallback if table doesn't exist)
    const stateRows = await safeQueryUserState('resume_structured', userId);
    const resumeStructured = stateRows?.[0]?.resume_structured;
    const skills = (resumeStructured?.skills || []).slice(0, 8).join(', ') || '';
    const achievements = (resumeStructured?.achievements_with_metrics || []).slice(0, 3).join('; ') || '';
