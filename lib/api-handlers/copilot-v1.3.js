...
async function findMatchingJobs({ structured = null, manualDirection = '', positionType = 'full-time', resumeHints = [], resumeName = '', limit = 20, recentRecommendationIds = [] }) {
    ...
    const scored = rows.map(job => {
        ...
        const directionScore = computeDirectionScore(directionTerms, resolvedDirection, job);
    ...
    return diversified.map(({ job, score, signals }) => {
...
export async function handleHeroRecommendations(userId, body) {
    ...
    if (Array.isArray(storedTodayMatches) && storedTodayMatches.length > 0) {
        return {
            success: true,
            cached: true,
