...
let copilotUserStateTableAvailable = null;
async function hasCopilotUserStateTable() {
    if (copilotUserStateTableAvailable !== null) return copilotUserStateTableAvailable;
    try {
        const rows = await neonHelper.query(
            `SELECT EXISTS (
                SELECT 1
                FROM information_schema.tables
                WHERE table_schema = 'public' AND table_name = 'copilot_user_state'
            ) AS exists`
        );
        copilotUserStateTableAvailable = !!rows?.[0]?.exists;
    } catch {
        copilotUserStateTableAvailable = false;
    }
    return copilotUserStateTableAvailable;
}
...
async function findMatchingJobs({ structured = null, manualDirection = '', positionType = 'full-time', resumeHints = [], limit = 20 }) {
    ...
    const scored = rows.map(job => {
        ...
        const jobText = `${job.title || ''} ${job.category || ''} ${jobType} ${(job.description || '').substring(0, 900)}`.toLowerCase();
...
export async function handleGenerateInterviewPlan(userId, body = {}) {
    ...
    const focusTasks = tasks
        .filter((task) => ['interview', 'resume', 'apply'].includes(String(task.phase || '').toLowerCase()))
        .slice(0, 8)
        .map((task) => task.task_name)
        .filter(Boolean);

    const phases = Array.isArray(planData?.phases) ? planData.phases : [];
    const interviewItems = phases
        .flatMap((phase) => Array.isArray(phase?.tasks) ? phase.tasks : [])
        .map((task) => task?.task_name || task?.task || '')
        .filter(Boolean)
        .slice(0, 6);

    const plan = {
        summary: body?.jobTitle
            ? `已基于 ${body.jobTitle} 生成面试准备重点，建议优先围绕岗位核心职责做结构化演练。`
            : '已基于当前求职方案生成面试准备重点，建议先完成高频题结构化演练。',
        keyThemes: focusTasks.slice(0, 4),
        sampleQA: (interviewItems.length > 0 ? interviewItems : [
            '请用 STAR 结构介绍你最有代表性的项目。',
            '你如何处理远程协作中的沟通延迟与信息缺失？',
            '为什么你适合这个远程岗位？'
        ]).map((question) => ({ question })),
        languageTip: '优先准备 60 秒英文自我介绍，并将项目成果表达成可量化结果。'
    };

    return { success: true, interviewPlan: plan };
}
...
export async function handleGenerateAnswer(userId, body = {}) {
    ...
    const prompt = `请基于用户远程求职背景，为下面的面试问题生成一版简洁、可直接练习的中文回答草稿。
...
输出 JSON：
{
  ...
  "followUp": "<建议继续补充的一点>"
    ...
    const raw = await callModel(prompt, SYSTEM_PROMPT_JSON, 'qwen-plus', 600);
    if (!raw) {
        return {
            success: true,
            answer: '建议用 STAR 结构回答：先说明背景与目标，再讲你的关键动作，最后量化结果，并补充你如何在远程环境里推进协作。',
