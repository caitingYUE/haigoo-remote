 ...
 *   - extract-resume:   Deterministic local resume extraction (zero token cost)
...
import { extractStructuredResume } from '../services/resume-structure-extractor.js';
import { buildMatchingProfile, scoreJobForUserProfile } from '../services/matching-engine.js';
...
export function expandDirectionTerms(input = '') {
...
export function computeDirectionScore(directionTerms, manualDirection, job) {
...
async function hasHeroRecommendationRunsTable() {
    return hasTable('hero_job_recommendation_runs');
}
...
export function normalizeHeroRecommendationDateKey(value) {
    const raw = String(value || '').trim();
    if (/^\d{4}-\d{2}-\d{2}$/.test(raw)) return raw;
    return new Date().toISOString().slice(0, 10);
}

export function normalizeHeroRecommendationContextKey(value, fallback = 'default::full-time::profile') {
...
function mapHeroRecommendationRunToHistory(row = {}) {
    const jobIds = parseStoredJson(row?.job_ids, []);
    const generatedAt = row?.generated_at ? new Date(row.generated_at).getTime() : Date.now();
    return {
        contextKey: String(row?.context_key || '').trim(),
        dateKey: String(row?.recommendation_date || '').slice(0, 10),
        jobIds: Array.isArray(jobIds) ? jobIds.map(id => String(id || '').trim()).filter(Boolean) : [],
        updatedAt: Number.isFinite(generatedAt) ? generatedAt : Date.now(),
    };
}

export function findHeroRecommendationRunForDate(rows = [], contextKey, dateKey) {
    return (rows || []).find(row => (
        String(row?.context_key || '') === contextKey &&
        String(row?.recommendation_date || '').slice(0, 10) === dateKey
    )) || null;
}

export function getRecentHeroRecommendationJobIds(rows = [], runCount = 3) {
    return Array.from(new Set(
        (rows || [])
            .slice(0, Math.max(0, Number(runCount) || 0))
            .flatMap(row => {
                const jobIds = parseStoredJson(row?.job_ids, []);
...
async function readHeroRecommendationRuns(userId, contextKey, dateKey) {
    if (!userId || !(await hasHeroRecommendationRunsTable())) {
        return { today: null, recent: [], history: [] };
    }
    ...
    try {
        const rows = await neonHelper.query(
            `SELECT context_key,
                    ...
                    job_ids,
                    generated_at
             FROM hero_job_recommendation_runs
             WHERE user_id = $1
             ORDER BY generated_at DESC
             LIMIT 12`,
            [userId]
        ) || [];
        const today = findHeroRecommendationRunForDate(rows, contextKey, dateKey);
        return {
            today,
            recent: rows.slice(0, 3),
            history: rows.map(mapHeroRecommendationRunToHistory).filter(item => item.contextKey && item.dateKey && item.jobIds.length > 0),
        };
    } catch (error) {
        console.warn('[Copilot V1.3] Failed to read hero recommendation runs:', error.message);
        return { today: null, recent: [], history: [] };
    }
}

async function persistHeroRecommendationRun({ userId, contextKey, dateKey, recommendations, inputSnapshot }) {
    if (!userId || !(await hasHeroRecommendationRunsTable())) return null;

    const jobIds = (recommendations || []).map(item => String(item?.id || item?.jobId || '').trim()).filter(Boolean);
    try {
        const inserted = await neonHelper.query(
            `INSERT INTO hero_job_recommendation_runs
               (user_id, context_key, recommendation_date, recommendations, job_ids, input_snapshot, generated_at)
             VALUES ($1, $2, $3::date, $4::jsonb, $5::jsonb, $6::jsonb, NOW())
             ON CONFLICT (user_id, context_key, recommendation_date) DO NOTHING
        ...
        const existing = await neonHelper.query(
            `SELECT context_key,
                    ...
                    generated_at
             FROM hero_job_recommendation_runs
             WHERE user_id = $1 AND context_key = $2 AND recommendation_date = $3::date
             LIMIT 1`,
            [userId, contextKey, dateKey]
        ) || [];
        return existing[0] || null;
    } catch (error) {
        console.warn('[Copilot V1.3] Failed to persist hero recommendation run:', error.message);
        return null;
    }
}
...
async function queryHeroCandidates(query, params = []) {
    let lastError;
    for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
            return await neonHelper.query(query, params);
        } catch (error) {
            lastError = error;
...
async function findMatchingJobs({ structured = null, manualDirection = '', positionType = 'full-time', resumeHints = [], resumeName = '', limit = 20, recentRecommendationIds = [] }) {
    const rawManualDirection = String(manualDirection || '').trim();
    const isGenericRemoteDirection = /^(远程工作|remote work|remote job|remote)$/i.test(rawManualDirection);
    const structuredDirection = normalizeTerms([
        structured?.targetRole,
        structured?.target_role,
        structured?.roles,
        structured?.industries,
    ]).slice(0, 12);
    const resumeNameDirection = String(resumeName || '')
        .replace(/\.(pdf|docx?|rtf|txt)$/i, '')
        .trim();
    const resolvedDirection = isGenericRemoteDirection
        ? (structuredDirection.join(' ') || resumeNameDirection)
        : rawManualDirection;
    const directionTerms = normalizeTerms([
        expandDirectionTerms(resolvedDirection),
        structuredDirection,
    ]);
    ...
    const heroProfile = buildMatchingProfile({
        targetRole: resolvedDirection,
        roles: structured?.roles,
        roleFamilies: structured?.roleFamilies || structured?.role_families,
        skills: skillTerms,
        tools: structured?.tools,
        experienceYears: structured?.experienceYears || structured?.years_of_experience,
        industries: structured?.industries,
        languages: structured?.languages,
        resumeText: resumeVectorTerms.join(' '),
        preferences: {
            jobTypes: positionType ? [positionType] : [],
        },
        hasResume: Boolean(structured || resumeHints.length),
        evidenceCoverage: Number(structured?.evidence_coverage) || (resolvedDirection ? 0.65 : 0.35),
        profileVersion: structured?.parser_version || 'hero-profile-v1'
    });
    let rows = [];
    try {
        const featureRows = await queryHeroCandidates(
            `SELECT job_id, title, category, industry, tags, requirements,
                    experience_level, job_type, location, region, timezone, published_at
             FROM jobs
        ...
        const preliminary = featureRows.map(row => {
            const result = scoreJobForUserProfile(heroProfile, row);
            const direction = Math.max(
                Number(result?.breakdown?.titleMatch || 0),
                Number(result?.breakdown?.roleTypeMatch || 0)
            );
            return {
                id: String(row.job_id),
                score: Number(result?.trueScore || 0),
                direction,
                publishedAt: new Date(row.published_at || 0).getTime()
            };
        });
        const directionCandidates = directionTerms.length
            ? preliminary.filter(item => item.direction >= 42)
            : preliminary;
        const candidatePool = (directionCandidates.length >= limit ? directionCandidates : preliminary)
            .sort((a, b) => b.score - a.score || b.publishedAt - a.publishedAt);
        const candidateLimit = Math.min(100, Math.max(40, limit * 4));
        const candidateIds = candidatePool.slice(0, candidateLimit).map(item => item.id);
        if (!candidateIds.length) return [];

        let rawRows = [];
        try {
            rawRows = await queryHeroCandidates(
                `SELECT job_id, title, company, company_id, category, industry, description,
                        tags, requirements, experience_level, job_type, location, region,
                        timezone, published_at, created_at, salary, is_translated, is_trusted
                 FROM jobs
                 WHERE job_id = ANY($1)`,
                [candidateIds]
            ) || [];
        } catch (detailError) {
            console.warn('[Copilot V1.3] candidate details query failed, using feature rows:', detailError.message);
            const candidateIdSet = new Set(candidateIds);
            rawRows = featureRows.filter(row => candidateIdSet.has(String(row.job_id)));
        }
    ...
    const scored = rows.map(job => {
        const result = scoreJobForUserProfile(heroProfile, job);
        const trueScore = Number(result?.trueScore ?? result?.totalScore) || 0;
        const score = trueScore / 100;
        const hasStrongDirectionHit = directionTerms.length === 0 ||
            Number(result?.breakdown?.titleMatch || 0) >= 55 ||
            Number(result?.breakdown?.roleTypeMatch || 0) >= 70;
        ...
        return {
            ...
            signals: {
                direction: Number(result?.breakdown?.titleMatch || result?.breakdown?.roleTypeMatch || 0),
                positionType: Number(result?.breakdown?.preferenceMatch || 0),
                skillTags: Number(result?.breakdown?.skillMatch || 0),
                resumeSemantic: Number(result?.breakdown?.keywordSimilarity || 0),
                experience: Number(result?.breakdown?.experienceMatch || 0),
            },
    ...
    const diversified = selectDiversifiedHeroMatches(ranked, limit, recentRecommendationIds);

    const presentationIds = diversified.map(item => String(item.job?.id || '')).filter(Boolean);
    let presentationLookup = new Map();
    if (presentationIds.length) {
        try {
            const presentationRows = await queryHeroCandidates(
                `SELECT job_id, company_logo, company_website, company_description, url, translations
                 FROM jobs
                 WHERE job_id = ANY($1)`,
                [presentationIds]
            ) || [];
            presentationLookup = new Map(presentationRows.map(row => [String(row.job_id), row]));
        } catch (error) {
            console.warn('[Copilot V1.3] presentation fields query error:', error.message);
        }
    }

    return diversified.map(({ job, score, signals, breakdown, constraintFlags }) => {
        const presentation = presentationLookup.get(String(job.id)) || {};
        ...
        return {
            ...
            companyIntro: presentation.company_description || job.description || '',
            companyDescription: presentation.company_description || '',
            companyTranslations: null,
            logo: presentation.company_logo || '',
            companyWebsite: presentation.company_website || '',
            sourceUrl: presentation.url || '',
            url: presentation.url || '',
            timezone: job.timezone || '',
            translations: presentation.translations || null,
...
function normalizeRecentRecommendationIds(raw = []) {
...
function getRecommendationCompanyKey(job = {}) {
    const companyId = job?.companyId ?? job?.company_id ?? job?.company_id_snapshot;
    if (companyId !== null && companyId !== undefined && String(companyId).trim()) {
        return `id:${String(companyId).trim().toLowerCase()}`;
    }
...
export function selectDiversifiedHeroMatches(scored = [], limit = 5, recentRecommendationIds = []) {
    const normalizedLimit = Math.max(1, Number(limit) || 1);
    const recentIds = new Set(normalizeRecentRecommendationIds(recentRecommendationIds));
    const selected = [];
    const selectedIds = new Set();
    const companyCounts = new Map();

    const tryAppend = (item, allowCompanyRepeat = false) => {
        if (!item || selected.length >= normalizedLimit) return false;

        const jobId = String(item?.job?.id || '').trim();
        if (!jobId || selectedIds.has(jobId)) return false;
        if (recentIds.has(jobId)) return false;

        const companyKey = getRecommendationCompanyKey(item?.job);
        const companyCount = companyKey ? (companyCounts.get(companyKey) || 0) : 0;
        if (!allowCompanyRepeat && companyKey && companyCount >= 2) return false;
...
export async function handleExtractResume(userId, resumeId) {
    ...
    // 2. Deterministic local extraction. Matching must not consume AI tokens.
    try {
...
export async function handleHeroRecommendations(userId, body) {
    const {
        ...
        resumeId,
        resumeName = '',
        resumeHints: incomingResumeHints = [],
        forceResumeExtraction = false,
        recentRecommendationIds = [],
        contextKey: incomingContextKey,
    ...
    const dateKey = normalizeHeroRecommendationDateKey(incomingDateKey);
    const fallbackContextKey = [
        String(jobDirection || '').trim().toLowerCase(),
        String(positionType || 'full-time').trim().toLowerCase(),
        resumeId ? `resume:${String(resumeId).trim()}` : 'profile',
    ].join('::');
    const contextKey = normalizeHeroRecommendationContextKey(incomingContextKey, fallbackContextKey);
    const storedRuns = userId
        ? await readHeroRecommendationRuns(userId, contextKey, dateKey)
        : { today: null, recent: [], history: [] };
    const storedTodayMatches = parseStoredJson(storedRuns.today?.recommendations, []);

    const hasCurrentRecommendationVersion = Array.isArray(storedTodayMatches) &&
        storedTodayMatches.length > 0 &&
        storedTodayMatches.every(item =>
            item?.algorithmVersion === MATCH_ALGORITHM_VERSION &&
            item?.calibrationVersion === MATCH_CALIBRATION_VERSION
        );

    if (hasCurrentRecommendationVersion) {
        return {
            success: true,
            cached: true,
            matches: storedTodayMatches.slice(0, Math.min(Math.max(Number(limit) || 1, 1), 5)),
            total: storedTodayMatches.length,
            generatedAt: storedRuns.today.generated_at,
            contextKey,
            recommendationHistory: storedRuns.history,
        };
    }
    ...
    if (userId && resumeId && (!structured || forceResumeExtraction)) {
    ...
    const serverRecentRecommendationIds = getRecentHeroRecommendationJobIds(storedRuns.recent, 3);
    const exclusionIds = normalizeRecentRecommendationIds([
        ...serverRecentRecommendationIds,
        ...normalizeRecentRecommendationIds(recentRecommendationIds),
    ]);
    ...
    const matches = await findMatchingJobs({
        ...
        resumeHints,
        resumeName,
        recentRecommendationIds: exclusionIds,
    ...
    const persistedRun = userId
        ? await persistHeroRecommendationRun({
            userId,
            contextKey,
            dateKey,
            recommendations: matches,
            inputSnapshot: {
                jobDirection: String(jobDirection).trim(),
                positionType,
                resumeId: resumeId || null,
                resumeName: String(resumeName || '').slice(0, 240),
                hasStructuredResume: Boolean(structured),
            },
        })
        : null;
    const persistedMatches = parseStoredJson(persistedRun?.recommendations, matches);
    const latestRuns = userId
        ? await readHeroRecommendationRuns(userId, contextKey, dateKey)
        : storedRuns;
    ...
    return {
        success: true,
        cached: false,
        matches: Array.isArray(persistedMatches) ? persistedMatches : matches,
        total: Array.isArray(persistedMatches) ? persistedMatches.length : matches.length,
        generatedAt: persistedRun?.generated_at || new Date().toISOString(),
        contextKey,
        recommendationHistory: latestRuns.history,
    };
}
