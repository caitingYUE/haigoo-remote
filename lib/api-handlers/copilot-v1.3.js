...
async function findMatchingJobs({ structured = null, manualDirection = '', positionType = 'full-time', resumeHints = [], limit = 20 }) {
    ...
    const conditions = likeTerms.length > 0
        ? likeTerms.map((_, i) => `(LOWER(title) LIKE $${i + 1} OR LOWER(COALESCE(category, '')) LIKE $${i + 1} OR LOWER(COALESCE(description, '')) LIKE $${i + 1} OR LOWER(COALESCE(job_type, '')) LIKE $${i + 1} OR LOWER(COALESCE(tags::text, '')) LIKE $${i + 1})`).join(' OR ')
        : `TRUE`;
    ...
    try {
        rows = await neonHelper.query(
            `SELECT job_id, title, company, category, location, job_type,
              experience_level, salary, description, published_at, logo, company_website, tags
             FROM jobs
  ...
  "remote_related_experience": ["经历1"]
    ...
    try {
       ...
       VALUES ($1, 'extract', '', '{}', $2, false, 'extract')`,
            ...
            console.warn('[Copilot V1.3] extract-resume session log skipped:', e.message);
        }
  ...
  "estimated_timeline": "预估拿到offer的时间"
    ...
    try {
       ...
       VALUES ($1, $2, $3, $4, $5, false, 'assess')`,
            ...
            console.warn('[Copilot V1.3] assess session log skipped:', e.message);
        }
...
请生成3-5个阶段，每个阶段3-5个任务，覆盖${timelineWeeks}周时间线。`;
    ...
    try {
        ...
        try {
            await neonHelper.query(
                `INSERT INTO copilot_sessions (user_id, goal, timeline, background, plan_data, is_trial, module)
       VALUES ($1, $2, $3, $4, $5, false, 'plan')`,
                [userId, goal, timeline, JSON.stringify(background || {}), JSON.stringify(planData)]
            );
        } catch (e) {
            console.warn('[Copilot V1.3] create-plan session log skipped:', e.message);
        }
