...
let jobsTagsColumnCache = null;
async function hasJobsTagsColumn() {
    if (jobsTagsColumnCache !== null) return jobsTagsColumnCache;
    try {
        ...
        jobsTagsColumnCache = Array.isArray(rows) && rows.length > 0;
    } catch {
        jobsTagsColumnCache = false;
    }
    return jobsTagsColumnCache;
}
...
async function findMatchingJobs({ structured = null, manualDirection = '', positionType = 'full-time', resumeHints = [], limit = 20 }) {
    ...
    const tagsAvailable = await hasJobsTagsColumn();
    const conditions = likeTerms.length > 0
        ? likeTerms.map((_, i) => {
            const base = `(LOWER(title) LIKE $${i + 1} OR LOWER(COALESCE(category, '')) LIKE $${i + 1} OR LOWER(COALESCE(description, '')) LIKE $${i + 1} OR LOWER(COALESCE(job_type, '')) LIKE $${i + 1}`;
    ...
    try {
        rows = await neonHelper.query(
            `SELECT job_id, title, company, category, location, job_type,
              experience_level, salary, description, published_at, logo, company_website${tagsAvailable ? ', tags' : ''}
             FROM jobs
