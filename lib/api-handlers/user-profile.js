...
async function getUserFavorites(userId) {
  ...
  try {
    ...
    const query = `
      ...
      FROM favorites 
      WHERE user_id = $1 
      AND (created_at IS NULL OR created_at > NOW() - INTERVAL '1 year')
    `
    ...
    if (action === 'record_interaction' && req.method === 'POST') {
      ...
      try {
        if (NEON_CONFIGURED) {
          ...
          await neonHelper.query(`
                    INSERT INTO user_job_interactions (user_id, job_id, interaction_type, notes, application_source, updated_at, job_title_snapshot, company_name_snapshot)
                    SELECT $1, $2, $3, $4, $5, NOW(), j.title, j.company
