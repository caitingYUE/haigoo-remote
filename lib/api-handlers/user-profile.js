...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'save_preferences' && req.method === 'POST') {
      ...
      try {
        const email = preferences.contactEmail || user.email
        if (email) {
          // Check if subscription exists for this user/email
          const existingSubs = await neonHelper.select('subscriptions', {
            channel: 'email',
            identifier: email
          })
          ...
          if (existingSubs && existingSubs.length > 0) {
            ...
            subId = existingSubs[0].subscription_id
          ...
          console.log(`[API save_preferences] Synced to subscriptions table: ${subId}`)
        }
      } catch (syncError) {
        console.error(`[API save_preferences] Failed to sync to subscriptions:`, syncError)
      }
        ...
        if (NEON_CONFIGURED) {
          const query = `
                    SELECT 
                        ...
                        j.company_industry as "jobCompanyIndustry",
