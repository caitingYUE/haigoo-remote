        ...
        if (NEON_CONFIGURED) {
          const query = `
                    SELECT 
                        ...
                        j.salary_range as "jobSalaryRange",
                        j.work_experience as "jobWorkExperience",
                        j.education_requirement as "jobEducationRequirement",
                        j.job_type as "jobJobType",
                        j.tags as "jobTags",
                        j.is_trusted as "jobIsTrusted",
                        j.pub_date as "jobPubDate",
          ...
          const applications = result ? result.map(row => ({
            ...
            job: {
              ...
              salary_range: row.jobSalaryRange,
              work_experience: row.jobWorkExperience,
              education_requirement: row.jobEducationRequirement,
              job_type: row.jobJobType,
              tags: Array.isArray(row.jobTags) ? row.jobTags : (typeof row.jobTags === 'string' ? JSON.parse(row.jobTags) : []),
              is_trusted: row.jobIsTrusted,
              pub_date: row.jobPubDate
            },
