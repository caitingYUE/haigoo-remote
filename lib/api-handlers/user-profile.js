    ...
    if (action === 'favorites_add' && (req.method === 'POST' || req.method === 'PUT')) {
      ...
      try {
        ...
        if (NEON_CONFIGURED && job) {
          ...
          // Use INSERT ON CONFLICT DO UPDATE to ensure we have the record
          const jobData = {
            job_id: job.id,
        ...
        const added = await addFavorite(user.user_id, targetJobId)
        if (!added) {
        ...
        if (NEON_CONFIGURED) {
          const query = `
                    SELECT 
                        uji.id, 
                        uji.job_id as "jobId", 
                        COALESCE(j.title, '职位已失效') as "jobTitle", 
                        COALESCE(j.company, '未知企业') as "company", 
