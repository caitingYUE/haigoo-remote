...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'favorites') {
      ...
      try {
        if (NEON_CONFIGURED && ids.length > 0) {
          ...
          const query = `SELECT * FROM jobs WHERE job_id IN (${placeholders}) AND status = 'active' ORDER BY published_at DESC`
    ...
    if (action === 'favorites_add' && (req.method === 'POST' || req.method === 'PUT')) {
      ...
      try {
        const added = await addFavorite(user.id, jobId)
        if (!added) {
    ...
    if (action === 'submit_recommendation' && req.method === 'POST') {
      ...
      try {
        ...
        console.log('[API] Recommendation submitted:', { userId: user.id })
    ...
    if (action === 'favorite_remove' && (req.method === 'POST' || req.method === 'PUT')) {
      ...
      try {
        const removed = await removeFavorite(user.id, jobId)
        if (!removed) {
          return res.status(503).json({ success: false, error: 'Service unavailable: Database not configured' })
        }
    ...
    if (action === 'save_preferences' && req.method === 'POST') {
      ...
      console.log(`[API save_preferences] User: ${user.id}`)
      ...
      const saved = await saveUserProfile(user.id, updatedProfile)
    ...
    if (action === 'get_preferences') {
      console.log(`[API get_preferences] User: ${user.id}`)
