...
async function ensureFeedbackReviewColumns() {
  ...
  try {
    ...
    console.error('[user-profile] ensureFeedbackReviewColumns error:', error)
  }
}
...
async function getUserFavorites(userId) {
  ...
  try {
    ...
    try {
      ...
      `
      result = await neonHelper.query(query, [userId])
    } catch (err) {
      if (err.message && err.message.includes('job_title_snapshot')) {
        console.warn('[user-profile] Column job_title_snapshot missing, falling back to basic query...');
        const fallbackQuery = `
           ...
           AND created_at > NOW() - INTERVAL '1 year'
         `
        result = await neonHelper.query(fallbackQuery, [userId]);
      } else {
        throw err;
      }
    }
    ...
    return result ? result.map(row => ({
      jobId: row.job_id,
      titleSnapshot: row.job_title_snapshot || null,
      companySnapshot: row.company_name_snapshot || null,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'public_feedbacks') {
      try {
        const source = params.get('source') || 'about_testimonial'
        let feedbacks = []
        if (NEON_CONFIGURED) {
          await ensureFeedbackReviewColumns()
          const result = await neonHelper.query(`
            SELECT f.content, f.display_name, f.display_title, f.created_at, u.username
            FROM feedbacks f
            LEFT JOIN users u ON f.user_id = u.user_id
            WHERE f.source = $1
              AND f.is_public = TRUE
              AND f.review_status = 'approved'
              AND COALESCE(NULLIF(f.content, ''), '') <> ''
            ORDER BY f.reviewed_at DESC NULLS LAST, f.created_at DESC
            LIMIT 20
          `, [source])
          feedbacks = (result || []).map(row => ({
            content: row.content,
            displayName: row.display_name || row.username || 'Haigoo 用户',
            displayTitle: row.display_title || '远程工作探索者',
            createdAt: row.created_at
          }))
        }
        return res.status(200).json({ success: true, feedbacks })
      } catch (e) {
        console.error('[user-profile] public_feedbacks error:', e)
        return res.status(200).json({ success: true, feedbacks: [] })
      }
    }
    ...
    if (action === 'feedbacks_list') {
      // 严格鉴权：检查管理员权限
      const isAdmin = !!(user.roles?.admin || SUPER_ADMIN_EMAILS.includes(String(user.email || '').toLowerCase()))
      if (!isAdmin) {
    ...
    if (action === 'notifications') {
      try {
        ...
        const notifications = result ? result
          .filter((row) => {
            if (!['membership_expired', 'membership_activated'].includes(String(row.type || ''))) {
              return true
            }

            const dedupeKey = [row.type, row.title, row.content].join('::')
    ...
    if (action === 'delete_account' && req.method === 'POST') {
      try {
        ...
        if (!user?.email) {
          return res.status(400).json({ success: false, error: '当前账号缺少邮箱信息，暂无法注销' })
        }
        ...
        if (NEON_CONFIGURED) {
          await neonHelper.query(
            `INSERT INTO deleted_account_locks (email, user_id, blocked_until, reason, created_at, updated_at)
             VALUES ($1, $2, NOW() + INTERVAL '30 days', 'account_deletion', NOW(), NOW())`,
        ...
        if (success) {
          ...
          return res.status(200).json({ success: true, message: '账号已注销，30天内该邮箱无法重新注册' })
        } else {
          if (lockCreated && NEON_CONFIGURED) {
            await neonHelper.query(
              `DELETE FROM deleted_account_locks
                WHERE email = $1
                  AND user_id = $2
    ...
    if (action === 'favorites') {
      ...
      const canAccessMemberOnlyJobs = isMembershipActive(user)
      let favoriteItems = await getUserFavorites(user.user_id)
        ...
        if (NEON_CONFIGURED && ids.length > 0) {
          ...
          const query = `
            ...
            WHERE j.job_id IN (${placeholders})
              ${canAccessMemberOnlyJobs ? '' : 'AND COALESCE(j.member_only, false) = false'}
          ...
          if (!canAccessMemberOnlyJobs) {
            // Do not fall back to saved title/company snapshots for Club roles:
            // stale favorites must disappear instead of revealing private jobs.
            const visibleIds = new Set((result || []).map(row => String(row.job_id)))
            favoriteItems = favoriteItems.filter(item => visibleIds.has(String(item.jobId)))
          }
    ...
    if (action === 'favorites_add' && (req.method === 'POST' || req.method === 'PUT')) {
      ...
      try {
        if (NEON_CONFIGURED) {
          const accessRows = await neonHelper.query(
            'SELECT member_only, status, is_approved FROM jobs WHERE job_id = $1 LIMIT 1',
            [targetJobId]
          )
          const targetJob = accessRows?.[0]
          if (!targetJob || targetJob.status !== 'active' || targetJob.is_approved !== true) {
            return res.status(404).json({ success: false, error: '岗位不存在或已下线' })
          }
          if (targetJob.member_only === true && !isMembershipActive(user)) {
            return res.status(404).json({ success: false, error: '岗位不存在或已下线' })
          }
        }
    ...
    if (action === 'submit_feedback' && (req.method === 'POST' || req.method === 'PUT')) {
      const { jobId, accuracy, content, contact, source, sourceUrl, displayName, displayTitle } = req.body || {}
      ...
      try {
        ...
        const isAboutTestimonial = feedbackSource === 'about_testimonial'
        if (NEON_CONFIGURED) {
          await ensureFeedbackReviewColumns()
          await neonHelper.insert(FEEDBACKS_TABLE, {
            ...
            is_public: false,
            review_status: isAboutTestimonial ? 'pending' : null,
            display_name: isAboutTestimonial ? String(displayName || '').slice(0, 120) : null,
            display_title: isAboutTestimonial ? String(displayTitle || '').slice(0, 120) : null,
        ...
        if (NEON_CONFIGURED) {
          try {
            await neonHelper.query(
              ...
              ['user_feedback', isAboutTestimonial ? '收到新的关于我们留言' : '收到新用户反馈', `用户: ${user.user_id}\n内容: ${content || ''}\n联系方式: ${contact || '无'}`]
            )
          } catch (err) {
            console.error('[API] Failed to insert admin message for user feedback', err)
          }
        }
    ...
    if (action === 'record_interaction' && req.method === 'POST') {
      ...
      console.log(`[user-profile] record_interaction: user=${user.user_id}, job=${jobId}, type=${type}`)
      
      if (!jobId || !type) return res.status(400).json({ success: false, error: 'Missing jobId or type' })

      const applicationInteractionTypes = new Set(['apply', 'apply_redirect', 'pending_apply', 'email', 'referral'])
      if (applicationInteractionTypes.has(type) && user.emailVerified !== true && user.email_verified !== true) {
    ...
    if (action === 'submit_referral' && req.method === 'POST') {
      ...
      try {
        if (NEON_CONFIGURED) {
          ...
          const isMember = membership.canAccessReferral
          ...
          await trackServerAnalyticsEvent({
            event: 'referral_submit_success',
            properties: {
              feature_key: 'referral',
              source_key: req.body?.source_key || 'referral_submit',
              entity_type: 'job',
              entity_id: jobId,
              resume_id: resumeId,
            ...
            user,
            userId: user.user_id,
            anonymousId: req.body?.anonymous_id || `user_${user.user_id}`,
            pageKey: req.body?.page_key || 'job_detail',
            module: 'application',
            featureKey: 'referral',
            sourceKey: req.body?.source_key || 'referral_submit',
            entityType: 'job',
            entityId: jobId,
    ...
    if (action === 'my_applications') {
      console.log(`[user-profile] my_applications: fetching for user=${user.user_id}`)
      try {
        if (NEON_CONFIGURED) {
          const query = `
                    SELECT 
                        ...
                        j.job_id as "activeJobId",
                        j.status as "jobStatus",
                        COALESCE(tc.logo, j.company_logo) as "jobLogo",
                        j.category as "jobCategory",
                        j.location as "jobLocation",
                        j.salary as "jobSalaryRange",
                        j.job_type as "jobJobType",
                        j.tags as "jobTags",
                        j.is_trusted as "jobIsTrusted",
                        j.published_at as "jobPubDate",
                        j.description as "jobDescription",
                        j.requirements as "jobRequirements",
                        j.benefits as "jobBenefits",
                        j.experience_level as "jobExperienceLevel",
                        j.is_remote as "jobIsRemote",
                        j.source_type as "jobSourceType",
                        j.url as "jobUrl",
                        j.can_refer as "jobCanRefer",
                        j.member_only as "jobMemberOnly",
                        j.translations as "jobTranslations",
                        j.is_translated as "jobIsTranslated",
                        j.company_id as "jobCompanyId",
                        j.industry as "jobCompanyIndustry",
                    ...
                    LEFT JOIN trusted_companies tc ON (j.company_id = tc.company_id OR (j.company_id IS NULL AND LOWER(TRIM(j.company)) = LOWER(TRIM(tc.name))))
          ...
          console.log(`[user-profile] my_applications: found ${result ? result.length : 0} records for user ${user.user_id}`)
          if (result && result.length > 0) {
             console.log('[user-profile] sample record:', {
               id: result[0].id,
          ...
          const applications = result ? result.map(row => ({
            ...
            job: {
              ...
              salary: row.jobSalaryRange ? { min: 0, max: 0, currency: 'USD', display: row.jobSalaryRange } : undefined,
              type: row.jobJobType || 'full-time',
              tags: Array.isArray(row.jobTags) ? row.jobTags : (typeof row.jobTags === 'string' ? JSON.parse(row.jobTags || '[]') : []),
              isTrusted: row.jobIsTrusted || false,
              publishedAt: row.jobPubDate || null,
              description: row.jobDescription,
              requirements: typeof row.jobRequirements === 'string' ? JSON.parse(row.jobRequirements || '[]') : (row.jobRequirements || []),
              responsibilities: [],
              benefits: typeof row.jobBenefits === 'string' ? JSON.parse(row.jobBenefits || '[]') : (row.jobBenefits || []),
              skills: typeof row.jobTags === 'string' ? JSON.parse(row.jobTags || '[]') : (row.jobTags || []),
              isRemote: row.jobIsRemote || false,
              sourceType: row.jobSourceType || 'rss',
              url: row.jobUrl || undefined,
              sourceUrl: row.jobUrl || undefined,
              canRefer: Boolean(row.jobCanRefer),
              memberOnly: Boolean(row.jobMemberOnly),
              experienceLevel: row.jobExperienceLevel,
              companyIndustry: row.jobCompanyIndustry,
              recommendationScore: 0,
              translations: typeof row.jobTranslations === 'string' ? JSON.parse(row.jobTranslations || '{}') : (row.jobTranslations || {}),
              isTranslated: row.jobIsTranslated || false,
              companyId: row.jobCompanyId || null,
              status: !row.activeJobId || String(row.jobStatus || '').toLowerCase() !== 'active' ? '已失效' : '进行中'
            },
