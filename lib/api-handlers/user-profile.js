    ...
    if (action === 'record_interaction' && req.method === 'POST') {
        ...
        try {
            if (NEON_CONFIGURED) {
                // Upsert interaction
                await neonHelper.query(`
                    INSERT INTO user_job_interactions (user_id, job_id, interaction_type, notes, updated_at)
                    VALUES ($1, $2, $3, $4, NOW())
                    ON CONFLICT (user_id, job_id, interaction_type) 
                    DO UPDATE SET updated_at = NOW(), notes = EXCLUDED.notes
                `, [user.user_id, jobId, type, notes || '']);
    ...
    if (action === 'submit_referral' && req.method === 'POST') {
        ...
        try {
            if (NEON_CONFIGURED) {
                ...
                await neonHelper.query(`
                    INSERT INTO user_job_interactions (user_id, job_id, interaction_type, resume_id, notes, status, updated_at)
                    VALUES ($1, $2, 'referral', $3, $4, 'applied', NOW())
                    ON CONFLICT (user_id, job_id, interaction_type) 
                    DO UPDATE SET 
        ...
        try {
            if (NEON_CONFIGURED) {
                const query = `
                    ...
                    WHERE uji.user_id = $1 AND uji.interaction_type IN ('apply', 'referral')
