...
async function getLocationCategories() {
  ...
  try {
    const result = await neonHelper.select(LOCATION_CATEGORIES_TABLE, { config_type: 'location_categories' })
    return result && result.length > 0 ? result[0].config_data : DEFAULT_LOCATION_CATEGORIES
  } catch (error) {
...
async function saveLocationCategories(categories) {
  ...
  try {
    const existing = await neonHelper.select(LOCATION_CATEGORIES_TABLE, { config_type: 'location_categories' })
    
    if (existing && existing.length > 0) {
      await neonHelper.update(LOCATION_CATEGORIES_TABLE, {
        config_data: categories,
        updated_at: new Date().toISOString()
      }, { config_type: 'location_categories' })
    } else {
