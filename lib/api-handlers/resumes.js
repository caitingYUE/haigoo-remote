...
async function parseMultipartSimple(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      ...
      if (receivedBytes > MAX_RESUME_UPLOAD_BYTES) {
        settled = true
        reject(new Error('Resume file exceeds 10MB limit'))
        return
      }
      chunks.push(chunk)
    })
...
async function handleUpload(req, res) {
  ...
  try {
    ...
    if (contentLength > MAX_RESUME_UPLOAD_BYTES) {
      return sendJson(res, { success: false, error: 'Resume file exceeds 10MB limit' }, 413)
    }
    ...
    if (buffer.length > MAX_RESUME_UPLOAD_BYTES) return sendJson(res, { success: false, error: 'Resume file exceeds 10MB limit' }, 413)
    if (!SUPPORTED_RESUME_EXTENSIONS.has(extension)) {
      return sendJson(res, { success: false, error: 'Unsupported resume format. Please upload PDF, DOCX or TXT.' }, 415)
    }

    // Save to temp file
    const tempDir = os.tmpdir()
    const safeFilename = path.basename(filename).replace(/[^a-zA-Z0-9._\-\u3400-\u9fff]/g, '_')
    tempFilePath = path.join(tempDir, `resume-${Date.now()}-${safeFilename}`)
    ...
    if (pythonResult && pythonResult.success) {
      ...
      if (text) {
        const structured = extractStructuredResume(text)
        parsedData = {
          ...structured,
          text: text.trim(),
          fallback: true
        }
        parseStatus = structured.evidence_coverage >= 0.35 ? 'success' : 'partial'
      }
    }

    // Normalize every successful extraction into the same deterministic shape.
    // Python parsing may provide richer contact/education data, while the shared
    // extractor guarantees roles, skills, experience and confidence fields.
    const extractedText = parsedData?.text || parsedData?.content || ''
    if (extractedText) {
      const structured = extractStructuredResume(extractedText)
      parsedData = {
        ...structured,
        ...parsedData,
        skills: Array.isArray(parsedData.skills) && parsedData.skills.length
          ? parsedData.skills
          : structured.skills,
        roles: Array.isArray(parsedData.roles) && parsedData.roles.length
          ? parsedData.roles
          : structured.roles,
        roleFamilies: Array.isArray(parsedData.roleFamilies) && parsedData.roleFamilies.length
          ? parsedData.roleFamilies
          : structured.roleFamilies,
        targetRole: parsedData.targetRole || parsedData.target_role || structured.targetRole,
        experienceYears: Number(parsedData.experienceYears || parsedData.total_experience || structured.experienceYears) || 0,
        evidence_coverage: Math.max(
          Number(parsedData.evidence_coverage) || 0,
          Number(structured.evidence_coverage) || 0
        )
      }
    }
    ...
    const status = String(error?.message || '').includes('exceeds 10MB') ? 413 : 500
    return sendJson(res, { success: false, error: error.message }, status)
  }
}
...
function buildLocalResumeAssistantFramework(text, targetRole = '') {
  ...
  const rewriteDirections = [
    ...
    },
    {
      ...
      example: '这样做能让招聘方更快理解你的岗位画像，减少不必要的猜测。'
    },
  ]
  ...
  const confidenceSummary = {
    ...
    summary: '保留现有基础，把最能代表你的经历、成果与目标岗位之间的关系表达得更集中，并补充必要的 STAR 信息和量化结果，整体说服力会更强。'
  }
...
async function buildInterviewPolishResult(userId, targetRole, framework, isMember) {
  ...
  return {
    ...
    sections: [
      {
        heading: '新增练习重点',
        body: '建议优先练习 2-3 道与你最强经历相关的问题，熟悉英文表达后逐步扩展到其他题型。',
