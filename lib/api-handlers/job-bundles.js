...
import { getPreviewJobBundlesResponse } from '../dev-preview-data.js';

const CEO_VIDEO_PREFIX = 'ceo:';

function isCeoCareerVideoId(videoId) {
    return String(videoId || '').startsWith(CEO_VIDEO_PREFIX)
}

function getCeoMaterialId(videoId) {
    return String(videoId || '').slice(CEO_VIDEO_PREFIX.length)
}

function isLocalPreviewRequest(req) {
    const host = String(req?.headers?.host || req?.headers?.['x-forwarded-host'] || '').toLowerCase();
    return (host.includes('localhost') || host.includes('127.0.0.1'))
        && process.env.HAIGOO_LOCAL_USE_REMOTE_JOBS !== 'true';
}
...
function getBundleAccess(bundle, user, { allowUnauthenticatedLoginPrompt = false } = {}) {
    ...
    if (visibility === 'specified') {
        ...
        // A bundle can be configured with users selected from the admin search
        // and/or imported registration emails. Both lists grant access; treating
        // user IDs as an exclusive source makes valid email-based grants vanish
        // as soon as an admin adds one searched user.
        const isAllowedByUserId = Boolean(userId && allowedUserIds.includes(userId))
        const isAllowedByEmail = Boolean(email && normalizeEmails(bundle?.allowed_emails).includes(email))
        const isAllowed = isAllowedByUserId || isAllowedByEmail
        return { visible: isAllowed, locked: false }
    }
    if (visibility === 'member') {
        // Legacy member collections remain login-gated for anonymous visitors, but
        // signed-in free users can read the public job information during the
        // compliance period. The visibility type is retained for later recovery.
        return { visible: true, locked: !user }
    }
...
async function hydrateCareerItems(bundle) {
    ...
    const moduleIds = items.map((item) => item.video_id).filter((videoId) => !isCeoCareerVideoId(videoId))
    const ceoMaterialIds = items.map((item) => item.video_id).filter(isCeoCareerVideoId).map(getCeoMaterialId).filter(Boolean)
    const [moduleRows, ceoRows] = await Promise.all([
        moduleIds.length > 0
            ? neonHelper.query(
            ...
            : Promise.resolve([]),
        ceoMaterialIds.length > 0
            ? neonHelper.query(
                `SELECT material_id, material_title, video_summary, company_name_snapshot,
                        duration_ms, cover_image_hash
                   FROM corporate_english_materials
                  WHERE material_id = ANY($1) AND deleted_at IS NULL AND status = 'published'`,
                [ceoMaterialIds]
            )
            : Promise.resolve([])
    ])
    const byId = new Map((moduleRows || []).map((row) => [String(row.video_id), { source: 'module', row }]))
    for (const row of ceoRows || []) {
        byId.set(`${CEO_VIDEO_PREFIX}${row.material_id}`, { source: 'ceo', row })
    }
    bundle.career_items = items.map((item) => {
        const entry = byId.get(item.video_id)
        ...
        if (entry.source === 'ceo') {
            return {
                ...
                access_tier: 'vip',
                href: `/careerlearning/watch/ceo/${encodeURIComponent(video.material_id)}`,
                cover_image_url: `/api/corporate-english-public?resource=cover-image&ownerType=material&ownerId=${encodeURIComponent(video.material_id)}&variant=thumb${video.cover_image_hash ? `&v=${encodeURIComponent(String(video.cover_image_hash).slice(0, 16))}` : ''}`
            }
        }
        return {
            ...item,
...
async function getEligibleJobIdSet(jobIds = [], canAccessMemberOnly = false) {
    ...
    const jobsResult = await neonHelper.query(
        `SELECT job_id FROM jobs
          WHERE job_id = ANY($1)
            ...
            AND status = 'active'
            ${canAccessMemberOnly ? '' : 'AND COALESCE(member_only, false) = false'}`,
        [ids]
    );
...
async function applyJobAccessFilter(bundle, user) {
    ...
    const eligibleIds = await getEligibleJobIdSet(ids, isMembershipActive(user));
    ...
    try {
        if (req.method === 'GET') {
            ...
            if (isLocalPreviewRequest(req)) {
                const preview = getPreviewJobBundlesResponse({ id, is_active });
                preview.data = (preview.data || []).map((bundle) => ({
                    ...bundle,
                    access: { visible: true, locked: false },
                    career_items: Array.isArray(bundle.career_items) ? bundle.career_items : [],
                    progress: null
                }));
            ...
            if (id) {
                ...
                await applyJobAccessFilter(bundle, user);
            ...
            for (let b of validBundles) {
                await applyJobAccessFilter(b, user);
            }
        ...
        if (req.method === 'POST' && req.query.action === 'progress') {
            ...
            if (action === 'toggle_video') {
                ...
                if (eventType === 'video_open') {
                    ...
                    const videoRows = isCeoCareerVideoId(videoId)
                        ? await neonHelper.query(
                            `SELECT material_title AS video_title FROM corporate_english_materials
                              WHERE material_id = $1 AND deleted_at IS NULL AND status = 'published'`,
                            [getCeoMaterialId(videoId)]
                        )
                        : await neonHelper.query(
