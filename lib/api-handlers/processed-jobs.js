...
function buildWhereClause(queryParams) {
  ...
  if (queryParams.sourceType) {
    const types = queryParams.sourceType.split(',');
    
    // Check for specific source types
    const hasClubReferral = types.includes('club-referral');
    const hasCurated = types.includes('curated');
    const hasThirdParty = types.includes('third-party');
    ...
    if (hasCurated) {
      // Curated means is_trusted = true OR source_type = 'trusted'
      sourceConditions.push('(is_trusted = true OR source_type = \'trusted\')');
    }
    
    if (hasThirdParty) {
      // Third party means NOT can_refer AND NOT is_trusted
      sourceConditions.push('(can_refer = false AND is_trusted = false AND (source_type IS NULL OR source_type = \'rss\' OR source_type = \'third-party\'))');
    }
  ...
  if (queryParams.location) {
    // If multiple locations are provided, match any of them
    // Construct patterns like %loc1%, %loc2%
    // We use a subquery to expand the comma-separated string into patterns
    conditions.push(`EXISTS (
      SELECT 1 FROM unnest(string_to_array($${paramIndex}, ',')) AS loc_pattern
      WHERE location ILIKE '%' || loc_pattern || '%'
    )`)
  ...
  // Handle industry (multi-select) - assume it maps to tags or category if no industry column
  // If we assume 'industry' filters should check tags:
  if (queryParams.industry) {
    // Check if any of the provided industries exist in the tags JSONB array
    // Convert comma-separated string to array, then check overlap
    // Note: This assumes tags is JSONB array of strings
...
async function readJobsFromNeon(queryParams = {}, pagination = {}) {
  ...
  try {
    ...
    return result.map(row => {
      // 临时修复：读取时重新分类 Region，以解决旧数据污染问题
      // 这是一个临时的性能折衷，直到所有数据被批量更新
      // 只有当 region 为 'both' 时才重新检查，因为 'overseas' 和 'domestic' 通常是准确的
      // 但为了保险，对所有数据重新计算（特别是 'both' 和 'domestic'，防止误判）
      // 注意：这不会修改数据库，只会影响当前返回结果
      let effectiveRegion = row.region;
      if (effectiveRegion === 'both' || effectiveRegion === 'domestic') {
        const reClassified = classifyRegion(row.location);
        // 如果重新分类结果比原结果更严格（如原为 both，现为 overseas），则采用新结果
        // 如果原为 domestic，现为 overseas，也采用新结果
        if (reClassified === 'overseas') {
          effectiveRegion = 'overseas';
        } else if (reClassified === 'domestic' && effectiveRegion === 'both') {
          effectiveRegion = 'domestic';
        }
      }

      // 如果当前查询有 region 筛选，且重新分类后不符合筛选条件，则应该过滤掉
      // 但由于这是分页查询后的结果，直接过滤会导致每页数量不一致甚至为空
      // 理想做法是写入数据库。这里我们只能尽量返回正确的数据给前端。
      // 前端如果发现 region 不对，可能会显示出来但用户会困惑。
      // 最好的办法是：如果这里发现数据不对，异步触发一个更新操作？太复杂。
      // 简单点：直接返回修正后的 region，让前端显示正确的标签。
      // 至于筛选不准的问题（比如查 domestic 查出了实际是 overseas 的），
      // 由于 SQL 是基于旧数据查的，这里无法挽回。
      // 唯一能做的是：在返回给前端前，如果发现不匹配，就把它剔除？
      // 这样会导致页面条数变少（比如请求50条，实际返回48条），但比显示错误数据好。
      ...
      const job = {
        ...
        url: row.url,
        sourceUrl: row.url, // Alias for frontend compatibility
        ...
        status: row.status,
        region: effectiveRegion, // 使用修正后的 region
...
async function getJobsWithMatchScores(userId, queryParams = {}, pagination = {}) {
  ...
  try {
    ...
    if (queryParams.sortBy === 'recent') {
      ...
      // 默认排序：相关性(>=30) > 内推 > 精选 > 时间
      // 匹配度 < 30% 的视为 0，排序不靠前
      orderByClause = `
      ORDER BY 
        CASE WHEN COALESCE(ujm.match_score, 0) >= 30 THEN COALESCE(ujm.match_score, 0) ELSE 0 END DESC,
        CASE WHEN j.can_refer = true THEN 1 ELSE 0 END DESC,
        CASE WHEN j.is_trusted = true THEN 1 ELSE 0 END DESC,
        j.published_at DESC
      `
    }
...
function classifyRegion(location) {
  ...
  // 优先级分类逻辑 (修复: 明确海外地点优先于 Global 关键词)
  
  // 1. 如果包含明确的海外关键词 (如 US, India, Hawaii)，强制归为 overseas
  // 即使它同时包含 Remote/Global
  if (isOverseas) {
    // 除非它同时也明确包含了中国 (比如 "US or China")
  ...
  // 3. 如果包含 APAC (且前面已经排除了明确的 Overseas 国家如 India/Singapore)
  if (isAPAC) {
    return 'domestic' // 视为国内可申
  }

  // 4. 最后才判断 Global/Remote
  // 只有在没有明确 Overseas 地点的情况下，Remote 才算 both
  if (isGlobal) {
    return 'both' 
  }

  // 默认: 如果完全无法判断,标记为both(保守策略,避免误判)
  // 但为了避免把 obscure 的海外地点误判为 both，我们可以改为默认为 overseas
  // 用户反馈说有很多非中国国家出现在"中国可申"里，说明默认 both 过于宽容
  return 'overseas'
}
      ...
      for (const job of unique) {
        ...
        if (!job.sourceType) {
          if (job.canRefer) {
            ...
            job.sourceType = 'trusted' // or 'curated'
          } else {
        ...
        await sql.query(`
          INSERT INTO ${JOBS_TABLE} (
            ...
            is_translated, translated_at, company_id, source_type, is_trusted, can_refer, is_featured,
