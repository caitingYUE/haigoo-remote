  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'backfill-industry') {
        ...
        try {
          // Step 1: Backfill from trusted_companies for jobs that have company_id linked
          const sqlBackfill = `
            UPDATE ${JOBS_TABLE} j
            SET industry = tc.industry
            FROM trusted_companies tc
            WHERE j.company_id = tc.company_id
              AND j.industry IS NULL
              AND tc.industry IS NOT NULL
          `
          const sqlResult = await neonHelper.query(sqlBackfill)
          const linkedCount = sqlResult?.rowCount ?? 0

          // Step 2: For remaining NULL industry jobs, use classifyCompany
          const jobsWithNullIndustry = await neonHelper.query(
            `SELECT job_id, company, description FROM ${JOBS_TABLE} WHERE industry IS NULL LIMIT 2000`
          )
