...
const RESTRICTED_JOB_DISCOVERY_QUERY_KEYS = [
  'search', 'category', 'experienceLevel', 'industry', 'regionType', 'region',
  'source', 'sourceType', 'location', 'timezone', 'type', 'jobType', 'salary',
  'tags', 'skills', 'company', 'companyId', 'status', 'dateFrom', 'dateTo',
  'isRemote', 'isApproved', 'isFeatured', 'copilotGoal', 'boostSearch',
  'boostCategory', 'boostIndustry', 'boostJobType', 'boostLocation'
]

function hasRestrictedJobDiscoveryQuery(query = {}) {
  if (String(query?.sortBy || '').toLowerCase() === 'recent') return true
  if (String(query?.memberOnly || '').toLowerCase() === 'true') return true
  if (String(query?.isTrusted || '').toLowerCase() === 'true') return true
  if (String(query?.isNew || '').toLowerCase() === 'true') return true
  return RESTRICTED_JOB_DISCOVERY_QUERY_KEYS.some((key) => String(query?.[key] || '').trim().length > 0)
}
...
const MATCH_SCORE_VISIBLE_THRESHOLD = 70
...
function buildDisplayScorePayload({ trueScore = 0, constraintFlags = {}, goalFitScore = null, row = {}, details = null } = {}) {
  const calibrated = calibrateDisplayScore({ trueScore, constraintFlags })
  ...
  try {
    ...
    const ensurePagedRowsMatchScores = async (rows = []) => {
      ...
      for (const row of rows) {
        ...
        if (!isFresh || !hasNewScoreShape || !hasConstraintFlags) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (shouldUseLocalJobsPreview(req)) {
        ...
        return res.status(200).json({
          ...preview,
          jobs: metadataOnly === 'true' || (previewIsGuestListRequest && pageNum > 1) ? [] : preview.jobs,
      ...
      // Fail closed until the user record explicitly confirms verification.
      // A lookup failure must not be treated as a verified account.
      ...
      if (hasValidRequester && !requesterEmailVerified && hasRestrictedJobDiscoveryQuery(req.query || {})) {
        return res.status(403).json({
          success: false,
          error: '请先验证邮箱后再搜索或筛选岗位',
          emailVerificationRequired: true
      ...
      const isGuestListRequest = !hasValidRequester && !id && !ids && metadataOnly !== 'true'
      ...
      if (!hasValidRequester && !effectiveIsAdmin) {
        res.setHeader('Cache-Control', 'public, s-maxage=60, stale-while-revalidate=300')
      } else {
        res.setHeader('Cache-Control', 'private, no-cache, no-store, must-revalidate')
      }
