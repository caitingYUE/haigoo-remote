...
async function getJobsWithMatchScores(userId, queryParams = {}, pagination = {}) {
  ...
  try {
    ...
    if (queryParams.sortBy === 'recent') {
      // 最新排序：严格按发布时间倒序
      // 必须带上前缀 j. 以防 join 时的歧义
      orderByClause = `ORDER BY j.published_at DESC`
    } else {
      // 默认/相关排序：
      // 1. 优先按照岗位匹配度展示 (match_score)
      // 2. 发布时间
      orderByClause = `
      ORDER BY 
    ...
    // 构建带匹配分数的查询语句
    // 强制把 published_at 转换为 TIMESTAMP 比较 (如果它是字符串的话)
    // 但 DDL 显示它是 TIMESTAMP 类型，所以这里应该是数据问题或者前端传参问题
    // 让我们确保 j.* 包含 published_at
    const query = `
      SELECT 
        j.*,
