  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        COALESCE(ujm.updated_at, j.published_at) as match_updated_at
      FROM ${JOBS_TABLE} j
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (action === 'jobs_with_match_score') {
        ...
        if (!userId) {
          return res.status(400).json({ error: 'userId is required for jobs_with_match_score action' })
        }
        ...
        try {
          ...
          // 获取带匹配分数的岗位列表
          const result = await getJobsWithMatchScores(userId, queryParams, { page: pageNum, limit: pageSize })
