...
function buildWhereClause(queryParams) {
  ...
  */
  ...
  if (jobType) {
    conditions.push(`LOWER(${JOBS_TABLE}.job_type) = ANY(string_to_array(LOWER($${paramIndex}), ','))`)
    params.push(jobType)
    paramIndex++
  }
  ...
  if (queryParams.industry) {
    ...
    for (const ind of industries) {
        ...
        if (ind === '人工智能') {
             ...
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Software%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Internet%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Technology%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%SaaS%'); paramIndex++;
        }
        else if (ind === '金融/Fintech') {
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Finance%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Fintech%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Banking%'); paramIndex++;
        }
        else if (ind === '电子商务') {
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%E-commerce%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Retail%'); paramIndex++;
        }
        else if (ind === '游戏/娱乐') {
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Game%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Gaming%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Entertainment%'); paramIndex++;
        }
        else if (ind === '企业服务/SaaS') {
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Enterprise%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%SaaS%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%B2B%'); paramIndex++;
        }
        else if (ind === '硬件/物联网') {
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Hardware%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%IoT%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Semiconductor%'); paramIndex++;
        }
        else if (ind === '大健康/医疗') {
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Health%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Medical%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Bio%'); paramIndex++;
        }
        else if (ind === '教育') {
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Education%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%EdTech%'); paramIndex++;
        }
    }
  ...
  // Handle companySize (multi-select)
  if (queryParams.companySize) {
    const sizes = queryParams.companySize.split(',')
    const sizeChecks = []
    
    for (const size of sizes) {
       // Build variations for fuzzy matching against dirty data (e.g. "1,001-5,000" vs "1001-5000", "2-10" vs "1-10")
       const variations = [size];
       
       if (size === '1-10') {
           variations.push('2-10'); // Common LinkedIn format
       }
       else if (size === '1001-5000') {
           variations.push('1,001-5,000');
       }
       else if (size === '501-1000') {
           variations.push('501-1,000');
       }
       else if (size === '5000+') {
           variations.push('5,000+');
           variations.push('10,001+'); // Include larger bucket
           variations.push('10000+');
           variations.push('5,001-10,000'); // Include 5k-10k bucket
       }

       const subOrs = [];
       for (const v of variations) {
          subOrs.push(`tc.employee_count ILIKE $${paramIndex}`);
          params.push(`%${v}%`);
          paramIndex++;
       }
       sizeChecks.push(`(${subOrs.join(' OR ')})`);
    }
    
    if (sizeChecks.length > 0) {
        // Use EXISTS to filter jobs based on trusted_companies employee_count
        conditions.push(`EXISTS (
            SELECT 1 FROM trusted_companies tc 
            WHERE tc.company_id = ${JOBS_TABLE}.company_id 
            AND (${sizeChecks.join(' OR ')})
        )`)
    }
  }
      ...
      if (action === 'jobs_with_match_score') {
        ...
        try {
          ...
          const queryParams = {
            ...
            companySize, // Pass companySize to queryParams
            tags,
            dateFrom,
            dateTo,
