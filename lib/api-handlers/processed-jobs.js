...
function extractSalary(salaryStr, title, desc) {
  ...
  if (salaryStr && salaryStr !== 'Open' && salaryStr !== '面议' && salaryStr.length > 1) return salaryStr
      ...
      if (action === 'reprocess') {
        ...
        try {
          ...
          const candidatesForAI = jobs.filter(j => {
             const badLocation = !j.location || j.location === 'Remote' || j.location === 'Unspecified' || j.location.length < 3
             const noSalary = !j.salary || j.salary === 'null' || j.salary === 'Open'
          ...
          // Prioritize: 
          // 1. Jobs with BOTH missing location and salary
          // 2. Jobs with missing location
          // 3. Jobs with missing salary
          // 4. Others
          candidatesForAI.sort((a, b) => {
             const aScore = (!a.location ? 2 : 0) + (!a.salary ? 2 : 0)
             const bScore = (!b.location ? 2 : 0) + (!b.salary ? 2 : 0)
             return bScore - aScore
          })

          // Increase limit to 30 to process more jobs per batch
          // We rely on Vercel/Serverless timeouts, but 30 * 2s = 60s might be close.
          // Let's try 20 to be safe, but process them in parallel batches.
          ...
          if (selectedForAI.length > 0) {
              ...
              for (let i = 0; i < selectedForAI.length; i += chunkSize) {
                  ...
                  await Promise.all(chunk.map(async (job) => {
                      try {
                          const aiResult = await analyzeJobContent(job)
                          if (aiResult) {
                              // Track usage
                              if (aiResult.usage) {
                              ...
                              let aiChanged = false
                              if (aiResult.location && aiResult.location !== job.location) {
                                  job.location = truncateString(aiResult.location, FIELD_LIMITS.location)
                                  job.region = classifyRegion(job.location) // Re-classify region based on AI location
                                  aiChanged = true
                              }
                              if (aiResult.salary && aiResult.salary !== job.salary) {
                                  job.salary = truncateString(aiResult.salary, FIELD_LIMITS.salary)
                                  aiChanged = true
                              }
                              if (aiResult.tags && Array.isArray(aiResult.tags)) {
                                  // Truncate individual tags
                                  const safeTags = aiResult.tags.map(t => truncateString(t, 50)).slice(0, 20)
                                  job.tags = safeTags
                                  aiChanged = true
                              }
                              if (aiResult.category && aiResult.category !== job.category) {
                                  job.category = truncateString(aiResult.category, FIELD_LIMITS.category)
                                  aiChanged = true
                              }
                              // Only update description if it was reformatted meaningfully
                              if (aiResult.formattedDescription && aiResult.formattedDescription.length > job.description.length * 0.8) {
                                  job.description = truncateString(aiResult.formattedDescription, FIELD_LIMITS.description)
                                  aiChanged = true
                              }
