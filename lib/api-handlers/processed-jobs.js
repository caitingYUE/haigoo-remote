...
function normalizeBreakdown(details = {}) {
  ...
  return {
    titleMatch: Math.max(0, Math.min(100, Math.round(Number(raw.titleMatch) || 0))),
    roleTypeMatch: Math.max(0, Math.min(100, Math.round(Number(raw.roleTypeMatch) || 0))),
...
function synthesizeMatchDetails({ score = 0, details = {}, jobRow = {} }) {
  ...
  if (normalized.titleMatch >= 85) strengths.push('岗位标题与求职方向高度一致')
  if (normalized.roleTypeMatch >= 82) strengths.push('岗位角色类型与职业定位贴合')
  if (normalized.skillMatch >= 72) strengths.push('核心技能契合度较高')
  if (normalized.keywordSimilarity >= 66) strengths.push('简历经历与岗位职责关联度较高')
  if (normalized.experienceMatch >= 70) strengths.push('经验层级与岗位要求接近')
  if (normalized.preferenceMatch >= 65) strengths.push('岗位类型与求职偏好一致')

  if (normalized.titleMatch < 55) suggestions.push('把目标岗位名称和目标职责写得更明确')
  if (normalized.roleTypeMatch < 55) suggestions.push('补充更贴近该岗位方向的项目或成果')
