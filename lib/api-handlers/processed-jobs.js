...
async function getJobsWithMatchScores(userId, queryParams = {}, pagination = {}) {
  ...
  try {
    ...
    if (queryParams.sortBy === 'recent') {
      ...
      orderByClause = `ORDER BY 
        CASE WHEN COALESCE(ujm.match_score, 0) >= 80 THEN 1 ELSE 0 END DESC, -- 强相关优先
        CASE WHEN j.is_featured = true THEN 1 ELSE 0 END DESC,               -- 置顶次之
        COALESCE(ujm.match_score, 0) DESC,                                   -- 其他相关度
        j.published_at DESC`                                                 -- 最后按时间
    }
...
function scatterJobs(jobs) {
  ...
  const canInsert = (job, currentList) => {
    ...
    // 3. 岗位类型打散
    if (backlog.length < 10 && recentJobs.length >= 3) {
