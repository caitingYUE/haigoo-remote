...
function extractSalary(salaryStr, title, desc) {
  ...
  const salaryPatterns = [
    // Standard Global: $100k-$150k
    /(\$\d{2,3}k\s*-\s*\$\d{2,3}k)/i,
    // Standard Global: $100,000 - $150,000
    /(\$\d{1,3}(?:,\d{3})*\s*-\s*\$\d{1,3}(?:,\d{3})*)/i,
    // Chinese specific: 20k-30k/月, 20-30k*14 (Prioritize this to capture suffix)
    /(\d{1,3}k\s*[-–—]\s*\d{1,3}k(?:\/[\w\u4e00-\u9fa5]+)?)/i,
    // Chinese/Short: 15k-25k, 15k-25k RMB
    /(\d{2,3}k\s*-\s*\d{2,3}k(?:\s*RMB|\s*CNY)?)/i,
    // Monthly/Specific Currency: 3000-4000 USD/Month (Support numbers without commas)
    /((?:\d{1,3}(?:,\d{3})*|\d{3,})\s*-\s*(?:\d{1,3}(?:,\d{3})*|\d{3,})\s*(?:USD|EUR|GBP|CNY|RMB|SGD|AUD|CAD|JPY|KRW))/i,
    // Explicit Labels: Salary: ...
    /(?:Salary|Pay|Compensation|薪资|待遇|月薪|年薪)[:\s]*([\d,]+(?:\.\d+)?\s*[kK万]?\s*[-–—to至]\s*[\d,]+(?:\.\d+)?\s*[kK万]?\s*(?:[A-Z]{3}|USD|RMB|元|美元)?)/i,
    // Simple Range with k: 20-40k (Very common in Chinese JDs)
