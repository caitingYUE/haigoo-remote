  ...
  */
  ...
  if (queryParams.location) {
    ...
    for(const loc of locs) {
       ...
       if (loc === 'China') {
           const keywords = [...MAINLAND_KEYWORDS, ...GREATER_CHINA_KEYWORDS];
           const keywordPatterns = keywords.map(k => `%${k}%`);
           locChecks.push(`${JOBS_TABLE}.location ILIKE ANY($${paramIndex})`);
           ...
           const keywords = EUROPE_KEYWORDS;
           const keywordPatterns = keywords.map(k => `%${k}%`);
           locChecks.push(`${JOBS_TABLE}.location ILIKE ANY($${paramIndex})`);
           params.push(keywordPatterns);
           paramIndex++;
           ...
           const keywords = APAC_KEYWORDS_EXTENDED;
           const keywordPatterns = keywords.map(k => `%${k}%`);
           locChecks.push(`${JOBS_TABLE}.location ILIKE ANY($${paramIndex})`);
           params.push(keywordPatterns);
           paramIndex++;
