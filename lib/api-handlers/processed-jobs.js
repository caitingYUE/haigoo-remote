...
function classifyRegion(location) {
  ...
  // 全球不限地点关键词
  const globalKeywords = ['anywhere', 'everywhere', 'worldwide', 'global', '不限地点', 'remote']
  const isGlobal = globalKeywords.some(k => loc.includes(k))

  // 中国可申关键词
  const domesticKeywords = [
    'china', '中国', 'cn', 'chinese', 'apac', 'asia pacific', 'east asia',
    'greater china', 'utc+8', 'gmt+8', 'cst', 'beijing', 'shanghai',
    'shenzhen', 'guangzhou', 'hangzhou', 'chengdu', 'taiwan', 'taipei',
    'hong kong', 'hongkong', 'hk', '台湾', '香港', '深圳',
    '北京', '上海', '广州', '杭州', '成都', '重庆', '南京', '武汉',
    '西安', '苏州', '天津', '大连', '青岛', '厦门', '澳门', 'macau'
  ]

  const isDomestic = domesticKeywords.some(k => loc.includes(k))

  // 海外关键词（明确的海外地区）
  const overseasKeywords = [
    'usa', 'united states', 'us', 'america', 'uk', 'united kingdom', 'england',
    'britain', 'canada', 'mexico', 'brazil', 'argentina', 'chile', 'peru',
    'colombia', 'latam', 'latin america', 'europe', 'eu', 'emea', 'germany',
    'france', 'spain', 'italy', 'netherlands', 'belgium', 'sweden', 'norway',
    'denmark', 'finland', 'poland', 'czech', 'ireland', 'switzerland',
    'australia', 'new zealand', 'oceania', 'india', 'pakistan', 'bangladesh',
    'sri lanka', 'nepal', 'japan', 'korea', 'south korea', 'singapore',
    'malaysia', 'indonesia', 'thailand', 'vietnam', 'philippines', 'uae',
    'dubai', 'saudi', 'turkey', 'russia', 'israel', 'africa', 'egypt'
  ]
