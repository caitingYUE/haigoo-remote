  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'reprocess') {
        ...
        try {
          // Fetch all jobs
          // Limit to 200 (was 5000) for Regex, to keep it fast. AI will pick from these.
          const jobs = await neonHelper.query(`SELECT * FROM ${JOBS_TABLE} ORDER BY created_at DESC LIMIT 200`)
          let updatedCount = 0
          let aiCount = 0
          ...
          }).slice(0, 5) // Limit to 5 per request to save time/cost

          if (candidatesForAI.length > 0) {
              console.log(`[processed-jobs] Starting AI analysis for ${candidatesForAI.length} jobs...`)
              await Promise.all(candidatesForAI.map(async (job) => {
                  try {
