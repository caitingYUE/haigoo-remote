  ...
  */
  ...
  if (typeof queryParams.isTranslated !== 'undefined') {
    ...
    if (boolVal) {
      ...
      // When querying for untranslated (false), also include:
      // 1. Explicitly untranslated (IS NOT TRUE)
      // 2. "Fake" translated (IS TRUE but null/empty translations)
      // 3. "English" content (IS TRUE but NO Chinese characters in title AND description)
      //    This handles "C++" case correctly (if desc has Chinese, it's valid)
      conditions.push(`(
        is_translated IS NOT TRUE 
        OR (
          is_translated IS TRUE AND (
            translations IS NULL 
            OR translations::text = 'null' 
            OR translations::text = '{}'
            OR (
              COALESCE(translations->>'title', '') !~ E'[\\\\u4e00-\\u9fa5]'
              AND
              COALESCE(translations->>'description', '') !~ E'[\\\\u4e00-\\u9fa5]'
            )
          )
        )
      )`)
      ...
      if (action === 'jobs_with_match_score') {
        ...
        try {
          ...
          const queryParams = {
            ...
            isAdmin // Pass isAdmin status from token
          }
