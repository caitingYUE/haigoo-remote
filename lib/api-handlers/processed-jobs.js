...
function buildSearchOrderClause(searchTerm, startIndex) {
  ...
  if (tokens.length > 1) {
    for (const token of tokens) {
      ...
      tokenMetaChecks.push(`(${JOBS_TABLE}.category ILIKE ${tokenParam} OR COALESCE(${JOBS_TABLE}.industry, tc.industry, '') ILIKE ${tokenParam} OR ${JOBS_TABLE}.location ILIKE ${tokenParam} OR ${JOBS_TABLE}.job_type ILIKE ${tokenParam} OR ${JOBS_TABLE}.tags::text ILIKE ${tokenParam} OR COALESCE(${JOBS_TABLE}.translations::text, '') ILIKE ${tokenParam})`)
    }
  }
  ...
  return {
    clause: `ORDER BY (
      ...
      CASE WHEN COALESCE(${JOBS_TABLE}.translations::text, '') ILIKE ${phraseParam} THEN 260 ELSE 0 END +
      CASE WHEN ${JOBS_TABLE}.experience_level ILIKE ${phraseParam} THEN 220 ELSE 0 END +
