    ...
    if (req.method === 'GET') {
        ...
        if (payload) {
          ...
          if (payload.userId) {
            try {
              ...
              isMember = Boolean(requester?.roles?.admin || isMembershipActive(requester))
            } catch (memberErr) {
      ...
      if (!isMember && items.length > 0) {
        items = items.map(job => {
          ...
          return {
            ...rest,
            hiringEmail: job?.memberOnly ? null : rest.hiringEmail,
            emailType: job?.memberOnly ? null : rest.emailType,
