  ...
  */
  ...
  if (queryParams.industry) {
    ...
    for (const ind of industries) {
      ...
      indChecks.push(`(COALESCE(${JOBS_TABLE}.industry, tc.industry, '') ILIKE $${paramIndex})`)
      params.push(ind)
      paramIndex++
    }
...
function buildSearchOrderClause(searchTerm, startIndex) {
  ...
  if (tokens.length > 1) {
    for (const token of tokens) {
      ...
      tokenMetaChecks.push(`(${JOBS_TABLE}.category ILIKE ${tokenParam} OR COALESCE(${JOBS_TABLE}.industry, tc.industry, '') ILIKE ${tokenParam})`)
    }
  }
