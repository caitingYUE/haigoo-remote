...
function buildWhereClause(queryParams) {
  ...
  if (regionParam) {
    ...
    if (regions.includes('overseas')) {
      ...
      // 注意：如果同时选了 domestic 和 overseas，这里简单的 OR 会导致 overseas 覆盖 domestic 的 exclude 逻辑
      // 但这里的 overseas 逻辑是 "is overseas", 不是 "is only overseas".
      // 原逻辑: (is overseas keywords) AND (NOT china keywords)
      // 如果 OR domestic，那么结果是 (is china) OR (is overseas AND NOT china). 结果是合理的。
      regionConditions.push(`(
