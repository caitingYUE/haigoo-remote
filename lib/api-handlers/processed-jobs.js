  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'translate') {
        ...
        if (!jobIds.length || jobIds.length > 5) return res.status(400).json({ error: '每次最多翻译 5 个岗位' })
        ...
        try {
          ...
          if (translateJobs) {
            ...
            return res.status(200).json({ success: true, count: translatedJobs.length, failedCount: 0 })
          } else {
