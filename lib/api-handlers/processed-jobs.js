...
const VIP_DIVERSITY_WINDOW = 10
const VIP_DIVERSITY_LIMIT = 4
const VIP_DIVERSITY_FIRST_N = 30
...
function canPlaceRankedJob(job, currentList, options = {}) {
  ...
  if (enforceVipDiversity && currentList.length < VIP_DIVERSITY_FIRST_N && isVipRankedJob(job)) {
    const recentVipWindow = currentList.slice(-VIP_DIVERSITY_WINDOW)
    const vipCount = recentVipWindow.filter(isVipRankedJob).length
    if (vipCount >= VIP_DIVERSITY_LIMIT) return false
  }
