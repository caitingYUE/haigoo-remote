...
async function getJobsWithMatchScores(userId, queryParams = {}, pagination = {}, options = {}) {
  ...
  try {
    ...
    const shouldUseLegacySqlRanking = isRecentSort || !!queryParams.search
    ...
    if (shouldUseLegacySqlRanking) {
      ...
      const scoredRows = candidateRows.map(row => {
        ...
        const visibleScore = rawScore >= MATCH_SCORE_VISIBLE_THRESHOLD ? rawScore : 0
      ...
      pagedRows = scoredRows.slice(offset, offset + limit)
    }

    const rowsWithMatchLevel = (pagedRows || []).map(row => {
      const normalizedScore = Math.max(0, Math.min(100, Math.round(Number(row.match_score) || 0)))
