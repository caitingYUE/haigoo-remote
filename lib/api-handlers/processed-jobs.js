...
function buildWhereClause(queryParams) {
  ...
  // 基本字段过滤
  if (queryParams.ids) {
    const delimiter = queryParams.ids.includes('|') ? '|' : ',';
    conditions.push(`${JOBS_TABLE}.job_id = ANY(string_to_array($${paramIndex}, '${delimiter}'))`)
    ...
    if (req.method === 'GET') {
      ...
      if (action === 'featured_home') {
        ...
        try {
          ...
          const buildDailyFeaturedSelection = (jobs = [], limit = 6) => {
            const companyCounts = {}
            const dispersedJobs = []

            for (const job of jobs) {
              if (dispersedJobs.length >= Math.max(limit * 4, 24)) break
            ...
            if (dispersedJobs.length <= limit) {
              return dispersedJobs.slice(0, limit)
            }

            const fixedJobs = dispersedJobs.slice(0, Math.min(2, limit))
            const remainingSlots = Math.max(limit - fixedJobs.length, 0)
            if (remainingSlots === 0) return fixedJobs

            const rotatingPool = dispersedJobs.slice(fixedJobs.length, Math.min(dispersedJobs.length, fixedJobs.length + 12))
            if (rotatingPool.length <= remainingSlots) {
              return [...fixedJobs, ...rotatingPool].slice(0, limit)
            }
            ...
            for (let index = 0; index < rotatingPool.length; index += 1) {
              rotatedJobs.push(rotatingPool[(offset + index) % rotatingPool.length])
              if (rotatedJobs.length >= remainingSlots) break
            }

            return [...fixedJobs, ...rotatedJobs].slice(0, limit)
          }
          ...
          const jobs = buildDailyFeaturedSelection(allJobs, 6)
