      ...
      if (action === 'reprocess') {
        ...
        try {
          ...
          const candidatesForAI = jobs.filter(j => {
             ...
             const noSalary = !j.salary || j.salary === 'null'
          ...
          // Shuffle candidates to ensure we don't get stuck on the same unfixable jobs
          const selectedForAI = candidatesForAI.sort(() => Math.random() - 0.5).slice(0, 5)
          ...
          if (selectedForAI.length > 0) {
              ...
              await Promise.all(selectedForAI.map(async (job) => {
                  try {
