...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (shouldUseLocalJobsPreview(req)) {
        const preview = getPreviewJobsResponse(req.query || {}, { page: pageNum, limit: pageSize })
        return res.status(200).json({
          ...preview,
          jobs: metadataOnly === 'true' ? [] : preview.jobs,
      ...
      if (shouldUseLocalPreviewFallback) {
        const preview = getPreviewJobsResponse(req.query || {}, { page: pageNum, limit: pageSize })
        items = metadataOnly === 'true' ? [] : preview.jobs
        total = preview.total
