...
function buildSearchOrderClause(searchTerm, startIndex) {
  ...
  if (tokens.length > 1) {
    for (const token of tokens) {
      ...
      tokenMetaChecks.push(`(${JOBS_TABLE}.category ILIKE ${tokenParam} OR COALESCE(${JOBS_TABLE}.industry, tc.industry, '') ILIKE ${tokenParam} OR ${JOBS_TABLE}.location ILIKE ${tokenParam} OR ${JOBS_TABLE}.job_type ILIKE ${tokenParam} OR ${JOBS_TABLE}.tags::text ILIKE ${tokenParam})`)
    }
  }
