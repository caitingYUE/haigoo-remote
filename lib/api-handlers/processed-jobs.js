...
/**
 * 岗位打散算法 (Scatter Jobs) - P1 Optimized Version
 * 优化展示体验，避免同一企业/类型岗位过于集中
 * 
 * 规则：
 * 1. 优先保留原有的排序权重 (内推/置顶/匹配度)
 * 2. 同一企业在局部窗口内不要出现过多 (滑动窗口检查)
 * 3. 岗位类型/类别多样性 (尽力打散)
 * 4. 防止 backlog 无限积压 (定期强制插入)
 * 
 * 2025-12-27 P1优化:
 * - 窗口大小: 8 -> 16
 * - 新增 category 多样性检查
...
function scatterJobs(jobs) {
  ...
  const canInsert = (job, currentList) => {
    ...
    // 4. P1 Fix: 新增 category 多样性检查
    if (shortWindow.length >= 4 && job.category) {
      const sameCategoryCount = shortWindow.filter(j => j.category === job.category).length
      if (sameCategoryCount >= 4) return false
    }
  ...
  // 遍历原始列表
  for (const job of jobs) {
    // P1 Fix: 定期强制处理 backlog，防止无限积压
    // 每插入10个岗位，强制插入一个 backlog 项（如果有的话）
    if (backlog.length > 0 && insertsSinceBacklogCheck >= 10) {
      result.push(backlog.shift())
      insertsSinceBacklogCheck = 0
    }

    // 尝试处理 backlog 中的积压项 (优先释放积压，避免无限推迟)
    for (let i = 0; i < Math.min(backlog.length, 5); i++) { // 限制检查数量提升性能
      if (canInsert(backlog[i], result)) {
        result.push(backlog[i])
        backlog.splice(i, 1)
        insertsSinceBacklogCheck++
        break
      }
    }
