...
import { extractSalary, extractLocation, classifyRegion } from '../services/classification-service.js'
  ...
  */
  ...
  if (queryParams.category) {
    ...
    for (const cat of categories) {
      ...
      // 3. 反向包含匹配 (e.g. "测试/QA" 匹配 "测试")
      
      // 注意：使用 ILIKE 进行包含匹配
      // 如果 cat 是 "测试/QA"，我们想匹配 "测试" 或 "QA" 吗？
      // 实际上，DB存的是 '测试/QA' (从 JOB_KEYWORDS 看)，所以精确匹配应该能行。
      // 但是 '运维/SRE' 在 DB 是 '运维/SRE'。
      // 如果前端传 '运维/SRE'，后端存 '运维/SRE'，精确匹配 OK。
      
      // 但是，为了防止万一（比如旧数据只存了 '测试'），我们加上 OR 逻辑
      // 将 "测试/QA" 拆分为 "测试", "QA"
      const subParts = cat.split(/[/\\]/)
      if (subParts.length > 1) {
         const partChecks = subParts.map(p => `category ILIKE $${paramIndex} OR $${paramIndex} ILIKE category`) // 双向包含
         // 这里参数化比较麻烦，简单点：直接用 ILIKE 匹配整个词，或者匹配其中一部分
         
         // 修正策略：
         // 如果是 "测试/QA"，我们生成 `category ILIKE '%测试%' OR category ILIKE '%QA%'`
         // 这样最稳
         subParts.forEach(p => {
             catChecks.push(`category ILIKE $${paramIndex}`)
             params.push(`%${p}%`)
             paramIndex++
         })
      }
      ...
      // 始终加上原始值的精确/模糊匹配
      catChecks.push(`category ILIKE $${paramIndex}`) 
      params.push(`%${cat}%`) // 使用模糊匹配更安全
      paramIndex++
      
      // 如果是英文输入 (e.g. "Backend")，尝试匹配中文 "后端开发" ?
      // 目前前端只传中文，所以暂时不需要反向映射
    }
  ...
  if (queryParams.industry) {
    conditions.push(`LOWER(industry) = ANY(string_to_array(LOWER($${paramIndex}), ','))`)
