...
import { verifyToken } from '../../server-utils/auth-helper.js'
import { getTrustedCompanies } from './trusted-companies.js'
    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        for (const job of normalized) {
          ...
          if (c) {
            let changed = false
            if (!job.companyWebsite && c.url) { job.companyWebsite = c.url; changed = true }
