    ...
    if (req.method === 'POST') {
      ...
      if (action === 'cleanup') {
        ...
        try {
          ...
          // Note: We keep jobs with invalid published_at (NULL) just in case, or we can choose to delete them.
          // The previous logic was: if (!Number.isFinite(t)) return true (keep it).
          // So we only delete if published_at IS NOT NULL AND published_at < cutoff
          
          const result = await neonHelper.query(
            `DELETE FROM ${JOBS_TABLE} WHERE published_at IS NOT NULL AND published_at < $1 RETURNING job_id`,
            [cutoffDate]
          )
