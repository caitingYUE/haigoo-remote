...
function buildWhereClause(queryParams) {
  ...
  if (queryParams.sourceType) {
    ...
    const hasRss = types.includes('rss') || types.includes('third-party');
    ...
    if (hasTrusted) {
      // Trusted/Official means (is_trusted = true OR source_type = 'trusted') AND can_refer = false
      // This ensures strict separation from "Club Referral"
      sourceConditions.push('((is_trusted = true OR source_type = \'trusted\') AND can_refer = false)');
    }
    
    if (hasRss) {
      // RSS/Third-party means NOT can_refer AND NOT is_trusted (handling NULLs safely)
      sourceConditions.push('((can_refer IS NOT TRUE) AND (is_trusted IS NOT TRUE) AND (source_type IS NULL OR source_type = \'rss\' OR source_type = \'third-party\'))');
    }
...
async function getJobsWithMatchScores(userId, queryParams = {}, pagination = {}) {
  ...
  try {
    ...
    if (queryParams.sortBy === 'recent') {
      ...
      // 默认排序：内推 > 官网直申(精选) > 相关性(>=30) > 时间
      // 如果没有特别的相关性，这和时间排序很像，但内推和精选会置顶
      orderByClause = `
      ORDER BY 
    ...
    console.log(`[getJobsWithMatchScores] Sorting by: ${queryParams.sortBy || 'relevance'} (Clause: ${orderByClause.trim().substring(0, 50)}...)`)
      ...
      for (const job of unique) {
        ...
        await sql.query(`
          INSERT INTO ${JOBS_TABLE} (
            ...
            risk_rating, haigoo_comment, hidden_fields,
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    // Temporarily disable cache to ensure new fields (logo) are propagated
        ...
        try {
          ...
          if (payload && payload.userId) {
             ...
             if (userRes && userRes.length > 0) {
                 const u = userRes[0];
                 isMember = u.member_status === 'active' && u.member_expire_at && new Date(u.member_expire_at) > new Date();
             }
          }
        } catch (e) {
