...
function buildWhereClause(queryParams) {
  ...
  // 默认过滤掉 RSS/Third-party 和 Trusted/Official 来源的岗位，只保留内推
  // 除非是管理员 (isAdmin) 或者是指向特定ID的查询
  // 注意：这里是强制过滤。即使前端传了 sourceType=rss，也会因为 AND can_refer=true 而查不到数据（符合"网站侧不展示"的需求）
  if (!queryParams.isAdmin && !queryParams.id) {
    conditions.push('can_refer = true');
  }
