  ...
  */
  ...
  if (jobType) {
    ...
    for (const t of types) {
        ...
        if (cleanType === 'internship' || cleanType === 'intern') {
            ...
            subConditions.push(`${JOBS_TABLE}.job_type ILIKE $${paramIndex}`); params.push('%实习%'); paramIndex++;
            ...
            subConditions.push(`${JOBS_TABLE}.job_type ILIKE $${paramIndex}`); params.push('%全职%'); paramIndex++;
            ...
            subConditions.push(`${JOBS_TABLE}.job_type ILIKE $${paramIndex}`); params.push('%兼职%'); paramIndex++;
        }
        else if (cleanType === 'contract') {
            subConditions.push(`${JOBS_TABLE}.job_type ILIKE $${paramIndex}`); params.push('%contract%'); paramIndex++;
            subConditions.push(`${JOBS_TABLE}.job_type ILIKE $${paramIndex}`); params.push('%合同%'); paramIndex++;
            ...
            subConditions.push(`${JOBS_TABLE}.job_type ILIKE $${paramIndex}`); params.push('%自由%'); paramIndex++;
        }
  ...
  if (queryParams.industry) {
    ...
    for (const ind of industries) {
        ...
        indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`)
        params.push(ind)
        paramIndex++

        // 2. Mapping for English/Fuzzy matching
        if (ind === '人工智能') {
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%AI%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Artificial Intelligence%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Machine Learning%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Technology%'); paramIndex++; // Fallback for general tech if labeled AI
        }
        else if (ind === 'Web3/区块链') {
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Web3%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Blockchain%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Crypto%'); paramIndex++;
        }
        else if (ind === '互联网/软件') {
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Software%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Internet%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Technology%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%SaaS%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%IT%'); paramIndex++;
        }
        else if (ind === '金融/Fintech') {
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Finance%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Fintech%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Banking%'); paramIndex++;
             indChecks.push(`tags ?| ARRAY['Fintech', 'Finance', 'Banking', '金融', '银行']`);
        }
        else if (ind === '电子商务') {
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%E-commerce%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Retail%'); paramIndex++;
        }
        else if (ind === '游戏/娱乐') {
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Game%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Gaming%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Entertainment%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Media%'); paramIndex++;
        }
        else if (ind === '企业服务/SaaS') {
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Enterprise%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%SaaS%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%B2B%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Business%'); paramIndex++;
        }
        else if (ind === '硬件/物联网') {
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Hardware%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%IoT%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Semiconductor%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Electronics%'); paramIndex++;
        }
        else if (ind === '大健康/医疗') {
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Health%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Medical%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Bio%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Pharma%'); paramIndex++;
        }
        else if (ind === '教育') {
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Education%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%EdTech%'); paramIndex++;
             indChecks.push(`${JOBS_TABLE}.industry ILIKE $${paramIndex}`); params.push('%Training%'); paramIndex++;
        }
    }
...
async function readJobsFromNeon(queryParams = {}, pagination = {}) {
  ...
  try {
    ...
    // Default / "Relevance" (Business Logic)
      ...
      orderByClause = `ORDER BY 
        CASE WHEN is_featured = true THEN 1 ELSE 0 END DESC,
    ...
    const query = `
      SELECT *, 
        ...
        (SELECT industry FROM trusted_companies tc WHERE tc.company_id = ${JOBS_TABLE}.company_id) as trusted_industry,
        (SELECT hiring_email FROM trusted_companies tc WHERE tc.company_id = ${JOBS_TABLE}.company_id) as trusted_hiring_email,
        (SELECT email_type FROM trusted_companies tc WHERE tc.company_id = ${JOBS_TABLE}.company_id) as trusted_email_type
      FROM ${JOBS_TABLE}
      ${where}
...
async function getJobsWithMatchScores(userId, queryParams = {}, pagination = {}) {
  ...
  try {
    ...
    // 优化：如果包含搜索词或特定过滤，使用相关性排序；否则默认按时间排序
    // 防止无筛选条件时 ORDER BY match_score 导致全表扫描性能问题
    ...
    if (queryParams.sortBy === 'recent' || queryParams.sortBy === 'published_at_desc') {
      ...
      // 默认推荐排序：
      // 1. 如果有搜索词，按相关性（DB层已处理）
      // 2. 如果无搜索词，按 匹配分(AI) > 岗位追踪 > 置顶 > 内推 > 时间

      // 注意：ujm.match_score 已经包含了 AI 评分
      // 我们可以进一步联合其他信号，比如是否在追踪列表（这里暂未关联追踪表，但可以根据 match_score >= 80 视为高相关）

      // 优化逻辑：即使无搜索词，只要是已登录用户（match_score > 0），就优先展示高分岗位
      // 对于未登录用户（match_score 全为 0），回退到 Feature/Trust/Time 排序
    ...
    // ⚠️ 2026-01-07 Fix: Removed redundant filter, buildWhereClause already handles it
    ...
    // 获取聚合数据 (Facets) - Use the same query params
