...
async function getApprovedLocationTaxonomyAggregations() {
  const options = JOB_LOCATION_FILTER_OPTIONS
    .map(option => ({ option, pattern: buildPostgresLocationPattern(option) }))
    .filter(item => item.pattern)

  if (options.length === 0) return []

  const selectColumns = options.map((_, index) => (
    `COUNT(*) FILTER (WHERE ${JOBS_TABLE}.location ~* $${index + 1})::int AS "location_${index}"`
  ))
  ...
  `, options.map(item => item.pattern))
  const counts = rows?.[0] || {}

  return options
    .map(({ option }, index) => ({
      value: option.value,
      count: Number(counts[`location_${index}`] || 0)
    }))
