import { analyzeJobContent } from '../../lib/deepseek-parser.js'
...
import { parseSalaryString } from '../services/classification-service.js'
...
function extractLocationFromText(text) {
  ...
  if (/\b(remote|wfh|work from home|distributed|anywhere|远程|在家办公)\b/i.test(cleanText)) {
      ...
      const remoteRegion = cleanText.match(/(?:remote|远程)\s*[-–—]\s*([A-Za-z\s]+)/i);
      if (remoteRegion && remoteRegion[1] && remoteRegion[1].length < 20) {
          return `Remote - ${remoteRegion[1].trim()}`;
      }
      return 'Remote';
  }
...
function extractSalary(salaryStr, title, desc) {
  ...
  if (salaryStr && salaryStr !== 'Open' && salaryStr !== '面议' && salaryStr !== 'Competitive' && salaryStr !== 'null' && salaryStr.length > 1) return salaryStr
  ...
  // Patterns for salary: 
  // $100k-$150k, $100,000 - $150,000, 15k-25k RMB
  // 3000-4000 USD/Month
  // 15-25k
  const salaryPatterns = [
    ...
    /(\d{2,3}k\s*-\s*\d{2,3}k)/i,
    /(\d{1,3}(?:,\d{3})*\s*-\s*\d{1,3}(?:,\d{3})*\s*(?:USD|EUR|GBP|CNY|RMB|SGD|AUD|CAD|JPY|KRW))/i,
    /(?:Salary|Pay|Compensation|薪资|待遇|月薪|年薪)[:\s]*([\d,]+(?:\.\d+)?\s*[kK万]?\s*[-–—to至]\s*[\d,]+(?:\.\d+)?\s*[kK万]?\s*(?:[A-Z]{3}|USD|RMB|元|美元)?)/i,
    /(\d+\s*[-–—]\s*\d+\s*k)/i
  ]
...
function mapRowToJob(row) {
  ...
  return {
    ...
    salary: typeof row.salary === 'string' ? parseSalaryString(row.salary) : row.salary,
  ...
  try {
    ...
    if (queryParams.sortBy === 'recent') {
      ...
      orderByClause = `ORDER BY 
        ...
        CASE WHEN j.is_featured = true THEN 1 ELSE 0 END DESC,               
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'reprocess') {
        ...
        try {
          ...
          // 1. Regex Processing (Fast, for all)
          const updates = jobs.map(job => {
            // Skip manually edited jobs
            if (job.is_manually_edited) return null
          ...
          // 2. AI Processing (Slow, for top candidates)
          // Filter jobs that still need help after Regex
          // Criteria: Bad Location OR No Salary OR Few Tags
          const candidatesForAI = jobs.filter(j => {
             // 提取失败的定义：
             // 1. 地点：为空、Unspecified、长度小于3（除非是 'US', 'UK' 等合法缩写，但 extractLocationFromText 应该已经处理过了）
             //    如果 location 包含 'Remote' 且没有后续区域（如 'Remote - US'），我们认为它可能还需要进一步提取具体区域（如果 JD 里有）
             //    但用户主要抱怨提取不到，所以重点是空值或 'Unspecified'。
             //    如果 location 已经是 'Remote'，我们暂且认为它是“提取成功”的，除非我们要追求 'Remote - China'。
             //    用户说“隐藏在jd里的地点和薪资几乎都没提取出来”，说明现在很多是 null 或 Unspecified。
             const badLocation = !j.location || j.location === 'Unspecified' || j.location.length < 2
             
             // 2. 薪资：为空、null、Open、Competitive、面议
             const noSalary = !j.salary || j.salary === 'null' || j.salary === 'Open' || j.salary === 'Competitive' || j.salary === '面议'
             
             // Handle tags being potentially string or array
             let tagCount = 0
             try {
                if (Array.isArray(j.tags)) tagCount = j.tags.length
                else if (typeof j.tags === 'string') tagCount = JSON.parse(j.tags).length
             } catch (e) {}
             const fewTags = tagCount < 2
             
             // 必须有足够的描述内容供 AI 分析
             return (badLocation || noSalary || fewTags) 
                && j.description && j.description.length > 50
                && !j.is_manually_edited
          })
          
          // Prioritize: 
          // 1. Jobs with BOTH missing location and salary (Score 4)
          // 2. Jobs with missing salary (Score 2) - User emphasized salary extraction
          // 3. Jobs with missing location (Score 2)
          // 4. Others (Score 0)
          candidatesForAI.sort((a, b) => {
             const getScore = (job) => {
                 let score = 0;
                 if (!job.location || job.location === 'Unspecified') score += 2;
                 if (!job.salary || job.salary === 'null' || job.salary === 'Open' || job.salary === 'Competitive') score += 2;
                 return score;
             }
             return getScore(b) - getScore(a); // Descending order
          })

          // Process ALL candidates if possible, but limit to avoid timeout
          // User complained about "nothing extracted", so we should try to process as many as possible within limits.
          // Vercel limit is 10s (hobby) or 60s (pro).
          // 50 jobs * 2s = 100s, too risky. 
          // Let's stick to 20 but ensure they are the MOST important ones.
          const selectedForAI = candidatesForAI.slice(0, 20)

          if (selectedForAI.length > 0) {
              console.log(`[processed-jobs] Starting AI analysis for ${selectedForAI.length} jobs (pool: ${candidatesForAI.length})...`)
              
              // Process in chunks of 5 to avoid rate limits
              const chunkSize = 5
              for (let i = 0; i < selectedForAI.length; i += chunkSize) {
                  const chunk = selectedForAI.slice(i, i + chunkSize)
                  await Promise.all(chunk.map(async (job) => {
                      try {
                          // Strip HTML from description to save tokens and reduce noise for AI
                          // We create a temporary job object with clean description for AI
                          ...
                          if (aiResult) {
                              ...
                              // Update Location
                              // Only update if current is bad OR AI found something better (e.g. Remote -> Remote - US)
                              const isCurrentLocationBad = !job.location || job.location === 'Unspecified' || job.location.length < 2;
                              const isAiLocationBetter = aiResult.location && aiResult.location !== 'Unspecified' && aiResult.location !== 'Remote';
                              
                              if (aiResult.location && (isCurrentLocationBad || (job.location === 'Remote' && isAiLocationBetter))) {
                              ...
                              // Update Salary
                              // Only update if current is bad OR AI found specific numbers
                              const isCurrentSalaryBad = !job.salary || job.salary === 'null' || job.salary === 'Open' || job.salary === 'Competitive';
                              const isAiSalarySpecific = aiResult.salary && /\d/.test(aiResult.salary);
                              
                              if (aiResult.salary && (isCurrentSalaryBad || isAiSalarySpecific)) {
                              ...
                              if (aiResult.tags && Array.isArray(aiResult.tags) && aiResult.tags.length > 0) {
                                  ...
                                  const mergedTags = [...new Set([...existingTags, ...newTags])].slice(0, 20);
                              ...
                              // Update Category
                              if (aiResult.category && (!job.category || job.category === '其他' || job.category === 'Other')) {
                              ...
                              if (aiChanged) {
                                  aiCount++
                                  // Add to updates array if not already there
                                  if (!updates.find(u => u.job_id === job.job_id)) {
                                      updates.push(job)
                                  }
                              }
                          }
                      } catch (err) {
                          console.error(`[processed-jobs] AI analysis failed for job ${job.job_id}:`, err)
                      }
                  }))
              }
          }
          ...
          console.log(`[processed-jobs] Reprocessed jobs. Regex updates: ${updatedCount - aiCount}, AI updates: ${aiCount}`)
          return res.status(200).json({ success: true, processed: jobs.length, updated: updatedCount, ai_updated: aiCount })
        } catch (e) {
