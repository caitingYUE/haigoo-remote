...
function buildWhereClause(queryParams) {
  ...
  // 基本字段过滤
  if (queryParams.ids) {
    conditions.push(`job_id = ANY(string_to_array($${paramIndex}, ','))`)
  ...
  if (queryParams.source) {
    if (queryParams.source === 'special:official') {
      conditions.push(`(is_trusted = true OR source_type = 'official')`)
    } else if (queryParams.source === 'special:manual') {
      conditions.push(`(source = 'manual' OR is_manually_edited = true)`)
    } else {
  ...
  */
  ...
  if (queryParams.category && typeof queryParams.category === 'string' && queryParams.category.trim() !== '') {
    ...
    for (const cat of categories) {
      ...
      // A. 原始值模糊匹配
      subConditions.push(`category ILIKE $${paramIndex}`)
      params.push(`%${cat}%`)
      paramIndex++
      ...
      englishKeywords.forEach(kw => {
          subConditions.push(`category ILIKE $${paramIndex}`) // Match category field
          params.push(`%${kw}%`)
          paramIndex++
  ...
  if (queryParams.location) {
    ...
    for(const loc of locs) {
       locChecks.push(`location ILIKE $${paramIndex}`);
       params.push(`%${loc}%`);
       paramIndex++;
    }
  ...
  if (queryParams.timezone) {
    conditions.push(`timezone = ANY(string_to_array($${paramIndex}, ','))`)
  ...
  if (queryParams.isFeatured === 'true' || queryParams.isFeatured === true) {
    ...
    conditions.push(`(is_featured IS NOT TRUE)`)
  }
  ...
  if (jobType) {
    conditions.push(`LOWER(job_type) = ANY(string_to_array(LOWER($${paramIndex}), ','))`)
    params.push(jobType)
    paramIndex++
  }
  ...
  if (queryParams.experienceLevel) {
    conditions.push(`LOWER(experience_level) = ANY(string_to_array(LOWER($${paramIndex}), ','))`)
  ...
  if (queryParams.industry) {
    ...
    for (const ind of industries) {
        ...
        indChecks.push(`industry ILIKE $${paramIndex}`)
        params.push(ind)
        paramIndex++

        // 2. Mapping for English/Fuzzy matching
        if (ind === '人工智能') {
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%AI%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Artificial Intelligence%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Machine Learning%'); paramIndex++;
        }
        else if (ind === 'Web3/区块链') {
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Web3%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Blockchain%'); paramIndex++;
             indChecks.push(`industry ILIKE $${paramIndex}`); params.push('%Crypto%'); paramIndex++;
        }
        else if (ind === '互联网/软件') {
  ...
  try {
    ...
    const query = `
      SELECT 
        ...
        (SELECT industry FROM trusted_companies tc WHERE tc.company_id = j.company_id) as trusted_industry,
    ...
    // 如果是最新排序，不进行打散，保持时间顺序
    const finalJobs = queryParams.sortBy === 'recent' ? formattedJobs : scatterJobs(formattedJobs)
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'translate') {
        ...
        try {
            ...
            // 2. Translate
            if (translateJobs) {
                console.log(`[processed-jobs] Translating ${jobs.length} jobs...`)
                // Force translation even if already translated (user request)
                jobs = await translateJobs(jobs)
