...
function buildWhereClause(queryParams) {
  ...
  if (queryParams.sourceType) {
    ...
    if (hasCurated) {
      // Curated means (is_trusted = true OR source_type = 'trusted') AND can_refer = false
...
async function readJobsFromNeon(queryParams = {}, pagination = {}) {
  ...
  try {
    ...
    const query = `
      SELECT *, (SELECT website FROM trusted_companies tc WHERE tc.company_id = ${JOBS_TABLE}.company_id) as trusted_website
      FROM ${JOBS_TABLE}
      ${where}
      ORDER BY 
