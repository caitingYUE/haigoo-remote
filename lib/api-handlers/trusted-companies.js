...
export async function getCompaniesWithJobStats({ page = 1, limit = 20, sortBy = 'updatedAt', sortOrder = 'desc', industry, search, canRefer, region, minJobs = 1, jobCategories }) {
    try {
        if (NEON_CONFIGURED) {
            ...
            // 2. Fetch Job Samples for these companies
            // We only need samples for the companies on this page
            const companyIds = companyResult.map(c => c.company_id)
            let jobSamplesMap = {}
            
            if (companyIds.length > 0) {
                // Limit samples per company to avoid huge payload? 
                // The original query used jsonb_agg(DISTINCT ...) which gets all categories.
                // We want all categories/titles for the frontend to display stats.
                // Let's fetch basic job info.
                
                // Use placeholders for IN clause
                const idPlaceholders = companyIds.map((_, i) => `$${i + 1}`).join(',')
                const jobsQuery = `
                    SELECT company_id, category, title 
                    FROM jobs 
                ...
                if (jobsResult) {
                    jobsResult.forEach(job => {
                        if (!jobSamplesMap[job.company_id]) {
                            jobSamplesMap[job.company_id] = []
                        }
                        jobSamplesMap[job.company_id].push({
            ...
            if (minJobs > 0 || conditions.length > 0) {
                ...
                // Optimization: If no search/industry filter, we can use a pre-calculated list or simpler query?
                // For now, let's use a dynamic query to ensure accuracy (as requested "avoid empty results")
                // We need to join jobs table here.
                
                // Note: We need to use the SAME conditions to filter companies, 
                // THEN get active jobs from those companies.
                
                // Reconstruct params for this specific query to avoid conflict?
                // Actually we can reuse 'params' but we need to be careful if we added extra params for companyQuery pagination.
                // 'params' currently contains only filter params. Pagination params were added to a copy 'queryParams'.
                // So 'params' is safe to reuse.
                
                // Query: Get distinct categories from jobs where company_id is in (filtered companies)
                // BUT 'baseQuery' + 'whereClause' defines the filtered companies.
                // So: SELECT DISTINCT j.category FROM jobs j WHERE j.status='active' AND j.company_id IN (SELECT tc.company_id ${baseQuery} ${whereClause})
                
                // Performance warning: Nested IN with complex subquery might be slow.
                // Alternative: SELECT DISTINCT category FROM jobs j JOIN trusted_companies tc ON j.company_id = tc.company_id WHERE j.status='active' AND ... (tc filters)
                
                // Let's use JOIN approach for category aggregation
                const categoryParams = [...params]; // Copy params
                let catParamIndex = params.length + 1;
                
                // We need to map 'tc.' conditions to the join query.
                // Luckily our conditions use 'tc.' prefix or 'EXISTS'.
                ...
                // Limit to top categories to avoid huge lists? Frontend usually handles 20-50 ok.
                // Let's just get all distinct.
                ...
                // Global categories (from all active trusted jobs)
                // Use cache or simple query
            ...
            // Map results
            const companies = companyResult.map(row => {
                const jobSamples = jobSamplesMap[row.company_id] || []
                const categories = {}
                    ...
                    if (jobsResult) {
                        jobsResult.forEach(job => {
                            if (!statsMap[job.company_id]) {
                                statsMap[job.company_id] = { total: 0, categories: {} }
                            }
                            statsMap[job.company_id].total++
                            const cat = job.category || '其他'
                            statsMap[job.company_id].categories[cat] = (statsMap[job.company_id].categories[cat] || 0) + 1
                        })
                    }
                }
                ...
                try {
                    ...
                    if (NEON_CONFIGURED) {
                        await neonHelper.transaction(async (client) => {
                            ...
                            await writeJobsToNeon(enrichedJobs, 'upsert', true, client)
