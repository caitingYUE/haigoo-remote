...
export async function getCompaniesWithJobStats({ page = 1, limit = 20, sortBy = 'job_count', sortOrder = 'desc', industry, search, canRefer, region, minJobs = 0 }) {
    try {
        if (NEON_CONFIGURED) {
            const offset = (page - 1) * limit
            ...
            if (minJobs > 0) {
                 // If we are filtering by minJobs, we can sum up job_count from the filtered result
                 // But wait, the main query has pagination. We need a separate sum query.
                 // Let's create a sum query reusing the conditions
                 
                 let sumQuery = `
                    SELECT SUM(tc.job_count) as total_jobs
                    FROM trusted_companies tc
                    LEFT JOIN jobs j ON tc.company_id = j.company_id
                 `
                 ...
                 // Reuse conditions
                 if (conditions.length > 0) {
                    sumQuery += ' WHERE ' + conditions.join(' AND ')
                 }
                 
                 // We don't need group by for sum
                 // But wait, the WHERE clause might contain aggregate conditions?
                 // No, conditions are on tc or j columns.
                 // BUT, the `minJobs` condition `tc.job_count >= X` is valid.
                 // However, the original query had GROUP BY tc.company_id
                 // If we join jobs, we get multiple rows per company.
                 // If we SUM(tc.job_count), we will sum it N times (N = number of jobs).
                 // That's wrong.
                 // We should query trusted_companies table directly for sum, if possible.
                 // But the filter conditions might involve jobs table (e.g. search j.title).
                 
                 // Correct approach:
                 // 1. Get distinct companies matching criteria
                 // 2. Sum their job_count
                 
                 sumQuery = `
                    SELECT SUM(sub.job_count) as total_jobs
                    FROM (
                        SELECT DISTINCT tc.company_id, tc.job_count
                        FROM trusted_companies tc
                        LEFT JOIN jobs j ON tc.company_id = j.company_id
                 ...
                 const sumResult = await neonHelper.query(sumQuery, params)
