...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'GET' && action === 'featured_home') {
            ...
            try {
                // 新逻辑：先获取最新的精选职位，找出它们对应的精选企业
                // 确保“精选企业”模块展示的公司和“全部精选”岗位的公司一一对应
                const jobsQuery = `
                    SELECT company_id
                    FROM jobs
                    WHERE status = 'active'
                    ...
                    LIMIT 100
                `
                const jobsResult = await neonHelper.query(jobsQuery)
                
                const uniqueCompanyIds = []
                for (const row of (jobsResult || [])) {
                    if (uniqueCompanyIds.length >= 6) break
                    if (!uniqueCompanyIds.includes(row.company_id)) {
                        uniqueCompanyIds.push(row.company_id)
                    }
                }
                ...
                // Fallback: 如果符合条件的精选企业不足6个，补充一些活跃企业
                if (result.length < 6) {
                    const existingIds = result.map(r => r.company_id)
                    const limitNeeded = 6 - result.length
                    ...
                    if (existingIds.length > 0) {
                        ...
                        fallbackQuery += ` WHERE tc.company_id NOT IN (${placeholders})`
                        fallbackParams.push(...existingIds)
                    }
                    fallbackQuery += ` ORDER BY j_agg.approved_job_count DESC, tc.updated_at DESC LIMIT $${fallbackParams.length + 1}`
                    fallbackParams.push(limitNeeded)
                    
                    const fallbackResult = await neonHelper.query(fallbackQuery, fallbackParams)
                    if (fallbackResult) {
                        result = result.concat(fallbackResult)
                    }
                }
