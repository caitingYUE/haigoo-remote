    ...
    await neonHelper.transaction(async (client) => {
        ...
        for (const company of companies) {
            await client.query(`
                ...
                (company_id, name, website, careers_page, linkedin, description, logo, cover_image, industry, tags, source, job_count, can_refer, status, created_at, updated_at)
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            // Fetch LinkedIn Contacts (Experimental)
            if (action === 'fetch_linkedin_contacts') {
                const { url } = req.query
                if (!url) return res.status(400).json({ success: false, error: 'URL is required' })

                console.log(`[trusted-companies] Fetching LinkedIn contacts from: ${url}`)
                
                try {
                    // Dynamic import node-fetch if needed (or assume it's available globally or via require)
                    // In this project context, import might be needed or global fetch
                    ...
                    if (!response.ok) {
                        throw new Error(`LinkedIn responded with status: ${response.status}`)
                    }
                    ...
                    if (title.includes('Sign In') || title.includes('Log In') || title.includes('Login') || title.includes('auth')) {
                        return res.status(403).json({ 
                            success: false, 
                            error: 'LinkedIn requires login. Cannot fetch contacts automatically.',
                    ...
                    // Fallback: heuristic text search (very limited)
                    if (contacts.length === 0) {
                         // Check meta tags or other open graph data
                         const ogTitle = $('meta[property="og:title"]').attr('content')
                         if (ogTitle) {
                             // Sometimes title contains "X employees"
                         }
                    }
                    ...
                    return res.json({ 
                        ...
                        contacts, 
                        meta: { title, url },
                        message: contacts.length > 0 ? `Found ${contacts.length} contacts` : 'No public contacts found (likely hidden behind login)'
                    })

                } catch (e) {
                    console.error('[trusted-companies] LinkedIn fetch error:', e)
                    return res.status(500).json({ success: false, error: e.message || 'Failed to fetch LinkedIn page' })
                }
            }
                    ...
                    if (NEON_CONFIGURED) {
                        await neonHelper.transaction(async (client) => {
                            ...
                            await client.query(`
                                ...
                                SET job_count = $1, updated_at = NOW() 
                                WHERE company_id = $2
