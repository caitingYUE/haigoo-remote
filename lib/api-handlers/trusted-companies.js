    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            if ((!target && !req.query.action) || target === 'companies') {
                ...
                if (req.query.id) {
                    if (NEON_CONFIGURED) {
                        const result = await neonHelper.query('SELECT * FROM trusted_companies WHERE company_id = $1', [req.query.id])
                        if (result && result.length > 0) {
                            const {
                                companyRow: row,
                                referralContacts,
        ...
        if (req.method === 'DELETE') {
            ...
            try {
                ...
                await neonHelper.transaction(async (client) => {
                    ...
                    // 2. Hide associated current jobs; historical evidence is retained.
                    console.log(`[trusted-companies] Deactivating jobs for company ID: ${id}`)
                    await client.query("UPDATE jobs SET status = 'inactive', updated_at = NOW() WHERE company_id = $1", [id])

                    // Also delete by name if available
                    if (companyName) {
                        await client.query(
                            "UPDATE jobs SET status = 'inactive', updated_at = NOW() WHERE company = $1 AND (company_id IS NULL OR company_id != $2)",
                            [companyName, id]
                        )
                    }
                })
