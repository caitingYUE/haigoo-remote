    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            if (resource === 'companies') {
                ...
                if (action === 'extract') {
                    ...
                    try {
                        ...
                        if (jobsArray.length > 0) {
                            console.log('[companies] First job sample:', JSON.stringify(jobsArray[0]).substring(0, 200))
                        }
                        ...
                        if (extractedCompanies.length > 0) {
                            console.log('[companies] First extracted sample:', JSON.stringify(extractedCompanies[0]))
                        }
                ...
                if (NEON_CONFIGURED) {
                    try {
                        // Fetch both trusted and extracted companies
                        const [trustedResult, extractedResult] = await Promise.all([
                            neonHelper.query('SELECT * FROM trusted_companies ORDER BY name'),
                            neonHelper.query('SELECT * FROM extracted_companies ORDER BY name')
                        ])

                        console.log(`[trusted-companies] GET: Trusted=${trustedResult ? trustedResult.length : 0}, Extracted=${extractedResult ? extractedResult.length : 0}`)
                        ...
                        trustedCompanies.forEach(c => {
                            ...
                            if (existing) {
                                ...
                                companyMap.set(key, {
                                    ...c,
                                    jobCount: Math.max(c.jobCount, existing.jobCount) // Keep higher job count? Or sum? Trusted usually manually set?
                                    // Let's assume trusted might not have up-to-date job count if not synced
                                })
                            } else {
                        ...
                        // Fallback to trusted only if extracted table missing or query fails
                        const result = await neonHelper.query('SELECT * FROM trusted_companies ORDER BY name')
                        if (result && result.length > 0) {
        ...
        if (req.method === 'POST') {
            ...
            if (action === 'batch-import') {
                try {
                    ...
                    // Get existing companies from BOTH tables to avoid duplicates
                    const trustedCompanies = await getAllCompanies()
                    let extractedCompanies = []
                    if (NEON_CONFIGURED) {
                        const result = await neonHelper.query('SELECT * FROM extracted_companies')
                        if (result && result.length > 0) {
                            extractedCompanies = result.map(row => ({
                                name: row.name,
                                website: row.url
                            }))
                        }
                    ...
                    const allExisting = [...trustedCompanies, ...extractedCompanies]
                    const existingNames = new Set(allExisting.map(c => c.name.toLowerCase()))
                    const existingWebsites = new Set(allExisting.map(c => c.website?.toLowerCase()).filter(Boolean))
                    ...
                    if (crawlMetadata && newCompanies.length > 0) {
                        ...
                        const updatedAllCompanies = [...trustedCompanies, ...newCompanies]
