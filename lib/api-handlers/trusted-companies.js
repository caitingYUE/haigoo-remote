...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'POST') {
                    ...
                    if (NEON_CONFIGURED) {
                        try {
                            await neonHelper.transaction(async (client) => {
                                await client.query(`
                                    INSERT INTO trusted_companies 
                                    (company_id, name, website, careers_page, linkedin, description, logo, cover_image, industry, tags, source, job_count, can_refer, status, updated_at)
            ...
            const success = await saveAllCompanies([companyToSave])
            if (!success) {
