...
async function resolveCompanyReferralContactsForJob(client, companyRow, jobId) {
    ...
    try {
        const jobRows = await client.query(
            'SELECT referral_contact_mode FROM jobs WHERE job_id = $1 AND company_id = $2 LIMIT 1',
            [jobId, normalizedCompanyRow.company_id]
        )
        const mode = String(jobRows?.[0]?.referral_contact_mode || 'inherit_all').trim().toLowerCase()
        if (mode !== 'custom') {
            return { companyRow: normalizedCompanyRow, referralContacts: defaultContacts }
        }
