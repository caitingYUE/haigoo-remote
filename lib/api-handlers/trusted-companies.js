    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            if (resource === 'tags') {
                ...
                let config = null
                try {
                    const result = await neonHelper.query('SELECT config_value FROM app_config WHERE config_key = $1', [TAG_CONFIG_KEY])
            ...
            if (req.query.action === 'sync-jobs') {
                ...
                try {
                    ...
                    if (NEON_CONFIGURED) {
                        const result = await neonHelper.query('SELECT * FROM trusted_companies WHERE status = $1 ORDER BY name', ['active'])
                        if (result && result.rows) {
                            trustedCompanies = result.rows.map(row => ({
                                id: row.company_id,
                                name: row.name,
                                website: row.website,
                                description: row.description,
                    ...
                    let jobsArray = []
                    if (NEON_CONFIGURED) {
                        const result = await neonHelper.query('SELECT * FROM processed_jobs WHERE status = $1 ORDER BY created_at DESC', ['active'])
                        if (result && result.rows) {
                            jobsArray = result.rows.map(row => ({
                                id: row.job_id,
                                title: row.title,
                                company: row.company,
                        ...
                        await neonHelper.transaction(async (client) => {
                            for (const job of updatedJobs) {
                                await client.query(`
                                    ...
                                    SET company_id = $1, company_logo = $2, company_website = $3, 
                                        company_description = $4, company_industry = $5, company_tags = $6,
                                        updated_at = $7
                                    WHERE job_id = $8
                                `, [
                                    job.companyId,
            ...
            if (resource === 'companies') {
                    ...
                    try {
                        ...
                        try {
                            if (NEON_CONFIGURED) {
                                ...
                                if (result && result.rows) {
                                    existingCompanies = result.rows.map(row => ({
                                        id: row.company_id,
                                        name: row.name,
                                        url: row.url,
                                        description: row.description,
                ...
                if (NEON_CONFIGURED) {
                    const result = await neonHelper.query('SELECT * FROM extracted_companies ORDER BY name')
                    if (result && result.rows) {
                        companiesArray = result.rows.map(row => ({
                            id: row.company_id,
                            name: row.name,
                            url: row.url,
                            description: row.description,
                        ...
                        await neonHelper.transaction(async (client) => {
                            for (const job of enrichedJobs) {
                                await client.query(`
                                    ...
                                    SET company_id = $1, company_logo = $2, company_website = $3, 
                                        company_description = $4, company_industry = $5, company_tags = $6,
                                        tags = $7, is_trusted = $8, can_refer = $9, source_type = $10,
                                        updated_at = $11
                                    WHERE job_id = $12
                                `, [
                                    job.companyId,
            ...
            if (action === 'crawl-jobs') {
                ...
                try {
                    ...
                    if (NEON_CONFIGURED) {
                        const result = await neonHelper.query('SELECT * FROM processed_jobs WHERE status = $1 ORDER BY created_at DESC', ['active'])
                        if (result && result.rows) {
                            existingJobs = result.rows.map(row => ({
                                id: row.job_id,
                                title: row.title,
                                company: row.company,
                                companyId: row.company_id,
                    ...
                    if (NEON_CONFIGURED) {
                        await neonHelper.transaction(async (client) => {
                            ...
                            await client.query('DELETE FROM processed_jobs WHERE company_id = $1', [company.id])
                            
                            // Insert new jobs
                            for (const job of allJobs) {
                                await client.query(`
                                    INSERT INTO processed_jobs 
                                    (job_id, title, company, company_id, company_logo, company_website, 
                                     company_description, company_industry, company_tags, location, 
                                     description, url, source, tags, salary, remote, status, created_at, updated_at)
            ...
            try {
                ...
                if (NEON_CONFIGURED) {
                    await neonHelper.transaction(async (client) => {
                        ...
                        await client.query('DELETE FROM processed_jobs WHERE company_id = $1', [id])
