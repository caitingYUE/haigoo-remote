...
import { neon } from '../../server-utils/dal/neon-helper.js'
...
const NEON_CONFIGURED = !!NEON_DATABASE_URL
...
export async function getAllCompanies() {
    try {
        // Priority: Neon Database -> Upstash REST -> Redis TCP -> Memory
        if (NEON_CONFIGURED) {
            try {
                const result = await neon.query('SELECT * FROM trusted_companies WHERE status = $1 ORDER BY name', ['active'])
                ...
                console.warn('[trusted-companies] Neon database read failed, trying Upstash REST:', e?.message)
            }
        }
        ...
        if (UPSTASH_REST_CONFIGURED) {
            try {
                ...
                console.warn('[trusted-companies] Upstash REST read failed:', e?.message)
            }
        }
    } catch (e) {
...
export async function saveAllCompanies(companies) {
    ...
    let success = false
    try {
        // Priority: Neon Database -> Upstash REST -> Redis TCP
        if (NEON_CONFIGURED) {
            try {
