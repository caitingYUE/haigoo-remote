...
export async function getCompaniesPaginated({ page = 1, limit = 20, sortBy = 'updated_at', sortOrder = 'desc', industry, search, canRefer, source }) {
    try {
        if (NEON_CONFIGURED) {
            ...
            if (source && source !== 'all') {
                if (source === 'manual') {
                    conditions.push(`(source = 'manual' OR source IS NULL)`)
                } else if (source === 'rss') {
                    conditions.push(`source != 'manual'`)
                } else {
    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            if (target === 'companies') {
                ...
                const pageSize = parseInt(req.query.limit) || parseInt(req.query.pageSize) || 50
