...
function extractMetadata(html) {
    ...
    const metadata = { title: '', description: '', image: '', icon: '', culture: '', founder: '', cultureImage: '' }
    ...
    metadata.cultureImage = metadata.coverImage
        ...
        try {
            ...
            for (const item of items) {
                if (item['@type'] === 'Organization' || item['@type'] === 'Corporation') {
                    if (item.founder) {
                        if (typeof item.founder === 'string') metadata.founder = item.founder
                        if (Array.isArray(item.founder) && item.founder.length > 0) {
                            metadata.founder = item.founder.map(f => f.name || f).filter(Boolean).join('、')
                        }
                    }
                    if (!metadata.founder && item.founders && Array.isArray(item.founders) && item.founders.length > 0) {
                        metadata.founder = item.founders.map(f => f.name || f).filter(Boolean).join('、')
                    }
                    if (item.address) {
    ...
    if (!metadata.culture) {
        metadata.culture = findTextByKeywords(['culture', 'values', 'mission', '愿景', '使命', '文化', '价值'])
    }
    if (!metadata.founder) {
        metadata.founder = findTextByKeywords(['founder', 'co-founder', 'ceo', '创始人', '联合创始'])
    }
    ...
    if (!metadata.cultureImage) {
        ...
        for (let i = 0; i < imgs.length; i++) {
            ...
            if (alt.includes('culture') || alt.includes('vision') || alt.includes('mission') || alt.includes('文化') || alt.includes('愿景') || alt.includes('使命')) {
                const src = $(imgs[i]).attr('src')
                if (src) {
                    metadata.cultureImage = src
                    break
                }
            }
        }
        ...
        if (NEON_CONFIGURED) {
            ...
            if (result) {
                ...
                return result.map(row => ({
                    ...
                    culture: row.culture,
                    founderIntro: row.founder_intro,
                    cultureImage: row.culture_image,
                    showCultureOnHome: row.show_culture_on_home || false,
    ...
    await neonHelper.transaction(async (client) => {
        ...
        for (const company of companies) {
            await client.query(`
                ...
                (company_id, name, website, careers_page, linkedin, description, logo, cover_image, culture, founder_intro, culture_image, show_culture_on_home, industry, tags, source, job_count, can_refer, status, created_at, updated_at)
                ...
                ON CONFLICT (company_id) DO UPDATE SET
                    ...
                    founder_intro = EXCLUDED.founder_intro,
                    culture_image = EXCLUDED.culture_image,
                    show_culture_on_home = EXCLUDED.show_culture_on_home,
            ...
            `, [
                ...
                company.coverImage || '',
                company.culture || '',
                company.founderIntro || '',
                company.cultureImage || '',
                company.showCultureOnHome || false,
                company.industry || '其他',
    ...
    try {
        ...
        if (req.method === 'GET') {
            ...
            if (target === 'companies') {
                ...
                // Get single company
                if (id) {
                    const company = companiesArray.find(c => String(c.id) === String(id))
                    ...
                    if (NEON_CONFIGURED) {
                        try {
                            await neonHelper.transaction(async (client) => {
                                await client.query(`
                                    INSERT INTO trusted_companies 
                                    (company_id, name, website, careers_page, linkedin, description, logo, cover_image, culture, founder_intro, culture_image, show_culture_on_home, industry, tags, source, job_count, can_refer, status, updated_at)
                            ...
                            console.log(`[trusted-companies] Updated trusted company: ${body.name}`)
                            return res.status(200).json({ success: true, message: 'Trusted company updated' })
                        } catch (e) {
                            console.error('[trusted-companies] Failed to update trusted company:', e)
                            return res.status(500).json({ success: false, error: 'Failed to update trusted company' })
                        }
                    } else {
                        return res.status(500).json({ success: false, error: 'Database not configured' })
                    }
                    ...
                    if (NEON_CONFIGURED) {
                        await neonHelper.transaction(async (client) => {
                            ...
                            for (const job of allJobs) {
                                await client.query(`
                                    INSERT INTO jobs 
                                    (job_id, title, company, company_id, location, description, url, published_at,
                                ...
                                `, [
                                    ...
                                    job.description,
                                    job.url,
                                    job.publishedAt || job.createdAt || new Date().toISOString(),
                                    job.source || 'crawled',
                                    JSON.stringify(job.tags || []),
                                    job.salary,
                                    job.remote || false,
