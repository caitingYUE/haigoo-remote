...
// Data Access Layer
// Get only trusted companies from database
        ...
        if (NEON_CONFIGURED) {
            ...
            if (result) {
                ...
                return result.map(row => ({
                    id: row.company_id,
                    name: row.name,
                    url: row.url,  // Updated from website to url
...
// Get all companies (merged from trusted_companies and extracted_companies)
    ...
    try {
        if (NEON_CONFIGURED) {
            ...
            const trustedCompanies = (trustedResult || []).map(row => ({
                id: row.company_id,
                name: row.name,
                url: row.url,  // Now consistent after migration
                description: row.description,
                logo: row.logo,
                coverImage: row.cover_image,
                industry: row.industry,
        ...
        console.error('[trusted-companies] getAllCompanies error:', e)
    }
    return []
}
        ...
        await neonHelper.transaction(async (client) => {
            ...
            for (const company of companies) {
                await client.query(`
                    ...
                    (company_id, name, url, description, logo, cover_image, industry, tags, source, job_count, can_refer, status, created_at, updated_at)
                ...
                `, [
                    company.id,
                    company.name,
                    company.url || '',  // Changed from website to url
                    company.description || '',
                    company.logo || '',
                    company.coverImage || '',
                    company.industry || '其他',
            ...
            if (action === 'sync-jobs') {
                ...
                try {
                    ...
                    if (NEON_CONFIGURED) {
                        ...
                        const result = await neonHelper.query('SELECT company_id, name, url, logo, description, industry, tags FROM trusted_companies ORDER BY name')
                        if (result && result.length > 0) {
                            trustedCompanies = result.map(row => ({
                                id: row.company_id,
                                name: row.name,
                                url: row.url,  // Changed from website
                                description: row.description,
                                logo: row.logo,
                                industry: row.industry,
                                tags: row.tags || []
                ...
                try {
                    ...
                    paginatedCompanies.forEach(company => {
                        ...
                        if (trusted) {
                            ...
                            if (!company.url && trusted.url) company.url = trusted.url
        ...
        if (req.method === 'POST') {
            ...
            if (action === 'batch-import') {
                try {
                    ...
                    const existingWebsites = new Set(trustedCompanies.map(c => normalizeUrl(c.url)).filter(Boolean))
            ...
            if (action === 'crawl-jobs') {
                ...
                const company = companies.find(c => c.id === id)
                if (!company) return res.status(404).json({ success: false, error: 'Company not found' })
                if (!company.careersPage && !company.url) return res.status(400).json({ success: false, error: 'No careers page or website URL' })

                try {
                    const { crawlCompanyJobs } = await import('../../lib/job-crawler.js')
                    const url = company.careersPage || company.url
