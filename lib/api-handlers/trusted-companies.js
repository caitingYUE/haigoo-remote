...
async function resolveJobMemberOnly(jobId) {
    if (!jobId || !NEON_CONFIGURED) return false
    try {
        const result = await neonHelper.query(
            'SELECT member_only FROM jobs WHERE job_id = $1 LIMIT 1',
            [jobId]
        )
        return Boolean(result?.[0]?.member_only)
    } catch (error) {
        console.warn('[trusted-companies] Failed to resolve job member-only state:', error?.message || error)
        return false
    }
}
...
function sanitizeCompanyContactsForViewer(company, viewerAccess, options = {}) {
    ...
    const shouldHideContacts = Boolean(options?.jobMemberOnly || company?.memberOnly) && !viewerAccess?.canAccessCompanyContacts
    if (!shouldHideContacts) return company
                ...
                if (req.query.id) {
                    if (NEON_CONFIGURED) {
                        ...
                        if (result && result.length > 0) {
                            ...
                            const jobMemberOnly = await resolveJobMemberOnly(req.query.jobId)
