...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'GET' && action === 'featured_home') {
            ...
            try {
                // New Logic: Sort by APPROVED active jobs count DESC
                const query = `
                    SELECT tc.*, 
                        (SELECT COUNT(*) FROM jobs j WHERE j.company_id = tc.company_id AND j.status = 'active' AND j.is_approved = true) as approved_job_count
                    FROM trusted_companies tc
                    WHERE (SELECT COUNT(*) FROM jobs j WHERE j.company_id = tc.company_id AND j.status = 'active' AND j.is_approved = true) > 0
                    ORDER BY approved_job_count DESC, updated_at DESC
                    LIMIT 6
                `
        ...
        if (req.method === 'POST') {
                    ...
                    if (NEON_CONFIGURED) {
                        try {
                            ...
                            const savedCompany = {
                                ...
                                emailType: body.emailType || '通用支持邮箱',
            ...
            if (id) {
                ...
                companyToSave = {
                    ...
                    emailType: body.emailType !== undefined ? body.emailType : existing.email_type,
