    ...
    try {
        if (NEON_CONFIGURED) {
            ...
            let query = `
                SELECT 
                    tc.*,
                    COUNT(j.job_id) as actual_job_count,
                ...
                LEFT JOIN jobs j ON (
                    tc.company_id = j.company_id 
                    OR LOWER(REPLACE(tc.name, ' ', '')) = LOWER(REPLACE(j.company, ' ', ''))
                    OR tc.name ILIKE '%' || j.company || '%'
                    OR j.company ILIKE '%' || tc.name || '%'
                )
            `
            
            let countQuery = 'SELECT COUNT(*) FROM trusted_companies tc'
            const params = []
            const conditions = []
            let paramIndex = 1
            ...
            const sortColumn = allowedSorts[sortBy] || 'actual_job_count'
            const order = sortOrder.toUpperCase() === 'ASC' ? 'ASC' : 'DESC'
            ...
            const companies = result ? result.map(row => {
                ...
                return {
                    ...
                    tags: row.tags || [],
                    source: row.source,
                    jobCount: row.actual_job_count || 0,
                ...
                try {
                    ...
                    paginatedCompanies.forEach(company => {
                        ...
                        if (trusted) {
                            ...
                            if (!company.website && trusted.website) company.website = trusted.website
        ...
        if (req.method === 'POST') {
            ...
            if (action === 'sync-jobs') {
                ...
                try {
                    ...
                    // 5. Update Job Counts in trusted_companies
                    if (NEON_CONFIGURED) {
                        console.log('[trusted-companies] Recalculating job counts...')
                        await neonHelper.query(`
                            UPDATE trusted_companies tc
                            SET job_count = COALESCE(sub.count, 0), updated_at = NOW()
                            FROM (
                        ...
                        `)
                        // Also set count to 0 for companies with no jobs
                        await neonHelper.query(`
