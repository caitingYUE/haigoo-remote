            ...
            if (action === 'crawl-jobs') {
                ...
                try {
                    ...
                    for (const j of enrichedJobs) {
                        // 1. 过滤 30 天外的岗 (存在时间才强判断)
                        if (j.publishedAt) {
                            const pubDate = new Date(j.publishedAt).getTime()
                            if (!isNaN(pubDate)) {
                                const daysDiff = (Date.now() - pubDate) / (1000 * 3600 * 24)
                                if (daysDiff > 30) {
                                    filteredByTime++
                                    continue
                                }
                    ...
                    console.log(`[trusted-companies] Crawled ${enrichedJobs.length} jobs, Kept ${cleanEnrichedJobs.length} (Filtered Time:${filteredByTime}, Loc:${filteredByLocation}, Offline:${filteredByOffline})`)
