...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'GET' && action === 'featured_home') {
            ...
            try {
                // New Logic: Daily update (sort by updated_at DESC), at least 5 APPROVED active jobs
                const query = `
                    SELECT * FROM trusted_companies tc
                    WHERE (SELECT COUNT(*) FROM jobs j WHERE j.company_id = tc.company_id AND j.status = 'active' AND j.is_approved = true) >= 5 
