                 ...
                 if (companyIds.length > 0) {
                     ...
                     companiesWithStats.forEach(c => {
                         if (jobSamplesMap[c.id]) {
                             const categories = {}
                             jobSamplesMap[c.id].forEach(job => {
    ...
    try {
        ...
        if (req.method === 'GET' && action === 'featured_home') {
            ...
            try {
                ...
                // Cache optimization: Cache public featured companies list for 5 minutes
                // This significantly reduces DB load and latency for China users via Vercel Edge Cache
