...
async function resolveCompanyReferralContactsForJob(client, companyRow, jobId) {
    ...
    if (!jobId || !normalizedCompanyRow.company_id) {
        return { companyRow: normalizedCompanyRow, referralContacts: defaultContacts }
    }
    ...
    try {
        ...
        if (!jobRow) {
            ...
            return { companyRow: normalizedCompanyRow, referralContacts: [] }
        }
        ...
        return { companyRow: normalizedCompanyRow, referralContacts: filteredContacts }
    } catch (error) {
            ...
            if ((!target && !req.query.action) || target === 'companies') {
                ...
                if (req.query.id) {
                    if (NEON_CONFIGURED) {
                        ...
                        if (result && result.length > 0) {
                            const { companyRow: row, referralContacts } = await resolveCompanyReferralContactsForJob(
                                neonHelper,
                                result[0],
                                req.query.jobId
                            )
