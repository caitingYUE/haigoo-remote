...
export async function getCompaniesPaginated({ page = 1, limit = 20, sortBy = 'updated_at', sortOrder = 'desc', industry, search, canRefer }) {
    try {
        if (NEON_CONFIGURED) {
    ...
    await neonHelper.transaction(async (client) => {
        ...
        for (const company of companies) {
            await client.query(`
                ...
                (company_id, name, website, careers_page, linkedin, description, logo, cover_image, industry, tags, source, job_count, can_refer, status, translations, created_at, updated_at, last_crawled_at, address, employee_count, founded_year, specialties, company_rating, rating_source, aliases)
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (req.method === 'GET') {
            const { target, action, page, limit, sortBy, sortOrder, industry, search, canRefer } = req.query
                    ...
                    if (NEON_CONFIGURED) {
                        try {
                            await neonHelper.transaction(async (client) => {
                                await client.query(`
                                    INSERT INTO trusted_companies 
                                    (company_id, name, website, careers_page, linkedin, description, logo, cover_image, culture, founder_intro, culture_image, show_culture_on_home, industry, tags, source, job_count, can_refer, status, translations, updated_at, address, employee_count, founded_year, specialties, company_rating, rating_source)
