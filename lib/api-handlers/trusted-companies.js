...
function extractMetadata(html) {
    ...
    if (!metadata.description || metadata.description.length < 50) {
        ...
        const desc = $('.about-us p, #about p, .description, [class*="description"], main p, article p, .w-richtext p, .hero-text, .intro-text, .company-description, .overview').first().text().trim()
        if (desc.length > 20) {
            metadata.description = desc.substring(0, 500) + (desc.length > 500 ? '...' : '')
        }
    }
    ...
    try {
        ...
        if (req.method === 'GET' && action === 'crawl') {
            ...
            try {
                ...
                // Prioritize Clearbit logo with availability check
                const domain = new URL(targetUrl).hostname;
                const clearbitLogo = `https://logo.clearbit.com/${domain}`;
                let bestLogo = metadata.icon; // Default to favicon

                try {
                    // Quick check if Clearbit logo exists (timeout 2s)
                    const checkRes = await fetch(clearbitLogo, { method: 'HEAD', timeout: 2000 });
                    if (checkRes.ok) {
                        bestLogo = clearbitLogo;
                    } else {
                         // Fallback to Google Favicon if original icon is missing
                         if (!bestLogo) {
                             bestLogo = `https://www.google.com/s2/favicons?domain=${domain}&sz=128`;
                         }
                    }
                } catch (e) {
                    // Ignore check errors, stick to metadata.icon or Google fallback
                    if (!bestLogo) bestLogo = `https://www.google.com/s2/favicons?domain=${domain}&sz=128`;
                }
