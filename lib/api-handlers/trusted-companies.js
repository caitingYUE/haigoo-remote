...
import { checkUrlStatus } from '../cron-handlers/stream-verify-links.js'
...
import {
    cacheCompanyLogoBestEffort,
    resolveCachedLogoUrlFromRow
} from '../services/company-image-asset-service.js'
import {
    getPreviewCompaniesResponse,
    getPreviewCompaniesWithJobStatsResponse,
    getPreviewCompanyById,
    getPreviewCoverImageResponse,
    getPreviewFeaturedCompaniesResponse
} from '../dev-preview-data.js'
import { reconcileCompanyDirectoryHistory } from '../services/mini-company-match-service.js'
import { deactivateCompany } from '../services/company-deactivation.js'
...
async function resolveViewerContactAccess(req) {
    try {
        const token = extractToken(req)
        if (!token) {
            return { requester: null, canAccessCompanyContacts: false }
        }
        ...
        return {
            requester,
            canAccessCompanyContacts: isAdmin || isMembershipActive(requester)
        }
    } catch (error) {
        console.warn('[trusted-companies] Failed to resolve viewer access:', error?.message || error)
        return { requester: null, canAccessCompanyContacts: false }
    }
}

async function resolveTrustedCompaniesListAccess(req) {
    try {
        const token = extractToken(req)
        if (!token) {
            return { requester: null, canAccessTrustedCompaniesPage: false }
        }
        ...
        const requester = await userHelper.getUserById(payload.userId)
        const isAdmin = Boolean(requester?.roles?.admin || SUPER_ADMIN_EMAILS.includes(String(requester?.email || '').toLowerCase()))
        const capabilities = deriveMembershipCapabilities(requester)
        return {
            requester,
            canAccessTrustedCompaniesPage: isAdmin || Boolean(capabilities?.canAccessTrustedCompaniesPage)
        }
    } catch (error) {
        console.warn('[trusted-companies] Failed to resolve trusted list access:', error?.message || error)
        return { requester: null, canAccessTrustedCompaniesPage: false }
    }
}

function restrictTrustedCompaniesListResponse(result, limit = 8) {
    const safeLimit = Math.max(1, Math.min(parseInt(limit, 10) || 8, 8))
    const companies = Array.isArray(result?.companies) ? result.companies.slice(0, safeLimit) : []
    ...
    return {
        ...
        totalActiveJobs: companies.reduce((sum, company) => sum + (parseInt(company?.jobCount, 10) || 0), 0),
        availableCategories: [],
        page: 1,
        totalPages: 1,
        hasMore: false,
...
async function resolveJobContactAccessContext(jobId) {
    if (!jobId || !NEON_CONFIGURED) {
        return { memberOnly: false, allowPublicEmailOnly: false }
    }
    try {
        const result = await neonHelper.query(
            'SELECT member_only, url FROM jobs WHERE job_id = $1 LIMIT 1',
            [jobId]
        )
        const row = result?.[0]
        const memberOnly = Boolean(row?.member_only)
        return {
            memberOnly,
            allowPublicEmailOnly: Boolean(row) && !memberOnly && !String(row?.url || '').trim()
        }
    } catch (error) {
        console.warn('[trusted-companies] Failed to resolve job contact access context:', error?.message || error)
        return { memberOnly: false, allowPublicEmailOnly: false }
    }
}

function sanitizeCompanyContactsForViewer(company, viewerAccess, options = {}) {
    if (!company) return company

    if (viewerAccess?.canAccessCompanyContacts) return company

    const canExposePublicEmailOnly = Boolean(
        options?.allowPublicEmailOnly &&
        company?.jobReferralContactMode !== 'custom' &&
        String(company?.hiringEmail || '').trim()
    )

    if (canExposePublicEmailOnly) {
        return {
            ...company,
            // Public email-only roles expose the company mailbox, not named
            // contacts, LinkedIn profiles, or referral-specific metadata.
            referralContacts: []
        }
    }
    ...
    return {
        ...
        hiringEmail: null,
        emailType: null,
        referralContacts: Array.isArray(company.referralContacts)
            ? company.referralContacts.map((contact) => ({
                ...contact,
                hiringEmail: null,
                linkedin: null
            }))
...
function isJobReferralLinkSchemaMissing(error) {
    return error?.message?.includes(JOB_REFERRAL_CONTACT_LINKS_TABLE)
}
...
function extractMetadata(html) {
    ...
    if (!metadata.address) {
        ...
        if (country || locality) {
            metadata.address = [locality, country].filter(Boolean).join(', ')
        }
    }

    // 4. Enhanced Logo Extraction
    if (!metadata.icon || metadata.icon.length < 5) {
        // Try finding logo img tags
        // Look for common logo class names or IDs
        const logoImg = $('img[class*="logo"], img[id*="logo"], .logo img, header img, a[class*="brand"] img').first()
        if (logoImg.length > 0) {
            let src = logoImg.attr('src') || logoImg.attr('data-src')
            if (src) {
                // Handle relative URLs inside caller (crawler)
                // Just return raw src here
                metadata.icon = src
            }
        }
    }
...
const EMAIL_TYPE_MODERN_DEFAULT = '通用邮箱'
const EMAIL_TYPE_LEGACY_DEFAULT = '通用支持邮箱'

const LEGACY_TO_MODERN_EMAIL_TYPE = {
    '招聘专用邮箱': '招聘邮箱',
    '通用支持邮箱': '通用邮箱',
    '内部员工邮箱': '员工邮箱',
    '企业领导邮箱': '高管邮箱',
...
const MODERN_TO_LEGACY_EMAIL_TYPE = {
    '招聘邮箱': '招聘专用邮箱',
    '通用邮箱': '通用支持邮箱',
    '员工邮箱': '内部员工邮箱',
    '高管邮箱': '企业领导邮箱',
...
let cachedEmailTypeConstraintMode = null

function normalizeEmailTypeForClient(emailType) {
    const normalized = LEGACY_TO_MODERN_EMAIL_TYPE[String(emailType || '').trim()]
    return normalized || EMAIL_TYPE_MODERN_DEFAULT
}

function normalizeEmailTypeForDb(emailType, mode = 'modern') {
    const raw = String(emailType || '').trim()
    if (mode === 'legacy') {
        return MODERN_TO_LEGACY_EMAIL_TYPE[raw] || EMAIL_TYPE_LEGACY_DEFAULT
    }
    return LEGACY_TO_MODERN_EMAIL_TYPE[raw] || EMAIL_TYPE_MODERN_DEFAULT
}

function parseReferralContacts(raw) {
    if (!raw) return []
...
function buildReferralContactId(companyId = 'company') {
    const normalizedCompanyId = String(companyId || 'company').trim() || 'company'
    return `rc_${normalizedCompanyId}_${randomUUID().replace(/-/g, '')}`
}

function normalizeReferralContactsForDb(referralContacts, fallbackHiringEmail, fallbackEmailType, mode = 'modern', companyId = null) {
    const parsed = parseReferralContacts(referralContacts)
    const normalized = parsed
        .map((item) => {
            const id = String(item?.id || item?.contactId || '').trim() || buildReferralContactId(companyId)
            const hiringEmail = String(item?.hiringEmail || item?.email || '').trim()
            const emailType = normalizeEmailTypeForDb(item?.emailType || fallbackEmailType, mode)
            const name = String(item?.name || '').trim()
            const title = String(item?.title || item?.position || '').trim()
            const linkedin = String(item?.linkedin || item?.linkedinUrl || item?.linkedinProfile || '').trim()
            return { id, hiringEmail, emailType, name, title, linkedin }
        })
        .filter((item) => item.hiringEmail || item.name || item.title || item.linkedin)

    if (normalized.length > 0) return normalized

    const fallbackEmail = String(fallbackHiringEmail || '').trim()
    if (!fallbackEmail) return []

    return [{
        id: buildReferralContactId(companyId),
        hiringEmail: fallbackEmail,
        emailType: normalizeEmailTypeForDb(fallbackEmailType, mode),
        name: '',
        title: '',
        linkedin: ''
    }]
...
function normalizeReferralContactsForClient(referralContacts, fallbackHiringEmail, fallbackEmailType, companyId = null) {
    const parsed = parseReferralContacts(referralContacts)
    const normalized = parsed
        .map((item) => {
            const id = String(item?.id || item?.contactId || '').trim() || buildReferralContactId(companyId)
            const hiringEmail = String(item?.hiringEmail || item?.email || '').trim()
            const emailType = normalizeEmailTypeForClient(item?.emailType || fallbackEmailType)
            const name = String(item?.name || '').trim()
            const title = String(item?.title || item?.position || '').trim()
            const linkedin = String(item?.linkedin || item?.linkedinUrl || item?.linkedinProfile || '').trim()
            return { id, hiringEmail, emailType, name, title, linkedin }
    ...
    return [{
        id: buildReferralContactId(companyId),
        hiringEmail: fallbackEmail,
        emailType: normalizeEmailTypeForClient(fallbackEmailType),
        name: '',
        title: '',
        linkedin: ''
    }]
...
function referralContactsNeedIdBackfill(referralContacts) {
    return parseReferralContacts(referralContacts).some((item) => !String(item?.id || item?.contactId || '').trim())
}

function shouldCanonicalizeReferralContacts(referralContacts, fallbackHiringEmail) {
    const parsed = parseReferralContacts(referralContacts)
    if (parsed.length === 0 && String(fallbackHiringEmail || '').trim()) {
        return true
    }
    return parsed.some((item) => !String(item?.id || item?.contactId || '').trim())
}

async function cleanupInvalidJobReferralLinks(client, companyId, referralContacts = []) {
    if (!companyId) return
    try {
        const validIds = referralContacts
            .map((contact) => String(contact?.id || '').trim())
            .filter(Boolean)

        if (validIds.length === 0) {
            await client.query(`DELETE FROM ${JOB_REFERRAL_CONTACT_LINKS_TABLE} WHERE company_id = $1`, [companyId])
            return
        }

        await client.query(
            `DELETE FROM ${JOB_REFERRAL_CONTACT_LINKS_TABLE}
             WHERE company_id = $1
               AND NOT (contact_id = ANY($2::text[]))`,
            [companyId, validIds]
        )
    } catch (error) {
        if (isJobReferralLinkSchemaMissing(error)) {
            console.warn('[trusted-companies] job referral link schema missing, skipping cleanup...')
            return
        }
        throw error
    }
...
async function ensureCompanyReferralContactIdsPersisted(companyRow, client = neonHelper) {
    if (!companyRow?.company_id) return companyRow
    if (!shouldCanonicalizeReferralContacts(companyRow.referral_contacts, companyRow.hiring_email)) return companyRow

    const emailTypeConstraintMode = await getEmailTypeConstraintMode(client)
    const normalizedReferralContacts = normalizeReferralContactsForDb(
        companyRow.referral_contacts,
        companyRow.hiring_email,
        companyRow.email_type,
        emailTypeConstraintMode,
        companyRow.company_id
    )

    await client.query(
        'UPDATE trusted_companies SET referral_contacts = $2 WHERE company_id = $1',
        [companyRow.company_id, JSON.stringify(normalizedReferralContacts)]
    )
    await cleanupInvalidJobReferralLinks(client, companyRow.company_id, normalizedReferralContacts)

    return {
        ...companyRow,
        referral_contacts: normalizedReferralContacts
    }
}

async function resolveCompanyReferralContactsForJob(client, companyRow, jobId) {
    const normalizedCompanyRow = await ensureCompanyReferralContactIdsPersisted(companyRow, client)
    const defaultContacts = normalizeReferralContactsForClient(
        normalizedCompanyRow.referral_contacts,
        normalizedCompanyRow.hiring_email,
        normalizedCompanyRow.email_type,
        normalizedCompanyRow.company_id
    )

    if (!jobId || !normalizedCompanyRow.company_id) {
        return {
            companyRow: normalizedCompanyRow,
            referralContacts: defaultContacts,
            jobReferralContactMode: null,
            jobEffectiveReferralContactCount: null
        }
    }

    try {
        const jobRows = await client.query(
            'SELECT company_id, referral_contact_mode FROM jobs WHERE job_id = $1 LIMIT 1',
            [jobId]
        )
        const jobRow = jobRows?.[0] || null

        if (!jobRow) {
            console.warn(`[trusted-companies] No job row found for job ${jobId}, hiding referral contacts to avoid cross-job leakage.`)
            return {
                companyRow: normalizedCompanyRow,
                referralContacts: [],
                jobReferralContactMode: null,
                jobEffectiveReferralContactCount: 0
            }
        }

        const jobCompanyId = String(jobRow?.company_id || '').trim()
        const requestedCompanyId = String(normalizedCompanyRow.company_id || '').trim()
        if (jobCompanyId && requestedCompanyId && jobCompanyId !== requestedCompanyId) {
            console.warn(
                `[trusted-companies] Job ${jobId} belongs to company ${jobCompanyId}, but detail request asked for company ${requestedCompanyId}; hiding referral contacts.`
            )
            return {
                companyRow: normalizedCompanyRow,
                referralContacts: [],
        ...
        const mode = String(jobRow?.referral_contact_mode || 'inherit_all').trim().toLowerCase()
        if (mode !== 'custom') {
            return {
                companyRow: normalizedCompanyRow,
                referralContacts: defaultContacts,
                jobReferralContactMode: mode,
                jobEffectiveReferralContactCount: defaultContacts.length
            }
        }

        const mappingRows = await client.query(
            `SELECT contact_id FROM ${JOB_REFERRAL_CONTACT_LINKS_TABLE} WHERE job_id = $1 ORDER BY updated_at ASC, contact_id ASC`,
            [jobId]
        )
        const selectedIds = new Set((mappingRows || []).map((row) => String(row?.contact_id || '').trim()).filter(Boolean))
        const filteredContacts = defaultContacts.filter((contact) => selectedIds.has(String(contact?.id || '').trim()))

        return {
            companyRow: normalizedCompanyRow,
            referralContacts: filteredContacts,
            jobReferralContactMode: mode,
            jobEffectiveReferralContactCount: filteredContacts.length
        }
    } catch (error) {
        if (isJobReferralLinkSchemaMissing(error) || error?.message?.includes('referral_contact_mode')) {
            console.warn('[trusted-companies] job referral schema missing, falling back to company-wide contacts...')
            return {
                companyRow: normalizedCompanyRow,
                referralContacts: defaultContacts,
                jobReferralContactMode: null,
...
async function getEmailTypeConstraintMode(client = neonHelper) {
    if (cachedEmailTypeConstraintMode) return cachedEmailTypeConstraintMode
    ...
    try {
        const rows = await client.query(`
            ...
            WHERE c.conname = 'valid_email_type'
              AND t.relname = 'trusted_companies'
            LIMIT 1
        `)

        const definition = String(rows?.[0]?.definition || '')
        cachedEmailTypeConstraintMode = definition.includes('招聘专用邮箱') || definition.includes('通用支持邮箱')
            ? 'legacy'
            : 'modern'
    } catch (e) {
        console.warn('[trusted-companies] Failed to detect email_type constraint mode, fallback to modern:', e?.message || e)
        cachedEmailTypeConstraintMode = 'modern'
    }

    return cachedEmailTypeConstraintMode
}


// Data Access Layer
export async function getAllCompanies({ search, name, id } = {}) {
    try {
        if (NEON_CONFIGURED) {
            let query = "SELECT * FROM trusted_companies WHERE status = 'active'"
            const params = []
            ...
            if (id) {
                ...
                // Keep backward compatibility for search, but prefer exact name if possible
                // If search is provided but not name, stick to search logic or fuzzy logic
                // But this function getAllCompanies usually returns ALL.
                // The argument 'search' was used in CompanyDetailPage to filter IN MEMORY in old logic?
                // Wait, the previous implementation of getAllCompanies() in this file was:
                // export async function getAllCompanies() { ... SELECT * ... return result.map(...) }
                // It ignored arguments!
                // Ah, I need to check the previous turn's `Read` output.
                // It was: export async function getAllCompanies() { ... SELECT * FROM trusted_companies ORDER BY name ... }
                // So it fetched ALL companies. That's terrible if there are thousands.

                // Let's implement filtering here.
                query += ' AND (name ILIKE $1 OR description ILIKE $1)'
                params.push(`%${search}%`)
            } else {
                // Prevent full table dump if no filter is provided
                // This is a safety measure for legacy calls
            ...
            if (result) {
                ...
                return result.map(row => ({
                    ...
                    tags: row.tags || [],
                    source: row.source,
                    jobCount: row.approved_job_count ? parseInt(row.approved_job_count) : (row.job_count || 0),
                    ...
                    emailType: normalizeEmailTypeForClient(row.email_type),
                    referralContacts: normalizeReferralContactsForClient(row.referral_contacts, row.hiring_email, row.email_type, row.company_id),
...
export async function getCompaniesPaginated({ page = 1, limit = 20, sortBy = 'updated_at', sortOrder = 'desc', industry, search, name, memberOnly, isTrusted }) {
    try {
        if (NEON_CONFIGURED) {
            ...
            // Optimization: Exclude cover_image (base64) from list view to improve performance. Use getCompanyById for details.
            const trustedFields = `company_id, name, website, careers_page, linkedin, description, logo, NULL as cover_image, address, employee_count, hiring_email, email_type, referral_contacts, founded_year, specialties, company_rating, rating_source, industry, tags, source, job_count, can_refer, member_only, last_crawled_at, translations, created_at, updated_at`

            const extractedFields = `company_id, name, url as website, NULL as careers_page, NULL as linkedin, description, logo, NULL as cover_image, NULL as address, NULL as employee_count, NULL as hiring_email, NULL as email_type, NULL as referral_contacts, NULL as founded_year, NULL as specialties, NULL as company_rating, NULL as rating_source, industry, tags, source, job_count, false as can_refer, false as member_only, NULL as last_crawled_at, NULL as translations, created_at, updated_at`
            ...
            if (isTrusted === 'yes' || isTrusted === true) {
                ...
                conditions.push(`source != 'extracted'`, `status = 'active'`)
                ...
                useUnion = true
                query = `
                    SELECT ${trustedFields} FROM trusted_companies WHERE status = 'active'
                    UNION ALL
                ...
                countQuery = `
                    SELECT COUNT(*) FROM (
                        SELECT company_id FROM trusted_companies WHERE status = 'active'
                        UNION ALL
            ...
            if (name) {
                // Precise name search (case-insensitive) - MUCH FASTER
                conditions.push(`lower(name) = lower($${paramIndex})`)
                params.push(name)
                paramIndex++
            } else if (search) {
                // Optimization: If search matches a company name exactly or starts with it, use optimized query
                // Otherwise use the general fuzzy search
                // NOTE: 'description' search can be very slow.
                conditions.push(`(name ILIKE $${paramIndex} OR description ILIKE $${paramIndex})`)
                params.push(`%${search}%`)
                paramIndex++
            }

            if (memberOnly && memberOnly !== 'all') {
                if (useUnion || baseTable === 'trusted_companies') {
                    conditions.push(`member_only = $${paramIndex}`)
                    params.push(memberOnly === 'yes')
                    paramIndex++
                } else {
                    if (memberOnly === 'yes') {
            ...
            // Fix: Fetch real-time job counts for displayed companies to ensure accuracy
            if (result && result.length > 0) {
                try {
                    ...
                    const countsQuery = `
                        ...
                        GROUP BY company_id
                    `
                    const countsResult = await neonHelper.query(countsQuery, [companyIds])

                    const countsMap = {}
                    if (countsResult) {
                        countsResult.forEach(c => {
                            countsMap[c.company_id] = parseInt(c.count)
                        })
                    }

                    result.forEach(row => {
                        // Priority: Real-time count > Stored count
                        if (countsMap[row.company_id] !== undefined) {
                            row.approved_job_count = countsMap[row.company_id]
                        } else {
                            // If no jobs found in real-time query, but stored count > 0, 
                            // it implies data inconsistency or jobs were deleted/unapproved.
                            // However, strictly speaking, if GROUP BY returns nothing for an ID, count is 0.
                            // So we should set it to 0 if we want to be strict.
                            // But let's be safe: if companyId is in the list but not in countsResult, it means 0 matches.
                            // So we should set approved_job_count to 0.
                            row.approved_job_count = 0
                        }
                    })
                } catch (err) {
                    console.error('[trusted-companies] Failed to fetch real-time job counts:', err)
                }
            }

            const companies = result ? result.map(row => {
                // Use real-time count if available (and handle 0 correctly)
                const realCount = row.approved_job_count !== undefined ? parseInt(row.approved_job_count) : undefined;
                const finalCount = realCount !== undefined ? realCount : (row.job_count || 0);
...
export async function getCompaniesWithJobStats({ page = 1, limit = 20, sortBy = 'updatedAt', sortOrder = 'desc', industry, search, memberOnly, region, minJobs = 1, jobCategories }) {
    try {
        if (NEON_CONFIGURED) {
            const offset = (page - 1) * limit
            const params = []
            const conditions = ["tc.status = 'active'"]
            let paramIndex = 1
            ...
            if (minJobs > 0) {
                ...
                if (!isNaN(minJobsVal)) {
                    // Fix: Filter based on APPROVED jobs count only
                    conditions.push(`(SELECT COUNT(*) FROM jobs j WHERE j.company_id = tc.company_id AND j.status = 'active' AND j.is_approved = true) >= $${paramIndex}`)
            ...
            if (search) {
                ...
                conditions.push(`(tc.name ILIKE $${paramIndex} OR tc.description ILIKE $${paramIndex} OR EXISTS(SELECT 1 FROM jobs j WHERE j.company_id = tc.company_id AND j.status = 'active' AND j.is_approved = true AND j.title ILIKE $${paramIndex}))`)
                params.push(`%${search}%`)
                paramIndex++
            }

            if (memberOnly && memberOnly !== 'all') {
                conditions.push(`tc.member_only = $${paramIndex}`)
                params.push(memberOnly === 'yes')
                paramIndex++
            }
            ...
            // Calculate APPROVED job counts dynamically to ensure accuracy
            // We cannot rely solely on row.job_count because it might be stale or include unapproved jobs if sync hasn't run
            const companyIdsForStats = companyResult.map(c => c.company_id);
            let approvedCountsMap = {};

            if (companyIdsForStats.length > 0) {
                const idPlaceholders = companyIdsForStats.map((_, i) => `$${i + 1}`).join(',');
                // Query approved job counts
                const approvedCountQuery = `
                    SELECT company_id, COUNT(*) as count 
                    FROM jobs 
                    WHERE company_id IN (${idPlaceholders}) 
                    AND status = 'active' 
                    AND is_approved = true 
                    GROUP BY company_id
                 `;
                const approvedCountResult = await neonHelper.query(approvedCountQuery, companyIdsForStats);
                if (approvedCountResult) {
                    approvedCountResult.forEach(r => {
                        approvedCountsMap[r.company_id] = parseInt(r.count || '0');
                    });
                }
            }
            ...
            const companiesWithStats = companyResult.map(row => {
                ...
                return {
                    ...
                    tags: row.tags || [],
                    source: row.source,
                    jobCount: approvedCountsMap[row.company_id] || 0, // Use dynamic approved count
            ...
            if (companiesNeedingStats.length > 0) {
                ...
                const jobsQuery = `
                     ...
                     FROM jobs 
                     WHERE company_id IN (${idPlaceholders}) AND status = 'active' AND is_approved = true
            ...
            if (minJobs > 0 || conditions.length > 0) {
                ...
                // Fix: Count actual approved jobs instead of relying on potentially stale/unapproved job_count in trusted_companies
                const sumQuery = `
                    SELECT COUNT(j.job_id) as total_jobs 
                    ${baseQuery} 
                    JOIN jobs j ON j.company_id = tc.company_id
                    ${whereClause}
                    AND j.status = 'active' AND j.is_approved = true
                `
                ...
                try {
                    ...
                    if (catResult && catResult.length > 0) {
                        ...
                        const categoryQueryFallback = `
                            ...
                            ${whereClause} 
                            ${whereClause ? 'AND' : 'WHERE'} j.status = 'active' AND j.is_approved = true AND j.category IS NOT NULL
                        `;
...
export async function saveAllCompanies(companies, mode = 'upsert') {
    ...
    await neonHelper.transaction(async (client) => {
        ...
        for (const company of companies) {
            ...
            const normalizedReferralContacts = normalizeReferralContactsForDb(
                company.referralContacts,
                company.hiringEmail,
                company.emailType,
                emailTypeConstraintMode,
                company.id
            )
            const primaryReferralContact = normalizedReferralContacts[0]
            const normalizedEmailType = normalizeEmailTypeForDb(
                primaryReferralContact?.emailType || company.emailType,
                emailTypeConstraintMode
            )
            const normalizedHiringEmail = primaryReferralContact?.hiringEmail || company.hiringEmail || null

            await client.query(`
                INSERT INTO trusted_companies 
                (company_id, name, website, careers_page, linkedin, description, logo, cover_image, industry, tags, source, job_count, can_refer, member_only, status, translations, created_at, updated_at, last_crawled_at, address, employee_count, hiring_email, email_type, referral_contacts, founded_year, specialties, company_rating, rating_source)
                ...
                ON CONFLICT (company_id) DO UPDATE SET
                    ...
                    hiring_email = EXCLUDED.hiring_email,
                    email_type = EXCLUDED.email_type,
                    referral_contacts = EXCLUDED.referral_contacts,
            ...
            `, [
                ...
                normalizedHiringEmail,
                normalizedEmailType,
                JSON.stringify(normalizedReferralContacts),
            ...
            await cleanupInvalidJobReferralLinks(client, company.id, normalizedReferralContacts)
        }
    })
    for (const company of companies) {
        if (company?.id) await reconcileCompanyDirectoryHistory(company.id)
    }
...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET') {
        ...
        if (action !== 'crawl' && action !== 'sync-jobs') {
            // Shorter cache for individual queries to reflect admin updates quickly
            if (id || name) {
    ...
    if (req.method === 'GET' && !NEON_CONFIGURED && isLocalPreviewRequest(req)) {
        const { action, target, id, name } = req.query || {}

        if (action === 'featured_home') {
            return res.status(200).json(getPreviewFeaturedCompaniesResponse())
        }

        if (action === 'cover_image') {
            const payload = getPreviewCoverImageResponse(req.query?.company_id)
            if (!payload) {
        ...
        if (target === 'trusted_companies_with_jobs_info') {
            const { canAccessTrustedCompaniesPage } = await resolveTrustedCompaniesListAccess(req)
            const safeLimit = canAccessTrustedCompaniesPage
                ? Math.max(1, Math.min(parseInt(req.query?.limit, 10) || 20, 100))
                : Math.max(1, Math.min(parseInt(req.query?.limit, 10) || 8, 8))
            const previewResult = getPreviewCompaniesWithJobStatsResponse(canAccessTrustedCompaniesPage ? (req.query || {}) : {
                page: 1,
                limit: safeLimit,
                sortBy: 'jobCount',
                sortOrder: 'desc'
            })
            return res.status(200).json(canAccessTrustedCompaniesPage
                ? previewResult
                : restrictTrustedCompaniesListResponse(previewResult, safeLimit)
            )
        }

        if ((!target && !action) || target === 'companies') {
            if (id) {
                return res.status(200).json({ success: true, company: getPreviewCompanyById(id) })
            }

            if (name) {
                return res.status(200).json(getPreviewCompaniesResponse(req.query || {}))
            }
    ...
    try {
        ...
        if (req.method === 'POST' && action === 'deactivate') {
            ...
            if (!id) return res.status(400).json({ success: false, error: '企业 ID 不能为空' })
            if (!NEON_CONFIGURED) return res.status(503).json({ success: false, error: '数据库暂不可用，请稍后重试。' })
            try {
                const result = await deactivateCompany(neonHelper, id)
                if (!result) return res.status(404).json({ success: false, error: '企业不存在，请刷新列表。' })
                console.log('[trusted-companies] Company deactivated:', { id, hiddenJobs: result.hidden_jobs })
                return res.status(200).json({ success: true, id, hiddenJobs: result.hidden_jobs })
            } catch (error) {
                console.error('[trusted-companies] Deactivation failed:', { id, code: error?.code, message: error?.message })
                return res.status(500).json({ success: false, error: '停用企业失败，请稍后重试。' })
            }
        }
        ...
        if (req.method === 'GET' && action === 'featured_home') {
            ...
            try {
                const selectedFeaturedJobs = await getFeaturedHomeSelectedJobs({
                    neonHelper,
                    jobsTable: 'jobs',
                    limit: 6
                })
                const uniqueCompanyIds = []
                for (const row of selectedFeaturedJobs) {
                ...
                if (uniqueCompanyIds.length > 0) {
                    const placeholders = uniqueCompanyIds.map((_, i) => `$${i + 1}`).join(',')
                    ...
                    `
                    const unorderedResult = await neonHelper.query(query, uniqueCompanyIds)
                    
                    if (unorderedResult) {
                        result = uniqueCompanyIds.map(id => unorderedResult.find(r => r.company_id === id)).filter(Boolean)
                    }
                }
                ...
                const companies = (result || []).map(row => ({
                    ...
                    tags: row.tags || [],
                    source: row.source,
                    jobCount: parseInt(row.approved_job_count || '0'), // Use dynamic approved count
                    canRefer: row.can_refer || false,
                    memberOnly: row.member_only || false,
                    lastCrawledAt: row.last_crawled_at,
                    translations: row.translations,
                ...
                // Cache header shortened to allow admin updates to reflect quickly
        ...
        if (req.method === 'GET' && action === 'crawl') {
            // Security Check: Crawl action should be admin-only to prevent abuse
            const token = extractToken(req)
            const payload = token ? verifyToken(token) : null
            const requester = payload?.userId ? await userHelper.getUserById(payload.userId) : null
            const isAdmin = !!(requester?.roles?.admin || SUPER_ADMIN_EMAILS.includes(String(requester?.email || '').toLowerCase()))
            ...
            try {
                ...
                return res.status(200).json({
                    url: targetUrl,
                    logo: bestLogo,
                    icon: bestLogo, // Match CompanyMetadata interface
                    coverImage: metadata.coverImage,
                    image: metadata.coverImage, // Match CompanyMetadata interface
        ...
        if (req.method === 'GET') {
            const { target, action, page, limit, sortBy, sortOrder, industry, search, name, memberOnly, isTrusted } = req.query
            const viewerAccess = await resolveViewerContactAccess(req)
            ...
            if ((!target && !req.query.action) || target === 'companies') {
                // If ID is present, return single company
                if (req.query.id) {
                    if (NEON_CONFIGURED) {
                        const result = await neonHelper.query("SELECT * FROM trusted_companies WHERE company_id = $1 AND status = 'active'", [req.query.id])
                        if (result && result.length > 0) {
                            const {
                                companyRow: row,
                                referralContacts,
                                jobReferralContactMode,
                                jobEffectiveReferralContactCount
                            } = await resolveCompanyReferralContactsForJob(
                                neonHelper,
                                result[0],
                                req.query.jobId
                            )
                            const jobAccessContext = await resolveJobContactAccessContext(req.query.jobId)
                            const company = sanitizeCompanyContactsForViewer({
                                id: row.company_id,
                                name: row.name,
                                website: row.website,
                                careersPage: row.careers_page,
                                ...
                                jobMemberOnly: jobAccessContext.memberOnly,
                                allowPublicEmailOnly: jobAccessContext.allowPublicEmailOnly
                            })
                ...
                if (page || limit) {
                    ...
                    return res.status(200).json({
                        ...
                        companies: Array.isArray(result?.companies)
                            ? result.companies.map((company) => sanitizeCompanyContactsForViewer(company, viewerAccess))
                            : []
                    })
                }

                const companies = await getAllCompanies({ search, name })
                return res.status(200).json({
                    success: true,
                    companies: companies.map((company) => sanitizeCompanyContactsForViewer(company, viewerAccess))
                })
            }
                    ...
                    for (let offset = 0; offset < totalJobs; offset += BATCH_SIZE) {
                        ...
                        if (updates.length > 0) {
                            try {
                                ...
                                for (const companyId of new Set(updates.map((update) => update.company_id))) {
                                    await reconcileCompanyDirectoryHistory(companyId)
                                }
            ...
            if (target === 'companies') {
                ...
                if (id) {
                    if (NEON_CONFIGURED) {
                        try {
                            const result = await neonHelper.query("SELECT * FROM trusted_companies WHERE company_id = $1 AND status = 'active'", [id])
                            if (result && result.length > 0) {
                                const row = result[0]
                                const company = {
                                    id: row.company_id,
                                ...
                                return res.status(200).json({
                                    success: true,
                                    company: sanitizeCompanyContactsForViewer(company, viewerAccess)
                                })
                            }
                        } catch (e) {
                ...
                const memberOnly = req.query.memberOnly || ''
            ...
            }).then((items) => items.map((company) => sanitizeCompanyContactsForViewer(company, viewerAccess)))
            const { id } = req.query
            ...
            if (target === 'trusted_companies_with_jobs_info') {
                try {
                    const { page = 1, limit = 20, sortBy = 'job_count', sortOrder = 'desc', industry, search, memberOnly, region, minJobs, jobCategories } = req.query
                    const { canAccessTrustedCompaniesPage } = await resolveTrustedCompaniesListAccess(req)
                    const safeLimit = canAccessTrustedCompaniesPage
                        ? Math.max(1, Math.min(parseInt(limit, 10) || 20, 100))
                        : Math.max(1, Math.min(parseInt(limit, 10) || 8, 8))

                    // Cache optimization: Cache filtered results to balance freshness and performance
                    res.setHeader('Cache-Control', 'public, s-maxage=15, stale-while-revalidate=30')

                    const result = await getCompaniesWithJobStats({
                        page: canAccessTrustedCompaniesPage ? parseInt(page, 10) : 1,
                        limit: safeLimit,
                        sortBy: canAccessTrustedCompaniesPage ? sortBy : 'jobCount',
                        sortOrder: canAccessTrustedCompaniesPage ? sortOrder : 'desc',
                        industry: canAccessTrustedCompaniesPage ? industry : undefined,
                        search: canAccessTrustedCompaniesPage ? search : undefined,
                        memberOnly: canAccessTrustedCompaniesPage ? memberOnly : undefined,
                        region: canAccessTrustedCompaniesPage ? region : undefined,
                        minJobs: canAccessTrustedCompaniesPage ? minJobs : 1,
                        jobCategories: canAccessTrustedCompaniesPage ? jobCategories : undefined
                    })
                    if (isLocalPreviewRequest(req) && (!Array.isArray(result?.companies) || result.companies.length === 0) && Number(result?.total || 0) === 0) {
                        const previewResult = getPreviewCompaniesWithJobStatsResponse(canAccessTrustedCompaniesPage ? (req.query || {}) : {
                            page: 1,
                            limit: safeLimit,
                            sortBy: 'jobCount',
                    ...
                    return res.status(200).json(canAccessTrustedCompaniesPage
                        ? result
                        : restrictTrustedCompaniesListResponse(result, safeLimit)
                    )
                } catch (error) {
                    console.error('[trusted-companies] Error fetching companies with job stats:', error)
                    if (isLocalPreviewRequest(req)) {
                        const fallbackLimit = Math.max(1, Math.min(parseInt(req.query?.limit, 10) || 8, 8))
                        const previewResult = getPreviewCompaniesWithJobStatsResponse({
                            page: 1,
                            limit: fallbackLimit,
                            sortBy: 'jobCount',
                            sortOrder: 'desc'
                        })
                        return res.status(200).json(restrictTrustedCompaniesListResponse(previewResult, fallbackLimit))
                    }
                    return res.status(500).json({ error: 'Failed to fetch companies with job information' })
                }
            }

            if (isLocalPreviewRequest(req) && companies.length === 0) {
                return res.status(200).json(getPreviewCompaniesResponse(req.query || {}))
            }
        ...
        if (req.method === 'POST') {
                ...
                try {
                    ...
                    const updateJobsResult = await neonHelper.query(`
                        ...
                        WHERE 
                            ...
                            AND (
                                ...
                                j.member_only IS DISTINCT FROM tc.member_only OR
                                j.source_type IS DISTINCT FROM 'official'
                            )
                        RETURNING j.company_id
                    `)

                    const updatedCount = updateJobsResult?.rowCount || updateJobsResult?.length || 0
                    const updatedCompanyIds = [...new Set((updateJobsResult || []).map((row) => String(row.company_id || '')).filter(Boolean))]
                    for (const companyId of updatedCompanyIds) await reconcileCompanyDirectoryHistory(companyId)
                    ...
                    await neonHelper.query(`
                        ...
                        FROM (
                            SELECT company_id, COUNT(*) as count 
                            FROM jobs 
                            WHERE status = 'active' AND is_approved = true AND company_id IS NOT NULL 
                            GROUP BY company_id
                        ) sub
                ...
                if (body.isTrusted) {
                    if (NEON_CONFIGURED) {
                        try {
                            ...
                            let normalizedReferralContacts = []
                            let primaryReferralContact = null
                            let normalizedHiringEmail = body.hiringEmail || null
                            let normalizedEmailTypeForClient = normalizeEmailTypeForClient(body.emailType)

                            await neonHelper.transaction(async (client) => {
                                const emailTypeConstraintMode = await getEmailTypeConstraintMode(client)
                                normalizedReferralContacts = normalizeReferralContactsForDb(
                                    body.referralContacts,
                                    body.hiringEmail,
                                    body.emailType,
                                    emailTypeConstraintMode,
                                    companyId
                                )
                                primaryReferralContact = normalizedReferralContacts[0] || null
                                const normalizedEmailTypeForDb = normalizeEmailTypeForDb(
                                    primaryReferralContact?.emailType || body.emailType,
                                    emailTypeConstraintMode
                                )
                                normalizedHiringEmail = primaryReferralContact?.hiringEmail || body.hiringEmail || null
                                normalizedEmailTypeForClient = normalizeEmailTypeForClient(primaryReferralContact?.emailType || body.emailType)

                                await client.query(`
                                    INSERT INTO trusted_companies 
                                    (company_id, name, website, careers_page, linkedin, description, logo, cover_image, industry, tags, source, job_count, can_refer, member_only, status, translations, updated_at, address, employee_count, hiring_email, email_type, referral_contacts, founded_year, specialties, company_rating, rating_source, created_at)
                                ...
                                await cleanupInvalidJobReferralLinks(client, companyId, normalizedReferralContacts)
                            })
                            await reconcileCompanyDirectoryHistory(companyId)
                            ...
                            const savedCompany = {
                                id: companyId,
                                name: body.name,
                                website: body.url || body.website || '',
                                careersPage: body.careersPage || '',
                                ...
                                hiringEmail: normalizedHiringEmail,
                                emailType: normalizedEmailTypeForClient,
                                referralContacts: normalizeReferralContactsForClient(normalizedReferralContacts, normalizedHiringEmail, normalizedEmailTypeForClient, companyId),
                                foundedYear: body.foundedYear || null,
                                specialties: body.specialties || [],
                                companyRating: body.companyRating || null,
                                ratingSource: body.ratingSource || null,
                                createdAt: body.createdAt || now // Use existing if available, else now
                            };

                            const logoCacheResult = await cacheCompanyLogoBestEffort(neonHelper, companyId, body.logo)
                            if (logoCacheResult?.success && logoCacheResult.cachedLogoUrl) {
                                savedCompany.cachedLogoUrl = logoCacheResult.cachedLogoUrl
                            }

                            return res.status(200).json({ success: true, message: 'Trusted company saved', company: savedCompany })
                        } catch (e) {
            ...
            if (action === 'crawl-jobs') {
                ...
                const companies = await getAllCompanies({ id })
                const company = companies.find(c => String(c.id) === String(id))
                if (!company) return res.status(404).json({ success: false, error: 'Company not found' })
                ...
                if (!company.careersPage) {
                    ...
                    if (NEON_CONFIGURED) {
                        try {
                            await neonHelper.transaction(async (client) => {
                                // Keep current rows as historical evidence; only hide them.
                                await client.query("UPDATE jobs SET status = 'inactive', updated_at = NOW() WHERE company_id = $1", [company.id])
                                if (company.name) {
                                    await client.query("UPDATE jobs SET status = 'inactive', updated_at = NOW() WHERE company = $1 AND company_id IS NULL", [company.name])
                                }
                            ...
                            await reconcileCompanyDirectoryHistory(company.id)
                            return res.status(200).json({
                                success: true,
                                count: 0,
                                jobs: [],
                ...
                try {
                    const { dispatchSpecialCrawler } = await import('./special-crawlers.js')
                    ...
                    const crawlOptions = {
                        ...
                        maxDetailFetches: parseInt(maxDetails || '100', 10),
                        concurrency: 3
                    }

                    console.log('[trusted-companies] Crawl options:', crawlOptions)

                    // 1. Try special crawlers first
                    let crawlResult = { jobs: [], company: null };
                    const specialJobs = await dispatchSpecialCrawler(url);
                    if (specialJobs && specialJobs.length > 0) {
                        console.log(`[trusted-companies] Used special crawler for ${company.name}, found ${specialJobs.length} jobs`);
                        crawlResult.jobs = specialJobs;
                    } else {
                        // 2. Fallback to standard crawler
                        crawlResult = await crawlCompanyJobs(company.id, company.name, url, crawlOptions)
                    }
                    ...
                    if (crawledCompany) {
                        ...
                        if (idx !== -1) {
                            ...
                            if (updated) {
                                ...
                                if (crawledCompany.logo) {
                                    await cacheCompanyLogoBestEffort(neonHelper, company.id, crawledCompany.logo)
                                }
                            }
                        }
                    }
                    ...
                    // ---------------- 增加 3层清洗过滤漏斗 ------------------
                    const cleanEnrichedJobs = []
                    let filteredByLocation = 0
                    let filteredByOffline = 0

                    const strictEuroUSLocations = ['us', 'u.s.', 'usa', 'united states', 'uk', 'u.k.', 'united kingdom', 'europe', 'eu', 'canada', 'north america']
                    const offlineWords = ['on-site', 'in-office', 'no remote', '线下办公']

                    for (const j of enrichedJobs) {
                        // 1. 过滤严格定位在欧美的且不包含亚洲/Remote的岗
                        if (j.location) {
                            const locLower = j.location.toLowerCase();
                            // 如果地点完全只匹配欧美字典，且没提到 Global/Remote/Asia/APAC/Worldwide
                            const hasEuroUS = strictEuroUSLocations.some(l => locLower.includes(l));
                            const hasGlobalOrAsia = ['global', 'remote', 'worldwide', 'anywhere', 'apac', 'asia', 'china'].some(l => locLower.includes(l));
                            if (hasEuroUS && !hasGlobalOrAsia) {
                                filteredByLocation++
                                continue
                            }
                        }
                        ...
                        if (j.description) {
                            ...
                            if (offlineWords.some(w => descLower.includes(w))) {
                                filteredByOffline++
                                continue;
                            }
                        }

                        cleanEnrichedJobs.push(j)
                    }

                    console.log(`[trusted-companies] Crawled ${enrichedJobs.length} jobs, Kept ${cleanEnrichedJobs.length} (Filtered Loc:${filteredByLocation}, Offline:${filteredByOffline})`)

                    // Save back to database
                    if (NEON_CONFIGURED) {
                        try {
                            // --- Step A: 在线老岗位查存活 (复用验证器代替盲删) ---
                            // 查找本项目所有的已审核上架的老岗位 URL
                            ...
                            if (existingJobs && existingJobs.length > 0) {
                                console.log(`[trusted-companies] Found ${existingJobs.length} existing jobs, checking liveness...`)
                                for (const old of existingJobs) {
                                    const vRes = await checkUrlStatus(old.url)
                                    if (vRes.status === 'dead') {
                                        deadJobs.push(old.job_id)
                                    }
                                }
                                if (deadJobs.length > 0) {
                                    console.log(`[trusted-companies] Auto soft-deleted ${deadJobs.length} dead jobs.`)
                                    await neonHelper.query(`
                                        UPDATE jobs SET status = 'inactive', last_verified_at = NOW() 
                                        WHERE job_id = ANY($1)
                                    `, [deadJobs])
                                }
                                // Update alive ones timestamp
                                const aliveJobIds = existingJobs.map(j => j.job_id).filter(id => !deadJobs.includes(id))
                                if (aliveJobIds.length > 0) {
                                    await neonHelper.query(`UPDATE jobs SET last_verified_at = NOW() WHERE job_id = ANY($1)`, [aliveJobIds])
                                }
                            }

                            // --- Step B: 新职位插入 ---
                            // NOTE: 坚决摒弃了 DELETE 语法，转用 upsert 让库自己判断新增
                            if (cleanEnrichedJobs.length > 0) {
                                await writeJobsToNeon(cleanEnrichedJobs, 'upsert', true)
                            } else if (deadJobs.length > 0) {
                                await reconcileCompanyDirectoryHistory(company.id)
                            }

                            // --- Step C: 更新企业统计总数 ---
                            const finalActiveCountRes = await neonHelper.query(`
                                SELECT COUNT(*) as cnt FROM jobs 
                                WHERE company_id = $1 AND status = 'active' AND is_approved = true
                            `, [company.id])
                            const finalSavedCount = parseInt(finalActiveCountRes?.[0]?.cnt || '0')
                            ...
                            return res.status(200).json({
                                ...
                                jobs: cleanEnrichedJobs, // 回显这次成功插入并清洗完毕的队列
                                info: `新增清洗入库 ${cleanEnrichedJobs.length} 个；复查老站下架死链 ${deadJobs.length} 个`
                            })

                        } catch (txnError) {
                            console.error('[trusted-companies] DB Error during crawl-jobs:', txnError)
                            throw txnError;
                        }
                    }

                    return res.status(200).json({ success: true, count: cleanEnrichedJobs.length, jobs: cleanEnrichedJobs })
            ...
            const { id, name, website, careersPage, linkedin, description, logo, coverImage, tags, industry, canRefer, memberOnly } = body
            ...
            if (id) {
                ...
                companyToSave = {
                    ...
                    memberOnly: memberOnly !== undefined ? !!memberOnly : existing.member_only,
                    address: body.address !== undefined ? body.address : existing.address,
                    employeeCount: body.employeeCount !== undefined ? body.employeeCount : existing.employee_count,
                    hiringEmail: body.hiringEmail !== undefined ? body.hiringEmail : existing.hiring_email,
                    emailType: normalizeEmailTypeForClient(body.emailType !== undefined ? body.emailType : existing.email_type),
                    referralContacts: body.referralContacts !== undefined
                        ? normalizeReferralContactsForClient(body.referralContacts, body.hiringEmail, body.emailType, id)
                        : normalizeReferralContactsForClient(existing.referral_contacts, existing.hiring_email, existing.email_type, id),
                ...
                companyToSave = {
                    ...
                    hiringEmail: body.hiringEmail,
                    emailType: normalizeEmailTypeForClient(body.emailType),
                    referralContacts: normalizeReferralContactsForClient(body.referralContacts, body.hiringEmail, body.emailType, body.id),
                    foundedYear: body.foundedYear,
                    specialties: body.specialties,
                    companyRating: body.companyRating,
                    ratingSource: body.ratingSource,
                    createdAt: now,
                    updatedAt: now,
                    isTrusted: true,
                    source: 'manual',
            ...
            try {
                ...
                const savedCompany = {
                    ...
                    careersPage: companyToSave.careersPage || '',
                    referralContacts: normalizeReferralContactsForClient(
                        companyToSave.referralContacts,
                        companyToSave.hiringEmail,
                        companyToSave.emailType,
                        companyToSave.id
                    ),
                    hiringEmail: companyToSave.hiringEmail || '',
                    emailType: normalizeEmailTypeForClient(companyToSave.emailType),
                    isTrusted: true,
                    canRefer: !!companyToSave.canRefer,
                    memberOnly: !!companyToSave.memberOnly,
                    createdAt: companyToSave.createdAt || now,
                    updatedAt: companyToSave.updatedAt || now
                }
                const logoCacheResult = await cacheCompanyLogoBestEffort(neonHelper, companyToSave.id, companyToSave.logo)
                if (logoCacheResult?.success && logoCacheResult.cachedLogoUrl) {
                    savedCompany.cachedLogoUrl = logoCacheResult.cachedLogoUrl
                }
                return res.status(200).json({ success: true, message: 'Saved successfully', company: savedCompany })
            } catch (e) {
        ...
        if (req.method === 'DELETE') {
            ...
            try {
                ...
                const historyCount = await neonHelper.query('SELECT COUNT(*)::int AS count FROM company_job_history WHERE company_id = $1', [id])
                if (Number(historyCount?.[0]?.count || 0) > 0) {
                    console.warn('[trusted-companies] Delete blocked:', { id, code: 'COMPANY_HISTORY_EXISTS', historyCount: Number(historyCount[0].count) })
                    return res.status(409).json({ success: false, code: 'COMPANY_HISTORY_EXISTS', error: '该企业已有招聘历史，不能删除；请停用企业。' })
                }

                // Neon HTTP callback transactions are not atomic. Use a single statement;
                // DELETE RETURNING also avoids relying on node-postgres's rowCount shape.
                const deleted = await neonHelper.query(`
                    WITH deleted_trusted AS (
                        DELETE FROM trusted_companies WHERE company_id = $1 RETURNING company_id, name
                    ), deleted_extracted AS (
                        DELETE FROM extracted_companies WHERE company_id = $1
                        AND NOT EXISTS (SELECT 1 FROM deleted_trusted) RETURNING company_id, name
                    ), removed AS (
                        SELECT * FROM deleted_trusted UNION ALL SELECT * FROM deleted_extracted
                    ), hidden_jobs AS (
                    ...
                    SELECT company_id FROM removed
                `, [id])
                if (!deleted?.length) return res.status(404).json({ success: false, error: '企业不存在，请刷新列表。' })

                return res.status(200).json({ success: true, message: 'Deleted successfully' })
            } catch (error) {
                console.error('[trusted-companies] Delete failed:', error)
                if (error?.code === '23503') {
                    return res.status(409).json({ success: false, code: 'COMPANY_REFERENCES_EXIST', error: '该企业存在关联记录，无法直接删除；可改为停用并保留历史。' })
                }
        ...
        if (req.method === 'GET' && isLocalPreviewRequest(req)) {
            const { action, target, id } = req.query || {}
            if (action === 'featured_home') {
                return res.status(200).json(getPreviewFeaturedCompaniesResponse())
            }
            if (action === 'cover_image') {
            ...
            if (target === 'trusted_companies_with_jobs_info') {
                return res.status(200).json(getPreviewCompaniesWithJobStatsResponse(req.query || {}))
            }
            if ((!target && !action) || target === 'companies') {
                if (id) {
                    return res.status(200).json({ success: true, company: getPreviewCompanyById(id) })
