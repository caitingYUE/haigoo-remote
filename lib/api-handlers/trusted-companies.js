...
// 安全加载 Vercel KV - removed dynamic require
// let kv = null
// try {
...
export async function saveAllCompanies(companies) {
    ...
    try {
        ...
        if (UPSTASH_REST_CONFIGURED) {
            success = await upstashSet(STORAGE_KEY, JSON.stringify(companies))
            if (success) console.log('[trusted-companies] Saved to Upstash REST')
            else console.error('[trusted-companies] Failed to save to Upstash REST')
        } else if (REDIS_CONFIGURED) {
            const client = await getRedisClient()
            if (client) {
                await client.set(STORAGE_KEY, JSON.stringify(companies))
                success = true
            ...
            // If only memory is available, consider it a "soft" success but warn
            console.warn('[trusted-companies] No persistent storage configured, saved to memory only')
            success = true
        }
    } catch (e) {
            ...
            if (req.query.action === 'sync-jobs') {
                ...
                try {
                    ...
                    if (UPSTASH_REST_CONFIGURED) {
                        ...
                        trustedCompanies = data ? (typeof data === 'string' ? JSON.parse(data) : data) : []
                    } else if (REDIS_CONFIGURED) {
                        const client = await getRedisClient()
                        if (client) {
                    ...
                    let jobsArray = jobs ? (typeof jobs === 'string' ? JSON.parse(jobs) : jobs) : []
                    if (!Array.isArray(jobsArray)) jobsArray = []
                    ...
                    if (updatedCount > 0) {
                        if (UPSTASH_REST_CONFIGURED) {
                            await upstashSet(JOBS_KEY, JSON.stringify(updatedJobs))
                        } else if (REDIS_CONFIGURED) {
                            const client = await getRedisClient()
                            if (client) await client.set(JOBS_KEY, JSON.stringify(updatedJobs))
                        } else if (KV_CONFIGURED && kv) {
                            await kv.set(JOBS_KEY, updatedJobs)
                        }
                    }
            ...
            if (resource === 'companies') {
                ...
                if (action === 'extract') {
                    ...
                    try {
                        let jobs = null
                        if (UPSTASH_REST_CONFIGURED) {
                            console.log('[companies] Fetching from Upstash REST...')
                            jobs = await upstashGet(JOBS_KEY)
                        } else if (REDIS_CONFIGURED) {
                            console.log('[companies] Fetching from Redis TCP...')
                            const client = await getRedisClient()
                            if (client) jobs = await client.get(JOBS_KEY)
                        } else if (KV_CONFIGURED && kv) {
                            console.log('[companies] Fetching from Vercel KV...')
                            jobs = await kv.get(JOBS_KEY)
                        } else {
                            console.warn('[companies] No storage configured for jobs extraction')
                        }

                        let jobsArray = []
                        try {
                            jobsArray = jobs ? (typeof jobs === 'string' ? JSON.parse(jobs) : jobs) : []
                        } catch (e) {
                            console.error('[companies] Failed to parse jobs data:', e)
                        ...
                        try {
                            ...
                            console.log('[companies] Saved merged companies to storage')
                        } catch (e) {
                ...
                // Cache the list response to reduce Origin Transfer
        ...
        if (req.method === 'POST') {
            ...
            if (action === 'sync-jobs') {
                ...
                try {
                    ...
                    if (updatedCount > 0) {
                        // 4. Save back
                        const jobsStr = JSON.stringify(enrichedJobs)
                        if (UPSTASH_REST_CONFIGURED) {
                            await upstashSet(JOB_STORAGE_KEY, jobsStr)
                        } else if (REDIS_CONFIGURED) {
                            const client = await getRedisClient()
                            ...
                            await kv.set(JOB_STORAGE_KEY, enrichedJobs)
                        } else {
                            globalThis.__haigoo_processed_jobs_mem = enrichedJobs
                        }
            ...
            if (action === 'batch-import') {
                try {
                    ...
                    const { filePath, crawlMetadata = true } = req.body
