...
const FEATURE_KEYS = [
  ...
  'website_apply',
  'match_analysis',
  'corporate_english_page',
  'corporate_english_video_play',
  'corporate_english_clip_play',
];
...
const FEATURE_LABELS = {
  ...
  email_apply: '邮箱直申',
  company_info: '企业认证信息',
  membership_center: 'Club 权益页',
  membership_plan_trial_week: '体验会员（周）',
  membership_plan_starter: 'Club Starter',
  membership_plan_quarter: '季度会员',
  membership_plan_half_year: 'Club Member',
  membership_plan_year: '年度会员',
  membership_plan_annual: 'Club Partner',
  membership_advisor_added: '已添加顾问',
  ...
  corporate_english_page: '外企英语访问',
  corporate_english_video_play: '外企英语视频播放',
  corporate_english_clip_play: '外企英语跟读播放',
};

const ANALYTICS_TIME_ZONE = 'Asia/Shanghai';

const PERIOD_META = {
  day: { unit: 'day', label: '今日' },
  week: { unit: 'week', label: '本周' },
  month: { unit: 'month', label: '本月' },
};

function normalizePeriod(period = 'week') {
  return Object.prototype.hasOwnProperty.call(PERIOD_META, period) ? period : 'week';
}

function normalizeSegment(segment = 'all') {
  return ['all', 'guest', 'free', 'member'].includes(segment) ? segment : 'all';
}

function normalizeMetricMode(metricMode = 'total') {
  return metricMode === 'per_capita' ? 'per_capita' : 'total';
}

function normalizeView(view = 'core') {
  return ['core', 'copilot', 'membership'].includes(view) ? view : 'core';
}

function getPeriodStartExpression(period = 'week') {
  const normalizedPeriod = normalizePeriod(period);
  const unit = PERIOD_META[normalizedPeriod].unit;
...
function getDateFilter(period = 'week') {
  return getDateFilterForColumn('created_at', period);
}

function getDateFilterForColumn(column, period = 'week') {
  return `${column} >= ${getPeriodStartExpression(period)} AND ${column} < NOW()`;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const metricMode = normalizeMetricMode(req.query.metricMode);
    const view = normalizeView(req.query.view);
    const shouldLoadCore = view === 'core';
    const shouldLoadCopilot = view === 'copilot';
    const shouldLoadMembership = view === 'membership';
    const shouldLoadMonetization = shouldLoadCore || shouldLoadMembership;
    const dateFilter = getDateFilter(period);
    const membershipActivationDateFilter = getDateFilterForColumn('activation_at', period);
    ...
    `;
    const corporateEnglishPageCondition = `
      (
        event_name = 'corporate_english_page_view'
        OR (
          event_name = 'page_view' AND (
            COALESCE(page_key, properties->>'page_key') IN ('corporate_english', 'corporate-english')
            OR ${eventPathExpr} = '/careerlearning'
            OR ${eventPathExpr} LIKE '/careerlearning?%'
            OR ${eventPathExpr} = '/corporate-english'
            OR ${eventPathExpr} LIKE '/corporate-english?%'
          )
        )
      )
    `;
    const corporateEnglishVideoPlayCondition = `
      (
        event_name IN ('corporate_english_video_play', 'corporate_english_video_open')
        OR (
          event_name = 'feature_click'
          AND COALESCE(feature_key, properties->>'feature_key') = 'corporate_english_video_play'
        )
      )
    `;
    const corporateEnglishClipPlayCondition = `
      (
        event_name = 'corporate_english_clip_play'
        OR (
          event_name = 'feature_click'
          AND COALESCE(feature_key, properties->>'feature_key') = 'corporate_english_clip_play'
        )
      )
    `;
    const corporateEnglishSectionExpr = `COALESCE(
      NULLIF(properties->>'module_key', ''),
      NULLIF(properties->>'corporate_english_section', ''),
      NULLIF(properties->>'section', ''),
      CASE
        WHEN event_name IN ('corporate_english_video_play', 'corporate_english_video_open', 'corporate_english_clip_play') THEN 'ceo'
        ELSE NULL
      END
    )`;
    const corporateEnglishModuleViewCondition = (section) => `
      (
        event_name IN ('corporate_english_module_view', 'corporate_english_section_view')
        AND ${corporateEnglishSectionExpr} = '${section}'
      )
    `;
    const corporateEnglishDetailViewCondition = (section) => `
      (
        event_name = 'corporate_english_detail_view'
        AND ${corporateEnglishSectionExpr} = '${section}'
      )
    `;
    const corporateEnglishSectionVideoPlayCondition = (section) => `
      (
        ${corporateEnglishVideoPlayCondition}
        AND ${corporateEnglishSectionExpr} = '${section}'
      )
    `;
    const corporateEnglishSectionClipPlayCondition = (section) => `
      (
        ${corporateEnglishClipPlayCondition}
        AND ${corporateEnglishSectionExpr} = '${section}'
      )
    `;
    const corporateEnglishEntityIdExpr = `COALESCE(
      NULLIF(entity_id, ''),
      NULLIF(properties->>'entity_id', ''),
      NULLIF(properties->>'material_id', ''),
      NULLIF(properties->>'video_id', '')
    )`;
    ...
    const corporateEnglishFunnelQuery = `
      WITH base AS (
        SELECT ${actorExpr('ae')} AS actor_id, ae.*
        FROM analytics_events ae
        WHERE ${dateFilter} AND ${segmentFilter}
      ...
      actor_steps AS (
        SELECT
          actor_id,
          MIN(created_at) FILTER (WHERE ${corporateEnglishPageCondition}) AS corporate_english_visit_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishVideoPlayCondition}) AS corporate_english_video_play_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishClipPlayCondition}) AS corporate_english_clip_play_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishModuleViewCondition('ceo')}) AS corporate_english_ceo_module_view_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishDetailViewCondition('ceo')}) AS corporate_english_ceo_detail_view_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishSectionVideoPlayCondition('ceo')}) AS corporate_english_ceo_video_play_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishSectionClipPlayCondition('ceo')}) AS corporate_english_ceo_clip_play_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishModuleViewCondition('english_interview')}) AS corporate_english_interview_module_view_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishDetailViewCondition('english_interview')}) AS corporate_english_interview_detail_view_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishSectionVideoPlayCondition('english_interview')}) AS corporate_english_interview_video_play_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishModuleViewCondition('foreign_meeting')}) AS corporate_english_meeting_module_view_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishDetailViewCondition('foreign_meeting')}) AS corporate_english_meeting_detail_view_at,
          MIN(created_at) FILTER (WHERE ${corporateEnglishSectionVideoPlayCondition('foreign_meeting')}) AS corporate_english_meeting_video_play_at
        FROM base
        GROUP BY actor_id
      ),
      step_uv AS (
        SELECT 'corporate_english_visit' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_visit_at IS NOT NULL
        UNION ALL
        SELECT 'corporate_english_video_play' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_visit_at IS NOT NULL
          AND corporate_english_video_play_at IS NOT NULL
          AND corporate_english_video_play_at >= corporate_english_visit_at
        UNION ALL
        SELECT 'corporate_english_clip_play' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_visit_at IS NOT NULL
          AND corporate_english_clip_play_at IS NOT NULL
          AND corporate_english_clip_play_at >= corporate_english_visit_at
        UNION ALL
        SELECT 'corporate_english_ceo_module_view' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_ceo_module_view_at IS NOT NULL
        UNION ALL
        SELECT 'corporate_english_ceo_detail_view' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_ceo_detail_view_at IS NOT NULL
        UNION ALL
        SELECT 'corporate_english_ceo_video_play' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_ceo_detail_view_at IS NOT NULL
          AND corporate_english_ceo_video_play_at IS NOT NULL
          AND corporate_english_ceo_video_play_at >= corporate_english_ceo_detail_view_at
        UNION ALL
        SELECT 'corporate_english_ceo_clip_play' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_ceo_detail_view_at IS NOT NULL
          AND corporate_english_ceo_clip_play_at IS NOT NULL
          AND corporate_english_ceo_clip_play_at >= corporate_english_ceo_detail_view_at
        UNION ALL
        SELECT 'corporate_english_interview_module_view' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_interview_module_view_at IS NOT NULL
        UNION ALL
        SELECT 'corporate_english_interview_detail_view' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_interview_detail_view_at IS NOT NULL
        UNION ALL
        SELECT 'corporate_english_interview_video_play' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_interview_detail_view_at IS NOT NULL
          AND corporate_english_interview_video_play_at IS NOT NULL
          AND corporate_english_interview_video_play_at >= corporate_english_interview_detail_view_at
        UNION ALL
        SELECT 'corporate_english_meeting_module_view' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_meeting_module_view_at IS NOT NULL
        UNION ALL
        SELECT 'corporate_english_meeting_detail_view' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_meeting_detail_view_at IS NOT NULL
        UNION ALL
        SELECT 'corporate_english_meeting_video_play' AS step_id, COUNT(*) AS uv
        FROM actor_steps
        WHERE corporate_english_meeting_detail_view_at IS NOT NULL
          AND corporate_english_meeting_video_play_at IS NOT NULL
          AND corporate_english_meeting_video_play_at >= corporate_english_meeting_detail_view_at
      ),
      step_pv AS (
        SELECT 'corporate_english_visit' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishPageCondition}
        UNION ALL
        SELECT 'corporate_english_video_play' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishVideoPlayCondition}
        UNION ALL
        SELECT 'corporate_english_clip_play' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishClipPlayCondition}
        UNION ALL
        SELECT 'corporate_english_ceo_module_view' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishModuleViewCondition('ceo')}
        UNION ALL
        SELECT 'corporate_english_ceo_detail_view' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishDetailViewCondition('ceo')}
        UNION ALL
        SELECT 'corporate_english_ceo_video_play' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishSectionVideoPlayCondition('ceo')}
        UNION ALL
        SELECT 'corporate_english_ceo_clip_play' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishSectionClipPlayCondition('ceo')}
        UNION ALL
        SELECT 'corporate_english_interview_module_view' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishModuleViewCondition('english_interview')}
        UNION ALL
        SELECT 'corporate_english_interview_detail_view' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishDetailViewCondition('english_interview')}
        UNION ALL
        SELECT 'corporate_english_interview_video_play' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishSectionVideoPlayCondition('english_interview')}
        UNION ALL
        SELECT 'corporate_english_meeting_module_view' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishModuleViewCondition('foreign_meeting')}
        UNION ALL
        SELECT 'corporate_english_meeting_detail_view' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishDetailViewCondition('foreign_meeting')}
        UNION ALL
        SELECT 'corporate_english_meeting_video_play' AS step_id, COUNT(*) AS pv FROM base
        WHERE ${corporateEnglishSectionVideoPlayCondition('foreign_meeting')}
      )
      SELECT step_uv.step_id, step_uv.uv, COALESCE(step_pv.pv, 0) AS pv
      FROM step_uv
      LEFT JOIN step_pv USING (step_id)
    ...
    const corporateEnglishVideoStatsQuery = `
      WITH base AS (
        SELECT ${actorExpr('ae')} AS actor_id, ae.*
        FROM analytics_events ae
        WHERE ${dateFilter} AND ${segmentFilter}
      ...
      play_events AS (
        SELECT
          actor_id,
          created_at,
          ${corporateEnglishEntityIdExpr} AS entity_id,
          ${corporateEnglishSectionExpr} AS module_key
        FROM base
        WHERE ${corporateEnglishVideoPlayCondition}
          AND ${corporateEnglishEntityIdExpr} IS NOT NULL
      ),
      detail_events AS (
        SELECT
          actor_id,
          ...
          ${corporateEnglishSectionExpr} AS module_key
        FROM base
        WHERE event_name = 'corporate_english_detail_view'
          AND ${corporateEnglishEntityIdExpr} IS NOT NULL
      ),
      video_keys AS (
        SELECT entity_id FROM play_events
        UNION
        SELECT entity_id FROM detail_events
      ),
      play_stats AS (
        SELECT
          entity_id,
          COUNT(DISTINCT actor_id)::int AS play_uv,
          COUNT(*)::int AS play_pv
        FROM play_events
        GROUP BY entity_id
      ),
      detail_stats AS (
        SELECT
          entity_id,
          COUNT(DISTINCT actor_id)::int AS detail_uv,
          COUNT(*)::int AS detail_pv
        FROM detail_events
        GROUP BY entity_id
      ),
      video_meta AS (
        SELECT
          material_id::text AS entity_id,
          'ceo'::text AS module_key,
          COALESCE(NULLIF(material_title, ''), 'CEO访谈') AS video_title,
          COALESCE(NULLIF(company_name_snapshot, ''), NULLIF(speaker_name, ''), 'CEO访谈') AS source_label
        FROM corporate_english_materials
        WHERE deleted_at IS NULL
        UNION ALL
        SELECT
          video_id::text AS entity_id,
          module_key::text AS module_key,
          COALESCE(NULLIF(video_title, ''), '外企英语视频') AS video_title,
          COALESCE(NULLIF(video_source, ''), NULLIF(category, ''), '外企英语') AS source_label
        FROM corporate_english_module_videos
        WHERE deleted_at IS NULL
      )
      SELECT
        keys.entity_id,
        COALESCE(meta.module_key, 'ceo') AS module_key,
        COALESCE(meta.video_title, keys.entity_id) AS video_title,
        COALESCE(meta.source_label, '') AS source_label,
        COALESCE(play_stats.play_uv, 0)::int AS play_uv,
        COALESCE(play_stats.play_pv, 0)::int AS play_pv,
        COALESCE(detail_stats.detail_uv, 0)::int AS detail_uv,
        COALESCE(detail_stats.detail_pv, 0)::int AS detail_pv
      FROM video_keys keys
      LEFT JOIN LATERAL (
        SELECT *
        FROM video_meta
        WHERE video_meta.entity_id = keys.entity_id
        ORDER BY CASE WHEN video_meta.module_key = 'ceo' THEN 1 ELSE 0 END
        LIMIT 1
      ) meta ON TRUE
      LEFT JOIN play_stats
        ON play_stats.entity_id = keys.entity_id
      LEFT JOIN detail_stats
        ON detail_stats.entity_id = keys.entity_id
      ORDER BY play_uv DESC, play_pv DESC, detail_uv DESC, detail_pv DESC, video_title ASC
      LIMIT 12
    `;
    ...
    const monetizationFunnelQuery = `
      ...
      actor_steps AS (
        SELECT
          actor_id,
          MIN(created_at) FILTER (WHERE ${legacyMembershipFeatureCondition}) AS free_feature_exposure_at,
          MIN(created_at) FILTER (WHERE ${legacyMembershipClickCondition}) AS free_feature_click_at,
          MIN(created_at) FILTER (WHERE ${legacyMembershipConsumeCondition}) AS consume_or_limit_at,
          ...
          MIN(created_at) FILTER (WHERE event_name IN ('membership_plan_click', 'click_subscribe')) AS membership_plan_click_at,
      ...
      step_pv AS (
        ...
        WHERE actor_steps.free_feature_exposure_at IS NOT NULL
        ...
        INNER JOIN actor_steps ON actor_steps.actor_id = base.actor_id
        WHERE actor_steps.free_feature_exposure_at IS NOT NULL
          AND actor_steps.free_feature_click_at IS NOT NULL
          AND actor_steps.free_feature_click_at >= actor_steps.free_feature_exposure_at
        ...
        WHERE actor_steps.free_feature_exposure_at IS NOT NULL
          AND actor_steps.free_feature_click_at IS NOT NULL
          AND actor_steps.consume_or_limit_at IS NOT NULL
          AND actor_steps.free_feature_click_at >= actor_steps.free_feature_exposure_at
          AND actor_steps.consume_or_limit_at >= actor_steps.free_feature_click_at
        ...
        WHERE actor_steps.free_feature_exposure_at IS NOT NULL
          AND actor_steps.free_feature_click_at IS NOT NULL
          AND actor_steps.consume_or_limit_at IS NOT NULL
          AND actor_steps.upgrade_modal_view_at IS NOT NULL
          AND actor_steps.free_feature_click_at >= actor_steps.free_feature_exposure_at
          AND actor_steps.consume_or_limit_at >= actor_steps.free_feature_click_at
          AND actor_steps.upgrade_modal_view_at >= actor_steps.consume_or_limit_at
        ...
        WHERE actor_steps.free_feature_exposure_at IS NOT NULL
          ...
          AND actor_steps.consume_or_limit_at IS NOT NULL
          AND actor_steps.upgrade_modal_view_at IS NOT NULL
          AND actor_steps.membership_page_view_at IS NOT NULL
          AND actor_steps.free_feature_click_at >= actor_steps.free_feature_exposure_at
          AND actor_steps.consume_or_limit_at >= actor_steps.free_feature_click_at
          AND actor_steps.upgrade_modal_view_at >= actor_steps.consume_or_limit_at
          AND actor_steps.membership_page_view_at >= actor_steps.upgrade_modal_view_at
          AND (
            (event_name = 'page_view' AND COALESCE(page_key, properties->>'page_key') = 'membership')
            OR event_name = 'view_membership_page'
          )
        ...
        WHERE actor_steps.free_feature_exposure_at IS NOT NULL
          ...
          AND actor_steps.upgrade_modal_view_at IS NOT NULL
          AND actor_steps.membership_page_view_at IS NOT NULL
          AND actor_steps.membership_plan_click_at IS NOT NULL
          AND actor_steps.free_feature_click_at >= actor_steps.free_feature_exposure_at
          AND actor_steps.consume_or_limit_at >= actor_steps.free_feature_click_at
          AND actor_steps.upgrade_modal_view_at >= actor_steps.consume_or_limit_at
          AND actor_steps.membership_page_view_at >= actor_steps.upgrade_modal_view_at
          AND actor_steps.membership_plan_click_at >= actor_steps.membership_page_view_at
        ...
        WHERE actor_steps.free_feature_exposure_at IS NOT NULL
          ...
          AND actor_steps.membership_page_view_at IS NOT NULL
          AND actor_steps.membership_plan_click_at IS NOT NULL
          AND actor_steps.membership_payment_success_at IS NOT NULL
          AND actor_steps.free_feature_click_at >= actor_steps.free_feature_exposure_at
          AND actor_steps.consume_or_limit_at >= actor_steps.free_feature_click_at
          AND actor_steps.upgrade_modal_view_at >= actor_steps.consume_or_limit_at
          AND actor_steps.membership_page_view_at >= actor_steps.upgrade_modal_view_at
          AND actor_steps.membership_plan_click_at >= actor_steps.membership_page_view_at
          AND actor_steps.membership_payment_success_at >= actor_steps.membership_plan_click_at
          ...
          CASE
            ...
            WHEN event_name IN ('membership_plan_click', 'click_subscribe') THEN
              CASE
	                ...
	                WHEN COALESCE(properties->>'plan_id', '') = 'club_starter_monthly' THEN 'membership_plan_starter'
	                WHEN COALESCE(properties->>'plan_id', '') = 'club_go_quarterly' THEN 'membership_plan_quarter'
	                WHEN COALESCE(properties->>'plan_id', '') = 'club_half_year' THEN 'membership_plan_half_year'
	                WHEN COALESCE(properties->>'plan_id', '') = 'goo_plus_yearly' THEN 'membership_plan_year'
	                WHEN COALESCE(properties->>'plan_id', '') = 'club_annual' THEN 'membership_plan_annual'
	                ELSE 'membership_center'
	              END
	            WHEN event_name = 'membership_advisor_added_click' THEN 'membership_advisor_added'
    ...
    const paymentTruthQuery = `
      ...
      WHERE status = 'completed'
        AND ${getDateFilterForColumn('created_at', period)}
      GROUP BY plan_id
    `;

    const membershipActivationQuery = `
      WITH membership_users AS (
        SELECT
          user_id,
          CASE
            WHEN member_type IN ('trial_week', 'trial') THEN 'trial_week'
            WHEN member_type IN ('starter', 'monthly', 'club_starter') THEN 'starter'
            WHEN member_type IN ('quarter', 'club_go') THEN 'quarter'
            WHEN member_type IN ('year', 'goo_plus') THEN 'year'
            WHEN member_type IN ('half_year') THEN 'half_year'
            WHEN member_type IN ('annual') THEN 'annual'
            ELSE COALESCE(NULLIF(member_type, ''), 'none')
          END AS member_type,
          member_status,
          member_cycle_start_at,
          member_expire_at,
          COALESCE(member_cycle_start_at, member_since, created_at) AS activation_at
        FROM users
        WHERE COALESCE(NULLIF(member_type, ''), 'none') <> 'none'
          AND member_status IN ('active', 'expired', 'pro', 'lifetime')
      )
      SELECT
        member_type,
        COUNT(DISTINCT user_id) FILTER (WHERE ${membershipActivationDateFilter}) AS activated_users,
        COUNT(DISTINCT user_id) FILTER (
          WHERE member_status IN ('active', 'pro', 'lifetime')
            AND (member_cycle_start_at IS NULL OR member_cycle_start_at <= NOW())
            AND (member_expire_at IS NULL OR member_expire_at > NOW())
        ) AS current_active_users
      FROM membership_users
      GROUP BY member_type
      ORDER BY activated_users DESC, current_active_users DESC
    `;

    const membershipActivationExpr = 'COALESCE(member_cycle_start_at, member_since, created_at)';
    const membershipSuccessSummaryQuery = `
      ...
      membership_users AS (
        ...
        WHERE COALESCE(NULLIF(member_type, ''), 'none') <> 'none'
          AND member_status IN ('active', 'expired', 'pro', 'lifetime')
          AND ${getDateFilterForColumn(membershipActivationExpr, period)}
      ),
      unioned AS (
        SELECT user_id FROM payment_users
        UNION
        SELECT user_id FROM membership_users
      )
      SELECT COUNT(*)::int AS total_users FROM unioned
    `;

    const experienceQualityQuery = `
      WITH base AS (
        SELECT ${actorExpr('ae')} AS actor_id, ae.*
        FROM analytics_events ae
        WHERE ${dateFilter} AND ${segmentFilter}
      ...
      SELECT
        COUNT(*) FILTER (WHERE event_name IN ('search_result_rendered', 'search_empty')) AS search_result_pv,
        COUNT(*) FILTER (WHERE event_name = 'search_empty') AS search_empty_pv,
        COUNT(*) FILTER (WHERE COALESCE(event_family, properties->>'event_family') = 'api') AS api_request_pv,
        COUNT(*) FILTER (WHERE COALESCE(event_family, properties->>'event_family') = 'api' AND COALESCE(outcome, properties->>'outcome') IN ('failed', 'blocked')) AS api_failed_pv,
        percentile_cont(0.95) WITHIN GROUP (ORDER BY duration_ms) FILTER (WHERE COALESCE(event_family, properties->>'event_family') = 'api' AND duration_ms IS NOT NULL) AS api_p95_duration_ms,
        COUNT(DISTINCT actor_id) FILTER (WHERE COALESCE(event_family, properties->>'event_family') = 'client_error') AS client_error_uv,
        COUNT(*) FILTER (WHERE COALESCE(event_family, properties->>'event_family') = 'application' AND COALESCE(outcome, properties->>'outcome') = 'blocked') AS application_blocked_pv
      FROM base
    `;
    ...
    const [
      ...
      jobFunnelRows,
      jobBundleFunnelRows,
      corporateEnglishFunnelRows,
      corporateEnglishVideoStatsRows,
      ...
      trendRows,
      paymentTruthRows,
      membershipActivationRows,
      membershipSuccessSummaryRows,
      experienceQualityRows,
    ] = await Promise.all([
      neonHelper.query(overviewQuery),
      shouldLoadCore ? neonHelper.query(jobFunnelQuery) : [],
      shouldLoadCore ? neonHelper.query(jobBundleFunnelQuery) : [],
      shouldLoadCore ? neonHelper.query(corporateEnglishFunnelQuery) : [],
      shouldLoadCore ? neonHelper.query(corporateEnglishVideoStatsQuery) : [],
      shouldLoadMonetization ? neonHelper.query(monetizationFunnelQuery, FEATURE_KEYS) : [],
      shouldLoadCopilot ? neonHelper.query(copilotSummaryQuery) : [],
      shouldLoadCopilot ? neonHelper.query(copilotDirectionQuery) : [],
      shouldLoadCopilot ? neonHelper.query(copilotPositionTypeQuery) : [],
      shouldLoadCopilot ? neonHelper.query(copilotMatrixQuery) : [],
      shouldLoadCopilot ? neonHelper.query(copilotTrendQuery) : [],
      shouldLoadMembership ? neonHelper.query(membershipExperienceQuery, FEATURE_KEYS) : [],
      shouldLoadMembership ? neonHelper.query(resumeAssistantQuery) : [],
      neonHelper.query(trendQuery),
      neonHelper.query(paymentTruthQuery),
      neonHelper.query(membershipActivationQuery),
      neonHelper.query(membershipSuccessSummaryQuery),
      neonHelper.query(experienceQualityQuery),
    ]);
    ...
    const corporateEnglishMap = Object.fromEntries((corporateEnglishFunnelRows || []).map((row) => [row.step_id, row]));
    ...
    const membershipActivationTotalUsers = (membershipActivationRows || []).reduce((sum, row) => sum + toInt(row.activated_users), 0);
    const membershipCurrentActiveUsers = (membershipActivationRows || []).reduce((sum, row) => sum + toInt(row.current_active_users), 0);
    const membershipSuccessUnionUsers = toInt(membershipSuccessSummaryRows?.[0]?.total_users);
    const experienceQualityRow = experienceQualityRows?.[0] || {};
    ...
    const monetizationFunnel = computeStepMetrics([
      ...
      { stepId: 'upgrade_modal_view', label: '服务咨询弹窗曝光', uv: toInt(monetizationMap.upgrade_modal_view?.uv), pv: toInt(monetizationMap.upgrade_modal_view?.pv) },
      { stepId: 'membership_page_view', label: 'Club 权益页访问', uv: toInt(monetizationMap.membership_page_view?.uv), pv: toInt(monetizationMap.membership_page_view?.pv) },
      { stepId: 'membership_plan_click', label: 'Club 方案卡点击', uv: toInt(monetizationMap.membership_plan_click?.uv), pv: toInt(monetizationMap.membership_plan_click?.pv) },
      { stepId: 'membership_payment_success', label: '权益确认', uv: toInt(monetizationMap.membership_payment_success?.uv), pv: toInt(monetizationMap.membership_payment_success?.pv) },
    ]);
    ...
    const corporateEnglishModuleLabels = {
      ceo: 'CEO访谈',
      english_interview: '英语面试',
      foreign_meeting: '外企会议',
    };
    const corporateEnglishDetailUv = toInt(corporateEnglishMap.corporate_english_ceo_detail_view?.uv)
      + toInt(corporateEnglishMap.corporate_english_interview_detail_view?.uv)
      + toInt(corporateEnglishMap.corporate_english_meeting_detail_view?.uv);
    const corporateEnglishDetailPv = toInt(corporateEnglishMap.corporate_english_ceo_detail_view?.pv)
      + toInt(corporateEnglishMap.corporate_english_interview_detail_view?.pv)
      + toInt(corporateEnglishMap.corporate_english_meeting_detail_view?.pv);
    const corporateEnglishVideoCards = (corporateEnglishVideoStatsRows || []).map((row) => ({
      stepId: `corporate_english_video_${row.module_key}_${row.entity_id}`,
      label: row.video_title || row.entity_id,
      uv: toInt(row.play_uv),
      pv: toInt(row.play_pv),
      moduleKey: row.module_key,
      moduleLabel: corporateEnglishModuleLabels[row.module_key] || row.module_key || '外企英语',
      sourceLabel: row.source_label || '',
      entityId: row.entity_id,
      detailUv: toInt(row.detail_uv),
      detailPv: toInt(row.detail_pv),
      metricKind: 'video_play',
    }));

    const corporateEnglishFunnel = [
      ...computeStepMetrics([
      { stepId: 'corporate_english_visit', label: '外企英语访问', uv: toInt(corporateEnglishMap.corporate_english_visit?.uv), pv: toInt(corporateEnglishMap.corporate_english_visit?.pv) },
      { stepId: 'corporate_english_detail_view', label: '详情页访问汇总', uv: corporateEnglishDetailUv, pv: corporateEnglishDetailPv },
      { stepId: 'corporate_english_video_play', label: '全站视频播放/打开', uv: toInt(corporateEnglishMap.corporate_english_video_play?.uv), pv: toInt(corporateEnglishMap.corporate_english_video_play?.pv) },
      { stepId: 'corporate_english_clip_play', label: '全站跟读音频播放', uv: toInt(corporateEnglishMap.corporate_english_clip_play?.uv), pv: toInt(corporateEnglishMap.corporate_english_clip_play?.pv) },
      ]),
      ...corporateEnglishVideoCards,
    ];
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        experienceQuality: {
          searchResultPv: toInt(experienceQualityRow.search_result_pv),
          searchEmptyPv: toInt(experienceQualityRow.search_empty_pv),
          apiRequestPv: toInt(experienceQualityRow.api_request_pv),
          apiFailedPv: toInt(experienceQualityRow.api_failed_pv),
          apiP95DurationMs: toInt(experienceQualityRow.api_p95_duration_ms),
          clientErrorUv: toInt(experienceQualityRow.client_error_uv),
          applicationBlockedPv: toInt(experienceQualityRow.application_blocked_pv),
        },
        coreFunnels: {
          job: jobFunnel,
          jobBundle: jobBundleFunnel,
          corporateEnglish: corporateEnglishFunnel,
        ...
        membershipActivation: {
          activatedUsers: membershipActivationTotalUsers,
          currentActiveUsers: membershipCurrentActiveUsers,
          byType: (membershipActivationRows || []).map((row) => ({
            memberType: row.member_type,
            activatedUsers: toInt(row.activated_users),
            currentActiveUsers: toInt(row.current_active_users),
          })),
        },
      }
    });
