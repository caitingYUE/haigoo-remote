  ...
  try {
    ...
    const jobFunnelQuery = `
      ...
      actor_steps AS (
        SELECT
          ...
          MIN(created_at) FILTER (
            WHERE event_name = 'page_view' AND COALESCE(page_key, properties->>'page_key') = 'home'
          ) AS landing_home_visit_at,
          MIN(created_at) FILTER (
            WHERE event_name = 'copilot_hero_submit'
               OR event_name = 'click_job_card'
               OR (event_name = 'page_view' AND COALESCE(page_key, properties->>'page_key') = 'jobs')
          ) AS jobs_intent_at,
          MIN(created_at) FILTER (
            WHERE event_name = 'view_job_detail'
               OR (event_name = 'page_view' AND COALESCE(page_key, properties->>'page_key') = 'job_detail')
          ) AS job_detail_view_at,
          MIN(created_at) FILTER (WHERE event_name = 'click_apply_init') AS apply_init_click_at,
          MIN(created_at) FILTER (WHERE event_name = 'click_apply') AS apply_path_selected_at,
          MIN(created_at) FILTER (
            WHERE event_name IN ('click_apply_external', 'email_apply_success', 'referral_submit_success')
      ...
      step_pv AS (
        ...
        WHERE event_name = 'click_apply_init'
        UNION ALL
        SELECT 'apply_path_selected' AS step_id, COUNT(*) AS pv FROM base
        WHERE event_name = 'click_apply'
        UNION ALL
    ...
    const monetizationFunnelQuery = `
      ...
      step_pv AS (
        ...
        FROM base
        WHERE event_name = 'feature_exposure' AND COALESCE(feature_key, properties->>'feature_key') IN (${FEATURE_KEYS.map((_, index) => `$${index + 1}`).join(', ')})
        UNION ALL
        SELECT 'free_feature_click' AS step_id, COUNT(*) AS pv
        FROM base
        WHERE event_name = 'feature_click' AND COALESCE(feature_key, properties->>'feature_key') IN (${FEATURE_KEYS.map((_, index) => `$${index + 1}`).join(', ')})
        UNION ALL
        SELECT 'consume_or_limit' AS step_id, COUNT(*) AS pv
        FROM base
        WHERE event_name IN ('feature_consume', 'feature_limit_reached') AND COALESCE(feature_key, properties->>'feature_key') IN (${FEATURE_KEYS.map((_, index) => `$${index + 1}`).join(', ')})
        UNION ALL
    ...
    return res.status(200).json({
      ...
      data: {
        overview: {
          ...
          paymentSuccessUv: toInt(overviewRow.payment_success_uv),
        ...
        membershipExperience: {
          summary: {
            ...
            paymentCompletedUsers: paymentTruthTotalUsers,
          },
