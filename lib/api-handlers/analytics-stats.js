            ...
            WITH categorized_events AS (
                SELECT 
                    ...
                    CASE 
                        ...
                        -- Action: Click Apply
                        WHEN event_name = 'click_apply' OR event_name = 'click_apply_init' THEN 'step_4_action'
        ...
        const pageQuery = `
            WITH page_visits AS (
                SELECT 
                    anonymous_id,
                    CASE 
                        WHEN event_name = 'click_generate_copilot_plan' THEN 'Copilot-生成方案'
                        WHEN url LIKE '/copilot%' THEN 'Copilot主页'
                        WHEN url = '/' OR url = '' THEN '首页'
                        WHEN url LIKE '/jobs%' THEN '岗位列表/详情'
                        WHEN url LIKE '/companies%' THEN '企业库'
                        WHEN url LIKE '/job-bundles%' OR event_name = 'view_job_bundle' THEN '岗位合集'
                    ...
                    END as page_name
                FROM analytics_events
                WHERE ${dateFilter} AND (event_name = 'page_view' OR event_name LIKE 'view_%' OR event_name = 'click_generate_copilot_plan')
            )
        ...
        return res.status(200).json({
            ...
            data: {
                ...
                metrics: {
                    ...
                    applyConversion: jobViewStats.unique_viewers ? (((keyMetrics['click_apply_init'] || 0) + (keyMetrics['click_apply'] || 0)) / jobViewStats.unique_viewers) : 0, // Apply clicks / Job Viewers
