...
const TYPE_TO_COLUMN = {
    ...
    'website-apply': '__profile_website_apply__',
...
function getWebsiteApplyState(user) {
    ...
    const unlockedJobIds = Array.isArray(websiteApply.unlockedJobIds)
        ? websiteApply.unlockedJobIds.map((item) => String(item)).filter(Boolean)
        : [];

    return {
        usage: Number(websiteApply.count) || 0,
        unlockedJobIds,
    };
}
