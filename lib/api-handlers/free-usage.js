 ...
 * 管理免费用户功能使用额度（累计终身，不重置）:
 *   - company-info: 企业认证信息查看 (3次，共享)
 *   - email-apply:  邮箱直申 (3次，共享)
 *   - referral:     帮我内推 (用户级可配置，默认3次，共享使用记录)
 *   - website-apply: 外部申请跳转 (用户级可配置，默认20次)
 *
...
function getWebsiteApplyState(user) {
    const freeUsage = user?.profile?.preferences?.freeUsage || {};
    const websiteApply = freeUsage.websiteApply || {};
    const profileUnlockedJobIds = normalizeUnlockedJobIds(websiteApply.unlockedJobIds);
    const columnUnlockedJobIds = normalizeUnlockedJobIds(user?.[WEBSITE_APPLY_JOB_IDS_COLUMN]);
    const unlockedJobIds = Array.from(new Set([...columnUnlockedJobIds, ...profileUnlockedJobIds]));
    const usage = Math.max(
        Number(user?.[WEBSITE_APPLY_COUNT_COLUMN]) || 0,
        Number(websiteApply.count) || 0,
    ...
    try {
        await neonHelper.query(
            `UPDATE users
             SET ${WEBSITE_APPLY_COUNT_COLUMN} = $1,
                 ...
                 profile = jsonb_set(
                     ...
                     '{preferences,freeUsage,websiteApply}',
                     $3::jsonb,
                     true
                 ),
                 updated_at = NOW()
            ...
            [
                ...
                JSON.stringify({ count: usage, unlockedJobIds }),
                userId
            ]
        );
    } catch (error) {
        const message = String(error?.message || error || '');
        if (!/free_website_apply_count|free_website_apply_job_ids/i.test(message)) {
            throw error;
        }
