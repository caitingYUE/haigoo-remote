...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET') {
        ...
        if (type === 'website-apply') {
            ...
            return res.status(200).json({
                success: true,
                usage: state.usage,
                limit: WEBSITE_APPLY_FREE_LIMIT,
                remaining: Math.max(0, WEBSITE_APPLY_FREE_LIMIT - state.usage),
                isMember: false,
                unlocked_job_ids: state.unlockedJobIds
            });
        }
    ...
    if (req.method === 'POST') {
        ...
        if (type === 'website-apply') {
            ...
            if (state.usage >= WEBSITE_APPLY_FREE_LIMIT) {
                await trackFreeUsageEvent({ eventName: 'feature_limit_reached', type, user, userId, body, entityId: jobId });
                return res.status(403).json({
                    success: false,
                    error: 'Free usage limit reached',
            ...
            try {
                ...
                return res.status(200).json({
                    ...
                    usage: nextState.usage,
                    limit: WEBSITE_APPLY_FREE_LIMIT,
                    remaining: Math.max(0, WEBSITE_APPLY_FREE_LIMIT - nextState.usage),
                    isMember: false,
                    unlocked_job_ids: nextState.unlockedJobIds
                });
            } catch (e) {
