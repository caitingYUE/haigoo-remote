...
async function writeRawToUpstashREST(items) {
  const filtered = filterRecent(removeDuplicatesRaw(items), 30)  // ⚠️ 改为30天，与处理后数据保持一致
...
export async function readAllRawItems() {
  if (UPSTASH_REST_CONFIGURED) {
    try { return await readRawFromUpstashREST() } catch (e) { console.warn('[raw-rss] Upstash read failed', e) }
  }
  if (REDIS_CONFIGURED) {
    try { return await readRawFromRedis() } catch (e) { console.warn('[raw-rss] Redis read failed', e) }
  }
  if (kv) {
    try { return await readRawFromKV() } catch (e) { console.warn('[raw-rss] KV read failed', e) }
  }
  return readRawFromMemory()
}

export async function saveRawItems(items, mode = 'append') {
  const normalized = items.map(normalizeItem)
  let current = []
  
  if (mode === 'append') {
    current = await readAllRawItems()
  }
  
  const toWrite = mode === 'append' ? [...current, ...normalized] : [...normalized]
  
  if (UPSTASH_REST_CONFIGURED) {
    try { return await writeRawToUpstashREST(toWrite) } catch (e) { console.warn('[raw-rss] Upstash write failed', e) }
  }
  if (REDIS_CONFIGURED) {
    try { return await writeRawToRedis(toWrite) } catch (e) { console.warn('[raw-rss] Redis write failed', e) }
  }
  if (kv) {
    try { return await writeRawToKV(toWrite) } catch (e) { console.warn('[raw-rss] KV write failed', e) }
  }
  return writeRawToMemory(toWrite)
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (UPSTASH_REST_CONFIGURED) {
        try {
          written = await writeRawToUpstashREST(toWrite)
          provider = 'upstash-rest'
        } catch (e) {
          console.warn('[raw-rss] POST write Upstash failed, fallback:', e?.message || e)
          // Fallback to Redis
          if (REDIS_CONFIGURED) {
            try {
              written = await writeRawToRedis(toWrite)
              provider = 'redis'
            } catch (er) {
              console.warn('[raw-rss] POST write Redis failed:', er?.message || er)
              // Fallback to KV
              if (kv) {
                try {
                  written = await writeRawToKV(toWrite)
                  provider = 'vercel-kv'
                } catch (err2) {
                  console.warn('[raw-rss] POST write KV failed:', err2?.message || err2)
                  written = writeRawToMemory(toWrite)
                  provider = 'memory'
                }
              } else {
                written = writeRawToMemory(toWrite)
        ...
        try {
          ...
          console.warn('[raw-rss] POST write Redis failed:', e?.message || e)
          if (kv) {
            try {
              written = await writeRawToKV(toWrite)
              provider = 'vercel-kv'
            } catch (er) {
              console.warn('[raw-rss] POST write KV failed:', er?.message || er)
              written = writeRawToMemory(toWrite)
              provider = 'memory'
            }
          } else {
        ...
        try {
          ...
          console.warn('[raw-rss] POST write KV failed:', e?.message || e)
          written = writeRawToMemory(toWrite)
          provider = 'memory'
        }
      } else {
