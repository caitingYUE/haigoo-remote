...
function removeDuplicatesRaw(items) {
  ...
  for (const item of items) {
    ...
    const newScore = (item.description?.length || 0) + (item.rawContent?.length || 0)
    const existingScore = (existing.description?.length || 0) + (existing.rawContent?.length || 0)
    const newFetchedAt = new Date(item.fetchedAt || item.pubDate || 0).getTime()
    const existingFetchedAt = new Date(existing.fetchedAt || existing.pubDate || 0).getTime()

    if (newScore > existingScore || (newScore === existingScore && newFetchedAt > existingFetchedAt)) {
      bestByKey.set(key, item)
    }
  }
...
async function readRawFromNeon(queryParams = {}) {
  ...
  try {
    ...
    // 添加最近7天的过滤 (Unless ignored)
    if (!queryParams.ignoreTimeLimit) {
    ...
    if (filtered.length > 0) {
      ...
      const query = `
        ...
        ON CONFLICT (raw_id) DO UPDATE SET
          ...
          status = CASE
            WHEN EXCLUDED.status = 'raw' AND ${RAW_RSS_TABLE}.status IN ('processed', 'error')
              THEN ${RAW_RSS_TABLE}.status
            ELSE EXCLUDED.status
          END,
          processing_error = CASE
            WHEN EXCLUDED.status = 'raw' AND ${RAW_RSS_TABLE}.status = 'error'
              THEN ${RAW_RSS_TABLE}.processing_error
