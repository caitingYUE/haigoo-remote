...
async function readRawFromNeon(queryParams = {}) {
  ...
  try {
    // Auto-migration: Ensure company_name column exists
    try {
      await neonHelper.query(`
        ALTER TABLE ${RAW_RSS_TABLE} 
        ADD COLUMN IF NOT EXISTS company_name VARCHAR(500)
      `)
    } catch (e) {
      console.warn('[raw-rss] Auto-migration for company_name failed (might already exist):', e.message)
    }
    ...
    return result.map(row => ({
      ...
      url: row.url,
      title: row.title,
      company: row.company_name, // Extract company name
    ...
    if (filtered.length > 0) {
      ...
      const query = `
        INSERT INTO ${RAW_RSS_TABLE} (
          raw_id, source, category, url, title, company_name, description, link, 
...
function normalizeItem(item) {
  const safe = {
    ...
    company: truncateString(sanitizeHtml(String(item.company || item.company_name || '')), 500),
