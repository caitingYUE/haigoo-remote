...
function companyDirectorySearchSql(paramRef, { exact = false } = {}) {
  const compare = (expression) => exact
    ? `LOWER(BTRIM(COALESCE(${expression}, ''))) = LOWER(BTRIM(${paramRef}))`
    : `COALESCE(${expression}, '') ILIKE ${paramRef}`
  return `(
    ${compare('tc.name')}
    OR ${compare("tc.translations->>'name'")}
    OR ${compare("tc.translations->>'company'")}
    OR ${compare("tc.translations->>'companyName'")}
    OR EXISTS (
      SELECT 1
        FROM company_job_history search_job
        LEFT JOIN jobs search_job_record ON search_job_record.job_id = search_job.source_job_id
       WHERE search_job.company_id = tc.company_id
         AND search_job.closed_at IS NULL
         AND search_job.is_public_opportunity IS TRUE
         AND (
           ${compare('search_job.title')}
           OR ${compare("search_job_record.translations->>'title'")}
           OR ${compare("search_job_record.translations->>'company'")}
         )
    )
  )`
}

async function readMiniCompanies({ viewer, search = '', industry = '', page = 1, pageSize = 20 }) {
  ...
  const filters = ["tc.status = 'active'", 'COALESCE(hiring.open_job_count, 0) > 0']
  const params = []
  // first_seen_at is the stable "job newly listed" signal; last_seen_at changes on every crawl.
  const hiringJoin = `LEFT JOIN LATERAL (
    SELECT COUNT(*)::int AS open_job_count,
           MAX(h.first_seen_at) AS public_opportunity_updated_at,
           ARRAY(
             SELECT DISTINCT COALESCE(NULLIF(BTRIM(job_record.translations->>'title'), ''), open_job.title)
               FROM company_job_history open_job
               LEFT JOIN jobs job_record ON job_record.job_id = open_job.source_job_id
              WHERE open_job.company_id = tc.company_id
                AND open_job.closed_at IS NULL
                AND open_job.is_public_opportunity IS TRUE
                AND COALESCE(NULLIF(BTRIM(job_record.translations->>'title'), ''), NULLIF(BTRIM(open_job.title), '')) IS NOT NULL
              ORDER BY 1
              LIMIT 3
  ...
  if (normalizedSearch) {
    params.push(hasFullAccess ? `%${normalizedSearch}%` : normalizedSearch)
    filters.push(companyDirectorySearchSql(`$${params.length}`, { exact: !hasFullAccess }))
  }
  if (hasFullAccess && normalizedIndustry) {
    params.push(normalizedIndustry)
    filters.push(`tc.industry = $${params.length}`)
  }
  const where = filters.join(' AND ')
  const offset = (effectivePage - 1) * effectivePageSize
  const [rows, countRows, industryRows] = await Promise.all([
    neonHelper.query(
      `SELECT tc.company_id, tc.name, tc.description, tc.industry, tc.tags, tc.specialties, tc.address, tc.logo,
              tc.employee_count, tc.founded_year, tc.company_rating, tc.rating_source,
              ...
              hiring.public_job_titles
         FROM trusted_companies tc
         ${hiringJoin}
        WHERE ${where}
        ORDER BY hiring.public_opportunity_updated_at DESC NULLS LAST, tc.name ASC
        LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, effectivePageSize, offset]
    ),
    neonHelper.query(`SELECT COUNT(*)::int AS count FROM trusted_companies tc ${hiringJoin} WHERE ${where}`, params),
    hasFullAccess
      ? neonHelper.query(
          `SELECT industry, COUNT(*)::int AS count
            ...
            WHERE status = 'active' AND COALESCE(industry, '') <> ''
              AND EXISTS (
                SELECT 1 FROM company_job_history h
                 WHERE h.company_id = trusted_companies.company_id
                   AND h.closed_at IS NULL
                   AND h.is_public_opportunity IS TRUE
              )
...
async function handleMiniCompany(req, res) {
  ...
  const viewer = await resolveMiniViewer(String(req.query?.openid || '').trim())
  if (!viewer.hasCompanyDirectoryAccess) {
    const userId = String(viewer.user?.userId || '')
    const accessSearch = String(req.query?.search || '').trim().slice(0, 80)
    const freeAccess = await neonHelper.query(
      `SELECT 1
        WHERE EXISTS (
          SELECT 1
            FROM (
                ...
                JOIN LATERAL (
                  SELECT COUNT(*)::int AS open_job_count,
                   ...
                   WHERE h.company_id = tc.company_id
                     AND h.closed_at IS NULL
                     AND h.is_public_opportunity IS TRUE
                ) hiring ON hiring.open_job_count > 0
               WHERE tc.status = 'active'
               ORDER BY hiring.public_opportunity_updated_at DESC NULLS LAST, tc.name ASC
               LIMIT ${FREE_COMPANY_DIRECTORY_LIMIT}
            ) free_directory
           WHERE free_directory.company_id = $1
        ) OR ($2::text <> '' AND EXISTS (
          SELECT 1
            FROM career_watch_feed_snapshots snapshots,
                 jsonb_array_elements(${fixedMatchRecommendationsSql('snapshots')}) WITH ORDINALITY recommendation(item, ordinality)
           WHERE snapshots.user_id = $2
             AND ordinality <= 5
             AND item->>'companyId' = $1
          ...
          SELECT 1
           ...
           WHERE tc.company_id = $1
             ...
             AND EXISTS (
               SELECT 1 FROM company_job_history open_job
                WHERE open_job.company_id = tc.company_id
                  AND open_job.closed_at IS NULL
                  AND open_job.is_public_opportunity IS TRUE
             )
             AND ${companyDirectorySearchSql('$3', { exact: true })}
        ))
        LIMIT 1`,
      [companyId, userId, accessSearch]
    )
    if (!freeAccess?.[0]) {
      return res.status(403).json({
        success: false,
        code: 'COMPANY_NOT_IN_FREE_DIRECTORY',
        error: '该企业不在当前开放名单或你的固定匹配中'
      })
    }
  }
  const rows = await neonHelper.query(
  ...
  const directoryContacts = viewer.hasCompanyDirectoryAccess ? { contacts } : {}
  const company = {
    ...mapMiniCompany(row),
    ...
    ...(directoryContacts.contacts ? {
      ...(viewer.capabilities.canAccessCompanyContacts ? { contacts } : {})
    } : {})
  }
  return res.status(200).json({
    success: true,
...
async function handleCareerWatchState(req, res) {
  if (!ensureDatabase(res)) return
  const user = await requireBoundUser(String(req.query?.openid || '').trim(), res)
  if (!user) return
  const capabilities = deriveMembershipCapabilities(user)
  const wechatSubscription = miniWechatSubscriptionConfig()
  ...
  return res.status(200).json({
    ...
    ...access,
    entitlements: {
      ...careerWatchEntitlements(isMatchMember),
      wechatTemplateId: wechatSubscription.templateId,
      wechatSubscriptionAvailable: wechatSubscription.available
    },
    ...feed
...
export default async function miniGatewayHandler(req, res) {
  ...
  if (![
    ...
    'career_watch_state', 'career_watch_options', 'career_watch_save', 'career_watch_import', 'career_watch_notifications',
    ...
    'virtual_payment_create', 'virtual_payment_status', 'virtual_payment_list', 'virtual_payment_update', 'sync'
  ].includes(action)) {
  ...
  try {
    ...
    const safeErrorCodes = new Set([
      ...
      'MEMBERSHIP_REQUIRED', 'MEMBER_CONTACTS_REQUIRED', 'SUBSCRIBE_AUTH_REQUIRED',
