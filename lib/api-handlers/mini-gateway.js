  ...
  SELECT tc.company_id, tc.name, tc.translations, tc.description, tc.industry, tc.tags, tc.specialties,
    ...
    JOIN LATERAL (
      SELECT COUNT(*)::int AS open_job_count,
             MAX(CASE WHEN h.first_seen_at <= NOW() THEN h.first_seen_at END) AS public_opportunity_updated_at,
             MAX(CASE WHEN h.first_seen_at <= NOW() THEN h.first_seen_at END) + INTERVAL '72 hours' AS new_jobs_until,
             ARRAY(
               SELECT DISTINCT COALESCE(NULLIF(BTRIM(j.translations->>'title'), ''), NULLIF(BTRIM(j.title), ''), NULLIF(BTRIM(h.title), ''))
                 FROM company_job_history h
                 JOIN jobs j ON j.job_id = h.source_job_id
                WHERE h.company_id = tc.company_id AND h.closed_at IS NULL AND h.is_public_opportunity IS TRUE
             ...
             ARRAY(
               SELECT value FROM (
                 SELECT DISTINCT NULLIF(BTRIM(j.title), '') AS value
                   FROM company_job_history h JOIN jobs j ON j.job_id = h.source_job_id
                  WHERE h.company_id = tc.company_id AND h.closed_at IS NULL AND h.is_public_opportunity IS TRUE
                    AND j.status = 'active' AND j.is_approved IS TRUE AND COALESCE(j.member_only, FALSE) IS FALSE
                    AND (NULLIF(BTRIM(j.url), '') ~* '^https?://[^[:space:]]+$' OR NULLIF(BTRIM(tc.hiring_email), '') ~* '^[^[:space:]@]+@[^[:space:]@]+\\.[^[:space:]@]+$')
                 UNION
                 SELECT DISTINCT NULLIF(BTRIM(h.title), '') AS value
                   FROM company_job_history h JOIN jobs j ON j.job_id = h.source_job_id
                  WHERE h.company_id = tc.company_id AND h.closed_at IS NULL AND h.is_public_opportunity IS TRUE
                    AND j.status = 'active' AND j.is_approved IS TRUE AND COALESCE(j.member_only, FALSE) IS FALSE
                    AND (NULLIF(BTRIM(j.url), '') ~* '^https?://[^[:space:]]+$' OR NULLIF(BTRIM(tc.hiring_email), '') ~* '^[^[:space:]@]+@[^[:space:]@]+\\.[^[:space:]@]+$')
                 UNION
                 SELECT DISTINCT NULLIF(BTRIM(j.translations->>'title'), '') AS value
                   FROM company_job_history h JOIN jobs j ON j.job_id = h.source_job_id
                  WHERE h.company_id = tc.company_id AND h.closed_at IS NULL AND h.is_public_opportunity IS TRUE
                    AND j.status = 'active' AND j.is_approved IS TRUE AND COALESCE(j.member_only, FALSE) IS FALSE
                    AND (NULLIF(BTRIM(j.url), '') ~* '^https?://[^[:space:]]+$' OR NULLIF(BTRIM(tc.hiring_email), '') ~* '^[^[:space:]@]+@[^[:space:]@]+\\.[^[:space:]@]+$')
                 ...
                 SELECT DISTINCT NULLIF(BTRIM(translated.value), '') AS value
                   ...
                   CROSS JOIN LATERAL jsonb_each_text(
                     CASE WHEN jsonb_typeof(j.translations->'title') = 'object' THEN j.translations->'title' ELSE '{}'::jsonb END
                   ) translated
                  WHERE h.company_id = tc.company_id AND h.closed_at IS NULL AND h.is_public_opportunity IS TRUE
                    AND j.status = 'active' AND j.is_approved IS TRUE AND COALESCE(j.member_only, FALSE) IS FALSE
                    AND (NULLIF(BTRIM(j.url), '') ~* '^https?://[^[:space:]]+$' OR NULLIF(BTRIM(tc.hiring_email), '') ~* '^[^[:space:]@]+@[^[:space:]@]+\\.[^[:space:]@]+$')
             ...
             ARRAY(
               SELECT category_rollup.category FROM (
                 SELECT NULLIF(BTRIM(h.category), '') AS category, MAX(h.first_seen_at) AS latest_seen
                   FROM company_job_history h JOIN jobs j ON j.job_id = h.source_job_id
                  WHERE h.company_id = tc.company_id AND h.closed_at IS NULL AND h.is_public_opportunity IS TRUE
                    AND j.status = 'active' AND j.is_approved IS TRUE AND COALESCE(j.member_only, FALSE) IS FALSE
                    AND (NULLIF(BTRIM(j.url), '') ~* '^https?://[^[:space:]]+$' OR NULLIF(BTRIM(tc.hiring_email), '') ~* '^[^[:space:]@]+@[^[:space:]@]+\\.[^[:space:]@]+$')
                  GROUP BY NULLIF(BTRIM(h.category), '') ORDER BY latest_seen DESC NULLS LAST, category ASC LIMIT 12
               ) category_rollup
             ) AS open_role_categories,
             ARRAY(
               SELECT DISTINCT BTRIM(family)
                 FROM company_job_history role_history
                 JOIN jobs role_job ON role_job.job_id = role_history.source_job_id
                 CROSS JOIN LATERAL jsonb_array_elements_text(CASE WHEN jsonb_typeof(role_history.role_families) = 'array' THEN role_history.role_families ELSE '[]'::jsonb END) family
                WHERE role_history.company_id = tc.company_id AND role_history.closed_at IS NULL AND role_history.is_public_opportunity IS TRUE
                  AND role_job.status = 'active' AND role_job.is_approved IS TRUE AND COALESCE(role_job.member_only, FALSE) IS FALSE
                  AND (NULLIF(BTRIM(role_job.url), '') ~* '^https?://[^[:space:]]+$' OR NULLIF(BTRIM(tc.hiring_email), '') ~* '^[^[:space:]@]+@[^[:space:]@]+\\.[^[:space:]@]+$')
             ) AS role_families
        FROM company_job_history h
        JOIN jobs j ON j.job_id = h.source_job_id
       WHERE h.company_id = tc.company_id AND h.closed_at IS NULL AND h.is_public_opportunity IS TRUE
...
async function readMiniCompanies({ viewer, search = '', industry = '', page = 1, pageSize = 20, sortBy = 'latest' }) {
  ...
  const snapshot = await companyDirectoryCache('public-directory-v2', () => neonHelper.query(companyDirectorySnapshotSql))
