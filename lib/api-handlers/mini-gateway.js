...
async function readMiniCompanies({ viewer, search = '', industry = '', page = 1, pageSize = 20 }) {
  ...
  if (!hasFullAccess && !userId) {
    return {
      companies: [], total: 0, page: 1, pageSize: 5, hasMore: false,
      access: { scope: 'match_required', fullDirectory: false, previewLimit: 0, searchEnabled: false },
      industries: []
    }
  }
  ...
  const effectivePageSize = hasFullAccess
    ...
    : 5
  const filters = ["tc.status = 'active'"]
  const params = hasFullAccess ? [] : [userId]
  const fixedJoin = hasFullAccess ? '' : `JOIN LATERAL (
    SELECT item->>'companyId' AS company_id, ordinality
     ...
     WHERE snapshots.user_id = $1
       AND ordinality <= 5
  ) fixed ON fixed.company_id = tc.company_id`
  const hiringJoin = `LEFT JOIN LATERAL (
    SELECT COUNT(*)::int AS open_job_count,
           MAX(COALESCE(h.source_published_at, h.last_seen_at)) AS public_opportunity_updated_at
      FROM company_job_history h
     WHERE h.company_id = tc.company_id
       AND h.closed_at IS NULL
       AND h.is_public_opportunity IS TRUE
  ) hiring ON TRUE`
  if (hasFullAccess) filters.push('COALESCE(hiring.open_job_count, 0) > 0')
  if (hasFullAccess && normalizedSearch) {
    params.push(`%${normalizedSearch}%`)
    filters.push(`(tc.name ILIKE $${params.length} OR tc.description ILIKE $${params.length} OR tc.industry ILIKE $${params.length})`)
  }
  ...
  const [rows, countRows, industryRows] = await Promise.all([
    neonHelper.query(
      `SELECT tc.company_id, tc.name, tc.description, tc.industry, tc.tags, tc.specialties, tc.address,
         ...
         ${hiringJoin}
        WHERE ${where}
        ORDER BY ${hasFullAccess
          ? 'hiring.public_opportunity_updated_at DESC NULLS LAST, tc.name ASC'
          : 'fixed.ordinality ASC'}
        LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, effectivePageSize, offset]
    ),
    neonHelper.query(`SELECT COUNT(*)::int AS count FROM trusted_companies tc ${fixedJoin} ${hiringJoin} WHERE ${where}`, params),
    hasFullAccess
      ? neonHelper.query(
  ...
  return {
    ...
    access: {
      scope: hasFullAccess ? 'member_all' : total ? 'free_fixed' : 'match_required',
      fullDirectory: hasFullAccess,
      previewLimit: hasFullAccess ? null : total,
...
async function handleMiniCompany(req, res) {
  ...
  const userId = String(viewer.user?.userId || '')
  if (!viewer.bound) {
    return res.status(401).json({ success: false, code: 'LOGIN_REQUIRED', error: '请先登录并连接 Haigoo 账号' })
  }
  if (!viewer.hasCompanyDirectoryAccess) {
    const fixedAccess = await neonHelper.query(
      `SELECT 1
         FROM career_watch_feed_snapshots snapshots,
              jsonb_array_elements(${fixedMatchRecommendationsSql('snapshots')}) WITH ORDINALITY recommendation(item, ordinality)
        WHERE snapshots.user_id = $1
          AND ordinality <= 5
          AND item->>'companyId' = $2
        LIMIT 1`,
      [userId, companyId]
    )
    if (!fixedAccess?.[0]) {
      return res.status(403).json({
        success: false,
        code: 'COMPANY_NOT_IN_FREE_MATCH',
        error: '该企业不在你的匹配结果中'
      })
    }
  }
      ...
      WHERE tc.status = 'active' AND tc.company_id = $1
        AND ($2::boolean = TRUE OR EXISTS (
          SELECT 1
            FROM career_watch_feed_snapshots snapshots,
                 jsonb_array_elements(${fixedMatchRecommendationsSql('snapshots')}) WITH ORDINALITY recommendation(item, ordinality)
           WHERE snapshots.user_id = $3
             AND ordinality <= 5
             AND item->>'companyId' = tc.company_id
        ))
      LIMIT 1`,
    [companyId, viewer.hasCompanyDirectoryAccess, userId]
  )
  const row = rows?.[0]
  ...
  const company = {
    ...
    jobs: [],
    ...(viewer.hasCompanyDirectoryAccess ? { contacts } : {})
  }
  return res.status(200).json({
    success: true,
    company,
    access: {
      ...
      contacts: viewer.hasCompanyDirectoryAccess
    }
  })
}
...
export default async function miniGatewayHandler(req, res) {
  ...
  const tracedResponse = {
    setHeader(key, value) { rawResponse.setHeader(key, value); return tracedResponse },
    status(code) { rawResponse.status(code); return tracedResponse },
    json(payload) {
      return rawResponse.json(payload && typeof payload === 'object' && !Array.isArray(payload)
        ? { ...payload, requestId }
        : payload)
    }
  }
  ...
  try {
    ...
    const safeErrorCodes = new Set([
      'LOGIN_REQUIRED', 'FREE_MATCH_USED', 'COMPANY_NOT_IN_FREE_MATCH',
