    ...
    neonHelper.query(
      `SELECT tc.company_id, tc.name, tc.description, tc.industry, tc.tags, tc.specialties, tc.address,
        ...
        ORDER BY ${hasFullAccess
          ? "CASE WHEN tc.company_rating ~ '^[0-9]+([.][0-9]+)?$' THEN tc.company_rating::numeric ELSE 0 END DESC, tc.updated_at DESC NULLS LAST, tc.name ASC"
          : 'fixed.ordinality ASC'}
        LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, effectivePageSize, offset]
    ),
    neonHelper.query(`SELECT COUNT(*)::int AS count FROM trusted_companies tc ${fixedJoin} WHERE ${where}`, params),
    hasFullAccess
      ? neonHelper.query(
