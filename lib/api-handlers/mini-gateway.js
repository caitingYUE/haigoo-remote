...
function normalizeJobSnapshot(value, expectedId) {
  ...
  return {
    ...
    canRefer: value.canRefer === true,
    effectiveReferralContactCount: Math.max(0, Number(value.effectiveReferralContactCount || 0))
  }
}
...
async function handleBind(req, res) {
  ...
  if (userIdentityRows?.[0]?.openid && userIdentityRows[0].openid !== openid) {
    return res.status(409).json({ success: false, error: '该网站账号已绑定其他微信，请先在原微信中解绑' })
  }
  ...
  try {
    ...
    if (error?.code === '23505') {
      return res.status(409).json({ success: false, error: '微信或网站账号已被绑定，请刷新后重试' })
    }
    throw error
  }
...
async function handleRegister(req, res) {
  ...
  try {
    ...
    if (error?.code === '23505') {
      return res.status(409).json({
        success: false,
        error: '账号已创建，但微信或网站账号已绑定其他身份，请联系客服处理'
      })
    }
    throw error
  }
...
async function handleApplicationUsage(req, res) {
  ...
  const [website, email, referral] = await Promise.all([
    readUsage(user, 'website-apply'),
    readUsage(user, 'email-apply'),
    readUsage(user, 'referral')
  ])
  const failed = [website, email, referral].find((result) => result.statusCode < 200 || result.statusCode >= 300)
  if (failed) {
  ...
  return res.status(200).json({
    success: true,
    isMember: Boolean(website.payload?.isMember || email.payload?.isMember || referral.payload?.isMember),
...
export default async function miniGatewayHandler(req, res) {
  ...
  if (![
    ...
    'application', 'application_usage', 'application_status', 'sync'
  ].includes(action)) {
...
export { gatewaySecrets, hasGatewaySignature, normalizeEventId, normalizeJobSnapshot, requestSignature, stableJson }
