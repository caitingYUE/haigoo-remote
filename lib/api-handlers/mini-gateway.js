import { getContinuousMembershipExpireAt } from '../services/membership-redemption-code-service.js'
import { readJobFavorites, saveJobFavorite } from '../services/job-favorites-service.js'
import { rankDirectoryCompanies } from '../services/mini-company-search-service.js'
import {
  applyPreviewCompanyCatalogSnapshot,
  readPublicCompanyCatalogPage
} from '../services/mini-company-catalog-sync-service.js'
import { createPublicSnapshotCache } from '../services/public-snapshot-cache.js'
...
import { VIRTUAL_PAYMENT_PRODUCTS, parseProductMap, wechatVirtualPaymentService } from '../services/wechat-virtual-payment-service.js'
import { claimMiniMemberService, getMiniMemberServices } from '../services/mini-member-service.js'
...
const PRIVACY_VERSION = '2026-09-06'
// Keep installed clients working during rollout; consent rows retain the version actually presented.
const SUPPORTED_PRIVACY_VERSIONS = new Set(['2026-07-29', PRIVACY_VERSION])
...
function scopedGatewaySecrets(action) {
  if (action === 'company_catalog_snapshot') {
    return process.env.MINI_GATEWAY_READONLY_SECRET ? [process.env.MINI_GATEWAY_READONLY_SECRET] : []
  }
  if (action === 'company_catalog_import') {
    return process.env.MINI_GATEWAY_SHARED_SECRET ? [process.env.MINI_GATEWAY_SHARED_SECRET] : []
  }
  return [
    ...gatewaySecrets(),
    ...(new Set(['sync', 'company_catalog_snapshot']).has(action) && process.env.MINI_GATEWAY_READONLY_SECRET
...
async function userSummary(user) {
  ...
  return {
    ...
    memberExpireAt: await getContinuousMembershipExpireAt(user)
  }
}
...
async function handleSession(req, res) {
  ...
  if (user && user.status !== 'active') return res.status(403).json({ success: false, error: '账号已停用' })
  return res.status(200).json({
    success: true,
    openid,
    bound: Boolean(user),
    user: user ? await userSummary(user) : null
  })
}
...
async function handleBind(req, res) {
  ...
  if (!user || !passwordHash || !(await comparePassword(password, passwordHash))) {
    return res.status(401).json({ success: false, code: 'INVALID_CREDENTIALS', error: '邮箱或密码错误' })
  }
  ...
  try {
    const linked = await neonHelper.query(
      `INSERT INTO ${IDENTITY_TABLE} (app_id, openid, user_id, created_at, linked_at)
       ...
       WHERE ${IDENTITY_TABLE}.user_id IS NULL OR ${IDENTITY_TABLE}.user_id = EXCLUDED.user_id
       RETURNING user_id`,
      [appId, openid, user.user_id]
    )
    if (!linked?.length) return res.status(409).json({ success: false, error: '该微信已绑定其他 Haigoo 账号' })
  } catch (error) {
    if (error?.code === '23505') {
  ...
  return res.status(200).json({ success: true, bound: true, user: await userSummary(user) })
}
...
async function handleRegister(req, res) {
  ...
  if (
    ...
    !SUPPORTED_PRIVACY_VERSIONS.has(privacyVersion) ||
  ...
  try {
    ...
    if (!linked?.length) return res.status(409).json({ success: false, error: '账号已创建，但微信已连接其他账号。请先解除原有连接，再用邮箱和密码登录。' })
  } catch (error) {
    if (error?.code === '23505') {
      return res.status(409).json({
        success: false,
...
async function validateAccountPassword(user, password, res) {
  ...
  if (!password || !passwordHash || !(await comparePassword(password, passwordHash))) {
    res.status(401).json({ success: false, code: 'INVALID_CREDENTIALS', error: '账号密码验证失败' })
    return false
  }
  return true
}
...
async function handleUnbind(req, res) {
  ...
  if (!user) return
  if (req.body?.expectedUserId && req.body.expectedUserId !== user.user_id) {
    return res.status(409).json({ success: false, code: 'SESSION_CHANGED', error: '账号状态已变化，请重新登录后操作' })
  }
  const rateLimit = await consumeRateLimit(req, res, {
...
async function requireBoundUser(openid, res) {
  ...
  const { user } = await getIdentityUser(openid)
  if (!user?.user_id) {
    res.status(401).json({ success: false, code: 'LOGIN_REQUIRED', error: '请先登录并连接 Haigoo 账号' })
    return null
  }
  if (user.status !== 'active') {
    res.status(403).json({ success: false, error: '账号已停用' })
    return null
  }
  return user
}

async function miniViewer(user) {
  const capabilities = deriveMembershipCapabilities(user)
  return {
...
function safeCompanyLogoSource(value, companyId, fallbackValue = '') {
  const source = String(value || '').trim()
  const id = String(companyId || '').trim()
  if (!/^[A-Za-z0-9._:-]{1,255}$/.test(id)) return ''
  if (source.startsWith('cloud://')) return ''
  const fallback = String(fallbackValue || '').trim()
  if (/^https?:\/\//i.test(fallback)) return fallback
  if (source.startsWith('/api/company-assets?')) {
    try {
      const parsed = new URL(source, 'https://haigoo.invalid')
      if (parsed.pathname === '/api/company-assets'
        && parsed.searchParams.get('companyId') === id
        && ['', 'logo'].includes(parsed.searchParams.get('type') || '')) {
        return `${parsed.pathname}${parsed.search}`
      }
    } catch {
      // Fall through to the canonical first-party asset path.
    }
  }
  // The binary asset table is authoritative. cached_logo_url can be an old
  // public URL, an API path or empty, so its storage format must not decide
  // whether the Mini Program is allowed to resolve an existing company logo.
  return `/api/company-assets?companyId=${encodeURIComponent(id)}&type=logo`
}
...
function miniWechatSubscriptionConfig() {
  const appId = String(process.env.WECHAT_MINI_APP_ID || '').trim()
  const appSecret = String(process.env.WECHAT_MINI_APP_SECRET || '').trim()
  const templateId = String(process.env.WECHAT_MINI_COMPANY_UPDATE_TEMPLATE_ID || '').trim()
  const available = String(process.env.MINI_WECHAT_REMINDERS_ENABLED || '').toLowerCase() === 'true'
    && /^wx[0-9a-f]{16}$/i.test(appId) && appSecret.length >= 16 && /^[A-Za-z0-9_-]{10,128}$/.test(templateId)
  return { available, templateId: available ? templateId : '' }
}
...
function mapMiniCompany(row) {
  ...
  return {
    ...
    ratingSource: String(row.rating_source || '').trim(),
    logoFileId: safeCloudFileId(row.cached_logo_url),
    logoUrl: safeExternalUrl(row.logo),
    _logoSourcePath: safeCompanyLogoSource(row.cached_logo_url, companyId, row.logo),
    updatedAt: row.updated_at || null,
    websiteUrl: safeExternalUrl(row.website),
    careersUrl: safeExternalUrl(row.careers_page),
    hasPublicOpportunity: Boolean(row.has_public_opportunity),
    publicOpportunityUpdatedAt: row.public_opportunity_updated_at || null,
    latestPublicJobAt: row.public_opportunity_updated_at || null,
    newJobsUntil: row.new_jobs_until || null,
    openJobCount: Number(row.open_job_count || 0),
    publicJobTitles: safeJsonArray(row.public_job_titles).map((item) => String(item || '').trim()).filter(Boolean).slice(0, 3),
    openRoleCategories: normalizeOpenRoleCategories(safeJsonArray(row.open_role_categories), 6),
    roleFamilies: safeJsonArray(row.role_families).map((item) => String(item || '').trim()).filter(Boolean)
  }
}

function buildMiniCompanyContactPayload({ rawContacts, publicApplicationEmails, canAccessContacts = false }) {
  const publicEmails = publicApplicationEmails instanceof Set ? publicApplicationEmails : new Set()
  const contacts = safeJsonArray(rawContacts).map((item, index) => ({
    ...
    return Boolean(item.linkedin || email) && (!email || !publicEmails.has(email))
  })
  const contactPreviews = contacts.map((contact, index) => buildCompanyContactPreview({
    id: `preview-${index + 1}`,
    name: contact.name,
    title: contact.title
  }))
  return {
    ...
    contactPreviews,
    ...(canAccessContacts && contacts.length ? { contacts } : {})
  }
}
...
function mapMiniPlan(plan) {
  return {
    ...
    name: String(plan.name || plan.shortLabel || ''),
    shortLabel: String(plan.shortLabel || plan.name || ''),
    price: Number(plan.price ?? Number(plan.amountCents || 0) / 100),
    currency: 'CNY',
    durationDays: Number(plan.durationDays || 0),
    durationMonths: Number(plan.durationMonths || 0),
    description: String(plan.description || ''),
    featured: Boolean(plan.featured),
    features: safeJsonArray(plan.features).map((item) => String(item || '')).filter(Boolean)
  }
}

const FREE_COMPANY_DIRECTORY_LIMIT = 12

// Directory discovery is deliberately derived from current, approved, public jobs.
// History is evidence for recency, never a substitute for the current jobs row.
const companyDirectorySnapshotSql = `
  WITH eligible_directory_jobs AS (
    SELECT j.company_id, j.job_id, j.title, j.translations,
           h.title AS history_title,
           COALESCE(NULLIF(BTRIM(h.category), ''), NULLIF(BTRIM(j.category), '')) AS category,
           CASE WHEN jsonb_typeof(h.role_families) = 'array' THEN h.role_families ELSE '[]'::jsonb END AS role_families,
           COALESCE(
             CASE WHEN h.first_seen_at <= NOW() THEN h.first_seen_at END,
             CASE WHEN j.created_at <= NOW() THEN j.created_at END,
             CASE WHEN j.updated_at <= NOW() THEN j.updated_at END,
             CASE WHEN j.published_at <= NOW() THEN j.published_at END
  ...
  SELECT tc.company_id, tc.name, tc.translations, tc.description, tc.industry, tc.tags, tc.specialties,
         tc.address, tc.logo, tc.employee_count, tc.founded_year, tc.company_rating, tc.rating_source,
         tc.cached_logo_url, tc.updated_at, tc.website, tc.careers_page,
         hiring.open_job_count, hiring.public_opportunity_updated_at, hiring.new_jobs_until,
         hiring.public_job_titles, hiring.public_job_search_terms, hiring.open_role_categories,
         hiring.role_families,
         TRUE AS has_public_opportunity
    FROM trusted_companies tc
    JOIN LATERAL (
      SELECT COUNT(DISTINCT job.job_id)::int AS open_job_count,
             MAX(job.public_opportunity_updated_at) AS public_opportunity_updated_at,
             MAX(job.public_opportunity_updated_at) + INTERVAL '72 hours' AS new_jobs_until,
             ARRAY(
               SELECT DISTINCT COALESCE(
                 NULLIF(BTRIM(CASE WHEN jsonb_typeof(title_job.translations->'title') = 'string' THEN title_job.translations->>'title' END), ''),
                 NULLIF(BTRIM(title_job.title), ''),
                 NULLIF(BTRIM(title_job.history_title), '')
               )
                 FROM eligible_directory_jobs title_job
                WHERE title_job.company_id = tc.company_id
                ORDER BY 1
             ) AS public_job_titles,
             ARRAY(
               SELECT value FROM (
                 SELECT DISTINCT NULLIF(BTRIM(term_job.title), '') AS value
                   FROM eligible_directory_jobs term_job
                  WHERE term_job.company_id = tc.company_id
                 UNION
                 SELECT DISTINCT NULLIF(BTRIM(term_job.history_title), '') AS value
                   FROM eligible_directory_jobs term_job
                  WHERE term_job.company_id = tc.company_id
                 UNION
                 SELECT DISTINCT NULLIF(BTRIM(term_job.translations->>'title'), '') AS value
                   FROM eligible_directory_jobs term_job
                  WHERE term_job.company_id = tc.company_id AND jsonb_typeof(term_job.translations->'title') = 'string'
                 UNION
                 SELECT DISTINCT NULLIF(BTRIM(translated.value), '') AS value
                   FROM eligible_directory_jobs term_job
                   CROSS JOIN LATERAL jsonb_each_text(
                     CASE WHEN jsonb_typeof(term_job.translations->'title') = 'object' THEN term_job.translations->'title' ELSE '{}'::jsonb END
                   ) translated
                  WHERE term_job.company_id = tc.company_id
               ) terms WHERE value IS NOT NULL
             ) AS public_job_search_terms,
             ARRAY(
               SELECT category_rollup.category FROM (
                 SELECT category_job.category, MAX(category_job.public_opportunity_updated_at) AS latest_seen
                   FROM eligible_directory_jobs category_job
                  WHERE category_job.company_id = tc.company_id AND category_job.category IS NOT NULL
                  GROUP BY category_job.category ORDER BY latest_seen DESC NULLS LAST, category_job.category ASC LIMIT 12
               ) category_rollup
             ) AS open_role_categories,
             ARRAY(
               SELECT DISTINCT BTRIM(family)
                 FROM eligible_directory_jobs role_job
                 CROSS JOIN LATERAL jsonb_array_elements_text(role_job.role_families) family
                WHERE role_job.company_id = tc.company_id
             ) AS role_families
        FROM eligible_directory_jobs job
       WHERE job.company_id = tc.company_id
    ) hiring ON TRUE
   WHERE tc.status = 'active' AND hiring.open_job_count > 0`

const companyDirectoryCache = createPublicSnapshotCache({ ttlMs: 45000 })

function normalizeMiniDirectorySort(value) {
  return String(value || '').trim().toLowerCase() === 'relevance' ? 'relevance' : 'latest'
}

async function readMiniCompanies({ viewer, search = '', industry = '', page = 1, pageSize = 20, sortBy = 'latest' }) {
  const hasFullAccess = Boolean(viewer.hasCompanyDirectoryAccess)
  const requestedSort = String(sortBy || '').trim().toLowerCase()
  const query = {
    search: String(search || '').trim().slice(0, 80),
    industry: hasFullAccess ? String(industry || '').trim().slice(0, 80) : '',
    page: hasFullAccess ? Math.max(1, Number(page) || 1) : 1,
    pageSize: hasFullAccess ? Math.min(50, Math.max(1, Number(pageSize) || 20)) : FREE_COMPANY_DIRECTORY_LIMIT,
    sortBy: normalizeMiniDirectorySort(requestedSort)
  }
  const snapshot = await companyDirectoryCache('public-directory-v3', () => neonHelper.query(companyDirectorySnapshotSql))
  const result = await readMiniCompaniesFromSnapshot({ viewer, rows: snapshot, ...query })
  return { ...result, sortBy: requestedSort === 'newest' ? 'newest' : result.sortBy, serverTime: new Date().toISOString() }
}

async function readMiniCompaniesFromSnapshot({ viewer, rows = [], search = '', industry = '', page = 1, pageSize = 20, sortBy = 'latest' }) {
  ...
  const effectivePageSize = hasFullAccess
    ...
    : FREE_COMPANY_DIRECTORY_LIMIT
  let filtered = (rows || []).filter((row) => !normalizedIndustry || String(row.industry || '') === normalizedIndustry)
  let profile = null
  if (sortBy === 'relevance' && viewer.user?.userId) {
    try {
      profile = await getCareerWatchProfile(viewer.user.userId)
    } catch (error) {
      console.warn('[mini-gateway] company relevance profile unavailable', error?.message || error)
    }
  }
  const ranked = rankDirectoryCompanies(filtered, normalizedSearch, { mode: hasFullAccess ? 'member' : 'free', sortBy, profile })
  filtered = ranked.companies
  const total = filtered.length
  const offset = (effectivePage - 1) * effectivePageSize
  const paged = hasFullAccess ? filtered.slice(offset, offset + effectivePageSize) : filtered.slice(0, FREE_COMPANY_DIRECTORY_LIMIT)
  const industryCounts = new Map()
  for (const row of rows || []) if (row.industry) industryCounts.set(row.industry, (industryCounts.get(row.industry) || 0) + 1)
  return {
    companies: paged.map(mapMiniCompany),
    serverTime: new Date().toISOString(),
    searchOutcome: ranked.outcome || null,
    total,
    page: effectivePage,
    pageSize: effectivePageSize,
    hasMore: hasFullAccess && effectivePage * effectivePageSize < total,
    access: {
      scope: hasFullAccess ? 'member_all' : 'free_fixed',
      fullDirectory: hasFullAccess,
      previewLimit: hasFullAccess ? null : Math.min(FREE_COMPANY_DIRECTORY_LIMIT, total),
      searchEnabled: true,
      searchMode: hasFullAccess ? 'fuzzy' : 'exact'
    },
    sortBy,
    industries: [...industryCounts.entries()].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0])).slice(0, 24).map(([name, count]) => ({ name, count }))
  }
}
...
async function handleMiniCompany(req, res) {
  ...
  const viewer = await resolveMiniViewer(String(req.query?.openid || '').trim())
  // A specific active company is public, including links shared by operations.
  // Directory discovery and private contacts retain their separate entitlements.
  const rows = await neonHelper.query(
      `SELECT tc.company_id, tc.name, tc.description, tc.industry, tc.tags, tc.specialties, tc.logo,
            tc.address, tc.employee_count, tc.founded_year, tc.company_rating, tc.rating_source,
            tc.cached_logo_url, tc.updated_at, tc.website, tc.careers_page,
            tc.referral_contacts, tc.hiring_email,
            ...
            COALESCE(hiring.open_job_count, 0) > 0 AS has_public_opportunity,
            hiring.public_opportunity_updated_at
       FROM trusted_companies tc
       ...
       LEFT JOIN LATERAL (
         SELECT COUNT(*)::int AS open_job_count,
                MAX(h.first_seen_at) AS public_opportunity_updated_at
           FROM company_job_history h
           JOIN jobs current_job ON current_job.job_id = h.source_job_id
          WHERE h.company_id = tc.company_id
            AND h.closed_at IS NULL
            AND h.is_public_opportunity IS TRUE
            AND current_job.status = 'active'
            AND current_job.is_approved IS TRUE
            AND COALESCE(current_job.member_only, FALSE) IS FALSE
            AND (NULLIF(BTRIM(current_job.url), '') ~* '^https?://[^[:space:]]+$'
              OR NULLIF(BTRIM(tc.hiring_email), '') ~* '^[^[:space:]@]+@[^[:space:]@]+\\.[^[:space:]@]+$')
       ) hiring ON TRUE
      WHERE tc.company_id = $1
        AND (tc.status = 'active' OR EXISTS (
          SELECT 1
            FROM mini_company_follows follows
           WHERE follows.user_id::text = $2::text
             AND follows.company_id = tc.company_id
             AND follows.status = 'active'
        ))
      LIMIT 1`,
    [companyId, String(viewer.user?.userId || '')]
  )
  const row = rows?.[0]
  ...
  try {
    ...
    publicApplicationEmails = new Set((publicJobs || [])
      .filter((job) => !job.memberOnly && !job.url && !job.sourceUrl)
      .map((job) => normalizeEmail(job.hiringEmail))
      .filter((email) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)))
    if (!publicJobs?.length && Number(row.open_job_count || 0) > 0) {
      const fallbackEmail = normalizeEmail(row.hiring_email)
      if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(fallbackEmail)) publicApplicationEmails.add(fallbackEmail)
    }
  } catch (error) {
    console.warn('[mini-gateway] public application email lookup unavailable', error?.message || error)
    publicApplicationEmails = new Set([normalizeEmail(row.hiring_email)].filter(Boolean))
  }
  const rawContacts = safeJsonArray(row.referral_contacts)
  if (row.hiring_email && !rawContacts.some((item) => String(item?.hiringEmail || item?.email || '').trim() === String(row.hiring_email).trim())) {
    rawContacts.push({ hiringEmail: row.hiring_email, title: '招聘联系' })
  }
  const contactPayload = buildMiniCompanyContactPayload({
    rawContacts,
    publicApplicationEmails,
    canAccessContacts: viewer.capabilities.canAccessCompanyContacts
  })
  const company = {
    ...mapMiniCompany(row),
    ...contactPayload,
    ...
    // Jobs are attached by CloudRun from the formal read-only jobs source.
    // Keeping account/profile data separate prevents preview fixtures from
    // leaking into the Mini Program company directory.
    jobs: []
  }
  return res.status(200).json({
    success: true,
    ...
    access: {
      ...
      contacts: viewer.capabilities.canAccessCompanyContacts && Boolean(contactPayload.contacts?.length),
      contactPreview: contactPayload.contactPreviews.length > 0
    }
  })
}
...
async function handleMiniMembershipPlans(req, res) {
  if (!ensureDatabase(res)) return
  const viewer = await resolveMiniViewer(String(req.query?.openid || '').trim())
  const plans = Object.entries(VIRTUAL_PAYMENT_PRODUCTS).map(([id, plan]) => mapMiniPlan({ id, ...plan }))
  let paymentAvailable = false
  try {
    const productMap = parseProductMap()
    paymentAvailable = Object.entries(VIRTUAL_PAYMENT_PRODUCTS)
      .every(([id, plan]) => !productMap[id] || String(productMap[id]) === plan.productId)
  } catch {
    paymentAvailable = false
  }
  return res.status(200).json({ success: true, plans, membership: viewer.user, paymentAvailable })
}

async function handleMiniMemberServices(req, res) {
  if (!ensureDatabase(res)) return
  const user = await requireBoundUser(String(req.query?.openid || req.body?.openid || '').trim(), res)
  if (!user) return
  if (req.method === 'GET') {
    return res.status(200).json({ success: true, ...(await getMiniMemberServices(user)) })
  }
  const entitlement = await claimMiniMemberService(user, req.body?.entitlementKey)
...
async function handleMiniConsultations(req, res) {
  ...
  if (!SUPPORTED_PRIVACY_VERSIONS.has(privacyVersion) || Number.isNaN(acceptedAt.getTime())) {
...
async function readWebsiteResumeImport(userId) {
  try {
    const rows = await neonHelper.query(
      `SELECT resume_id, file_name, parse_status, parse_result, content_text, updated_at
         FROM resumes
        WHERE user_id = $1
          AND parse_status IN ('success', 'partial', 'completed')
...
async function handleMatchFeed(req, res) {
  ...
  const wechatSubscription = miniWechatSubscriptionConfig()
  ...
  return res.status(200).json({
    ...
    capabilities: {
      ...
      wechatTemplateId: wechatSubscription.templateId,
      wechatSubscriptionAvailable: wechatSubscription.available
    }
  })
}

async function handleCareerWatchState(req, res) {
  if (!ensureDatabase(res)) return
  const user = await requireBoundUser(String(req.method === 'POST' ? req.body?.openid || '' : req.query?.openid || '').trim(), res)
  if (!user) return
  const capabilities = deriveMembershipCapabilities(user)
  const wechatSubscription = miniWechatSubscriptionConfig()
  const isMatchMember = Boolean(capabilities.canAccessTrustedCompaniesPage)
  const [profile, importSources, filterOptions, access] = await Promise.all([
    getCareerWatchProfile(user.user_id),
    getCareerWatchImportSources(user.user_id),
    getCareerWatchFilterOptions(),
    getCareerWatchAccessState(user.user_id, isMatchMember)
  ])
  const feed = await getCareerWatchFeed({
    userId: user.user_id,
    profile,
    isMember: isMatchMember,
    fixedFree: access.matchState === 'fixed_free',
    refreshKey: isMatchMember && req.method === 'POST' ? String(req.body?.refreshKey || '').slice(0, 80) : ''
  })
  return res.status(200).json({
    success: true,
    profile,
    ...
    ...access,
    entitlements: {
      ...careerWatchEntitlements(isMatchMember, { refreshOnVisit: true }),
      wechatTemplateId: wechatSubscription.templateId,
      wechatSubscriptionAvailable: wechatSubscription.available
    },
    ...feed
...
async function handleCareerWatchOptions(_req, res) {
  if (!ensureDatabase(res)) return
  const wechatSubscription = miniWechatSubscriptionConfig()
  return res.status(200).json({
    success: true,
    filterOptions: await getCareerWatchFilterOptions(),
    capabilities: { wechatSubscriptionAvailable: wechatSubscription.available }
  })
}

async function handleCareerWatchSave(req, res) {
  if (!ensureDatabase(res)) return
  ...
  let profile
  try {
    profile = isMatchMember
      ? await saveCareerWatchProfile({
          userId: user.user_id,
          input: { ...(req.body || {}), sourcePlatform: 'mini' },
    ...
    if (error?.code !== 'FREE_MATCH_USED') throw error
    const [existingProfile, filterOptions, access] = await Promise.all([
      getCareerWatchProfile(user.user_id),
      getCareerWatchFilterOptions(),
      getCareerWatchAccessState(user.user_id, false)
    ])
    const feed = await getCareerWatchFeed({
      userId: user.user_id,
    ...
    return res.status(409).json({
      ...
      error: '匹配结果已生成，当前结果将继续保留',
      profile: existingProfile,
      filterOptions,
      ...access,
      entitlements: {
        ...careerWatchEntitlements(false),
        wechatTemplateId: wechatSubscription.templateId,
        wechatSubscriptionAvailable: wechatSubscription.available
      },
      ...feed
  ...
  const access = await getCareerWatchAccessState(user.user_id, isMatchMember)
  const [feed, filterOptions] = await Promise.all([
    getCareerWatchFeed({ userId: user.user_id, profile, isMember: isMatchMember, fixedFree: access.matchState === 'fixed_free' }),
    getCareerWatchFilterOptions()
  ])
  return res.status(200).json({
    success: true,
...
async function handleCareerWatchNotifications(req, res) {
  if (!ensureDatabase(res)) return
  const user = await requireBoundUser(String(req.body?.openid || '').trim(), res)
  if (!user) return
  const capabilities = deriveMembershipCapabilities(user)
  if (!capabilities.canAccessTrustedCompaniesPage) {
    return res.status(403).json({ success: false, code: 'MEMBERSHIP_REQUIRED', error: '会员可开启全库方向岗位提醒' })
  }
  const profile = await setCareerWatchNotifications(user.user_id, {
    enabled: req.body?.enabled === true,
    templateStatus: req.body?.templateStatus
  })
  return res.status(200).json({
    success: true,
    enabled: profile.wechatEnabled,
    templateStatus: profile.wechatTemplateStatus
  })
}

async function handleCareerWatchImport(req, res) {
  if (!ensureDatabase(res)) return
  const user = await requireBoundUser(String(req.body?.openid || '').trim(), res)
  if (!user) return
  return res.status(200).json({
...
async function handleMatchFollows(req, res) {
  ...
  if (req.method === 'DELETE') return res.status(200).json(await setCompanyFollow({ user, companyId: req.query?.companyId, active: false, isMember: capabilities.canAccessTrustedCompaniesPage }))
  const companyId = String(req.body?.companyId || '').trim()
  const active = req.body?.followed !== false
  return res.status(200).json(await setCompanyFollow({ user, companyId, active, isMember: capabilities.canAccessTrustedCompaniesPage }))
}
...
async function handleMatchNotifications(req, res) {
  ...
  return res.status(200).json(await setFollowNotifications({
    user,
    isMember: deriveMembershipCapabilities(user).canAccessTrustedCompaniesPage,
...
async function handleCareerState(req, res) {
  ...
  const runRows = entitlement.profile
    ...
    : []
  const importedResume = await readWebsiteResumeImport(user.user_id)
...
async function handleCareerResumeParse(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      rawFileStored: false,
      message: '简历已读取，原文件未保存。请检查识别出的岗位方向是否准确。'
    })
  } catch (error) {
...
async function handleCareerResumeSync(req, res) {
  if (!ensureDatabase(res)) return
  const user = await requireBoundUser(String(req.body?.openid || '').trim(), res)
  if (!user) return
  const filename = String(req.body?.filename || '').trim().slice(0, 160)
  ...
  const existing = await neonHelper.query(
    `SELECT resume_id FROM resumes
      WHERE user_id = $1 AND metadata->>'sha256' = $2
      ORDER BY created_at DESC LIMIT 1`,
    [user.user_id, digest]
  )
  if (existing?.[0]?.resume_id) {
    return res.status(200).json({ success: true, saved: true, duplicate: true, resumeId: existing[0].resume_id })
  }
  const parsed = await parseCareerResumeBuffer({ filename, buffer })
  const resumeId = `resume_${Date.now()}_${crypto.randomBytes(5).toString('hex')}`
  const extension = filename.toLowerCase().split('.').pop() || 'txt'
  await neonHelper.query(
    `INSERT INTO resumes (
       resume_id, user_id, file_name, file_size, file_type, parse_status,
       parse_result, content_text, metadata, file_content, created_at, updated_at
     ) VALUES ($1, $2, $3, $4, $5, 'completed', $6::jsonb, $7, $8::jsonb, $9, NOW(), NOW())`,
    [
      resumeId,
      user.user_id,
      ...
      String(parsed.text || ''),
      JSON.stringify({ sha256: digest, source: 'mini_match', consentedAt: new Date().toISOString() }),
      buffer
    ]
  )
  return res.status(201).json({ success: true, saved: true, duplicate: false, resumeId })
}
...
async function handleCareerAnalyze(req, res) {
  ...
  if (!entitlement.canAssess) {
    return res.status(409).json({ success: false, code: 'FREE_MATCH_USED', error: '匹配结果已生成，当前结果将继续保留' })
  }
  ...
  if (status === 'ready') {
    if (!entitlement.isMember) {
      const structured = buildStructuredCareerProfile(careerText, intake)
      await createFixedCareerWatchMatch({
        userId: user.user_id,
        input: {
          sourceMode: entitlement.profile?.source_type === 'resume' ? 'resume' : 'manual',
          roleFamilies: structured.matchingProfile.roleFamilies.slice(0, 5),
          customRoleTerms: String(intake.targetRoles || '').trim() ? [String(intake.targetRoles).trim().slice(0, 80)] : [],
          companyPreferences: {},
          activePreferenceKeys: [],
          toleranceMode: 'balanced',
          status: 'active',
          careerProfileId: entitlement.profile?.profile_id || null,
          sourcePlatform: 'mini'
        }
      })
    } else {
...
async function handleCareerDelete(req, res) {
  ...
  if (scope === 'resume') {
    ...
    if (!resumeId) return res.status(400).json({ success: false, error: '请选择要删除的简历' })
    const rows = await neonHelper.query(
      `WITH deleted AS (
         DELETE FROM resumes WHERE user_id = $1 AND resume_id = $2 RETURNING resume_id
       ), cleared_watch AS (
         UPDATE career_watch_profiles
            SET resume_id = NULL, updated_at = NOW()
          WHERE user_id = $1 AND resume_id = $2 AND EXISTS (SELECT 1 FROM deleted)
         RETURNING profile_id
       )
       SELECT resume_id FROM deleted`,
      [user.user_id, resumeId]
    )
    await neonHelper.query(
      `INSERT INTO ${CAREER_PRIVACY_EVENTS_TABLE} (user_id, action, metadata)
       VALUES ($1, 'career_resume_deleted', $2::jsonb)`,
      [user.user_id, JSON.stringify({ resumeId, deleted: Boolean(rows?.[0]) })]
    )
    return res.status(200).json({ success: true, deleted: Boolean(rows?.[0]), message: rows?.[0] ? '简历已删除' : '简历已经不存在' })
  }
  const rows = await neonHelper.query(
    `WITH deleted_profile AS (
       DELETE FROM ${CAREER_PROFILES_TABLE} WHERE user_id = $1 RETURNING profile_id
     ), deleted_watch AS (
       DELETE FROM career_watch_profiles WHERE user_id = $1 RETURNING profile_id
     ), deleted_exposures AS (
       DELETE FROM mini_match_exposures WHERE user_id = $1 RETURNING exposure_id
     )
     SELECT
       EXISTS (SELECT 1 FROM deleted_profile) AS deleted_profile,
       EXISTS (SELECT 1 FROM deleted_watch) AS deleted_watch`,
    [user.user_id]
  )
  const deleted = Boolean(rows?.[0]?.deleted_profile || rows?.[0]?.deleted_watch)
  await neonHelper.query(
    `INSERT INTO ${CAREER_PRIVACY_EVENTS_TABLE} (user_id, action, metadata)
     VALUES ($1, 'career_data_deleted', $2::jsonb)`,
    [user.user_id, JSON.stringify({ deletedProfile: Boolean(rows?.[0]?.deleted_profile), deletedWatch: Boolean(rows?.[0]?.deleted_watch) })]
  )
  return res.status(200).json({ success: true, deleted, message: '求职资料已删除，已有匹配结果仍会保留' })
}
...
async function handleVirtualPaymentRefundReconciliation(req, res) {
  if (!ensureDatabase(res)) return
  const openid = String(req.body?.openid || '').trim()
  const user = await requireBoundUser(openid, res)
  if (!user) return
  const result = await wechatVirtualPaymentService.reconcileRefund({
    ...
    leftFee: req.body?.leftFee,
    queriedAt: req.body?.queriedAt
  })
  return res.status(200).json({ success: true, ...result })
}

async function handleVirtualPaymentUpdate(req, res) {
  if (!ensureDatabase(res)) return
  const openid = String(req.body?.openid || '').trim()
  const user = await requireBoundUser(openid, res)
  if (!user) return
  ...
  if (!/^[A-Za-z0-9_*-]{6,32}$/.test(paymentId) || !['cancelled', 'failed'].includes(status)) {
    return res.status(400).json({ success: false, error: '支付状态参数无效' })
  }
  const order = await wechatVirtualPaymentService.updateClientStatus(paymentId, {
    userId: user.user_id,
    openid,
    status
  })
...
async function handleFavorites(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    return respondWithIdempotency(res, {
      ...
      if (!job) return { status: 404, payload: { success: false, error: '岗位不存在或已下线' } }
      await saveJobFavorite(user.user_id, jobId, job)
      return { status: 200, payload: { success: true, jobId, favorite: true } }
    })
  }

  const rows = await readJobFavorites(user.user_id)
  return res.status(200).json({
    success: true,
    favorites: (rows || []).map((row) => ({
      jobId: String(row.job_id || ''),
      createdAt: row.created_at || null,
      title: row.job_title_snapshot || '',
...
async function handleCompanyCatalogSnapshot(req, res) {
  if (!ensureDatabase(res)) return
  const page = Math.max(1, Number(req.query?.page) || 1)
  const pageSize = Math.min(250, Math.max(1, Number(req.query?.pageSize || req.query?.limit) || 100))
  const snapshot = await readPublicCompanyCatalogPage({ page, pageSize })
  return res.status(200).json({ success: true, ...snapshot })
}

async function handleCompanyCatalogImport(req, res) {
  if (!ensureDatabase(res)) return
  if (String(process.env.VERCEL_ENV || '') !== 'preview') {
    return res.status(403).json({ success: false, error: 'Catalog import is Preview-only' })
  }
  const result = await applyPreviewCompanyCatalogSnapshot(req.body || {}, { allowImport: true })
  return res.status(200).json({ success: true, ...result })
}

function attachRequestTrace(res, requestId) {
  const rawJson = res.json.bind(res)
  res.setHeader('X-Haigoo-Request-Id', requestId)
  res.json = function tracedJson(payload) {
    return rawJson(payload && typeof payload === 'object' && !Array.isArray(payload)
      ? { ...payload, requestId }
      : payload)
  }
  return res
...
export default async function miniGatewayHandler(req, res) {
  ...
  const receivedRequestId = String(req.headers?.['x-haigoo-request-id'] || req.query?.requestId || '').trim()
  ...
  if (![
    ...
    'membership_plans', 'member_services', 'consultations',
    'career_state', 'career_resume_parse', 'career_resume_sync', 'career_profile', 'career_analyze', 'career_delete',
    'career_watch_state', 'career_watch_refresh', 'career_watch_options', 'career_watch_save', 'career_watch_import', 'career_watch_notifications',
    ...
    'virtual_payment_create', 'virtual_payment_status', 'virtual_payment_list', 'virtual_payment_update',
    'virtual_payment_reconcile_refund', 'sync', 'company_catalog_snapshot', 'company_catalog_import'
  ].includes(action)) {
  ...
  try {
    ...
    if (action === 'member_services' && ['GET', 'POST'].includes(req.method)) return await handleMiniMemberServices(req, res)
    ...
    if (action === 'career_resume_sync' && req.method === 'POST') return await handleCareerResumeSync(req, res)
    ...
    if (action === 'career_watch_refresh' && req.method === 'POST') return await handleCareerWatchState(req, res)
    if (action === 'career_watch_state' && req.method === 'GET') return await handleCareerWatchState(req, res)
    if (action === 'career_watch_options' && req.method === 'GET') return await handleCareerWatchOptions(req, res)
    if (action === 'career_watch_save' && req.method === 'PUT') return await handleCareerWatchSave(req, res)
    if (action === 'career_watch_import' && req.method === 'POST') return await handleCareerWatchImport(req, res)
    if (action === 'career_watch_notifications' && req.method === 'POST') return await handleCareerWatchNotifications(req, res)
    ...
    if (action === 'virtual_payment_update' && req.method === 'PUT') return await handleVirtualPaymentUpdate(req, res)
    if (action === 'virtual_payment_reconcile_refund' && req.method === 'POST') return await handleVirtualPaymentRefundReconciliation(req, res)
    ...
    if (action === 'company_catalog_snapshot' && req.method === 'GET') return await handleCompanyCatalogSnapshot(req, res)
    if (action === 'company_catalog_import' && req.method === 'POST') return await handleCompanyCatalogImport(req, res)
    return res.status(405).json({ success: false, error: 'Method not allowed' })
  } catch (error) {
    console.error('[mini-gateway] request failed', { requestId, action, message: error?.message || String(error) })
    const statusCode = Number(error?.statusCode || error?.status || 500)
    const errorCode = String(error?.code || '')
    const safeErrorCodes = new Set([
      'LOGIN_REQUIRED', 'FREE_MATCH_USED', 'COMPANY_NOT_IN_FREE_MATCH', 'COMPANY_NOT_IN_FREE_DIRECTORY',
      'MEMBERSHIP_REQUIRED', 'MEMBER_CONTACTS_REQUIRED', 'SUBSCRIBE_AUTH_REQUIRED', 'COMPANY_FOLLOW_LIMIT_REACHED', 'COMPANY_REMINDER_LIMIT_REACHED', 'COMPANY_NOT_AVAILABLE',
      'WATCH_ROLE_REQUIRED', 'WATCH_OPTION_STALE', 'WATCH_VERSION_CONFLICT',
      'WATCH_NOT_CONFIGURED', 'MATCH_DATA_NOT_READY'
    ])
    ...
    return res.status(statusCode >= 400 && statusCode <= 599 ? statusCode : 500).json({
      ...
      error: safeVirtualPaymentError || safeErrorCodes.has(errorCode)
        ? error.message
        : statusCode === 503
          ? '服务繁忙，请稍后重试'
          : '当前服务暂时不可用，请稍后重试'
...
export {
  gatewaySecrets,
  attachRequestTrace,
  buildMiniCompanyContactPayload,
  hasGatewaySignature,
  mapMiniCompany,
  mapMiniNote,
  mapMiniPlan,
