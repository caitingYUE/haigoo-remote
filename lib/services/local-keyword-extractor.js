...
const STOP_WORDS = new Set([
    ...
    // Boring / File / Admin words to block
    'ppt', 'pptx', 'doc', 'docx', 'pdf', 'xls', 'xlsx', 'file', 'files',
    'k12', 'boss', 'manager', 'director', 'report', 'reports', 'daily', 'weekly',
    'monthly', 'meeting', 'meetings', 'slide', 'slides', 'sheet', 'sheets',
    'word', 'excel', 'powerpoint', 'office', 'windows', 'mac', 'linux',
    'tool', 'tools', 'app', 'apps', 'software', 'hardware', 'system', 'systems'
]);
...
export function extractKeywordsLocal(text, maxKeywords = 80) {
    ...
    // Boost weights of high-value career words found in text
    const HIGH_VALUE_WORDS = new Set([
        'strategy', 'leadership', 'management', 'development', 'innovation',
        'creative', 'design', 'architecture', 'engineering', 'product',
        'analysis', 'research', 'communication', 'collaboration', 'vision',
        'execution', 'optimization', 'efficiency', 'quality', 'performance',
        'growth', 'impact', 'solution', 'delivery', 'planning', 'mentoring',
        'coaching', 'building', 'scaling', 'driving', 'leading', 'creating'
    ]);

    // Apply boost
    sorted.forEach(item => {
        if (HIGH_VALUE_WORDS.has(item.text.toLowerCase())) {
            item.weight = Math.min(10, item.weight + 2);
        }
    });
