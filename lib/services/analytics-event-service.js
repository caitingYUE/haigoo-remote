...
function normalizeLimitedString(value, maxLength, fallback = null) {
  const text = normalizeString(value, fallback);
  if (text === null || text.length <= maxLength) return text;
  const suffix = crypto.createHash('sha256').update(text).digest('hex').slice(0, 12);
  return `${text.slice(0, maxLength - suffix.length - 1)}_${suffix}`;
}
...
export function derivePageKey(pathname = '') {
  ...
  if (path.startsWith('/careerlearning') || path.startsWith('/corporate-english')) return 'corporate_english';
...
export function normalizeAnalyticsEvent(event = {}, options = {}) {
  ...
  const normalized = {
    ...
    flowId: normalizeLimitedString(options.flowId || event.flow_id || mergedProperties.flow_id, 64, null),
