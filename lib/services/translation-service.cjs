...
async function translateText(originalText, targetLang, sourceLang = 'auto') {
  ...
  // 1. 优先使用配置的首选服务
  if (PREFERRED_PROVIDER && byProvider[PREFERRED_PROVIDER]) {
    // Special handling for AI: only if enabled
    if (PREFERRED_PROVIDER === 'ai') {
        if (config.aiEnabled) services.push(byProvider.ai)
    } else {
        services.push(byProvider[PREFERRED_PROVIDER])
    }
  }

  // 2. 如果 AI 启用且未被选为首选，将其作为高优先级备选 (因为 Google/Libre 翻译质量较差)
  if (config.aiEnabled && PREFERRED_PROVIDER !== 'ai') {
     // Check keys
