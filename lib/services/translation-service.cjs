...
const PREFERRED_PROVIDER = (process.env.PREFERRED_TRANSLATION_PROVIDER || 'libretranslate').toLowerCase()
...
// 并发与速率控制（默认每分钟18次，请求间隔 ~3333ms）
const TRANSLATE_CONCURRENCY = Number(process.env.TRANSLATE_CONCURRENCY || '1')
const REQUESTS_PER_MINUTE = Math.max(1, Number(process.env.TRANSLATE_REQUESTS_PER_MINUTE || '18'))
...
async function translateBatch(texts, targetLang = 'zh-CN', sourceLang = 'en') {
  ...
  try {
    ...
    try {
      ...
      console.warn('⚠️ 代理调用失败，返回原文:', e?.message || e)
    }
