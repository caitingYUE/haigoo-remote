...
async function translateJob(job) {
  ...
  const descHasChinese = hasChinese(translations.description);
  const originalHasChinese = hasChinese(job.title);

  // 如果目标是中文，但翻译结果没有中文（且原文也不是中文），视为翻译失败
  // 宽松检查：只要标题或描述中包含中文，即视为成功（处理类似 "C++" 这种纯英文标题的情况）
  // 🔧 FIX: 移除 !originalHasChinese 检查。即使原文包含中文，如果翻译结果（针对zh目标）不包含中文，也是无效的。
  // 除非原文本身就是纯中文（这种情况下 translateText 通常会直接返回原文，所以 titleHasChinese 应该为 true）
  // 🔧 FIX V2: 更加严格的检查。描述必须包含至少20个中文字符，防止只有"地点：上海"这种假翻译通过。
  const chineseCharCount = (translations.description || '').match(/[\u4e00-\u9fa5]/g)?.length || 0;
  
  if (chineseCharCount < 20) {
    console.warn(`⚠️ 岗位 ${job.id} 翻译校验失败: 描述中文字符数不足 (${chineseCharCount} < 20)`);
    console.warn(`   Result Snippet: ${translations.description ? translations.description.substring(0, 100) : 'null'}...`);
    return {
      ...job,
