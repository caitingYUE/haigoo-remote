...
async function translateText(originalText, targetLang, sourceLang = 'auto', context = {}) {
  ...
  // Google Cloud is only eligible when a server-side API key is configured.
  if (PREFERRED_PROVIDER === 'google') {
    addProvider('google', Boolean(GOOGLE_TRANSLATE_API_KEY))
  } else if (PREFERRED_PROVIDER && byProvider[PREFERRED_PROVIDER] && PREFERRED_PROVIDER !== 'ai' && PREFERRED_PROVIDER !== 'googleUnofficial') {
