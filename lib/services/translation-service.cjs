...
const GOOGLE_TRANSLATE_API_KEY = String(process.env.GOOGLE_TRANSLATE_API_KEY || '').trim()
const ALLOW_UNOFFICIAL_GOOGLE = String(process.env.GOOGLE_TRANSLATE_ALLOW_UNOFFICIAL_FALLBACK || '').toLowerCase() === 'true'
const GOOGLE_MAX_RETRIES = 2
const GOOGLE_RETRY_BASE_MS = 500
...
function googleRetryDelay(response, attempt) {
  ...
  return GOOGLE_RETRY_BASE_MS * (2 ** attempt)
}
...
async function fetchGoogleWithRetry(url, options) {
  ...
  for (let attempt = 0; attempt <= GOOGLE_MAX_RETRIES; attempt += 1) {
    try {
      ...
      if (!GOOGLE_RETRYABLE_STATUSES.has(response.status) || attempt === GOOGLE_MAX_RETRIES) return response
      await new Promise(resolve => setTimeout(resolve, googleRetryDelay(response, attempt)))
    } catch (error) {
      lastError = error
      if (attempt === GOOGLE_MAX_RETRIES) throw error
      await new Promise(resolve => setTimeout(resolve, GOOGLE_RETRY_BASE_MS * (2 ** attempt)))
    }
  }

  throw lastError || new Error('Google Cloud Translation request failed')
}
...
async function translateWithGoogle(text, targetLang, sourceLang = 'auto') {
  const doTranslate = async (inputText) => {
      try {
        ...
        const response = await fetchGoogleWithRetry(TRANSLATION_SERVICES.google.baseUrl, {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          ...
          const reason = String(errorPayload?.error?.message || '').replace(/\s+/g, ' ').slice(0, 240)
          throw new Error(`Google Cloud Translation API error: ${response.status}${reason ? ` - ${reason}` : ''}`)
        }

        const data = await response.json()
        const translation = data?.data?.translations?.[0]
        if (!translation?.translatedText) throw new Error('Invalid Google Cloud Translation response')
...
async function translateWithGoogleUnofficial(text, targetLang, sourceLang = 'auto') {
  const doTranslate = async (inputText) => {
    try {
      ...
      const response = await fetchWithTimeout(`${TRANSLATION_SERVICES.googleUnofficial.baseUrl}?${params}`)
      if (!response.ok) throw new Error(`Google Translate unofficial API error: ${response.status}`)

      const data = await response.json()
      if (!Array.isArray(data?.[0])) throw new Error('Invalid Google Translate unofficial response')
      ...
      return { success: false, error: error.message, provider: 'Google Translate (unofficial)' }
    }
  }

  return translateWithChunking(text, TRANSLATION_SERVICES.googleUnofficial.maxLength, doTranslate)
}
...
async function translateText(originalText, targetLang, sourceLang = 'auto', context = {}) {
  ...
  const byProvider = {
    ...
    googleUnofficial: () => translateWithGoogleUnofficial(text, normalizedTargetLang, normalizedSourceLang),
  ...
  // Authenticated Google Cloud is the primary provider whenever configured.
  addProvider('google', Boolean(GOOGLE_TRANSLATE_API_KEY))
  if (PREFERRED_PROVIDER && PREFERRED_PROVIDER !== 'google' && byProvider[PREFERRED_PROVIDER] && PREFERRED_PROVIDER !== 'ai' && PREFERRED_PROVIDER !== 'googleUnofficial') {
  ...
  addProvider('googleUnofficial', ALLOW_UNOFFICIAL_GOOGLE)
