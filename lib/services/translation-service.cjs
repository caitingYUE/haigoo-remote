...
async function translateJob(job) {
  ...
  // 规则：
  // - 如果总长度 < 50，只看绝对数量 (防止短描述被误判)
  // - 如果总长度 >= 50，要求中文比例至少 20% (宽松) 或者 中文字符数 > 100 (确保有足够的内容)
  // 很多技术JD会有大量英文术语，所以比例不能太高，但也不能只有页脚是中文。

  const isValid = chineseCharCount >= 20 && (totalLength < 100 || chineseRatio >= 0.1 || chineseCharCount >= 100);

  if (!isValid) {
    console.warn(`⚠️ 岗位 ${job.id} 翻译校验失败: 中文不足 (Count: ${chineseCharCount}, Ratio: ${chineseRatio.toFixed(2)})`);
