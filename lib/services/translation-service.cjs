...
async function fetchGoogleWithRetry(options) {
  ...
  for (let attempt = 0; attempt <= GOOGLE_MAX_RETRIES; attempt += 1) {
    try {
      ...
      await sleep(googleRetryDelay(response, attempt))
    } catch (error) {
      lastError = error
      if (attempt === GOOGLE_MAX_RETRIES) throw error
      await sleep(GOOGLE_RETRY_BASE_MS * (2 ** attempt) + Math.floor(Math.random() * 250))
    }
  }
 ...
 */
async function translateWithAIRequest(text, targetLang, sourceLang = 'auto') {
...
async function translateWithAI(text, targetLang, sourceLang = 'auto') {
  return translateWithChunking(text, TRANSLATION_SERVICES.ai.maxLength, (chunk) => (
    translateWithAIRequest(chunk, targetLang, sourceLang)
  ))
}
