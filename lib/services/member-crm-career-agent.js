...
export const RESUME_DIAGNOSIS_PROMPT_VERSION = 'resume-diagnosis-prompt-2026-08-15-v2'
...
function buildPrompt({ resumeText, localProfile, crmContext, userContext }) {
...
用户会直接看到 summary、strengths、careerPaths 和 clarificationQuestions。这些字段使用自然、简洁的中文，像职业顾问在当面交流：先说结论，再说依据；一句只表达一件事；不用“赋能、画像、抓手、闭环、底层逻辑、基于以上分析”等套话；不提 AI、模型、算法、系统或内部流程；不重复产品规则；信息不足时直接说明还需要了解什么。headline 不超过 24 个汉字，whyFit、strength.explanation 和 clarificationQuestions.reason 各不超过 80 个汉字。
...
用户确认的补充信息（可用于判断现实限制、职业方向和是否需要继续追问，但不能伪装成简历原文证据）：
${userContext || '未提供'}
...
export async function runResumeDiagnosis({ resumeText, crmContext = '', userContext = '' }) {
  ...
  const redactedUserContext = redactResumeForModel(userContext).slice(0, MAX_CONTEXT_CHARS)
  const localProfile = extractStructuredResume(redactedResume)
  const prompt = buildPrompt({ resumeText: redactedResume, localProfile, crmContext: redactedContext, userContext: redactedUserContext })
