...
export async function prepareCompanyUpdateEvents({ limit = 100 } = {}) {
  ...
  for (const event of events) {
    ...
    // Classify the actual role, not other teams mentioned in the description.
    const families = roleFamiliesForText(`${job.title || ''} ${job.category || ''}`)
    await neonHelper.query(
      `WITH event AS (
         ...
         JOIN career_watch_profiles p ON p.user_id=f.user_id
         WHERE f.status='active' AND f.in_app_enabled AND f.subscribed_at<=e.occurred_at
           AND mini_reminder_role_matches(to_jsonb(p),e.role_families,e.job_snapshot->>'title',e.job_snapshot->>'category')
         UNION ALL
      ...
      [event.event_id, JSON.stringify(families)])
  }
...
export async function listCompanyFollows(user) {
  const profiles = await neonHelper.query(
    `SELECT 1 FROM career_watch_profiles WHERE user_id=$1 AND status='active' AND in_app_enabled
      AND jsonb_array_length(COALESCE(role_families,'[]'::jsonb))>0`, [user.user_id])
  ...
  return {
    success: true,
    matchingPreferencesReady: Boolean(profiles?.length),
...
export async function setFollowNotifications({ user, companyId, enabled, templateStatus = 'not_requested', isMember = false }) {
  if (enabled) {
    const profiles = await neonHelper.query(
      `SELECT 1 FROM career_watch_profiles WHERE user_id=$1 AND status='active' AND in_app_enabled
        AND jsonb_array_length(COALESCE(role_families,'[]'::jsonb))>0`, [user.user_id])
    if (!profiles?.length) throw Object.assign(new Error('请先在 Match 中设置岗位类型或确认简历匹配方向'), {
      statusCode: 400, code: 'WATCH_ROLE_REQUIRED'
    })
  }
