...
export function computeCompanyMatch(profile, company, historyRows) {
  ...
  return {
    ...
    evidenceSummary: `${features.length} 条招聘证据，最近更新于 ${new Date(features[0].last_seen_at || Date.now()).toLocaleDateString('zh-CN')}`,
  ...
  try {
    return await neonHelper.query(`
      SELECT
        ...
        COALESCE(NULLIF(BTRIM(j.title), ''), '未命名岗位') AS title,
        ...
        COALESCE(j.published_at, j.created_at, j.updated_at, NOW()) AS first_seen_at,
        COALESCE(j.updated_at, j.published_at, j.created_at, NOW()) AS last_seen_at,
...
export async function getMatchFeed({ user, profileRow, isMember = false }) {
  ...
  let run = cached?.[0] ? { recommendations: recommendationItems(cached[0].recommendations), catalogVersion: cached[0].catalog_version, source: 'cached', generatedAt: cached[0].generated_at, fallbackUsed: String(cached[0].catalog_version || '').endsWith(':carry') } : null
