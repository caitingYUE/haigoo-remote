...
export async function recordCompanyUpdateEvent({ companyId, eventType = 'job_added', job = null }) {
  ...
  if (job) {
    ...
    const eligible = String(company?.status || '').toLowerCase() === 'active'
      && isEligibleDirectoryJob({ ...job, hiringEmail: job.hiringEmail || job.hiring_email || company?.hiring_email })
    if (!eligible) return { created: false, skipped: 'ineligible_job' }
  }
  const publicOpportunity = Boolean(job)
