...
export async function archiveJobSnapshot(job, { closed = false } = {}) {
  ...
  const rows = await neonHelper.query(
    ...
    [companyId, sourceJobId, hash(sourceUrl), title, String(job.description || ''), String(job.category || ''), JSON.stringify(roleFamilies), JSON.stringify(skills), String(job.industry || ''), String(job.experienceLevel || job.experience_level || ''), String(job.location || job.region || ''), String(job.timezone || ''), job.publishedAt || job.published_at || null, closed ? new Date().toISOString() : null, payloadHash, publicOpportunity ? 0.9 : 0.65, publicOpportunity]
  )
  return rows?.[0] || null
}
  ...
  await neonHelper.query(
    `WITH recipients AS (
       SELECT follows.user_id,
              BOOL_OR(wechat_enabled AND wechat_template_status = 'accepted') AS wechat_authorized
         FROM ${FOLLOWS_TABLE} follows
         JOIN users ON users.user_id = follows.user_id
        WHERE follows.company_id = $2 AND follows.status = 'active' AND follows.in_app_enabled = TRUE
          AND (
            (
              users.member_status IN ('active', 'pro', 'lifetime')
            ...
            OR EXISTS (
              SELECT 1
               ...
               WHERE snapshots.user_id = follows.user_id
                 AND ordinality <= 5
                 AND item->>'companyId' = follows.company_id
            )
          )
        GROUP BY follows.user_id
       UNION
       SELECT profiles.user_id,
              profiles.wechat_enabled AND profiles.wechat_template_status = 'accepted' AS wechat_authorized
         FROM career_watch_profiles profiles
         JOIN users ON users.user_id = profiles.user_id
        WHERE profiles.status = 'active' AND profiles.in_app_enabled = TRUE
          AND users.member_status IN ('active', 'pro', 'lifetime')
          AND COALESCE(users.member_type, '') NOT IN ('none', 'trial_week')
          AND (users.member_cycle_start_at IS NULL OR users.member_cycle_start_at <= NOW())
          AND (users.member_expire_at IS NULL OR users.member_expire_at > NOW())
          AND EXISTS (
            SELECT 1 FROM jsonb_array_elements_text($3::jsonb) event_role(value)
             WHERE COALESCE(profiles.role_families, '[]'::jsonb) ? event_role.value
          )
          AND (
            profiles.source_mode <> 'manual'
            OR jsonb_array_length(COALESCE(profiles.custom_role_terms, '[]'::jsonb)) = 0
            OR EXISTS (
              SELECT 1 FROM jsonb_array_elements_text(COALESCE(profiles.custom_role_terms, '[]'::jsonb)) role_term(value)
               WHERE $4 ILIKE '%' || role_term.value || '%'
            )
          )
     )
     INSERT INTO ${INBOX_TABLE} (user_id, event_id, notification_status)
     SELECT user_id, $1,
            CASE WHEN BOOL_OR(wechat_authorized) THEN 'pending' ELSE 'not_requested' END
       FROM recipients
      GROUP BY user_id
     ON CONFLICT (user_id, event_id) DO NOTHING`,
    [eventId, companyId, JSON.stringify(roleFamilies), `${job?.title || ''} ${job?.category || ''}`.trim()]
  )
  try {
...
async function deliverCompanyUpdateNotifications(eventId) {
  ...
  const rows = await neonHelper.query(
    `SELECT inbox.inbox_id, inbox.user_id, identities.openid, events.company_id,
            ...
            COALESCE(NULLIF(BTRIM(jobs.location), ''), NULLIF(BTRIM(jobs.region), '')) AS job_location,
            jobs.job_type,
            CASE
              WHEN follows.wechat_enabled = TRUE AND follows.wechat_template_status = 'accepted' THEN 'follow'
              WHEN profiles.wechat_enabled = TRUE AND profiles.wechat_template_status = 'accepted' THEN 'direction'
              ELSE ''
       ...
       JOIN users ON users.user_id = inbox.user_id
       LEFT JOIN ${FOLLOWS_TABLE} follows
         ON follows.user_id = inbox.user_id AND follows.company_id = events.company_id AND follows.status = 'active'
       LEFT JOIN career_watch_profiles profiles ON profiles.user_id = inbox.user_id AND profiles.status = 'active'
      WHERE inbox.event_id = $1 AND inbox.notification_status = 'pending'
        AND events.has_public_opportunity = TRUE
        AND (
          (
            follows.wechat_enabled = TRUE AND follows.wechat_template_status = 'accepted'
            AND (
              (
                users.member_status IN ('active', 'pro', 'lifetime')
                AND COALESCE(users.member_type, '') NOT IN ('none', 'trial_week')
              ...
              OR EXISTS (
                SELECT 1
                  FROM career_watch_feed_snapshots snapshots,
                       jsonb_array_elements(${fixedMatchRecommendationsSql('snapshots')}) WITH ORDINALITY recommendation(item, ordinality)
                 WHERE snapshots.user_id = inbox.user_id
                   AND ordinality <= 5
                   AND item->>'companyId' = events.company_id
              )
            )
          )
          OR (
            ...
            AND (users.member_cycle_start_at IS NULL OR users.member_cycle_start_at <= NOW())
            AND (users.member_expire_at IS NULL OR users.member_expire_at > NOW())
            AND profiles.wechat_enabled = TRUE
            AND profiles.wechat_template_status = 'accepted'
            AND EXISTS (
              SELECT 1 FROM jsonb_array_elements_text(COALESCE(events.role_families, '[]'::jsonb)) event_role(value)
               WHERE COALESCE(profiles.role_families, '[]'::jsonb) ? event_role.value
            )
            AND (
              profiles.source_mode <> 'manual'
              ...
              OR EXISTS (
                SELECT 1 FROM jsonb_array_elements_text(COALESCE(profiles.custom_role_terms, '[]'::jsonb)) role_term(value)
                 WHERE CONCAT_WS(' ', jobs.title, jobs.category) ILIKE '%' || role_term.value || '%'
              )
            )
          )
        )
  ...
  for (const row of rows) {
    ...
    if (status === 'sent' && row.authorization_source === 'follow') {
      await neonHelper.query(
    ...
    } else if (status === 'sent' && row.authorization_source === 'direction') {
      await neonHelper.query(
        `UPDATE career_watch_profiles
            SET wechat_enabled = FALSE, wechat_template_status = 'not_requested', updated_at = NOW()
          WHERE user_id = $1`,
        [row.user_id]
      )
    }
  ...
  if (active && !isMember) {
    const allowed = await neonHelper.query(
      `SELECT 1
         FROM career_watch_feed_snapshots snapshots,
              jsonb_array_elements(${fixedMatchRecommendationsSql('snapshots')}) WITH ORDINALITY recommendation(item, ordinality)
        WHERE snapshots.user_id = $1 AND ordinality <= 5 AND item->>'companyId' = $2
        LIMIT 1`,
      [user.user_id, id]
    )
    if (!allowed?.[0]) {
      throw Object.assign(new Error('免费用户只能关注本次 Match 的 5 家企业'), {
        statusCode: 403,
...
export async function setFollowNotifications({ user, companyId, enabled, templateStatus = 'not_requested' }) {
  ...
  const rows = await neonHelper.query(
    ...
    [Boolean(enabled), safeStatus, user.user_id, String(companyId)]
  )
  if (!rows?.[0]) {
    throw Object.assign(new Error('请先关注企业再开启微信提醒'), {
      statusCode: 403,
      code: 'SUBSCRIBE_AUTH_REQUIRED'
    })
  }
