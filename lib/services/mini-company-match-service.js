...
async function loadRecommendations(profileRow, userId) {
  ...
  await neonHelper.query(
    ...
    [profileRow.profile_id, userId, profileRow.version, catalogVersion, MINI_MATCH_ALGORITHM_VERSION, inputHash, JSON.stringify(recommendations)]
  )
...
export async function getMatchFeed({ user, profileRow, isMember = false }) {
  ...
  if (!run || String(run.catalogVersion) !== currentCatalog) run = await loadRecommendations(profileRow, userId)
