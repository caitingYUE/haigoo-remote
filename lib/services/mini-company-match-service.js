...
export function computeCompanyMatch(profile, company, historyRows) {
  ...
  const hasDirectRoleEvidence = direct > 0 || scored.some(({ score }) => Number(score?.breakdown?.titleMatch || 0) >= 68)
  const currentOpportunity = scored.some(({ feature, score }) => feature.is_public_opportunity && (
    ...
    (Number(score?.breakdown?.titleMatch || 0) >= 68 && Number(score?.breakdown?.roleTypeMatch || 0) >= 78)
  ))
  const band = total >= 80 ? 'high' : total >= 68 ? 'notable' : total >= 60 ? 'explore' : null
...
async function loadRecommendations(profileRow) {
  ...
  const recommendations = (companies || [])
    ...
    .slice(0, 12)
  const inputHash = hash({ profile: profileRow.profile_hash, catalogVersion, algorithm: MINI_MATCH_ALGORITHM_VERSION })
  await neonHelper.query(
    ...
    [profileRow.profile_id, profileRow.user_id, profileRow.version, catalogVersion, MINI_MATCH_ALGORITHM_VERSION, inputHash, JSON.stringify(recommendations)]
  )
  await neonHelper.query(`UPDATE mini_career_profiles SET last_match_at = NOW() WHERE profile_id = $1`, [profileRow.profile_id])
  return { recommendations, catalogVersion, source: 'recomputed', generatedAt: new Date().toISOString() }
}
...
export async function getMatchFeed({ user, profileRow, isMember = false }) {
  ...
  const cached = await neonHelper.query(
    ...
    [user.user_id, profileRow.profile_id, profileRow.version, MINI_MATCH_ALGORITHM_VERSION]
  )
  ...
  const currentCatalog = await getCatalogVersion()
  if (!run || String(run.catalogVersion) !== currentCatalog) run = await loadRecommendations(profileRow)

  const [follows, exposures, updates] = await Promise.all([
    neonHelper.query(`SELECT company_id, status, in_app_enabled, wechat_enabled FROM ${FOLLOWS_TABLE} WHERE user_id = $1 AND status = 'active'`, [user.user_id]),
    neonHelper.query(`SELECT company_id, show_count, last_shown_at, last_opened_at, dismissed_at FROM ${EXPOSURES_TABLE} WHERE user_id = $1`, [user.user_id]),
    neonHelper.query(
  ...
  const eligible = pool.filter((item) => {
    const exposure = exposureMap.get(String(item.companyId))
    if (!exposure?.last_shown_at) return true
    if (item.hasUpdate) return true
    return daysSince(exposure.last_shown_at) >= 7
  })
  const chosen = []
  const first = pool[0]
  if (first && eligible.some((item) => item.companyId === first.companyId)) chosen.push(first)
  for (const item of eligible) {
    if (chosen.some((current) => current.companyId === item.companyId)) continue
    chosen.push(item)
    if (chosen.length >= limit) break
  }
  for (const item of chosen) {
    await neonHelper.query(
      `INSERT INTO ${EXPOSURES_TABLE} (user_id, company_id, show_count, last_shown_at, updated_at)
       VALUES ($1, $2, 1, NOW(), NOW())
       ON CONFLICT (user_id, company_id) DO UPDATE SET show_count = ${EXPOSURES_TABLE}.show_count + 1, last_shown_at = NOW(), updated_at = NOW()`,
      [user.user_id, item.companyId]
    )
  }
  return {
    profile: {
...
export async function recordMatchFeedback({ user, companyId, action }) {
  ...
  if (safeAction === 'dismissed') {
    await neonHelper.query(`UPDATE ${EXPOSURES_TABLE} SET dismissed_at = NOW(), updated_at = NOW() WHERE user_id = $1 AND company_id = $2`, [user.user_id, companyId])
  } else {
    await neonHelper.query(`UPDATE ${EXPOSURES_TABLE} SET last_opened_at = CASE WHEN $3 = 'opened' THEN NOW() ELSE last_opened_at END, updated_at = NOW() WHERE user_id = $1 AND company_id = $2`, [user.user_id, companyId, safeAction])
  }
  return { success: true }
}
