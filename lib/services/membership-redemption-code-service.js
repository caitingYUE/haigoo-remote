...
export async function reconcileUserMembershipEntitlements(userId) {
  if (!userId || !(await isMembershipRedemptionReady())) {
...
export async function rebasePendingMembershipEntitlements(userId, baseAt) {
  if (!userId || !baseAt || !(await isMembershipRedemptionReady())) return 0
  try {
    const rows = await neonHelper.query(
...
export async function getUpcomingMembershipEntitlements(userId) {
  if (!userId || !(await isMembershipRedemptionReady())) return []
  try {
    const rows = await neonHelper.query(
      `SELECT segment_id, member_type, duration_months, starts_at, ends_at
         FROM membership_entitlement_segments
        WHERE user_id = $1
          AND activated_at IS NULL
          AND superseded_at IS NULL
...
export async function listMembershipCodes({
  ...
  const [rows, countRows, summaryRows, batchRows, channelRows] = await Promise.all([
    neonHelper.query(
      `SELECT c.code_id,
              ...
              c.voided_at, c.void_reason, b.batch_id, b.name AS batch_name,
  ...
  return {
    ...
    summary: summaryRows?.[0] || { total: 0, unused: 0, used: 0, expired: 0, voided: 0, starter: 0, half_year: 0, annual: 0 },
