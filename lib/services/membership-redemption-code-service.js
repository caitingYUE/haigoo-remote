...
import { deriveMembershipCapabilities, resolveMemberTypeFromUser } from '../shared/membership.js'
...
// Keep the per-order ledger for refunds and quota cycles, but present uninterrupted
// monthly/quarterly membership as one validity period to the customer.
export function continuousMembershipExpireAt(user, upcoming = []) {
  let expiresAt = user?.memberExpireAt || user?.member_expire_at || null
  const memberType = resolveMemberTypeFromUser(user)
  if (!deriveMembershipCapabilities(user).isActive || !['starter', 'quarter'].includes(memberType)) return expiresAt
  for (const segment of upcoming) {
    if (segment.memberType !== memberType || new Date(segment.startsAt).getTime() !== new Date(expiresAt).getTime()) break
    if (new Date(segment.expiresAt).getTime() <= new Date(expiresAt).getTime()) break
    expiresAt = segment.expiresAt
  }
  return expiresAt
}

export async function getContinuousMembershipExpireAt(user) {
  if (!user || !deriveMembershipCapabilities(user).isActive || !['starter', 'quarter'].includes(resolveMemberTypeFromUser(user))) {
    return user?.memberExpireAt || user?.member_expire_at || null
  }
  return continuousMembershipExpireAt(user, await getUpcomingMembershipEntitlements(user.user_id || user.userId))
}
