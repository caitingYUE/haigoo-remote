...
export function determineExperienceLevel(title, description) {
    ...
    // Priority 2: Senior (Explicitly check Senior BEFORE Lead to avoid "Senior Lead" confusion, though usually Lead > Senior)
    // Actually, usually Lead > Senior. But we must ensure "Senior Product Designer" is Senior.
    // If we have "Senior Lead", it's Lead.
    // But "Product Manager" should NOT be Lead just because of "Manager".

    // Priority 2: Lead / Principal / Staff / Architect
    // Removed 'manager' to avoid misclassifying Product Manager / Engineering Manager (unless it's Engineering Manager, which IS Lead)
    // Added specific manager roles that are Lead
    if (/\b(lead|principal|staff|architect|engineering manager|team lead|tech lead)\b/.test(titleLower)) {
        return 'Lead';
    }
    ...
    // Do NOT fallback to Entry based on description if Title didn't say so. 
    // Many JD mentions "mentoring junior developers", which triggers Entry.
    // So we default to Mid.
...
export function classifyCompany(companyName, description = '') {
    ...
    // Industry Classification - values must match canonical DB values in trusted_companies.industry
    let industry = '其他';
    ...
    if (/\b(ai|artificial intelligence|machine learning|llm|gpt|neural|deep learning)\b/.test(text)) {
        ...
        industry = '金融/Fintech';
    } else if (/\b(ecommerce|e-commerce|shop|marketplace|dtc|shopping|apparel|fashion)\b/.test(text)) {
        industry = '电子商务';
    } else if (/\b(game|gaming|esports|video game|mobile game|console|unity|unreal)\b/.test(text)) {
        industry = '游戏';
    } else if (/\b(media|entertainment|streaming|music|podcast|video|content|news|broadcast)\b/.test(text)) {
        industry = '媒体/娱乐';
    } else if (/\b(health|medical|biotech|pharma|care|patient|doctor|hospital|fitness|wellness)\b/.test(text)) {
        industry = '大健康/医疗';
    } else if (/\b(education|edtech|learning|school|university|course|training|tutor)\b/.test(text)) {
        industry = '教育';
    } else if (/\b(hardware|iot|device|robotics|chip|semiconductor|manufacturing|drone|embedded)\b/.test(text)) {
        industry = '硬件/物联网';
    } else if (/\b(consumer|lifestyle|travel|food|beverage|hospitality|restaurant)\b/.test(text)) {
        industry = '消费生活';
    } else if (/\b(saas|enterprise|b2b|crm|erp|productivity|workflow|collaboration)\b/.test(text)) {
        industry = '企业服务/SaaS';
    } else if (/\b(software|internet|technology|platform|app|cloud|mobile app|social|network)\b/.test(text)) {
        // General tech fallback - only if no more specific industry matched
        industry = '互联网/软件';
    }
...
export function classifyRegion(location) {
  ...
  // 默认: 如果完全无法判断，或者地点为空，或者只有 Unspecified 等无意义词汇
  // 用户要求：没有特别标识的，统一判断为【不限地点】(both)
  return 'both'
}

/**
 * 判断是否为明确的海外岗位（非中国/APAC区域）
 * 用于爬虫过滤：去除明显不适合中国申请者的岗位
 * @param {string} location 
 * @returns {boolean}
 */
export function isExplicitlyOverseas(location) {
  const loc = (location || '').toLowerCase().trim()
  if (!loc) return false

  // Check strict overseas keywords
  const isOverseas = OVERSEAS_KEYWORDS.some(k => loc.includes(k)) || 
                     ['us', 'uk', 'eu'].some(k => new RegExp(`\\b${k}\\b`, 'i').test(loc));
  ...
  const isGreaterChina = GREATER_CHINA_KEYWORDS.some(k => loc.includes(k))
  const isAPAC = APAC_KEYWORDS.some(k => loc.includes(k))
  const isAsia = ASIA_COUNTRY_KEYWORDS.some(k => loc.includes(k))

  // If it mentions China/APAC/Asia, it's not "strictly" overseas (could be 'both' or Asian)
  // 2026-01-16 Update: Removed isAsia from protection list to filter out specific non-China Asian countries (e.g. India, Philippines)
  if (isMainland || isGreaterChina || isAPAC) {
    return false
  }

  // If it's overseas and NOT China/APAC/Asia, then it's explicitly overseas
  return isOverseas
}

/**
 * 从文本中提取时区信息
 * @param {string} text - 包含时区的文本 (Location, Description, etc.)
 * @returns {string | null} 提取到的时区 (e.g. "GMT+8", "PST", "EST/PST")
 */
export function extractTimezone(text) {
    if (!text) return null;
    const cleanText = text.replace(/<[^>]*>?/gm, ' ').replace(/\s+/g, ' ').trim();
    
    // 1. Common Timezone Abbreviations
    // PST, PDT, EST, EDT, CST, CDT, MST, MDT, GMT, UTC, CET, CEST, BST, IST, JST, SGT, HKT, AEST, AEDT
    const tzRegex = /\b(PST|PDT|EST|EDT|CST|CDT|MST|MDT|GMT|UTC|CET|CEST|BST|IST|JST|SGT|HKT|AEST|AEDT)\b/gi;
    const matches = cleanText.match(tzRegex);
    if (matches) {
        // Deduplicate and join
        const uniqueTz = [...new Set(matches.map(m => m.toUpperCase()))];
        return uniqueTz.join(', ');
    }

    // 2. GMT/UTC Offsets (e.g. GMT+8, UTC-5)
    const offsetRegex = /\b(GMT|UTC)\s*[+-]\s*\d{1,2}(?::\d{2})?\b/gi;
    const offsetMatches = cleanText.match(offsetRegex);
    if (offsetMatches) {
        const uniqueOffsets = [...new Set(offsetMatches.map(m => m.replace(/\s+/g, '').toUpperCase()))];
        return uniqueOffsets.join(', ');
    }

    // 3. Region-based Timezones (Americas, EMEA, APAC)
    // Sometimes jobs say "Americas Timezones" or "EMEA Friendly"
    const regionTz = [];
    if (/\b(Americas|North America|South America|LATAM|US|Canada)\b/i.test(cleanText) && /\b(Timezone|Hours|Friendly)\b/i.test(cleanText)) {
        regionTz.push('Americas');
    }
    if (/\b(EMEA|Europe|UK|Africa)\b/i.test(cleanText) && /\b(Timezone|Hours|Friendly)\b/i.test(cleanText)) {
        regionTz.push('EMEA');
    }
    if (/\b(APAC|Asia|Pacific)\b/i.test(cleanText) && /\b(Timezone|Hours|Friendly)\b/i.test(cleanText)) {
        regionTz.push('APAC');
    }
    
    if (regionTz.length > 0) {
        return regionTz.join(', ');
    }
