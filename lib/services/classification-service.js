...
export function classifyCompany(companyName, description = '') {
    ...
    if (/\b(ai|artificial intelligence|machine learning|data|analytics|big data)\b/.test(text)) {
        industry = '人工智能/大数据';
        tags.push('AI', 'Data');
    } else if (/\b(crypto|blockchain|web3|defi|nft|bitcoin|ethereum)\b/.test(text)) {
        industry = 'Web3/区块链';
        tags.push('Web3', 'Blockchain');
    } else if (/\b(saas|software|platform|cloud|devops|api)\b/.test(text)) {
        industry = '企业服务/SaaS';
        tags.push('SaaS', 'Software');
    } else if (/\b(fintech|finance|banking|trading|payment|wallet)\b/.test(text)) {
        industry = '金融科技';
        tags.push('Fintech');
    } else if (/\b(ecommerce|e-commerce|retail|shop|marketplace)\b/.test(text)) {
        industry = '电商/零售';
        tags.push('E-commerce');
    } else if (/\b(game|gaming|esports)\b/.test(text)) {
        industry = '游戏/娱乐';
        tags.push('Gaming');
    } else if (/\b(health|medical|biotech|pharma)\b/.test(text)) {
        industry = '医疗健康';
        tags.push('Healthcare');
    } else if (/\b(education|edtech|learning|school|university)\b/.test(text)) {
        industry = '教育科技';
        tags.push('EdTech');
    }
