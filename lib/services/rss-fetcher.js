...
async function fetchAndParseFeed(source) {
  try {
    ...
    $('item').each((i, elem) => {
      ...
      let company = $item.find('company').text().trim() || 
                    $item.find('dc\\:creator').text().trim() || 
                    $item.find('creator').text().trim();

      // Fallback: Try to extract from title (e.g. "Company: Job Title" or "Job Title at Company")
      if (!company) {
        if (title.includes(':')) {
           // "Company: Job Title"
           company = title.split(':')[0].trim();
        } else if (title.includes(' at ')) {
           // "Job Title at Company" - risky, but common
           const parts = title.split(' at ');
           if (parts.length > 1) {
             company = parts[parts.length - 1].trim();
           }
        } else if (title.includes(' - ')) {
           // "Job Title - Company" or "Company - Job Title"
           // Hard to know which one, but WWR uses "Job Title: Company" sometimes? 
           // Actually WWR uses "Company: Job Title"
        }
      }

      // Specific handling for WeWorkRemotely if needed (they usually put company in title)
      if (source.name === 'WeWorkRemotely' && !company && title.includes(':')) {
        company = title.split(':')[0].trim();
      }
