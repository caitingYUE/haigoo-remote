...
import { MAX_SUBSCRIPTION_TOPICS } from '../shared/subscription-limits.js';

const SUBSCRIPTION_LIMIT_ERROR = '已触达订阅上限，无法继续添加';
...
function normalizeTopic(topic) {
  return [...new Set(String(topic || '').split(',').map(item => item.trim()).filter(Boolean))].join(',');
}
...
function assertSubscriptionTopicLimit(topics, customTopics = []) {
  const standardCount = (Array.isArray(topics) ? topics : [])
    .filter(item => String(item || '').trim().toLowerCase() !== 'other')
    .length;
  if (standardCount + customTopics.length > MAX_SUBSCRIPTION_TOPICS) {
    const error = new Error(SUBSCRIPTION_LIMIT_ERROR);
    error.code = 'SUBSCRIPTION_TOPIC_LIMIT';
    throw error;
  }
}
...
export const subscriptionsService = {
  ...
  async add(email, topic) {
    ...
    try {
      const normalizedTopics = normalizeTopic(topic).split(',').filter(Boolean);
      assertSubscriptionTopicLimit(normalizedTopics);
      const result = await neonHelper.query(
    ...
    const result = await neonHelper.query(
      `SELECT *
        ...
        ORDER BY
          ...
          CASE WHEN status = 'bounced' THEN 1 ELSE 0 END,
  ...
  async upsertForUser(user, { topics, customTopic, customTopics, status }) {
    if (!neonHelper.isConfigured) throw new Error('Database not configured');
    ...
    assertSubscriptionTopicLimit(normalizedTopics, normalizedCustomTopics);
    ...
    if (existing?.[0]?.subscription_id) {
      ...
      const result = await neonHelper.query(
        `UPDATE subscriptions
          ...
          RETURNING *`,
        [userId, email, topic, JSON.stringify(preferences), nextStatus, existing[0].subscription_id]
      );
      return result[0] || null;
    }
