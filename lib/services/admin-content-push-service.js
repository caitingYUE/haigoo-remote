...
function buildLocalSummary(job = {}, maxLength = 110) {
  const seeds = [
    job.summary,
    job.description
  ]
...
export async function getXiaohongshuJobList(query = {}) {
  ...
  const items = (rows || []).map((row) => {
    ...
    return {
      ...
      location: normalizeText(row.location, '地点待补充'),
      category: normalizeText(row.category, '角色待补充'),
      jobType: normalizeText(row.job_type, '类型待补充'),
      experienceLevel: normalizeText(row.experience_level, '级别待补充'),
      description: normalizeText(row.description),
      updatedAt: row.updated_at || row.created_at || null,
      shareUrl: buildShareUrl(row.job_id),
      employeeCount: normalizeText(row.employee_count, '待补充'),
      address: normalizeText(row.address, '待补充'),
      foundedYear: normalizeText(row.founded_year ? `${row.founded_year}年` : '', '待补充'),
      companyRating: normalizeText(row.company_rating, '待补充'),
      industry: normalizeText(row.industry || row.trusted_industry, '待补充'),
...
async function callBailianSummary(job = {}, localSummary = '') {
  ...
  const response = await fetch('https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: '你是一个招聘运营文案压缩助手，只负责将岗位详情压缩成适合社媒海报展示的简洁中文摘要。'
        },
        {
          role: 'user',
          content: prompt
...
export async function generateXiaohongshuSummary(payload = {}) {
  ...
  const localSummary = buildLocalSummary(job, 110)
  const cacheKey = [
    SUMMARY_TEMPLATE_VERSION,
    job.id || 'unknown',
    job.updatedAt || 'unknown',
    Buffer.from(localSummary).toString('base64').slice(0, 32)
  ].join(':')
  ...
  if (shouldUseAiSummary({ localSummary, description: job.description })) {
    try {
      result = {
        ...(await callBailianSummary(job, localSummary)),
        cacheHit: false
      }
    } catch (error) {
      result = {
