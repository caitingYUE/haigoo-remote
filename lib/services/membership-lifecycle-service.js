import neonHelper from '../../server-utils/dal/neon-helper.js'
import {
  buildMembershipExpiredEventKeySql,
  ensureMembershipNotificationLogTable,
  notifyMembershipActivated,
  notifyMembershipExpired
} from './membership-notification-service.js'
import {
  isMembershipRedemptionEnabled,
  isMembershipRedemptionReady,
  isMembershipEntitlementScheduleReady,
  reconcileDueMembershipEntitlements
} from './membership-redemption-code-service.js'

function hydrateUserRecord(user) {
  if (!user) return null
  const hydrated = { ...user }
  if (hydrated?.roles && typeof hydrated.roles === 'string') {
    try { hydrated.roles = JSON.parse(hydrated.roles) } catch (_error) { hydrated.roles = {} }
  }
  if (hydrated?.profile && typeof hydrated.profile === 'string') {
    try { hydrated.profile = JSON.parse(hydrated.profile) } catch (_error) { hydrated.profile = {} }
  }
  return hydrated
}

export async function sweepExpiredMemberships({ limit = 200 } = {}) {
  if (!neonHelper?.isConfigured) {
    return { success: true, scanned: 0, expired: 0, notified: 0, skipped: 0, scheduledActivated: 0 }
  }

  const parsedLimit = Number(limit)
  const safeLimit = Number.isFinite(parsedLimit)
    ? Math.min(2000, Math.max(1, Math.floor(parsedLimit)))
    : 200
  const redemptionReady = await isMembershipRedemptionReady()
  const entitlementScheduleReady = await isMembershipEntitlementScheduleReady()
  if (isMembershipRedemptionEnabled() && !redemptionReady) {
    return {
      success: true,
      scanned: 0,
      expired: 0,
      notified: 0,
      skipped: 0,
      scheduledActivated: 0,
      deferred: true,
      reason: 'membership_redemption_schema_not_ready'
    }
  }
  const scheduled = entitlementScheduleReady
    ? await reconcileDueMembershipEntitlements({ limit: safeLimit })
    : { scanned: 0, activated: 0, activatedUserIds: [] }
  for (const userId of scheduled.activatedUserIds || []) {
    try {
      const rows = await neonHelper.select('users', { user_id: userId })
      const user = hydrateUserRecord(rows?.[0])
      if (user) await notifyMembershipActivated(user)
    } catch (error) {
      console.error('[membership-lifecycle] Failed to notify scheduled membership activation:', userId, error)
    }
  }

  await ensureMembershipNotificationLogTable()
  const eventKeySql = buildMembershipExpiredEventKeySql({
    userIdColumn: 'u.user_id',
    expireAtColumn: 'u.member_expire_at'
  })
  const pendingEntitlementGuardSql = entitlementScheduleReady
    ? `AND NOT EXISTS (
          SELECT 1
            FROM membership_entitlement_segments segment
           WHERE segment.user_id = u.user_id
             AND segment.activated_at IS NULL
             AND segment.superseded_at IS NULL
             AND segment.starts_at <= NOW()
        )`
    : ''

  const rows = await neonHelper.query(
    `SELECT u.*
       FROM users u
       LEFT JOIN membership_notification_log log
         ON log.event_key = ${eventKeySql}
      WHERE u.member_status IN ('active', 'pro', 'lifetime', 'expired')
        AND u.member_expire_at IS NOT NULL
        AND u.member_expire_at <= NOW()
        ${pendingEntitlementGuardSql}
        AND log.event_key IS NULL
        AND NOT EXISTS (
          SELECT 1
            FROM notifications n
           WHERE n.user_id = u.user_id
             AND n.type = 'membership_expired'
             AND n.content LIKE '%' || TO_CHAR(u.member_expire_at AT TIME ZONE 'Asia/Shanghai', 'YYYY/MM/DD HH24:MI') || '%'
        )
      ORDER BY u.member_expire_at ASC
      LIMIT $1`,
    [safeLimit]
  )

  let expired = 0
  let notified = 0

  for (const rawRow of rows || []) {
    const user = hydrateUserRecord(rawRow)
    if (!user?.user_id) continue

    let updatedUser = user
    if (['active', 'pro', 'lifetime'].includes(String(user.member_status || '').toLowerCase())) {
      const updateResult = await neonHelper.query(
        `UPDATE users
            SET member_status = 'expired',
                updated_at = NOW()
          WHERE user_id = $1
            AND member_status IN ('active', 'pro', 'lifetime')
            AND member_expire_at IS NOT NULL
            AND member_expire_at <= NOW()
        RETURNING *`,
        [user.user_id]
      )

      updatedUser = hydrateUserRecord(updateResult?.[0] || null)
      if (!updatedUser) continue
    }

    expired += 1

    try {
      const didNotify = await notifyMembershipExpired(updatedUser)
      if (didNotify) notified += 1
    } catch (error) {
      console.error('[membership-lifecycle] Failed to notify expired member:', updatedUser.user_id, error)
    }
  }

  return {
    success: true,
    scanned: (rows || []).length,
    expired,
    notified,
    skipped: Math.max(0, (rows || []).length - expired),
    scheduledActivated: scheduled.activated || 0
  }
}
