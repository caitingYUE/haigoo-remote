...
2. **Keywords for Word Cloud**:
   - Extract **60-80 keywords** (Mix of single words and short 2-3 word phrases).
   - **CRITICAL: NO DUPLICATES**. Do not repeat the same word (e.g., do not output "Learning" 5 times). Ensure high variety.
...
export async function generateChristmasTree(resumeText) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 55000); // 55s timeout (safe for Vercel 60s)

    try {
        console.log(`[ChristmasService] Sending request to AI API... (Timeout: 55s)`);
        ...
        console.log(`[ChristmasService] AI Request completed in ${duration}ms. Status: ${response.status}`);
