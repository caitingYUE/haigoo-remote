...
function calculateTextSimilarity(text1, text2, titleText = '', prioritizedTerms = []) {
    ...
    // Jaccard 相似度本身偏低，经过放大和平滑处理
    const boostedJaccard = Math.min(1, jaccard * 4);
    const rawScore = boostedJaccard * 0.85 + titleBonus * 0.15;
    const score = Math.sqrt(rawScore) * 100;
    return Math.max(0, Math.min(100, Math.round(score)));
}
...
function applyHardConstraintCaps(userProfile, jobFeatures, score, skillScore) {
    ...
    // 放宽惩罚：如果完全没用匹配到技能，分数上限控制在 62 (中匹配之下)，而不是 58
...
function calculateMatchScore(userProfile, jobFeatures) {
    ...
    let baseTone = 50;
    const pref = userProfile.preferences || {};
    const hasCategoriesPref = pref.categories && pref.categories.length > 0;

    if (hasCategoriesPref) {
        // 1. Title/Category 命中的绝对主因 (+30分)
        const jobCategory = String(jobFeatures.category || '').toLowerCase();
        const jobTitle = String(jobFeatures.title || '').toLowerCase();
        const matchedCategory = pref.categories.some(c => {
            const lp = String(c).toLowerCase().trim();
            if (!lp) return false;
            return jobCategory.includes(lp) || jobTitle.includes(lp);
        });
        if (matchedCategory) baseTone += 30;

        // 2. 级别命中的补充基准 (+10分)
        if (pref.levels && pref.levels.length > 0) {
            const jobLevel = String(jobFeatures.experienceLevel || '').toLowerCase();
            const matchedLevel = pref.levels.some(l => {
                const lp = String(l).toLowerCase().trim();
                if (!lp) return false;
                return jobLevel.includes(lp) || lp === jobLevel;
            });
            if (matchedLevel) baseTone += 10;
        }
    }
    ...
    if (hasCategoriesPref) {
        // 如果有明确求职意向，大方向（50基础+30+10 = 90分）+ 细节分（10分）
        // detailScore 满分是 100
        const detailScore = (skillScore * 0.45) + (keywordScore * 0.35) + (expScore * 0.20);
        // 按 10% 比例加入基调分
        unCappedScore = Math.min(100, Math.round(baseTone + (detailScore * 0.1)));
    } else {
        // 如果用户没有偏好，为了归一化到 50-100，传统算完之后除以2加上50
        ...
        unCappedScore = 50 + Math.round(traditionalScore / 2);
    }
