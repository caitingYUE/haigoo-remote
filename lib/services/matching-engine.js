...
function calculateTitleMatch(userProfile, jobFeatures) {
    ...
    if (!userSignals.length) {
        const fallbackFamilies = countOverlap(toArray(userProfile.roleFamilies), toArray(jobFeatures.roleFamilies));
        if (fallbackFamilies > 0) return 82;
        return 18;
    }
...
function calculateRoleTypeMatch(userProfile, jobFeatures) {
    ...
    if (familyOverlap > 0) {
        return clampScore(94 + Math.min(6, familyOverlap * 3));
    }
    ...
    const sharedTerms = countOverlap(userTerms, jobTerms);
    if (sharedTerms >= 2) return 74;
    if (sharedTerms === 1) return 52;
    return 20;
}
...
async function getUserProfile(userId) {
    try {
        ...
        const resumeKeywords = extractKeywords(`${profile.targetRole || ''} ${profile.title || ''} ${resumeText || ''}`).slice(0, 20);
        const roleSignals = uniqueStrings([
            ...
            ...normalizedSkills.slice(0, 12),
            ...resumeKeywords.slice(0, 8)
        ], 16);
...
function buildJobFeaturesFromRow(jobRow) {
    ...
    const roleSignals = uniqueStrings([title, category, industry, ...rawSkills.slice(0, 10)], 14);
    const roleTerms = buildRoleTerms(roleSignals);
    const roleFamilies = enrichRoleFamilies(inferRoleFamilies(roleSignals), roleTerms);
...
function applyHardConstraintCaps(userProfile, jobFeatures, score, scoreParts = {}) {
    ...
    if (hasStrictLocations && !jobFeatures.isRemote) {
        ...
        if (!matched) {
            cappedScore = Math.min(cappedScore, 56);
            constraintFlags.strictLocationMismatch = true;
        }
    }
    ...
    if (!hasStrongRoleAlignment && userProfile.skills?.length && jobFeatures.skills?.length >= 3 && skillScore < 20) {
        cappedScore = Math.min(cappedScore, 57);
        constraintFlags.severeSkillMismatch = true;
    }

    if (hasStrongRoleAlignment) {
        cappedScore = Math.max(cappedScore, titleScore >= 88 ? 78 : 74);
    }
...
function calculateMatchScore(userProfile, jobFeatures) {
    ...
    let trueScore = detailScore;

    if (titleScore >= 92 && roleTypeScore >= 94) {
        trueScore = Math.max(trueScore, 92);
    } else if (roleTypeScore >= 94 && titleScore >= 80) {
        trueScore = Math.max(trueScore, 88);
    } else if (roleTypeScore >= 90 && skillScore >= 45) {
        trueScore = Math.max(trueScore, 84);
    ...
    return {
        ...
        breakdown: {
            ...
            rawDetailScore: clampScore(detailScore)
        }
    };
}
        ...
        if (result?.[0]) {
            ...
            if (Date.now() - cachedAt < CACHE_TTL) {
                ...
                return {
                    ...
                    breakdown: cached.score_breakdown || cached.match_details || {},
