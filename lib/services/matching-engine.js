...
function calculateMatchScore(userProfile, jobFeatures) {
    ...
    if (hasCategoriesPref) {
        ...
        if (matchedCategory) baseTone = 55;
    ...
    if (hasCategoriesPref) {
        // 如果有明确求职意向，使用新版【大方向打底 + 细节加分】策略
        const detailScore = (skillScore * 0.45) + (keywordScore * 0.35) + (expScore * 0.20);
        // 最高 65 + 100*0.45 = 110，封顶 100
        unCappedScore = Math.min(100, Math.round(baseTone + (detailScore * 0.45)));
    } else {
        // 退回传统计算（防无明确偏好的用户吃亏）
        unCappedScore = Math.round(
