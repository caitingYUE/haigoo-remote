...
/**
 * Parse LinkedIn URL to extract name/handle
 */
function parseLinkedInHandle(url) {
    try {
        ...
        // Handle /in/username
        if (path.includes('/in/')) {
            const parts = path.split('/in/')[1].split('/')[0];
            // Clean up: remove id numbers if they are at the end (e.g. john-doe-12345)
            // But sometimes the id is important. 
            // Heuristic: Replace dashes with spaces, capitalize words
            let name = parts.replace(/-/g, ' ');
            
            // Remove trailing numbers if they look like an ID (more than 4 digits)
            // e.g. "john doe 12345678" -> "john doe"
            // But "john doe 123" might be the handle. 
            // Let's just return the clean name for display.
        ...
        if (path.includes('/company/')) {
            const parts = path.split('/company/')[1].split('/')[0];
...
/**
 * Clean extracted email from common artifacts
 */
function cleanExtractedEmail(email) {
    if (!email) return null;
    let cleaned = email.toLowerCase().trim();

    // 1. Handle encoding artifacts at start (e.g. u003e -> >, 3a -> :)
    // Common unicode escape sequences or hex codes that might be matched
    cleaned = cleaned.replace(/^(u003[ce]|u002[27]|u00a0|3a|22|20|0a|0d|nbsp)/, '');
    
    // Also remove leading special chars if they remain (e.g. >contact@...)
    cleaned = cleaned.replace(/^[^a-z0-9]+/, '');

    // 2. Handle merged suffixes
    // Heuristic: TLDs shouldn't end with these common HTML/text words
    const suffixPatterns = [
        /full$/, /bold$/, /strong$/, /span$/, /div$/, /br$/, /li$/, /ul$/, /tr$/, /td$/, 
        /click$/, /here$/, /contact$/, /email$/, /address$/, /info$/, /tel$/, /fax$/,
        /font$/, /class$/, /style$/, /src$/, /href$/
    ];

    // Split to get TLD
    const lastDotIndex = cleaned.lastIndexOf('.');
    if (lastDotIndex !== -1) {
        let tld = cleaned.substring(lastDotIndex + 1);
        let domainPart = cleaned.substring(0, lastDotIndex);
        
        for (const pattern of suffixPatterns) {
            if (pattern.test(tld)) {
                const newTld = tld.replace(pattern, '');
                // If trimming leaves a valid TLD length (2+ chars), accept it
                // e.g. comfull -> com
                if (newTld.length >= 2) {
                    cleaned = domainPart + '.' + newTld;
                    tld = newTld; // update
                }
            }
        }
    ...
    const finalTld = parts[parts.length - 1];
    if (finalTld.length < 2 || finalTld.length > 15) return null;
 ...
 * @param {Object} options - { useGoogle: boolean }
 */
export async function mineCompanyContacts(input, options = {}) {
    console.log(`[ContactMiner] Starting mining for: ${input} (Options: ${JSON.stringify(options)})`);
    ...
    const socialLinks = new Map(); // Key: url, Value: { platform, url, handle, context }
    ...
    // Advanced Feature: Manual Trigger Only + Daily Limit Check
    if (options.useGoogle) {
        const allowed = await systemSettingsService.checkAndIncrementGoogleSearchUsage();
        if (allowed) {
            console.log('[ContactMiner] Google Search authorized (within daily limit)');
            
            // 1. Site-specific search (Existing)
            const siteQuery = `site:${domain} (contact OR about OR team OR "email us")`;
            // 2. Broad Email Pattern Search (New - Finds mentions on external sites like GitHub, SlideShare, etc.)
            const emailQuery = `"${domain}" email -site:${domain}`;

            // Execute in parallel if possible, but limit to 10 results total to save quota/time
            const [siteResults, emailResults] = await Promise.all([
                searchGoogle(siteQuery, 5),
                searchGoogle(emailQuery, 5)
            ]);

            const allResults = [...(siteResults || []), ...(emailResults || [])];

            if (allResults.length > 0) {
                console.log(`[ContactMiner] Found ${allResults.length} pages via Google Search`);
            ...
            console.warn('[ContactMiner] Google Search skipped: Daily limit (100) reached');
        }
    } else {
        console.log('[ContactMiner] Google Search skipped: Not requested');
    }
    ...
    while (urlsToCrawl.length > 0 && pagesCrawled < MAX_PAGES_TO_CRAWL) {
        ...
        matches.forEach(rawEmail => {
            ...
            if (/\.(png|jpg|jpeg|gif|css|js|svg|webp|woff|woff2|ttf|eot|mp4|mp3|wav|bmp|ico)$/.test(email)) return;
        ...
        while ((obfuscatedMatch = OBFUSCATED_EMAIL_REGEX.exec(bodyText)) !== null) {
            let email = `${obfuscatedMatch[1]}@${obfuscatedMatch[2]}.${obfuscatedMatch[3]}`;
            email = cleanExtractedEmail(email);
        ...
        while ((obfuscatedMatch = OBFUSCATED_EMAIL_SIMPLE_REGEX.exec(bodyText)) !== null) {
             let email = `${obfuscatedMatch[1]}@${obfuscatedMatch[2]}`;
             email = cleanExtractedEmail(email);
            ...
            for (const [platform, regex] of Object.entries(SOCIAL_PLATFORMS)) {
                if (regex.test(href)) {
                    ...
                    if (!socialLinks.has(href)) {
                        let handle = '';
                        let context = $el.text().trim();
                        
                        // Try to find context from title or img alt if text is empty
                        if (!context) {
                            context = $el.attr('title') || $el.attr('aria-label') || $el.find('img').attr('alt') || '';
                        }
                        
                        // Truncate context
                        if (context.length > 50) context = context.substring(0, 50) + '...';

                        // Specific Parser for LinkedIn
                        if (platform === 'linkedin') {
                            const extractedName = parseLinkedInHandle(href);
                            if (extractedName) {
                                handle = extractedName;
                                // If context is generic (like "LinkedIn"), use the handle as context/name
                                if (!context || context.toLowerCase() === 'linkedin') {
                ...
                items.forEach(item => {
                    ...
                    if (item.sameAs) {
                        ...
                        links.forEach(link => {
                            for (const [platform, regex] of Object.entries(SOCIAL_PLATFORMS)) {
                                if (regex.test(link)) {
                                    if (!socialLinks.has(link)) {
                                        let handle = '';
                                        // Specific Parser for LinkedIn
                                        if (platform === 'linkedin') {
                                            const extractedName = parseLinkedInHandle(link);
                                            if (extractedName) handle = extractedName;
                                        }

                                        socialLinks.set(link, {
                                            platform,
                                            url: link,
                                            handle,
