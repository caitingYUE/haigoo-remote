...
const DIMENSION_CONFIG = [
  ...
  },
  {
    ...
    improveText: '如岗位本身足够优质，可确认工作类型与时区安排后再决定'
  }
]
...
export function normalizeMatchBreakdown(details = {}) {
  ...
  return {
    titleMatch: normalizeOptionalScore(raw.titleMatch),
    roleTypeMatch: normalizeOptionalScore(raw.roleTypeMatch),
    skillMatch: normalizeOptionalScore(raw.skillMatch),
    keywordSimilarity: normalizeOptionalScore(raw.keywordSimilarity),
    experienceMatch: normalizeOptionalScore(raw.experienceMatch),
    preferenceMatch: normalizeOptionalScore(raw.preferenceMatch),
    evidenceCoverage: Math.max(0, Math.min(1, Number(raw.evidenceCoverage ?? raw.evidence_coverage) || 0))
  }
}

function inferConfidence(score, dimensions = [], evidenceCoverage = 1) {
  if (evidenceCoverage < 0.55 || dimensions.length < 3) {
    return {
      label: '有限',
      reason: '岗位或简历的关键证据不足，暂不宜据此判断完整匹配程度。'
    }
  }
...
export function buildMatchInsights({ score = 0, details = {}, jobRow = {} } = {}) {
  ...
  const confidence = inferConfidence(clampScore(score), dimensions, breakdown.evidenceCoverage)
