...
function isDefaultGroupName(value) {
  return normalizeText(value) === DEFAULT_GROUP_NAME
}
...
async function ensureDefaultGroupSeed() {
  ...
  await neonHelper.query(
    ...
    [groupId, DEFAULT_GROUP_NAME, 1, true, JSON.stringify(JOB_CATEGORY_OPTIONS)]
  )
}
...
function normalizePersistedShareUrl(jobId, shareUrl) {
  const fallback = buildShareUrl(jobId)
  const normalized = normalizeText(shareUrl)
...
function getTranslatedField(translations, field) {
  const parsed = safeParseJson(translations, {})
...
function mapCandidateRow(row) {
  ...
  return {
    ...
    title: getTranslatedField(row.translations, 'title') || normalizeText(row.title, '未命名岗位'),
    company: getTranslatedField(row.translations, 'company') || normalizeText(row.company, '未知企业'),
    location: getTranslatedField(row.translations, 'location') || normalizeText(row.location),
    category: getTranslatedField(row.translations, 'category') || normalizeText(row.category, '未分类'),
...
function buildPreviewJob(job, audienceKey) {
  return {
    ...
    shareUrl: normalizePersistedShareUrl(job.id, job.shareUrl),
...
function parseRunPayload(run) {
  const payload = safeParseJson(run?.payload, null)
  if (!payload?.card) return payload

  const card = payload.card
  const originalJobs = Array.isArray(card.jobs) ? card.jobs : []
  const jobs = originalJobs.map((job) => buildPreviewJob(job, card.key === 'member' ? 'member' : 'public'))
  let normalizedCopyText = normalizeText(card.copyText)

  for (let index = 0; index < jobs.length; index += 1) {
    const job = jobs[index]
    const rawShareUrl = normalizeText(originalJobs[index]?.shareUrl)
    const safeShareUrl = normalizePersistedShareUrl(job?.id, rawShareUrl || job?.shareUrl)
    if (!rawShareUrl || rawShareUrl === safeShareUrl) continue
    normalizedCopyText = normalizedCopyText.split(rawShareUrl).join(safeShareUrl)
  }
  ...
  return {
    ...
    card: {
      ...card,
      jobs,
      copyText: normalizedCopyText || card.copyText
    }
  }
}
...
async function getEffectiveGroups(batchDate) {
  ...
  const customGroups = groups.filter((group) => !isDefaultGroupName(group.internalName))
  return customGroups.length > 0 ? customGroups : groups
}
...
function deriveRoleOptions(groups = []) {
  const options = [...JOB_CATEGORY_OPTIONS]
  const seen = new Set(options)

  for (const group of groups) {
    const mergedRoles = [
      ...(Array.isArray(group.currentRoles) ? group.currentRoles : []),
      ...(Array.isArray(group.pendingRoles) ? group.pendingRoles : [])
    ]
...
function getNextSortOrder(groups = []) {
  return groups.reduce((maxValue, group) => {
    const sortOrder = Number(group.pendingSortOrder || group.currentSortOrder || group.sortOrder || 0)
...
export async function listSocialPushGroups(options = {}) {
  ...
  return {
    success: true,
    roleOptions: deriveRoleOptions(groups),
    today,
    tomorrow,
    groups
  }
...
export async function saveSocialPushGroup(input = {}) {
  ...
  const currentSettings = await listSocialPushGroups()
  ...
  const existingGroup = (currentSettings.groups || []).find((group) => group.id === groupId)
  const sortOrder = Number.isFinite(Number(input.sortOrder))
    ? clampSortOrder(input.sortOrder)
    : existingGroup
      ? Number(existingGroup.pendingSortOrder || existingGroup.currentSortOrder || existingGroup.sortOrder || 100)
      : getNextSortOrder(currentSettings.groups || [])
...
export async function searchSocialPushManualJobs(input = {}) {
  if (!neonHelper.isConfigured) {
    throw new Error('Database not configured')
  }
  ...
  const audienceKey = input.audienceKey === 'member' ? 'member' : 'public'
  const limit = Math.min(Math.max(Number.parseInt(input.limit, 10) || 6, 1), 12)
  const batchDate = normalizeText(input.batchDate, getScheduleContext(new Date(), DEFAULT_TIMEZONE).displayBatchDate)
  const batchLabel = getBatchLabelFromBatchDate(batchDate, new Date(), DEFAULT_TIMEZONE)
  const like = `%${query}%`
  ...
  const rows = await neonHelper.query(
    `SELECT
      ...
      CASE
        WHEN LOWER(BTRIM(COALESCE(j.translations->>'title', ''))) = LOWER(BTRIM($1)) THEN 140
        WHEN LOWER(BTRIM(COALESCE(j.translations->>'company', ''))) = LOWER(BTRIM($1)) THEN 130
        WHEN LOWER(BTRIM(j.title)) = LOWER(BTRIM($1)) THEN 120
        WHEN LOWER(BTRIM(j.company)) = LOWER(BTRIM($1)) THEN 110
        WHEN COALESCE(j.translations->>'title', '') ILIKE $2 THEN 105
        WHEN COALESCE(j.translations->>'company', '') ILIKE $2 THEN 100
        WHEN j.title ILIKE $2 THEN 90
        WHEN j.company ILIKE $2 THEN 80
        WHEN COALESCE(j.translations->>'title', '') ILIKE $3 THEN 75
        WHEN COALESCE(j.translations->>'company', '') ILIKE $3 THEN 70
        WHEN j.title ILIKE $3 THEN 60
        WHEN j.company ILIKE $3 THEN 55
        ELSE 10
      END AS manual_match_score
    FROM jobs j
    LEFT JOIN trusted_companies tc
      ON (j.company_id = tc.company_id OR (j.company_id IS NULL AND LOWER(TRIM(j.company)) = LOWER(TRIM(tc.name))))
    LEFT JOIN (
    ...
    WHERE j.status = 'active'
      ...
      AND COALESCE(NULLIF(BTRIM(j.title), ''), NULL) IS NOT NULL
      AND COALESCE(NULLIF(BTRIM(j.company), ''), NULL) IS NOT NULL
      AND ($4 = 'member' OR COALESCE(j.member_only, false) = false)
      AND (
        j.title ILIKE $3
        OR j.company ILIKE $3
        OR COALESCE(j.translations::text, '') ILIKE $3
        OR COALESCE(j.category, '') ILIKE $3
      )
    ORDER BY manual_match_score DESC,
      COALESCE(j.published_at, j.updated_at, j.created_at) DESC NULLS LAST,
      j.job_id DESC
    LIMIT $5`,
    [query, prefixLike, like, audienceKey, limit]
  )

  const jobs = (rows || []).map((row) => {
    const job = {
      ...mapCandidateRow(row),
      recommendationScore: Number(row.manual_match_score || 0)
    }
    const preview = buildPreviewJob(job, audienceKey)
    return {
      ...preview,
      companyInfoLine: buildCompanyInfoLine(job),
      referralInfoLines: buildReferralInfoLines(job),
...
export async function disableSocialPushGroup(input = {}) {
  ...
  if (isDefaultGroupName(group.pendingInternalName || group.currentInternalName || group.internalName)) {
