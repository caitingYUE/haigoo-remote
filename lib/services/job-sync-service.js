...
export async function syncCompanyJobs(companyId, companyName, crawledJobs) {
    ...
    for (let i = newCandidates.length - 1; i >= 0; i--) {
        ...
        for (const oldJob of obsoleteCandidates) {
            ...
            if (oldJob.title.toLowerCase().trim() === newJob.title.toLowerCase().trim()) {
                // For manual jobs, Title match is sufficient (description might differ significantly)
                const isManual = oldJob.source_type === 'manual';
                const score = calculateSimilarity(newJob.description, oldJob.description);
                
                // If manual, we accept even low similarity (trust the title)
                // If not manual, require high similarity (0.8)
                const threshold = isManual ? 0.1 : 0.8;
                ...
                // If manual and title matches, we can break early if we trust title enough
    ...
    for (const id of finalObsoleteIds) {
        ...
        if (job && (job.source_type === 'manual' || job.is_manually_edited)) {
            console.log(`[JobSync] Manual job '${job.title}' not found in crawl. Marking as Pending (Review Required).`);
