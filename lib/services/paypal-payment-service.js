...
function publicOrder(row) {
  if (!row) return null
  const snapshot = row.metadata?.paypal?.planSnapshot || row.metadata?.virtualPayment?.planSnapshot || {}
  return {
  ...
  async listAdminOrders({ page = 1, pageSize = 25, status = 'all', search = '' } = {}) {
    ...
    const rows = await neonHelper.query(
      `SELECT p.*, u.email AS user_email, u.username AS user_name,
       ...
       LEFT JOIN membership_entitlement_segments s ON s.source_payment_id = p.payment_id
       LEFT JOIN LATERAL (SELECT * FROM payment_refunds WHERE payment_id = p.payment_id ORDER BY (status = 'review_required') DESC, created_at DESC LIMIT 1) r ON TRUE
       WHERE p.provider IN ('paypal', 'wechat_virtual')
    ...
    return {
      orders: (rows || []).map(row => ({
        ...publicOrder(row), provider: row.provider, userEmail: row.user_email || '', userName: row.user_name || '',
  ...
  async reviewRefund({ refundId, adminUserId, approve, note }) {
    const rows = await neonHelper.query(
      `SELECT r.*, p.provider_capture_id, p.expected_amount_cents, p.refunded_amount_cents,
       ...
       LEFT JOIN membership_entitlement_segments s ON s.source_payment_id = p.payment_id
       WHERE r.refund_id = $1::uuid AND p.provider = 'paypal' AND r.provider = 'paypal' LIMIT 1`,
      [refundId]
    )
