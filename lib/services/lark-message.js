 ...
 */
async function sendUserMessage(openId, content, userId = '') {
    try {
        // 构造请求参数
 ...
 * @param {string} userId - 用户user_id（可选）
 */
async function sendUserCard(openId, content, color = 'green', extraContent = '', userId = '') {
    try {
        // 构造卡片元素
 ...
 * @param {string} content - 消息内容
 */
async function sendChatMessage(chatId, openId, content) {
    try {
        // 构造请求参数
        const request = {
 ...
 */
async function sendChatCard(chatId, content, color = 'green', title = '岗位推荐') {
    try {
        // 构造卡片元素
        const cardFooterElements = [
            { tag: 'hr' },
 ...
 */
async function replyChatMessage(senderId, messageId, content, replyInThread = false) {
    try {
        // 构造请求参数
        const request = {
 ...
 * @param {string} extraContent - 额外内容（可选）
 */
async function replyChatCard(senderId, messageId, content, color = 'green', replyInThread = false, extraContent = '') {
    try {
        // 构造卡片元素
        const cardFooterElements = [
            { tag: 'hr' },
 ...
 */
async function downloadMessageFile(messageId, fileKey, downloadPath) {
    try {
        // 构造请求参数
        const request = {
            message_id: messageId,
...
async function sendLog(content, color = 'red', title = '异常告警') {
    if (!DEBUG_CHAT_ID) {
