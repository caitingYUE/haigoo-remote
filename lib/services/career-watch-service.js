...
async function getLiveFollowedUpdates(userId, limit = 50) {
  const rows = await neonHelper.query(
    `SELECT inbox.inbox_id, inbox.status, events.company_id, companies.name AS company_name,
            ...
            history.category AS job_category, CASE WHEN history.closed_at IS NULL AND history.is_public_opportunity IS TRUE THEN 'active' ELSE 'inactive' END AS job_status,
            history.source_published_at, follows.created_at AS subscribed_at
