...
import { createPublicSnapshotCache } from './public-snapshot-cache.js'
...
// Migration 084 makes immutable free snapshots mandatory for this release.
const FIXED_MATCH_SNAPSHOT_ENABLED = true
...
export function careerWatchEntitlements(isMember = false, { refreshOnVisit = false } = {}) {
  return {
    isMember: Boolean(isMember),
    maxRoleFamilies: 5,
    maxPreferenceTypes: null,
    maxFollows: isMember ? null : 5,
    refreshHours: isMember ? (refreshOnVisit ? 0 : 6) : null,
...
const watchCandidateCache = createPublicSnapshotCache({ ttlMs: 45000, maxEntries: 1 })
const filterOptionsCache = createPublicSnapshotCache({ ttlMs: 5 * 60 * 1000, maxEntries: 1 })

export async function getCareerWatchFilterOptions() {
  return filterOptionsCache(String(new Date().getFullYear()), readCareerWatchFilterOptions)
}

async function readCareerWatchFilterOptions() {
...
function mapFollowedUpdates(rows) {
  return (rows || []).map((row) => ({
    ...
    ,jobStatus: row.job_status || 'unknown'
    ,publishedAt: row.source_published_at || row.occurred_at
...
export async function getPublicCareerWatchFeed(limit = 6) {
  ...
  return (rows || []).map((row) => {
    ...
    return {
      ...
      jobId: canonicalJobId(row.job_id),
...
async function applyLiveRecommendationState(userId, recommendations, { preserveFixed = false } = {}) {
  ...
  const [follows, exposures, liveJobs] = await Promise.all([
    ...
    neonHelper.query(
      `SELECT tc.company_id, tc.description, tc.translations AS company_translations, tc.employee_count, tc.cached_logo_url, tc.logo, tc.address,
  ...
  return (recommendations || [])
    .filter((item) => preserveFixed || (liveByCompany.has(String(item.companyId)) && !dismissedIds.has(String(item.companyId))))
    .map((item) => {
      const companyId = String(item.companyId)
      const follow = followMap.get(companyId)
      const jobs = liveByCompany.get(companyId) || []
      if (!jobs.length) return {
        ...item, jobId: '', jobTitle: '暂无开放岗位', applyUrl: '', openJobCount: 0, openRoleLabels: [],
        hasUpdate: false, reasons: ['保留首次匹配企业，当前暂无开放岗位'],
        isFollowed: Boolean(follow), isSubscribed: Boolean(follow?.wechat_enabled && follow?.wechat_template_status === 'accepted')
      }
      const latest = jobs[0]
      const publicRating = supportedCompanyRating(latest)
      return {
        ...item,
        description: localizedCompanyDescription(latest),
        jobId: canonicalJobId(latest.source_job_id || item.jobId),
...
function careerWatchSnapshotMeta(profile, generatedAt, isMember) {
  ...
  return {
    ...
    validUntil: isMember ? new Date(new Date(safeGeneratedAt).getTime() + 6 * 3600000).toISOString() : null
  }
}
...
async function getLiveFollowedUpdates(userId, limit = 50) {
  const rows = await neonHelper.query(
    `SELECT inbox.inbox_id, inbox.status, events.company_id, companies.name AS company_name,
            ...
            events.source_job_id, COALESCE(history.title, jobs.title) AS job_title,
            COALESCE(history.category, jobs.category) AS job_category,
            CASE WHEN jobs.status = 'active' AND jobs.is_approved IS TRUE AND COALESCE(jobs.member_only, FALSE) IS FALSE
                 THEN 'active' ELSE 'inactive' END AS job_status,
            COALESCE(history.source_published_at, jobs.published_at) AS source_published_at,
            follows.created_at AS subscribed_at
      ...
      LEFT JOIN company_job_history history ON history.source_job_id = events.source_job_id AND history.company_id = events.company_id
      LEFT JOIN jobs ON jobs.job_id = events.source_job_id
...
export async function computeCareerWatchFeed({ userId, profile, isMember = false, limit = null, fixedFree = false, recentBatches = [] }) {
  ...
  const [rows, follows, updates, exposures] = await Promise.all([
    watchCandidateCache('open-opportunities', () => neonHelper.query(
  ...
  for (const { company, jobs } of grouped.values()) {
    ...
    recommendations.push({
      ...
      jobId: canonicalJobId(latest.source_job_id),
  ...
  // Batches are newest first. Unseen companies lead; when the pool is small,
  // reuse the least recently seen before reusing the immediately previous batch.
  const lastSeen = new Map()
  for (const [index, batch] of recentBatches.slice(0, 3).entries()) {
    for (const id of batch.companyIds || []) if (!lastSeen.has(String(id))) lastSeen.set(String(id), index)
  }
  const freshness = (item) => lastSeen.has(item.companyId) ? lastSeen.get(item.companyId) : 3
  recommendations.sort((left, right) => (isMember ? freshness(right) - freshness(left) : 0) || Number(right.isFollowed) - Number(left.isFollowed) || right.directionSpecificity - left.directionSpecificity || right.score - left.score || new Date(right.updatedAt || 0) - new Date(left.updatedAt || 0))
  ...
  return {
    ...
    unreadFollowedUpdateCount: (updates || []).filter((row) => row.status === 'unread').length,
    generatedAt: new Date().toISOString(),
    emptyReason: recommendations.length ? null : profile.toleranceMode === 'strict' ? 'strict_filters' : 'no_role_update'
  }
}

const pendingWatchRefreshes = new Map()

export async function getCareerWatchFeed(options) {
  // Collapse same-instance entry requests; the conditional database write below
  // also prevents two instances from advancing the same snapshot concurrently.
  const key = `${options.userId}:${options.profile?.version || 0}:${Boolean(options.isMember)}:${Boolean(options.fixedFree)}`
  if (!options.refreshKey) return readCareerWatchFeed(options)
  if (pendingWatchRefreshes.has(key)) return pendingWatchRefreshes.get(key)
  const pending = readCareerWatchFeed(options).finally(() => pendingWatchRefreshes.delete(key))
  pendingWatchRefreshes.set(key, pending)
  return pending
}

async function readCareerWatchFeed({ userId, profile, isMember = false, fixedFree = false, refreshKey = '', refreshExpired = false, retry = false }) {
  ...
  let snapshot = null
  const snapshotItems = () => fixedFree
    ? (parseJsonArray(snapshot?.fixed_recommendations).length ? parseJsonArray(snapshot.fixed_recommendations) : parseJsonArray(snapshot?.recommendations).slice(0, 5))
    : parseJsonArray(snapshot?.recommendations).slice(0, 5)
  const snapshotTime = () => fixedFree ? snapshot?.fixed_generated_at || snapshot?.generated_at : snapshot?.generated_at
  const restore = async (stale = false) => responseWithSnapshot({
    recommendations: await applyLiveRecommendationState(userId, snapshotItems(), { preserveFixed: fixedFree }),
  ...
  try {
    ...
    const batches = parseJsonArray(snapshot?.recent_match_batches).slice(0, 3)
    const cacheExpired = refreshExpired && Date.now() - new Date(snapshot?.generated_at || 0).getTime() >= 6 * 3600000
    const alreadyRefreshed = refreshKey && batches.some((batch) => batch.refreshKey === refreshKey)
    const snapshotMatchesProfile = fixedFree || Number(snapshot?.profile_version) === Number(profile?.version)
    if (snapshot && snapshotMatchesProfile && (fixedFree || !isMember || (!refreshKey && !cacheExpired) || alreadyRefreshed)) {
      // A read never rotates recommendations, and fixed results never expire.
      if (snapshotItems().length || (!fixedFree && snapshot.empty_reason)) return await restore()
    }
    if (fixedFree) {
      // Once the free quota is used, a missing snapshot is a recovery issue, not
      // permission to silently generate a different set of companies.
      throw Object.assign(new Error('原匹配结果暂时无法加载，请稍后重试'), { statusCode: 503, code: 'MATCH_DATA_NOT_READY' })
    }
    const previousBatches = batches.length ? batches : snapshot?.recommendations?.length ? [{
      companyIds: parseJsonArray(snapshot.recommendations).map((item) => String(item.companyId)),
      generatedAt: snapshot.generated_at
    }] : []
    const result = await computeCareerWatchFeed({ userId, profile, isMember, limit: 5, fixedFree, recentBatches: previousBatches })
    const nextBatches = [{ companyIds: result.recommendations.map((item) => item.companyId), refreshKey, generatedAt: result.generatedAt }, ...previousBatches].slice(0, 3)
    const savedSnapshots = await neonHelper.query(
      `INSERT INTO career_watch_feed_snapshots (
         user_id, profile_version, recommendations, followed_updates, empty_reason, generated_at, recent_match_batches
       ) SELECT $1::varchar, $2, $3::jsonb, $4::jsonb, $5, NOW(), $6::jsonb
          WHERE EXISTS (SELECT 1 FROM career_watch_profiles WHERE user_id = $1::varchar AND version = $2)
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         generated_at = NOW(),
         recent_match_batches = EXCLUDED.recent_match_batches,
         snapshot_revision = career_watch_feed_snapshots.snapshot_revision + 1
       WHERE career_watch_feed_snapshots.snapshot_revision = $7::bigint
         AND career_watch_feed_snapshots.profile_version = $8
         AND EXISTS (SELECT 1 FROM career_watch_profiles WHERE user_id = $1::varchar AND version = $2)
      ...
      [userId, profile.version, JSON.stringify(result.recommendations), JSON.stringify(result.followedUpdates), result.emptyReason,
        JSON.stringify(nextBatches), snapshot?.snapshot_revision ?? -1, snapshot?.profile_version || 0]
    )
    if (!savedSnapshots?.length) {
      if (retry) throw new Error('匹配结果正在更新，请稍后重试')
      return readCareerWatchFeed({ userId, profile, isMember, fixedFree, refreshKey, refreshExpired, retry: true })
    }
    const persistedGeneratedAt = savedSnapshots[0].generated_at
    return responseWithSnapshot({ ...result, generatedAt: persistedGeneratedAt, source: 'recomputed' }, persistedGeneratedAt)
  } catch (error) {
    if (snapshot && snapshotItems().length && (fixedFree || Number(snapshot.profile_version) === Number(profile?.version))) {
      return restore(true)
    }
    throw error
  }
...
export async function createFixedCareerWatchMatch({ userId, input, expectedVersion = null }) {
  ...
  const rows = await neonHelper.query(
    `WITH input_gate AS (
       ...
       INSERT INTO career_watch_feed_snapshots (
         user_id, profile_version, recommendations${fixedColumn}, followed_updates, empty_reason, generated_at, fixed_generated_at
       )
       SELECT $1, saved_profile.version, $13::jsonb${fixedValue}, $14::jsonb, NULL, NOW(), NOW()
         FROM saved_profile
       ON CONFLICT (user_id) DO UPDATE SET
         profile_version = EXCLUDED.profile_version,
