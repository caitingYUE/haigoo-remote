    ...
    try {
        ...
        if (jobUrl.includes('alphasights.com')) {
            ...
            if (singleRole.length > 0) {
                // Improved logic: Score all candidate columns to find the best one
                // This avoids picking the wrong column (e.g. the one with the form/scripts)
                const candidates = singleRole.find('.col-12, .col-md-10, .col-md-8');
                let bestCandidate = null;
                let maxScore = 0;

                const keywords = [
                    'The role', 'What you’ll do', "What you'll do", 'Who you are', 
                    'About AlphaSights', 'Requirements', 'Responsibilities', 'Qualifications',
                    'A day in the life', 'About the role'
                ];

                candidates.each((i, el) => {
                    const $el = $(el).clone();
                    // Clean critical noise first to get accurate text length/content
                    $el.find('script, style, noscript, form, button, input, select').remove();
                    ...
                    // Penalty for very short text (likely just a header or button wrapper)
                    if (text.length < 200) score -= 20;

                    // Bonus for reasonable length
                    if (text.length > 500 && text.length < 10000) score += 5;

                    if (score > maxScore) {
                        maxScore = score;
                        bestCandidate = $(el); // Use original element reference
                    }
                });

                if (bestCandidate && maxScore > 0) {
                    contentContainer = bestCandidate;
                    console.log(`[fetchJobDetails] AlphaSights: Found best container with score ${maxScore}`);
                } else {
                    // Fallback: Keyword search inside #single-role if scoring failed
                    console.log('[fetchJobDetails] AlphaSights: Scoring failed, trying keyword fallback');
                    for (const kw of keywords) {
                        const el = singleRole.find(`*:contains("${kw}")`).last();
                        if (el.length > 0) {
                             // Traverse up to find the column container
                             contentContainer = el.closest('.col-12, .col-md-10, .row');
                             break;
                        }
                    }
                }
            ...
            if (contentContainer && contentContainer.length > 0) {
                ...
                // Remove noise: buttons, forms, headers that are not part of description
                container.find('button, form, .role-location, h1, script, style, noscript').remove()
                // Remove the "All Open Roles" and "Apply Now" links if they are wrappers
                container.find('a[href*="open-roles"], a[href*="#application-section"]').remove()

                const text = cleanText(container.text())
                if (text.length > 100) {
                    description = text
                    console.log(`[fetchJobDetails] AlphaSights: Extracted ${description.length} chars from #single-role`)
                }
            }
        }
...
export async function crawlCompanyJobs(companyId, careersUrl, options = {}) {
    ...
    if (careersUrl.includes('ted.com') && careersUrl.includes('jobs')) {
    ...
    if (careersUrl.includes('greenhouse.io/frec')) {
    ...
    if (careersUrl.includes('dover.com/jobs/mixrank')) {
