...
async function parseJobsFromHtml(html, baseUrl, companyId) {
    ...
    if (baseUrl.includes('taskade.com/jobs')) {
        ...
        // Find buttons that look like job cards
        // Structure: <button> Title <p>Location</p> </button>
        // We look for any button that contains a p tag, or specific class if possible.
        // Based on analysis, the container has class sc-4df7bc25-0 TzeOO
        // But classes are generated.
        // We can search for the "Join our Team" header and traverse down.
        
        const $joinHeader = $('h1#join-our-team');
        if ($joinHeader.length > 0) {
            // The job list is in a sibling div
            const $container = $joinHeader.parent().find('button');
            $container.each((i, el) => {
                const $btn = $(el);
                const $p = $btn.find('p');
                ...
                if ($p.length > 0) {
                    ...
                    if (title && location) {
                        taskadeJobs.push({
                            title,
                            // Use the main careers page as the URL since there are no deep links
                            url: baseUrl, 
                            location,
                            type: 'Full-time',
                            // Add a helpful note since we can't extract the real JD
                            description: `
Role: ${title}
Location: ${location}

**Note**: This job was detected on Taskade's official careers page. 
Please visit [Taskade Careers](https://www.taskade.com/jobs) to view the full description and apply.
                            `.trim(),
             ...
             $('button').each((i, el) => {
                ...
                const $p = $btn.find('p');
                if ($p.length > 0) {
                     const loc = cleanText($p.text());
                     // Heuristic: Location usually contains specific words
                     if (loc.includes('Remote') || loc.includes('San Francisco') || loc.includes('Singapore')) {
                        const $clone = $btn.clone();
                        $clone.find('p').remove();
                        const title = cleanText($clone.text());
                        
                        if (title && title.length < 100) { // Avoid long button texts
                             taskadeJobs.push({
                                title,
                                url: baseUrl,
                                location: loc,
                                type: 'Full-time',
                                description: `Role: ${title}\nLocation: ${loc}\n\nPlease visit https://www.taskade.com/jobs to apply.`,
