            ...
            if (apiRes.ok) {
                ...
                if (data.jobs) {
                    ...
                    const dateFilteredJobs = filteredJobs.filter(job => {
                        if (!job.publishedAt) return false; // Strict: Drop if no date to avoid old jobs appearing as new
                        try {
                            const pubDate = new Date(job.publishedAt);
                            // Check if valid date
                            if (isNaN(pubDate.getTime())) return false; // Strict: Drop invalid dates
                            return pubDate >= thirtyDaysAgo;
                        } catch (e) {
                            return false; // Strict: Drop on error
                        }
                    });
