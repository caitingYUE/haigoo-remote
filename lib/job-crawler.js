...
export async function fetchJobDetails(jobUrl, useAI = false) {
    try {
        ...
        if (jobUrl.includes('alphasights.com')) {
            ...
            // The content is usually in .col-12 inside #single-role
            const singleRole = $('#single-role');
            let contentContainer = null;
            
            if (singleRole.length > 0) {
                // Try specific column class
                const specificCol = singleRole.find('.col-md-10');
                if (specificCol.length > 0) {
                     contentContainer = specificCol.first();
                } else {
                    // Fallback: Keyword search inside #single-role
                    const keywords = ['The role:', 'What you’ll do:', "What you'll do:", 'Who you are:'];
            ...
            if (contentContainer && contentContainer.length > 0) {
                ...
                container.find('button, form, .role-location, h1').remove()
