...
export async function fetchJobDetails(jobUrl, useAI = false) {
    try {
        ...
        if (jobUrl.includes('alphasights.com')) {
            ...
            // AlphaSights typically uses #main-content or specific section classes
            // Based on observation, content might be in a div with no clear class, or 'bg-white' sections
            
            // Try to grab the main container that holds headings like "The role", "What you'll do"
            // Look for H2/H3 headings that match common terms and grab their parent or siblings
            const headings = $('h2, h3').filter((i, el) => {
                const text = $(el).text().toLowerCase()
                return text.includes('role') || text.includes('looking for') || text.includes('requirements') || text.includes('what you')
            })

            if (headings.length > 0) {
                // If we found headings, their parent might be the container
                // Or we can construct description from these sections
                let extractedText = ''
                headings.each((i, el) => {
                    const $el = $(el)
                    const sectionContent = $el.nextUntil('h2, h3, section').text()
            ...
            // Fallback: Try specific IDs or Classes if known (e.g. #content)
            if (!description) {
                const contentDiv = $('#content, .content, main').first()
                if (contentDiv.length > 0) {
                    description = cleanText(contentDiv.text())
                }
            }
        }
