...
export async function fetchJobDetails(jobUrl, useAI = false) {
    try {
        ...
        if (jobUrl.includes('alphasights.com')) {
            ...
            const contentContainer = $('#single-role .col-12').first()
            ...
            // Fallback: Try to find sections starting with strong/bold tags (e.g. "The role:", "What you'll do:")
            if (!description) {
                const headings = $('p > strong, p > b').filter((i, el) => {
                    const text = $(el).text().toLowerCase()
                    return text.includes('role') || text.includes('what you') || text.includes('who you')
                })

                if (headings.length > 0) {
                    let extractedText = ''
                    headings.each((i, el) => {
                        const $p = $(el).parent() // The <p> containing the strong tag
                        const sectionContent = $p.nextUntil('p:has(strong), div, section').text()
                        extractedText += $p.text() + '\n' + sectionContent + '\n\n'
                    })
                    
                    if (extractedText.length > 100) {
                        description = cleanText(extractedText)
                    }
                }
            }
        }
