...
async function parseJobsFromHtml(html, baseUrl, companyId) {
    ...
    if (baseUrl.includes('taskade.com/jobs')) {
        ...
        // 1. Try to parse Next.js RSC payload for netlifyCMSContextValue
        // This contains the source of truth for jobs
        const scripts = $('script')
        scripts.each((i, el) => {
            const content = $(el).html() || ''
            if (content.includes('self.__next_f.push') && content.includes('netlifyCMSContextValue')) {
                 const matches = content.matchAll(/self\.__next_f\.push\(\[\d+,"(.*?[^\\])"\]\)/g)
                 for (const match of matches) {
                     let jsonStr = match[1].replace(/\\"/g, '"').replace(/\\\\/g, '\\')
                     // Look for "jobs":[{...}]
                     const jobsMatch = jsonStr.match(/"jobs":(\[{.*?}\])/)
                     if (jobsMatch) {
                         try {
                             const jobs = JSON.parse(jobsMatch[1])
                             jobs.forEach(j => {
                                 // Deduplicate
                                 if (taskadeJobs.some(existing => existing.title === j.title)) return

                                 taskadeJobs.push({
                                     title: j.title,
                                     url: baseUrl, // Taskade seems to be a single page
                                     location: j.location,
                                     type: 'Full-time',
                                     // Use a better description including apply link
                                     description: `Role: ${j.title}\nLocation: ${j.location}\nApply: ${j.apply_url || 'jobs@taskade.com'}\n\nPlease visit https://www.taskade.com/jobs to view full details.`,
                             ...
                             console.error('[JobCrawler] Error parsing Taskade jobs JSON:', e)
                         }
                     }
                 }
            }
        ...
        if (taskadeJobs.length > 0) {
             console.log(`[JobCrawler] Taskade parser (RSC) found ${taskadeJobs.length} jobs`)
