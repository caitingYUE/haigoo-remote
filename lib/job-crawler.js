...
export async function crawlCompanyJobs(companyId, companyName, careersUrl, options = {}) {
    ...
    try {
        ...
        // 2026-01-18 Update: Relaxed date filter for direct company crawls. 
        // If a job is listed on the company's career page, it's likely active regardless of published date.
        // We only log the count of old jobs but DO NOT filter them out, to avoid missing valid "evergreen" or "general" roles.
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        
        const oldJobsCount = filteredJobs.filter(job => {
        ...
        if (oldJobsCount > 0) {
            console.log(`[JobCrawler] Found ${oldJobsCount} jobs older than 30 days (kept as active since they are on the career page)`);
        }

        return {
            jobs: filteredJobs, // Return all valid location jobs
            company: companyInfo
        }
