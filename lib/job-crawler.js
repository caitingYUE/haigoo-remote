...
async function parseJobHtml(html, jobUrl, useAI = false) {
    ...
    if (foundInSchema && description) {
        return { description, requirements, benefits, company, applyUrl }
    }
    ...
    if (nextData.length > 0 && (!description || description.length < 100)) {
        try {
            ...
            if (foundDesc) {
                ...
                description = cleanText(foundDesc)
    ...
    if (jobUrl.includes('alphasights.com')) {
        ...
        if (singleRole.length > 0) {
            ...
            let text = cleanText(container.text());
...
export async function crawlCompanyJobs(companyId, companyName, careersUrl, options = {}) {
    ...
    // Specific Fix for Canonical (Ensure we hit the careers page that has the JSON)
    if (companyName === 'Canonical' || careersUrl.includes('canonical.com')) {
         if (!careersUrl.includes('/careers')) {
