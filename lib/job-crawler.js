...
export async function crawlCompanyJobs(companyId, companyName, careersUrl, options = {}) {
    ...
    // Specific Fix for Taskade (Use main careers page which is Next.js)
    if (companyName === 'Taskade' || careersUrl.includes('taskade.com')) {
        // Taskade's main site taskade.com/jobs lists jobs and is crawlable via Next.js strategy
        console.log('[JobCrawler] Taskade detected, using main careers page')
        // No redirect needed, or ensure we use the jobs page
        if (!careersUrl.includes('/jobs')) {
             careersUrl = 'https://www.taskade.com/jobs'
        }
    }

    // Specific Fix for Grafana Labs (Use Greenhouse)
    if (companyName === 'Grafana Labs' || careersUrl.includes('grafana.com')) {
    ...
    if (companyName === 'Docker' || careersUrl.includes('docker.com')) {
    ...
    if (companyName === 'Progress Chef' || careersUrl.includes('chef.io') || companyName === 'Progress Software' || careersUrl.includes('progress.com')) {
    ...
    if (companyName === 'Canonical' || careersUrl.includes('canonical.com')) {
    ...
    if (companyName === 'Osome' || careersUrl.includes('osome.com')) {
    ...
    if (companyName === 'Fueled' || careersUrl.includes('fueled.com')) {
        console.log('[JobCrawler] Fueled detected, enforcing correct careers page and custom parser');
        // Force correct URL
        if (!careersUrl.includes('/careers') || careersUrl.includes('/opportunities')) {
                    ...
                    for (let i = 0; i < jobsToFetch.length; i += concurrency) {
                        ...
                        const batchResults = await Promise.all(
                            batch.map(async (job, idx) => {
                                ...
                                // If location is "Remote" (default) or "Unspecified", try to extract from Title or Description
                                if (job.location === 'Remote' || job.location === 'Unspecified') {
                                    ...
                                    if (titleParts.length > 1) {
                                        ...
                                        // We can use ClassificationService.isExplicitlyOverseas to check if it's a known location?
                                        // Or just assume it is if it's short.
                                        if (potentialLoc.length > 2 && potentialLoc.length < 30 && /^[A-Z]/.test(potentialLoc)) {
                                            if (!['Manager', 'Director', 'Lead', 'Engineer', 'Developer', 'Remote', 'Full-time', 'Contract'].includes(potentialLoc)) {
