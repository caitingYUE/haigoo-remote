...
function dedupeLines(lines = [], limit = 10) {
    return Array.from(new Set(
        lines
            .map((line) => normalizeLine(line).replace(/^[-•*]\s*/, '').trim())
            .filter((line) => line.length >= 4 && line.length <= 220)
    )).slice(0, limit);
}

function extractSectionItems(description = '', headingPatterns = []) {
    ...
    for (const line of lines) {
        ...
        const isHeading = headingPatterns.some((pattern) => pattern.test(lower));
        ...
        if (!line) {
            blankStreak += 1;
            if (blankStreak >= 2 && results.length >= 2) break;
            continue;
        }
...
function localExtractRequirements(description = '') {
    const headingPatterns = [
        /requirements?/i,
        /qualifications?/i,
        /what (you|we)'?ll need/i,
    ...
    const sectionItems = extractSectionItems(description, headingPatterns);
    if (sectionItems.length >= 2) return sectionItems.slice(0, 10);
    ...
    const bulletLines = String(description || '')
        ...
        .filter((line) =>
            /^[-•*]/.test(line) &&
            /(experience|skill|ability|proficient|knowledge|degree|years|熟悉|经验|能力|技能|学历|年以上)/i.test(line)
        );

    return dedupeLines(bulletLines);
}

function localExtractBenefits(description = '') {
    const headingPatterns = [
        /benefits?/i,
        /perks?/i,
        /what we offer/i,
    ...
    const bulletLines = String(description || '')
        ...
        .filter((line) =>
            /^[-•*]/.test(line) &&
            /(benefit|health|insurance|pto|vacation|bonus|stock|remote stipend|福利|补贴|年假|保险|奖金|期权)/i.test(line)
        );

    return dedupeLines(bulletLines);
}

function localExtractPublishedDate(description = '') {
 ...
 */
export async function callBailianAPI(prompt, systemPrompt = '', retries = 2) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            if (attempt > 0) {
                console.log(`[ai-parser] Retrying API call (attempt ${attempt}/${retries})...`);
            ...
            lastError = error;
            console.error(`[ai-parser] Error calling ${config.provider} API (attempt ${attempt}):`, error.message);
        }
    }
    
    return { content: null, usage: null, error: lastError };
}
...
export async function extractRequirements(description) {
    ...
    const localResult = localExtractRequirements(description);
...
export async function extractBenefits(description) {
    ...
    const localResult = localExtractBenefits(description);
    if (localResult.length > 0) {
        return localResult;
    }
...
export async function extractPublishedDate(description) {
    return localExtractPublishedDate(description);
}
...
export async function enhanceJobDescription(job) {
    ...
    try {
        // Extract requirements, benefits, and published date in parallel
        const [requirements, benefits, publishedAt] = await Promise.all([
            extractRequirements(job.description),
            extractBenefits(job.description),
            !job.publishedAt ? extractPublishedDate(job.description) : Promise.resolve(null)
        ]);
        ...
        if (publishedAt) {
            ...
            console.log(`[ai-parser] Extracted published date: ${publishedAt}`);
        }
