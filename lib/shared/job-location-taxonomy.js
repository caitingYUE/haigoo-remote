...
function group(value, key, label, labelEn, keywords, children, exactValues = []) {
  const broadKeywords = unique(keywords)
  const broadExactValues = unique(exactValues)
  return {
    value,
    key,
    label,
...
const APAC_CHILDREN = [
  ...
  child('CentralAsia', '中亚', 'Central Asia', [
    'central asia', '中亚', 'kazakhstan', 'almaty', 'astana', '哈萨克斯坦', '阿拉木图',
    'uzbekistan', 'tashkent', '乌兹别克斯坦', '塔什干',
    'kyrgyzstan', 'bishkek', '吉尔吉斯斯坦', 'tajikistan', 'dushanbe', '塔吉克斯坦',
    'turkmenistan', 'ashgabat', '土库曼斯坦'
  ]),
  child('NorthAsia', '北亚', 'North Asia', [
    'north asia', 'northern asia', '北亚', 'siberia', '西伯利亚',
    'russian far east', 'far east russia', 'vladivostok', '俄罗斯远东', '海参崴'
  ]),
  child('Oceania', '大洋洲', 'Oceania', [
    'oceania', '大洋洲', 'australia', 'sydney', 'melbourne', 'brisbane', 'perth',
    '澳大利亚', '澳洲', '悉尼', '墨尔本',
    'new zealand', 'auckland', 'wellington', '新西兰', '奥克兰',
    'pacific islands', '太平洋岛国', 'fiji', '斐济', 'papua new guinea', '巴布亚新几内亚', 'guam', '关岛'
  ])
]
...
export const JOB_LOCATION_FILTER_OPTIONS = JOB_LOCATION_TAXONOMY.flatMap(locationGroup => [
  locationGroup,
  ...locationGroup.children.map(option => ({
    ...option,
    parentValue: locationGroup.value
  }))
])
...
export function resolveJobLocationPanelParent({
  selectedValues,
  visibleParentValues,
  toggledParentValue,
  willSelect
}) {
  if (willSelect) return toggledParentValue

  const selected = new Set(Array.isArray(selectedValues) ? selectedValues : [])
  const visibleParents = Array.isArray(visibleParentValues) ? visibleParentValues : []
  const remainingSelectedParent = visibleParents.find(parentValue => {
    if (parentValue === toggledParentValue) return false
    return [...selected].some(value => getJobLocationParentValue(value) === parentValue)
  })

  return remainingSelectedParent
    || visibleParents.find(parentValue => parentValue !== toggledParentValue)
    || toggledParentValue
}
...
function matchesNormalizedLocation(normalized, option) {
...
export function matchesJobLocationFilter(location, filterValue) {
  ...
  if (matchesNormalizedLocation(normalized, option)) return true

  if (!option.parentValue) return false
  const parentGroup = JOB_LOCATION_TAXONOMY.find(group => group.value === option.parentValue)
  if (!parentGroup || !matchesNormalizedLocation(normalized, {
    exactValues: parentGroup.broadExactValues,
    keywords: parentGroup.broadKeywords
  })) return false

  // Broad parent-only labels (for example "APAC Remote") remain eligible for
  // every child. Once a location names a child region, sibling filters must not
  // inherit the broad parent match.
  return !parentGroup.children.some(childOption => matchesNormalizedLocation(normalized, childOption))
}
