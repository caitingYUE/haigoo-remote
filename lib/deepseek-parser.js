/**
 * AI Parser (DeepSeek / Bailian)
 * Uses AI API to enhance job descriptions and extract structured data
 */

/**
 * Get AI Provider Configuration
 * Priorities:
 * 1. Bailian (Alibaba) - Preferred for stability/cost if configured
 * 2. DeepSeek - High performance alternative
 */
function getAiProviderConfig() {
    // 1. Check Bailian Keys
    const bailianKey = 
    ...
    for (const key of deepseekKeyVariants) {
        if (process.env[key]) {
            return {
                provider: 'DeepSeek',
...
async function callDeepSeekAPI(prompt, systemPrompt = '') {
    ...
    if (!config) {
        console.warn('[ai-parser] No AI API key found in environment variables');
        return null;
    }
...
Fields to extract:
1. location: Specific city, country, and timezone (e.g. "San Francisco, USA (GMT-8)"). If remote, specify "Remote" or "Remote - [Region] (Timezone)".
2. salary: Standardized salary range (e.g. "$100k - $150k", "€30k-€50k", "¥20k-40k/Month"). Extract from 'Compensation', 'Pay', 'Salary' sections. Keep original currency.
3. tags: Array of 5-10 keywords (Tech stack, skills, industry).
4. category: Best fit from standard categories (e.g. "前端开发", "后端开发", "产品经理").
5. formattedDescription: HTML formatted description. Use <h3> for section headers, <ul>/<li> for lists, <p> for paragraphs. Clean up layout. IMPORTANT: Do NOT translate technical terms (e.g. React, Python, AWS) - keep them in English.
6. language: "zh" or "en".
