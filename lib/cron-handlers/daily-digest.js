...
async function fetchActiveSubscriptions(targetEmail = '') {
  ...
  return await neonHelper.query(
    `SELECT
     ...
     WHERE s.status = 'active'
       ...
       AND ($1::text = '' OR LOWER(BTRIM(s.identifier)) = $1)
...
function buildSubject(selectedJobs) {
  const primaryCount = selectedJobs.filter((job) => job.matchTier === 'primary').length
  if (primaryCount > 0) {
    return `Haigoo 岗位订阅更新｜${selectedJobs.length} 个新机会`
  }
  return `Haigoo 岗位订阅更新｜${selectedJobs.length} 个相关机会`
}
...
export async function sendDailyDigests(reqOrRes, maybeRes) {
  ...
  const targetEmail = getRequestValue(req, 'targetEmail').toLowerCase()
  const now = new Date()
  ...
  if (!isAuthorizedRequest(req)) {
    writeEvent(res, {
      type: 'error',
      code: 'UNAUTHORIZED',
      message: 'This endpoint requires CRON_SECRET.'
    })
    res.end()
    return
  }
  ...
  try {
    ...
    const [subscriptions, jobs] = await Promise.all([
      fetchActiveSubscriptions(targetEmail),
      fetchCandidateJobs()
    ])
    ...
    if (!subscriptions || subscriptions.length === 0) {
      writeEvent(res, {
        ...
        message: targetEmail
          ? 'No active email subscription found for targetEmail'
    ...
    for (const subscription of subscriptions) {
      ...
      try {
        ...
        const subject = buildSubject(selection.selectedJobs)
        const payload = {
          topics: subscriptionTopics.displayTopics,
          reason: selection.reason,
