...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (neonHelper.isConfigured) {
            ...
            // Followed companies are checked daily; the rest rotate by oldest crawl.
            console.log('[Cron:CrawlTrustedJobs] Fetching next batch of companies to crawl...');
            const result = await neonHelper.query(`
                SELECT companies.* FROM trusted_companies companies
                WHERE companies.status = 'active'
                AND companies.careers_page IS NOT NULL
                AND length(companies.careers_page) > 0
                ORDER BY EXISTS (
                  SELECT 1 FROM mini_company_follows follows
                   WHERE follows.company_id = companies.company_id
                     AND follows.status = 'active'
                     AND (companies.last_crawled_at IS NULL OR companies.last_crawled_at < NOW() - INTERVAL '20 hours')
                ) DESC,
                companies.last_crawled_at ASC NULLS FIRST
                LIMIT 12
            `);
                    ...
                    if (neonHelper.isConfigured) {
                        try {
                            ...
                            await neonHelper.transaction(async (client) => {
                                // Keep current rows for auditability; matching history is
                                // written by the normal sync path before deactivation.
                                await client.query("UPDATE jobs SET status = 'inactive', updated_at = NOW() WHERE company_id = $1 AND (source != 'manual') AND (is_manually_edited IS NOT TRUE)", [company.id])
                                if (company.name) {
                                    await client.query("UPDATE jobs SET status = 'inactive', updated_at = NOW() WHERE company = $1 AND company_id IS NULL AND (source != 'manual') AND (is_manually_edited IS NOT TRUE)", [company.name])
                                }
