        ...
        const results = await pLimit(targetCompanies, 3, async (company, index) => {
            ...
            try {
                ...
                const result = await crawlCompanyJobs(company.id, url, {
                    fetchDetails: true,
                    maxDetailFetches: 20, // Increased to 20 to get more jobs with details
                    concurrency: 3,
                ...
                if (result.jobs && result.jobs.length > 0) {
                    ...
                    // 立即保存当前公司的 jobs
                    try {
                        // FIX: Delete existing jobs for this company before saving new ones to prevent duplicates
                        if (neonHelper.isConfigured) {
                            await neonHelper.transaction(async (client) => {
                                // 1. Delete by company_id (excluding manual jobs)
                                await client.query('DELETE FROM jobs WHERE company_id = $1 AND (source != \'manual\') AND (is_manually_edited IS NOT TRUE)', [company.id])
                                
                                // 2. Delete by company name if company_id is NULL (cleanup orphans)
                                if (company.name) {
                                    await client.query('DELETE FROM jobs WHERE company ILIKE $1 AND company_id IS NULL AND (source != \'manual\') AND (is_manually_edited IS NOT TRUE)', [company.name])
                                }
                            })
                        }
