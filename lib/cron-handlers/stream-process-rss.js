...
function extractCompany(title, description, url) {
  ...
  // Fallback 2: Check description for "About [Company]" pattern (Common in Himalayas RSS)
  ...
  // Fallback 3: Extract from URL (Himalayas specific)
  ...
  try {
    ...
    while (true) {
      ...
      const processedJobs = await pLimit(newItems, 3, async (item, index) => {
        ...
        let company = extractCompany(item.title, item.description, item.link);
