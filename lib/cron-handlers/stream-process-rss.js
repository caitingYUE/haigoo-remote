...
import { classifyJob, determineExperienceLevel, extractSalary, extractLocation, classifyRegion, isExplicitlyOverseas } from '../services/classification-service.js';
    ...
    while (true) {
      ...
      const results = await pLimit(newItems, 3, async (item, index) => {
        try {
          ...
          // ⏱ Expiry check: skip jobs published more than 30 days ago
          ...
          const industry = rawData.companyIndustry || rawData.industry || null; // Map industry field
          ...
          if (url && (!description || description.length < 500)) {
            try {
              // Enable AI for better extraction, especially for complex sites
              // This is expensive but worth it for RSS feeds where quality matters
