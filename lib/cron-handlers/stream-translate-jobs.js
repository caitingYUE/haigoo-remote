...
async function cleanFakeTranslations() {
  ...
  console.log('🧹 Pre-flight: Checking for fake translations (Aggressive Mode)...');
  try {
    const BATCH_SIZE = 1000;
    const MAX_CLEANUP_LOOPS = 10; // Clean up to 10,000 jobs per run
    let loops = 0;
    let totalFixed = 0;

    while (loops < MAX_CLEANUP_LOOPS) {
        // Fetch a batch of "Translated" jobs to audit
        const jobs = await neonHelper.query(`
            SELECT job_id, translations 
            FROM jobs 
            WHERE is_translated = true 
            ORDER BY updated_at ASC
            LIMIT ${BATCH_SIZE} OFFSET ${loops * BATCH_SIZE}
        `);
        ...
        // Stricter check: Must have at least 20 Chinese characters
        ...
        loops++;
        // If we found nothing in this batch, maybe subsequent batches are fine? 
        // No, we should check them all. But offset approach is safer.
    }
    console.log(`✅ Pre-flight cleanup finished. Fixed ${totalFixed} jobs.`);
  } catch (e) {
...
export default async function handler(req, res) {
  console.log('[Cron:TranslateJobs] VERSION: FIX_NUCLEAR_LOOP_CLEANUP');
