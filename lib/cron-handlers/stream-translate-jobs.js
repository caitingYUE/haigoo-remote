...
export default async function handler(req, res) {
  ...
  try {
    ...
    const aiEnabled = aiSetting?.value ?? false; // Default to false if not set
    console.log(`🔧 系统设置: AI 翻译 ${aiEnabled ? '已启用' : '已禁用'}`);
        ...
        if (toSave.length > 0) {
          try {
            ...
            if (batchUsage.total > 0) {
                await systemSettingsService.incrementTokenUsage(batchUsage);
