...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (NEON_CONFIGURED) {
      ...
      // 逐页处理
      // 注意：由于我们只查询未翻译的，且每次处理完会更新状态，所以理论上应该一直取第1页？
      // 不，如果使用分页，当第1页处理完变成已翻译后，原来的第2页会自动变成第1页。
      // 所以如果我们在循环中一直取 page=1，是可以的。
      // 但是为了避免某些顽固数据一直失败导致死循环，我们可以还是按照页码遍历，或者简单点：
      // 每次都取 page=1，但限制循环次数为 totalPages。
      // 更好策略：
      // 如果我们按 ID 排序，已处理的还在原位，只是状态变了。
      // 但我们的查询条件是 `isTranslated = false`。
      // 所以处理完一批，它们就会从查询结果中消失。
      // 因此，我们应该始终请求 `page: 1`。
      
      for (let i = 0; i < totalPages; i++) {
        const page = 1; // 始终处理第一页（因为已处理的会从队列中移出）
        ...
        console.log(`Processing batch ${currentBatchNum}/${totalPages}...`)
        ...
        // 读取一页未翻译数据
        const jobs = await readJobsFromNeon(queryParams, { page: 1, limit: pageSize })
        ...
        // 2. 筛选 (这里其实已经是筛选过的了，但保留"假翻译"检测逻辑作为双重保险)
        // 之前的逻辑是先读所有再筛选，现在读出来的就是未翻译的。
        // 但我们还是需要检测"假翻译"（虽然数据库里已经是false了，但以防万一）
        // 其实这里 jobs 都是 isTranslated=false 的，所以直接全部处理即可。
        
        const untranslated = jobs;
        const alreadyTranslated = 0;
        ...
        if (toSave.length > 0) {
          try {
            ...
            console.log(`✅ 保存处理结果 (page ${page}, count: ${toSave.length}, mode: upsert)`)
