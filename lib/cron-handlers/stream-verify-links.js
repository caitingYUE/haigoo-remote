...
async function checkUrlStatus(url) {
    ...
    try {
        ...
        if (typeof response.data === 'string') {
            ...
            // 只取 body 里的文字进行快速扫视（前3000个字符基本就够了，避免大页面正则卡顿）
            const bodyText = $('body').text().substring(0, 3000).toLowerCase().replace(/\s+/g, ' ');
        ...
        // 网络超时、DNS 错误、500 服务端异常等
        return { status: 'ambiguous', reason: `Network/Server Error: ${error.message}` };
    }
}
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const reportMsg = `验证完成！共扫 ${jobs.length} 岗位。判定存活: ${actions.alive}, 永久死亡剔除: ${actions.dead}, 触发安全盾转待审核: ${actions.ambiguous}。 (顺带 GC 物理清理 30天僵尸: 完成)`;
