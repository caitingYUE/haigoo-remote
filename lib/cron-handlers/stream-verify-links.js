...
import { archiveJobSnapshot, rebuildCompanyHiringProfile } from '../services/mini-company-match-service.js';
...
export async function checkUrlStatus(rawUrl) {
    const url = String(rawUrl || '').trim();
    try {
        const parsedUrl = new URL(url);
        if (!['http:', 'https:'].includes(parsedUrl.protocol)) throw new Error('Unsupported protocol');
    } catch (_) {
        // A malformed value is a data-quality issue, not proof that a role closed.
        return { status: 'ambiguous', reason: 'Invalid URL format' };
    }
    ...
    try {
        ...
        // 2. 401/403/Authwall 只能说明探针被拦截，不能证明岗位失效。
        if (statusCode === 403 || statusCode === 401 || url.includes('linkedin.com') || finalUrl.includes('linkedin.com/authwall')) {
            return { status: 'error_retry', reason: `Blocked or Authwall (HTTP ${statusCode})` };
        }
...
export default async function handler(req, res) {
    ...
    if (payload && payload.userId) {
        ...
        isAdmin = !!(requester?.roles?.admin || SUPER_ADMIN_EMAILS.includes(String(requester?.email || '').toLowerCase()))
    }
    ...
    try {
        ...
        // --- 1. 获取需要重检的链接 ---
        // Recheck automatically deactivated roles too, so a reopened posting can
        // recover instead of remaining permanently hidden. Manually archived
        // roles are intentionally excluded.
        const QUERY_LIMIT = 30;
        const jobsResult = await neonHelper.query(`
            SELECT jobs.*,
                   COALESCE(jobs.company_id, (
                     SELECT company_id FROM trusted_companies
                      WHERE LOWER(BTRIM(name)) = LOWER(BTRIM(jobs.company)) LIMIT 1
                   )) AS effective_company_id
            FROM jobs
            WHERE jobs.is_approved = true
              AND NULLIF(BTRIM(jobs.url), '') IS NOT NULL
              AND (
                jobs.status = 'active'
                OR (
                  jobs.status = 'inactive'
                  AND jobs.haigoo_comment LIKE '[自动巡查] 判定链接彻底死亡并下线，原因:%'
                )
              )
            ORDER BY jobs.last_verified_at ASC NULLS FIRST
            LIMIT $1
        `, [QUERY_LIMIT]);
        ...
        for (const job of results) {
            ...
            try {
                if (v.status === 'dead') {
                    await archiveJobSnapshot({ ...job, id: job.job_id, companyId: job.effective_company_id }, { closed: true });
                    ...
                    if (job.effective_company_id) await rebuildCompanyHiringProfile(job.effective_company_id);
                    actions.dead++;
                } else if (v.status === 'ambiguous') {
                    await archiveJobSnapshot({ ...job, id: job.job_id, companyId: job.effective_company_id }, { closed: false });
                    ...
                    // Reopen roles that were previously deactivated by this verifier.
                    await neonHelper.query(`
                        UPDATE jobs
                        SET status = 'active',
                            last_verified_at = NOW(),
                            haigoo_comment = CASE WHEN status = 'inactive' THEN NULL ELSE haigoo_comment END
                        WHERE job_id = $1
                    `, [job.job_id]);
                    if (job.status === 'inactive' && job.effective_company_id) {
                        await rebuildCompanyHiringProfile(job.effective_company_id);
                    }
                    actions.alive++;
                }
            } catch (err) {
        ...
        const reportMsg = `验证完成！共扫 ${jobs.length} 岗位。存活: ${actions.alive}, 永久剔除: ${actions.dead}, 模糊转待审: ${actions.ambiguous}, 网络/权限拦截容错: ${actions.error_retry}。`;
