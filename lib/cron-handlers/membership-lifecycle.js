...
export default async function handler(req, res) {
  ...
  try {
    const limit = req.query.limit || req.body?.limit || 200
    const [result, paypal] = await Promise.all([
      sweepExpiredMemberships({ limit }),
      paypalPaymentService.reconcilePendingOrders({ limit: Math.min(100, Number(limit) || 50) })
    ])

    return res.status(200).json({ ...result, paypal })
  } catch (error) {
