...
function ParagraphText({ text }: { text: string }) {
  ...
  return (
    <div className="hg-video-notes-paragraph break-words text-base leading-8 text-slate-700">
...
function VideoNoteBlock({ block }: { block: CorporateEnglishVideoNoteBlock }) {
  ...
  if (block.type === 'quote') return <blockquote className="hg-video-notes-quote border-l-2 px-5 py-3 text-base font-semibold leading-8 text-slate-700"><InlineText text={block.text || ''} /></blockquote>
...
export function VideoNotesArticle({ notes, emptyMessage = '该课程的视频笔记正在整理中。' }: { notes: CorporateEnglishVideoNoteBlock[]; emptyMessage?: string }) {
  if (!notes.length) {
    return <div className="hg-video-notes-empty px-5 py-10 text-center text-sm font-semibold leading-6 text-slate-500">{emptyMessage}</div>
  }
  return (
    <article className="hg-video-notes-article space-y-5">
