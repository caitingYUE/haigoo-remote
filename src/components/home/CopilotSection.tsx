...
export default function CopilotSection() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 403) {
          showWarning('本期体验次数已用完', '职业方案生成功能暂时不可继续使用，你仍可浏览全部公开岗位信息。')
        } else {
  ...
  if (result) {
    return (
      <div className="py-16 border-b border-slate-100 bg-gradient-to-br from-[#eff5fb]/50 to-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
              <Sparkles className="w-6 h-6 text-[#466f9d]" />
              您的专属远程求职方案
            </h2>
            <div className="flex gap-4">
              <button
                onClick={resetForm}
                className="text-sm text-slate-500 hover:text-[#466f9d] underline"
              >
                重新生成
              </button>
              <button
                onClick={() => navigate('/profile?tab=custom-plan')}
                className="text-sm font-bold text-[#466f9d] hover:text-[#345d88] flex items-center gap-1"
              >
                  ...
                  {result.applicationPlan.steps.slice(0, isCollapsed ? 3 : undefined).map((step, i) => (
                    <div key={i} className="flex gap-4 items-start group">
                      <div className="w-16 shrink-0 text-right">
                        <span className="text-xs font-bold text-[#466f9d] bg-[#eff5fb] px-2 py-1 rounded block w-full text-center truncate">
                          {step.week}
                        </span>
                      </div>
                      <div className="relative pb-4 border-l-2 border-slate-100 pl-4 last:border-0 last:pb-0">
                        <div className="absolute -left-[9px] top-1.5 w-4 h-4 rounded-full bg-white border-2 border-[#c9dce8] group-hover:border-[#587faa] transition-colors"></div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 animate-in fade-in slide-in-from-top-4 duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {result.recommendations.map((job, i) => {
                    ...
                    const matchColor = job.match === '高匹配' ? 'text-[#466f9d]' : job.match === '中匹配' ? 'text-amber-600' : 'text-slate-500';
                    return (
                      <div key={i} className="p-3 bg-slate-50 rounded-xl hover:bg-[#eff5fb] transition-colors cursor-pointer border border-slate-100" onClick={() => navigate('/jobs')}>
          ...
          <div className="mt-6 flex justify-center">
            <button
              ...
              className="flex items-center gap-2 px-6 py-2 bg-white border border-slate-200 rounded-full text-sm font-medium text-slate-600 hover:text-[#466f9d] hover:border-[#c9dce8] transition-all shadow-sm"
            >
              {isCollapsed ? (
          ...
          <div className="mt-4 text-center">
            <p className="text-xs text-slate-400">
              ...
              <button onClick={() => navigate('/profile?tab=custom-plan')} className="text-[#587faa] hover:underline ml-1">个人中心</button>
            </p>
          </div>
  ...
  return (
    <div className="py-20 border-b border-slate-100 bg-white relative overflow-hidden">
      {/* Background Decor */}
      <div className="absolute top-0 right-0 -mr-20 -mt-20 w-96 h-96 bg-[#eff5fb] rounded-full blur-3xl opacity-50 pointer-events-none"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="min-w-0 flex flex-col">
            <div className="mb-8">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#dce9f5] text-[#345d88] text-xs font-bold mb-4">
                <Sparkles className="w-3.5 h-3.5" />
                AI 远程求职助手 (Beta)
              </span>
              <h2 className="text-3xl font-bold text-slate-900 mb-4 leading-tight">
                定制您的 <span className="text-[#466f9d]">远程工作</span><br />
                全流程计划
              </h2>
              ...
              <div className="space-y-6">
                    ...
                    {[
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center justify-center gap-2 p-3 rounded-xl border transition-all ${formData.goal === opt.value
                          ? 'border-[#466f9d] bg-[#eff5fb] text-[#345d88] ring-1 ring-[#466f9d]'
                          : 'border-slate-200 text-slate-600 hover:border-[#9fbbd2] hover:bg-slate-50'
                          }`}
                      >
                        {opt.icon}
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-xl border-slate-200 focus:border-[#587faa] focus:ring-[#587faa] py-2.5 text-sm"
                  >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 rounded-xl border-slate-200 focus:border-[#587faa] focus:ring-[#587faa] py-2 text-sm"
                      />
                    </div>
                    <div className="relative">
                    ...
                    <div className="relative">
                      ...
                      <select
                        ...
                        className="w-full pl-9 rounded-xl border-slate-200 focus:border-[#587faa] focus:ring-[#587faa] py-2 text-sm text-slate-600"
                      >
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full bg-[#466f9d] text-white rounded-xl py-3.5 font-bold hover:bg-[#345d88] transition-all shadow-lg shadow-[#c9dce8] disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
          ...
          <div className="hidden lg:block relative">
            <div className="relative z-10 bg-slate-900 rounded-2xl shadow-2xl border border-slate-800 p-2 overflow-hidden transform rotate-2 hover:rotate-0 transition-transform duration-500">
                ...
                {loading ? (
                  ...
                  <div className="py-6 space-y-6">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="relative flex h-3 w-3">
                        <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#7f9fbc] opacity-75"></span>
                        <span className="relative inline-flex rounded-full h-3 w-3 bg-[#587faa]"></span>
                      </span>
                      <span className="text-[#7f9fbc] text-xs font-bold uppercase tracking-wider">正在为您生成专属远程求职方案...</span>
                    </div>

                    {/* Step 1: Diagnosis */}
                    <div className={`bg-slate-800/50 rounded-xl p-4 border transition-all duration-500 ${loadingStep >= 0 ? 'border-[#587faa]/50 shadow-lg shadow-[rgba(36,63,92,0.20)]' : 'border-slate-800 opacity-50'}`}>
                      <div className="flex items-start gap-3">
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${loadingStep > 0 ? 'bg-green-500 text-white' : 'bg-[#466f9d] text-white'}`}>
                          ...
                          {loadingStep === 0 && (
                            <div className="space-y-1.5 animate-in fade-in duration-700">
                              ...
                              <div className="h-1.5 w-full bg-slate-700 rounded-full overflow-hidden mt-2">
                                <div className="h-full bg-[#587faa] animate-progress"></div>
                              </div>
                            </div>
                          )}
                          {loadingStep > 0 && (
                    ...
                    {/* Step 2: Matching */}
                    <div className={`bg-slate-800/50 rounded-xl p-4 border transition-all duration-500 ${loadingStep >= 1 ? 'border-[#587faa]/50 shadow-lg shadow-[rgba(36,63,92,0.20)]' : 'border-slate-800 opacity-50'}`}>
                      <div className="flex items-start gap-3">
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${loadingStep > 1 ? 'bg-green-500 text-white' : loadingStep === 1 ? 'bg-[#466f9d] text-white' : 'bg-slate-700 text-slate-400'}`}>
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          {loadingStep === 1 && (
                            <div className="space-y-2 animate-in fade-in duration-500">
                              ...
                              <div className="text-xs text-[#9fbbd2]">正在检索全球 2000+ 远程企业数据库...</div>
                            </div>
                          )}
                          {loadingStep > 1 && (
                            <div className="flex gap-2 mt-1">
                              <span className="text-[10px] bg-[#587faa]/20 text-[#9fbbd2] px-2 py-0.5 rounded border border-[#587faa]/30">匹配到 5+ 核心岗位</span>
                            </div>
                          )}
                        </div>
                      </div>
                    ...
                    <div className={`bg-slate-800/50 rounded-xl p-4 border transition-all duration-500 ${loadingStep >= 2 ? 'border-[#587faa]/50 shadow-lg shadow-[rgba(36,63,92,0.20)]' : 'border-slate-800 opacity-50'}`}>
                      <div className="flex items-start gap-3">
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${loadingStep > 2 ? 'bg-green-500 text-white' : loadingStep === 2 ? 'bg-[#466f9d] text-white' : 'bg-slate-700 text-slate-400'}`}>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className={`flex gap-4 transition-opacity duration-500 ${demoStep >= 0 ? 'opacity-100' : 'opacity-30'}`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-colors duration-500 ${demoStep >= 0 ? 'bg-[#466f9d] text-white' : 'bg-slate-800 text-slate-500'}`}>
                        1
                      </div>
                      <div className="space-y-2 w-full">
                        <div className="text-slate-300 font-medium text-sm">AI 深度解析您的背景与目标</div>
                        <div className={`h-20 w-full bg-slate-800 rounded border border-slate-700 p-3 transition-all duration-500 ${demoStep === 0 ? 'ring-2 ring-[#587faa] bg-slate-800/80' : ''}`}>
                    ...
                    <div className={`flex gap-4 transition-opacity duration-500 ${demoStep >= 1 ? 'opacity-100' : 'opacity-30'}`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-colors duration-500 ${demoStep >= 1 ? 'bg-[#466f9d] text-white' : 'bg-slate-800 text-slate-500'}`}>
                        2
                      </div>
                      <div className="space-y-2 w-full">
                    ...
                    <div className={`flex gap-4 transition-opacity duration-500 ${demoStep >= 2 ? 'opacity-100' : 'opacity-30'}`}>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-colors duration-500 ${demoStep >= 2 ? 'bg-[#466f9d] text-white' : 'bg-slate-800 text-slate-500'}`}>
                        3
                      </div>
                      <div className="space-y-2 w-full">
                        <div className="text-slate-300 font-medium text-sm">生成面试策略与投递计划</div>
                        <div className="grid grid-cols-2 gap-2">
                          <div className={`h-20 bg-slate-800 rounded border border-slate-700 p-2 transition-all duration-500 ${demoStep === 2 ? 'bg-[#243f5c]/20 border-[#587faa]/50' : ''}`}>
              ...
              {/* Floating Badge */}
              {!loading && (
                <div className={`absolute bottom-8 right-8 bg-[#466f9d] text-white px-4 py-2 rounded-lg shadow-lg text-sm font-bold flex items-center gap-2 transition-all duration-500 transform ${demoStep === 3 ? 'translate-y-0 opacity-100 scale-110' : 'translate-y-4 opacity-0 scale-90'}`}>
            ...
            <div className="absolute -inset-4 bg-gradient-to-tr from-[#587faa] to-[#b7791f] rounded-[2rem] opacity-20 blur-2xl -z-10"></div>
          </div>
