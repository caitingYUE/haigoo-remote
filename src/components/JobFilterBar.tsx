...
const FilterDropdown: React.FC<FilterDropdownProps> = ({ label, activeLabel, isOpen, onToggle, onClose, children, isActive, variant = 'default', icon }) => {
  ...
  let buttonClass = "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all border whitespace-nowrap ";

  if (isActive || isOpen) {
    if (variant === 'solid-blue') {
      buttonClass += "bg-indigo-600 text-white border-indigo-600 shadow-sm hover:bg-indigo-700";
    } else if (variant === 'solid-purple') {
      buttonClass += "bg-indigo-600 text-white border-indigo-600 shadow-sm hover:bg-indigo-700";
    } else {
      buttonClass += "bg-indigo-50 text-indigo-600 border-indigo-200";
    }
  } else {
    buttonClass += "bg-white text-slate-600 border-slate-200 hover:bg-slate-50 hover:border-slate-300 shadow-sm";
  }
