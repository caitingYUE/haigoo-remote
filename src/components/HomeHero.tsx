...
// ── Generated Plan View ────────────────────────────────────────────────────────
function GeneratedPlanView({ plan, isGuest }: { plan: any, isGuest: boolean }) {
    if (!plan) return null;
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 flex-shrink-0 flex flex-col items-center gap-3">
                {isGuest && (
                    <div className="text-xs text-amber-600 bg-amber-50 rounded-lg px-4 py-2 border border-amber-100 font-medium w-full text-center">
                        <span className="inline-block mr-1">⚠️</span> 方案仅缓存 5 分钟，<a href="/login" className="underline font-bold">立即登录</a> 永久保存并解锁所有后续功能。
                    </div>
                )}
                <div className="flex w-full gap-2">
                    <a href="/jobs" className="flex-1 py-2.5 bg-indigo-600 text-white rounded-xl text-sm font-bold flex justify-center items-center gap-2 hover:bg-indigo-700 transition-colors shadow-sm">
                        去大厅查看更多岗位 <ArrowRight className="w-4 h-4" />
                    </a>
                    {!isGuest && (
                        <a href="/profile" className="flex-1 py-2.5 bg-slate-50 text-slate-700 rounded-xl text-sm font-bold flex justify-center items-center gap-2 hover:bg-slate-100 border border-slate-200 transition-colors">
                            前往个人中心查看完整版
                        </a>
                    )}
                </div>
