                ...
                <div>
                    ...
                    {showReferralModule && (
                        <section className="pb-5">
                            <div className="rounded-xl border border-indigo-100 bg-gradient-to-br from-indigo-50/60 via-white to-white p-4 space-y-3">
                                <div className="flex items-center justify-between gap-3">
                                    <h3 className="text-base font-bold text-slate-900">帮我内推</h3>
                                    <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border ${isMember ? 'bg-indigo-100 text-indigo-700 border-indigo-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                                        <Crown className="w-3 h-3" />
                                        会员专属功能
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2.5">
                                    {referralContacts.map((contact, index) => (
                                        <div key={`ref-contact-${index}`} className="rounded-lg border border-slate-200 bg-white px-3 py-3 flex flex-col gap-2.5">
                                            <div className="flex items-center justify-between gap-3">
                                                <div className="min-w-0">
                                                    <div className="text-sm font-semibold text-slate-900 truncate">
                                                        {isMember ? (contact.name || '-') : `${formatMaskedName(contact.name)}＊`}
                                                    </div>
                                                    <div className="text-xs text-slate-500 mt-0.5 truncate">{contact.title || '-'}</div>
                                                </div>
                                                <button
                                                    onClick={() => applyViaReferralContact(contact)}
                                                    className={`inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-colors ${isMember ? 'bg-slate-900 text-white hover:bg-indigo-700' : 'bg-slate-100 text-slate-400 cursor-pointer hover:bg-indigo-50 hover:text-indigo-500'}`}
                                                >
                                                    {isMember ? <Mail className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5" />}
                                                    邮箱直申
                                                </button>
                                            </div>
                                            <div className={`grid grid-cols-[1fr_auto] items-center gap-3 ${isMember ? '' : 'opacity-70'}`}>
                                                <div className={`text-xs ${isMember ? 'text-indigo-700 font-semibold' : 'text-slate-400 blur-[3px] select-none'}`}>
                                                    {isMember ? (contact.hiringEmail || '-') : '会员可见'}
                                                </div>
                                                {isMember ? (
                                                    <a
                                            ...
                                            {!isMember && (
                                                <div className="text-[11px] text-amber-600 bg-amber-50 border border-amber-100 rounded-md px-2 py-1">
                                                    开通会员后解锁联系人邮箱与领英主页
                                                </div>
                                            )}
                                        </div>
