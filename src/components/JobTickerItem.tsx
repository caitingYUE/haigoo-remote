...
export default function JobTickerItem({ job, onOpen }: { job: any, onOpen?: (job: any) => void }) {
    ...
    return (
        <div 
            className="flex-shrink-0 flex items-center gap-3 bg-white/80 backdrop-blur-sm rounded-full border border-slate-100 px-4 py-2 mx-2 shadow-sm hover:shadow-md hover:border-[#dce9f5] transition-all cursor-pointer group min-w-[240px]"
            onClick={handleOpen}
            onKeyDown={(e) => {
        ...
        >
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-center">
                <h4 className="font-bold text-slate-800 text-xs truncate max-w-[140px] group-hover:text-[#466f9d] transition-colors">
                    {job.title}
                </h4>
            ...
            <div className="w-6 h-6 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-[#466f9d] group-hover:text-white transition-colors flex-shrink-0">
