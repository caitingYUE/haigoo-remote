...
async function extractTextViaServer(file: File): Promise<string | null> {
  try {
    ...
    if (!json.success || !json.data?.text) {
      console.warn('Server parse returned no text')
      return null
    }
...
export async function parseResumeFileEnhanced(file: File): Promise<ParsedResume> {
  ...
  // 2. 如果前端解析失败，尝试服务端解析
  console.log(`[resume-parser] Local parse failed, trying server...`)
  const serverText = await extractTextViaServer(file)
  if (serverText && serverText.length > 0) {
    console.log(`[resume-parser] Server parsed, ${serverText.length} chars`)
    return extractResumeFields(serverText)
  }
