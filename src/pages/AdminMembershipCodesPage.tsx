        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="bg-[linear-gradient(120deg,#f5f3ff_0%,#ffffff_54%,#eff6ff_100%)] p-5 sm:p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h1 className="flex items-center gap-2 text-2xl font-black text-slate-950"><Ticket className="h-6 w-6 text-[#466f9d]" />会员兑换码管理</h1>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {data?.isSuperAdmin ? (
                  <button type="button" onClick={openGenerate} className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#466f9d] px-4 text-sm font-bold text-white shadow-sm hover:bg-[#345d88] focus:outline-none focus:ring-2 focus:ring-[#c9dce8]">
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-slate-600">
              <span className="rounded-full bg-[#dce9f5] px-3 py-1.5 text-[#345d88]">已分发 {Number(data?.summary?.distributed || 0)}</span>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <form onSubmit={event => { event.preventDefault(); void load(1) }} className="grid grid-cols-2 gap-2 md:grid-cols-3 xl:grid-cols-[minmax(190px,1fr)_132px_132px_150px_170px_auto]">
            <label className="relative col-span-2 md:col-span-3 xl:col-span-1">
              ...
              <input aria-label="搜索兑换码、批次或渠道" value={search} onChange={event => setSearch(event.target.value)} placeholder="搜索兑换码、批次或渠道" className="h-11 w-full rounded-xl border border-slate-200 pl-10 pr-3 text-sm outline-none focus:border-[#9fbbd2] focus:ring-2 focus:ring-[#dce9f5]" />
            </label>
          ...
          {data?.isSuperAdmin && selectedBatch ? (
            <div className="mt-3 flex flex-wrap justify-end gap-2 border-t border-slate-100 pt-3">
              <button type="button" onClick={() => void exportBatch(selectedBatch)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-700 hover:bg-[#eff5fb] hover:text-[#345d88]"><Download className="h-4 w-4" />导出所选批次</button>
            ...
            <table className="min-w-[1260px] w-full divide-y divide-slate-100">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  return (
                    <tr key={row.id} className={`${muted ? 'bg-slate-50/80 text-slate-400' : 'hover:bg-[#eff5fb]/30'}`}>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex flex-wrap items-center gap-2">
                          <label
                            className={`inline-flex h-9 cursor-pointer items-center gap-1.5 rounded-lg border px-2.5 text-xs font-bold transition ${row.distributed ? 'border-[#c9dce8] bg-[#eff5fb] text-[#345d88]' : 'border-slate-200 bg-white text-slate-600'} ${row.status !== 'unused' || distributionUpdating === row.id ? 'cursor-not-allowed opacity-60' : 'hover:border-[#c9dce8] hover:bg-[#eff5fb]'}`}
                          ...
                          >
                            <input
                              ...
                              className="h-4 w-4 accent-[#466f9d]"
                              aria-label={`将兑换码 ${row.code} 标记为${row.distributed ? '未分发' : '已分发'}`}
                            />
                            已分发
                          </label>
                          {data.isSuperAdmin && !row.isMasked ? <button type="button" aria-label={`复制兑换码 ${row.code}`} onClick={() => void copyText(row.code, row.id)} className="rounded-lg border border-slate-200 p-2 text-slate-600 hover:bg-white focus:outline-none focus:ring-2 focus:ring-[#c9dce8]" title="复制兑换码">{copied === row.id ? <Check className="h-4 w-4 text-emerald-600" /> : <Clipboard className="h-4 w-4" />}</button> : null}
      ...
      {showGenerate ? (
        <div className="fixed inset-0 z-[11000] flex items-center justify-center overflow-y-auto bg-slate-950/60 p-4 backdrop-blur-sm">
          <div role="dialog" aria-modal="true" aria-labelledby="membership-code-generate-title" className="relative w-full max-w-2xl rounded-3xl bg-white p-5 shadow-2xl sm:p-6">
            ...
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#dce9f5] text-[#345d88]"><KeyRound className="h-6 w-6" /></div>
            ...
            {generatedCodes.length ? (
              <div className="mt-5">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <button type="button" onClick={() => void copyText(generatedCodes.join('\n'), 'generated')} className="inline-flex items-center gap-2 rounded-xl bg-[#466f9d] px-4 py-2.5 text-sm font-bold text-white">{copied === 'generated' ? <Check className="h-4 w-4" /> : <Clipboard className="h-4 w-4" />}复制全部</button>
              ...
              <form onSubmit={submitGenerate} className="mt-5 grid gap-4 sm:grid-cols-2">
                <label className="sm:col-span-2"><span className="text-sm font-bold text-slate-700">批次名称</span><input autoFocus required value={form.name} onChange={event => setForm(prev => ({ ...prev, name: event.target.value }))} placeholder="例如：2026 Q3 淘宝月度会员" className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-[#9fbbd2] focus:ring-2 focus:ring-[#dce9f5]" /></label>
                ...
                <div className="sm:col-span-2 flex justify-end gap-2"><button type="button" disabled={generating} onClick={closeGenerate} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold disabled:opacity-50">取消</button><button type="submit" disabled={generating} className="inline-flex items-center gap-2 rounded-xl bg-[#466f9d] px-5 py-2.5 text-sm font-bold text-white disabled:opacity-60">{generating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}确认生成</button></div>
              </form>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[11000] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm">
          <div role="dialog" aria-modal="true" aria-labelledby="membership-code-edit-title" className="relative w-full max-w-lg rounded-3xl bg-white p-5 shadow-2xl sm:p-6">
            ...
            <form onSubmit={confirmEditBatch} className="mt-5 space-y-4">
              <label className="block"><span className="text-sm font-bold text-slate-700">批次名称</span><input autoFocus required value={editForm.name} onChange={event => setEditForm(prev => ({ ...prev, name: event.target.value }))} className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-[#9fbbd2] focus:ring-2 focus:ring-[#dce9f5]" /></label>
              <label className="block"><span className="text-sm font-bold text-slate-700">销售渠道</span><input required value={editForm.channel} onChange={event => setEditForm(prev => ({ ...prev, channel: event.target.value }))} className="mt-2 h-11 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none focus:border-[#9fbbd2] focus:ring-2 focus:ring-[#dce9f5]" /></label>
              ...
              <div className="flex justify-end gap-2"><button type="button" disabled={actionLoading} onClick={() => setEditTarget(null)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold disabled:opacity-50">取消</button><button type="submit" disabled={actionLoading} className="inline-flex items-center gap-2 rounded-xl bg-[#466f9d] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60">{actionLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : null}保存</button></div>
            </form>
          </div>
        </div>
      ) : null}
