  ...
  return <div className="space-y-4">
    <SectionCard title="新建简历诊断" description="CRM 内部底稿，不写入用户前台或匹配数据">
      ...
      <div className="mt-3 flex flex-col justify-between gap-2 border-t border-slate-100 pt-3 text-xs text-slate-500 md:flex-row md:items-center">
        <label className="inline-flex cursor-pointer items-center gap-2"><input type="checkbox" checked={includeCrmContext} onChange={event => setIncludeCrmContext(event.target.checked)} disabled={running} className="h-4 w-4 rounded border-slate-300 text-indigo-600" />带入 CRM 背景、需求与服务方案</label>
  ...
  return <><div className="space-y-4">
    <SectionCard title={`诊断成果 v${artifact.version}`} description={`${run.sourceResumeName} · ${run.model}${usage ? ` · ${usage.toLocaleString()} tokens` : ''}`} action={<div className="flex flex-wrap gap-2"><CareerRunStatus run={run} />{artifact.status === 'draft' && <button onClick={() => setProfileOpen(true)} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600">编辑画像</button>}<button disabled={running} onClick={() => void onRegenerate()} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 disabled:opacity-40">重新生成</button></div>}>
      <div className="space-y-5">
        <div><h4 className="text-lg font-bold text-slate-950">{content.summary.headline}</h4><p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-slate-700">{content.summary.positioning}</p><p className="mt-3 border-l-2 border-indigo-300 pl-3 text-sm leading-6 text-slate-700"><b>咨询重点：</b>{content.summary.consultantBrief}</p></div>
    ...
    <details className="rounded-xl border border-slate-200 bg-white"><summary className="cursor-pointer px-5 py-4 text-sm font-bold text-slate-800">查看证据账本（{content.evidenceLedger.length}）</summary><div className="divide-y divide-slate-100 border-t border-slate-100 px-5">{content.evidenceLedger.map(item => <div key={item.id} className="py-3"><div className="flex items-center gap-2"><span className="text-[11px] font-bold text-indigo-600">{item.id} · {item.grade}</span><span className="text-sm font-semibold text-slate-700">{item.statement}</span></div><blockquote className="mt-2 border-l-2 border-slate-200 pl-3 text-xs leading-5 text-slate-500">{item.sourceExcerpt}</blockquote></div>)}</div></details>
