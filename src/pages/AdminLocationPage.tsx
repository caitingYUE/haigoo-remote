...
export default function AdminLocationPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-4">
      ...
      <p className="text-sm text-slate-500">按行填写关键字，前端按包含匹配；anywhere/everywhere/worldwide/remote 属于 Global（两边都显示）。</p>
      ...
      <div className="flex justify-end">
        <button onClick={save} className="px-4 py-2 rounded-lg bg-[#466f9d] text-white shadow-sm hover:bg-[#345d88] transition-colors disabled:opacity-50" disabled={saving}>{saving ? '保存中...' : '保存'}</button>
      </div>
    </div>
  )
}
