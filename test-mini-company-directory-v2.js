...
const directorySql = gateway.slice(gateway.indexOf('const companyDirectorySnapshotSql'), gateway.indexOf('async function readMiniNotes('))
assert.match(directorySql, /JOIN jobs j ON j\.company_id = tc\.company_id/)
assert.match(directorySql, /LEFT JOIN company_job_history h ON h\.company_id = tc\.company_id[\s\S]+h\.source_job_id = j\.job_id/)
assert.doesNotMatch(directorySql, /job_record\.job_id IS NULL|current_job\.job_id IS NULL/)
