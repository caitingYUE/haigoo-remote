...
- 展示策略：优先显示处理后的 `tags`；若不存在则回退使用 `skills`。
- 兜底规则：当无任何标签或技能时，显示 `remote` 作为兜底，仅此一个标签。
- 组件接入：使用 `SingleLineTags` 组件渲染，`size` 为岗位卡片 `xs`、推荐卡片 `sm`。
- 交互与可读性：标签采用圆角徽标样式，`+N` 显示隐藏数量；内容缩放时自动适配，不引起卡片高度变化。