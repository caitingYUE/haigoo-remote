...
## 8. 岗位翻译展示策略 (Job Translation Display Strategy)

岗位详情页当前采用“前台默认中文、用户可切回原文”的展示策略：

*   **默认展示**: 若岗位已存在 `translations` 中文内容，详情页默认直接展示中文版本。
*   **切换方式**: 页面保留一个极简翻译切换入口，用户可手动切回原文查看。
*   **前台展示原则**: 不再对用户展示翻译次数、会员限制或其它策略性提示，避免影响浏览与申请动线。
*   **后台兜底**: 翻译次数统计与权限链路仍保留在后端，免费用户上限提高为高阈值兜底，仅用于兼容旧逻辑与异常回退，不作为前台策略展示内容。
