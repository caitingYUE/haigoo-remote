...
assert.equal(mapCompanyJobSummary({ id: 'Field Application Engineer  – legacy', companyId: 'company-1', title: 'Field Application Engineer' }, 'company-1')?.id, 'Field Application Engineer – legacy', 'legacy title-based job IDs must use one canonical whitespace form')
...
assert.match(gateway, /buildMiniCompanyContactPayload/)
assert.match(gateway, /canAccessContacts: viewer\.capabilities\.canAccessCompanyContacts/)
assert.match(gateway, /contactPreview: contactPayload\.contactPreviews\.length > 0/)
assert.match(gateway, /publicApplicationEmails/)
assert.match(gateway, /publicJobTitles:/)
assert.match(gateway, /openRoleCategories:/)
...
assert.match(gateway, /import \{ rankDirectoryCompanies \}/)
assert.match(gateway, /rankDirectoryCompanies\(filtered, normalizedSearch/)
assert.doesNotMatch(gateway, /tc\.aliases/)
const reminderService = fs.readFileSync(new URL('./lib/services/mini-wechat-reminder-service.js', import.meta.url), 'utf8')
assert.match(reminderService, /message\/subscribe\/send/)
assert.match(reminderService, /miniprogram_state: wechatMiniProgramState\(\)/)
assert.match(matchService, /member_status IN \('active', 'pro', 'lifetime'\)/)
assert.match(matchService, /member_cycle_start_at IS NULL/)
assert.match(matchService, /mini_reminder_role_matches/)
assert.match(fs.readFileSync('server-utils/dal/migrations/090_mini_wechat_matching_reminders.sql', 'utf8'), /custom_role_terms/)
assert.match(reminderService, /page: `pages\/company-detail\/index\?id=/)
assert.doesNotMatch(reminderService, /page: `pages\/web-view/)
...
assert.match(matchCardStyles, /grid-template-columns: minmax\(0, 1fr\) (?:132|144)px/)
assert.match(matchCardStyles, /\.match-company-card__identity-main \{ display: flex;[^}]+flex-direction: column/)
assert.match(matchCardStyles, /\.match-company-card__fit \{[^}]*min-height:\s*0/)
...
assert.match(watchPage, /recentMatchDate/)
assert.match(watchPage, /非会员仅匹配一次/)
assert.match(watchPage, /member_dynamic/)
assert.match(matchCard, /onOpenJob\(company\)/)
assert.match(watchPage, /watch-feed__freshness/)
assert.match(watchPage, /方向与企业/)
assert.match(watchPage, /fixed_free/)
assert.match(companyDetail, /setClipboardData/)
assert.match(companyDetail, /开放岗位/)
assert.match(companyDetail, /pages\/job-detail\/index/)
assert.match(companyDetail, /企业联系人/)
assert.match(companyDetail, /升级会员后可查看联系人信息/)
assert.match(companyDetail, /company-detail__contact-card--locked/)
assert.doesNotMatch(companyDetail, /company-job__source/)
assert.match(companyDetail, /company-detail__popular/)
...
assert.match(companyDetail, /access\?\.contacts && fullContacts.length/)
...
assert.match(companyCloudRun, /companyJobMetadata\(await readCurrentCompanyJobMetadata\(\), id, companyName\)/)
assert.match(companyCloudRun, /readFormalCompanyJobs\(id, String\(result\.company\?\.name/)
assert.match(companyCloudRun, /openJobCount > 0 && !jobs\.length/)
assert.match(companyCloudRun, /openJobCount: companyMetadata\.jobs\.length/)
assert.match(companyCloudRun, /openJobCount: jobs\.length/)
