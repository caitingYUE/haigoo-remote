...
export default function EditorialSearch({ value, placeholder, disabled = false, gateLabel = '', onInput, onGateClick }: EditorialSearchProps) {
  return (
    <View className='editorial-search' aria-role={disabled && onGateClick ? 'button' : undefined} aria-label={disabled ? gateLabel || placeholder : '搜索'} onClick={disabled ? onGateClick : undefined}>
      ...
      <Input className='editorial-search__input' value={value} disabled={disabled} placeholder={placeholder} onInput={(event) => onInput(event.detail.value)} />
