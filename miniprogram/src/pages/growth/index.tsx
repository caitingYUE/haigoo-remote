    ...
    <View className='page-shell growth-page'>
      ...
      <View className='growth-list'>
        ...
        {visibleNotes.map((note) => (
          <EditorialRow className='growth-card' key={note.id} label={`阅读 ${note.titleZh || note.title}`} onClick={() => open(note)}>
            <View className='growth-card__body'>
              ...
              <View className='growth-card__footer'><Text>{difficultyLabels[note.difficulty] || '阅读'} · {note.durationMinutes ? `${note.durationMinutes} 分钟` : '长文'} · {note.tags[0] || '职业成长'}</Text>{!note.unlocked ? <Text className='growth-card__access'>会员</Text> : null}</View>
            </View>
            {note.coverUrl ? <Image className='growth-card__cover' src={note.coverUrl} mode='aspectFill' lazyLoad /> : <View className='growth-card__cover growth-card__cover--fallback'><Text>HAIGOO</Text><Text>{note.tags[0] || '职业笔记'}</Text></View>}
          </EditorialRow>
        ))}
      </View>
    </View>
