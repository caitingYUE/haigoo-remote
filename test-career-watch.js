...
const miniApiClient = fs.readFileSync(new URL('./miniprogram/src/services/api-client.ts', import.meta.url), 'utf8')
const miniCareerService = fs.readFileSync(new URL('./miniprogram/src/services/career-match-service.ts', import.meta.url), 'utf8')
...
assert.match(service, /inbox\.status[\s,]/)
assert.match(service, /row\.status === 'unread'/)
...
assert.match(miniPage, /上传简历，快速识别方向/)
assert.match(miniPage, /可选择 1–5 个方向/)
assert.match(miniPage, /开始设置偏好/)
assert.match(miniPage, /roleGroups/)
assert.match(miniPage, /const resumeFlowActive = useRef\(false\)/)
// The retained-page guard also protects an in-progress setup screen.
assert.match(miniPage, /if \(resumeFlowActive\.current \|\| \(stepRef\.current === 'setup' && lastScope\.current === miniContentScope\(\)\)\) return/)
assert.match(miniPage, /if \(scope !== miniContentScope\(\) \|\| sequence !== loadSequence\.current \|\| resumeFlowActive\.current \|\| stepRef\.current === 'setup'\) return/)
assert.match(miniPage, /resumeFlowActive\.current = true/)
assert.match(miniPage, /resumeFlowActive\.current = false/)
assert.match(miniPage, /mapResumeCareerDirections\(parsed\.structured, result\.filterOptions\)/)
assert.match(miniPage, /title: '暂无匹配类型'/)
assert.match(miniPage, /const latest = await fetchCareerWatch\(\)/)
assert.match(miniPage, /latest\.matchState !== 'unused' && sameDirections/)
assert.match(miniApiClient, /Promise\.race\(\[/)
assert.match(miniApiClient, /request:fail timeout/)
assert.match(miniCareerService, /export function mapResumeCareerDirections/)
assert.match(miniCareerService, /const byValue = new Map\(options\.map/)
assert.match(miniCareerService, /\['产品经理', \['product manager'/)
